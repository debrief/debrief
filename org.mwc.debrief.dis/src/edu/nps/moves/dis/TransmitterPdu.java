...
public class TransmitterPdu extends RadioCommunicationsFamilyPdu implements Serializable {
	...
	public int getMarshalledSize() {
		...
		for (int idx = 0; idx < modulationParametersList.size(); idx++) {
			final ModulationType listElement = modulationParametersList.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
		for (int idx = 0; idx < antennaPatternList.size(); idx++) {
			final BeamAntennaPattern listElement = antennaPatternList.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
	...
	public void marshal(final DataOutputStream dos) {
		...
		try {
			...
			for (int idx = 0; idx < modulationParametersList.size(); idx++) {
				final ModulationType aModulationType = modulationParametersList.get(idx);
				aModulationType.marshal(dos);
			} // end of list marshalling

			for (int idx = 0; idx < antennaPatternList.size(); idx++) {
				final BeamAntennaPattern aBeamAntennaPattern = antennaPatternList.get(idx);
	...
	public void setAntennaPatternList(final List<BeamAntennaPattern> pAntennaPatternList) {
	...
	public void setModulationParametersList(final List<ModulationType> pModulationParametersList) {
	...
	public void setRadioEntityType(final RadioEntityType pRadioEntityType) {
	...
	public void unmarshal(final DataInputStream dis) {
		...
		try {
			...
			for (int idx = 0; idx < modulationParameterCount; idx++) {
				final ModulationType anX = new ModulationType();
				anX.unmarshal(dis);
				modulationParametersList.add(anX);
			}

			for (int idx = 0; idx < antennaPatternCount; idx++) {
				final BeamAntennaPattern anX = new BeamAntennaPattern();
				anX.unmarshal(dis);
