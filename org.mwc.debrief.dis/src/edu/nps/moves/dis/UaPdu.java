...
/**
 * Section 5.3.7.3. Information about underwater acoustic emissions. This
...
public class UaPdu extends DistributedEmissionsFamilyPdu implements Serializable {
	...
	public int getMarshalledSize() {
		...
		for (int idx = 0; idx < shaftRPMs.size(); idx++) {
			final ShaftRPMs listElement = shaftRPMs.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
		for (int idx = 0; idx < apaData.size(); idx++) {
			final ApaData listElement = apaData.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
		for (int idx = 0; idx < emitterSystems.size(); idx++) {
			final AcousticEmitterSystemData listElement = emitterSystems.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
	...
	public void marshal(final DataOutputStream dos) {
		...
		try {
			...
			for (int idx = 0; idx < shaftRPMs.size(); idx++) {
				final ShaftRPMs aShaftRPMs = shaftRPMs.get(idx);
				aShaftRPMs.marshal(dos);
			} // end of list marshalling

			for (int idx = 0; idx < apaData.size(); idx++) {
				final ApaData aApaData = apaData.get(idx);
				aApaData.marshal(dos);
			} // end of list marshalling

			for (int idx = 0; idx < emitterSystems.size(); idx++) {
				final AcousticEmitterSystemData aAcousticEmitterSystemData = emitterSystems.get(idx);
	...
	public void setApaData(final List<ApaData> pApaData) {
		apaData = pApaData;
	}

	public void setEmitterSystems(final List<AcousticEmitterSystemData> pEmitterSystems) {
	...
	public void setShaftRPMs(final List<ShaftRPMs> pShaftRPMs) {
	...
	public void unmarshal(final DataInputStream dis) {
		...
		try {
			...
			for (int idx = 0; idx < numberOfShafts; idx++) {
				final ShaftRPMs anX = new ShaftRPMs();
				anX.unmarshal(dis);
				shaftRPMs.add(anX);
			}

			for (int idx = 0; idx < numberOfAPAs; idx++) {
				final ApaData anX = new ApaData();
				anX.unmarshal(dis);
				apaData.add(anX);
			}

			for (int idx = 0; idx < numberOfUAEmitterSystems; idx++) {
				final AcousticEmitterSystemData anX = new AcousticEmitterSystemData();
				anX.unmarshal(dis);
