...
public class EspduSender {
	 ...
	 * network. (Comcast, NPS.) This determines the bcast address for all connected
	 * interfaces, based on the IP and subnet mask. If you have a dual-homed host it
	 * will return a bcast address for both. If you have some VMs running on your
	 * host this will pick up the addresses for those as well--eg running VMWare on
	 * your laptop with a local IP this will also pick up a 192.168 address assigned
	 * to the VM by the host OS.
	 *
	 * @return set of all bcast addresses
	 */
	public static Set<InetAddress> getBroadcastAddresses() {
		final Set<InetAddress> bcastAddresses = new HashSet<InetAddress>();
		...
		try {
			...
			while (interfaces.hasMoreElements()) {
				final NetworkInterface anInterface = (NetworkInterface) interfaces.nextElement();

				if (anInterface.isUp()) {
					final Iterator it = anInterface.getInterfaceAddresses().iterator();
					while (it.hasNext()) {
						final InterfaceAddress anAddress = (InterfaceAddress) it.next();
						if ((anAddress == null || anAddress.getAddress().isLinkLocalAddress()))
							continue;

						// System.out.println("Getting bcast address for " + anAddress);
						final InetAddress abcast = anAddress.getBroadcast();
						if (abcast != null)
	...
	public static void main(final String args[]) {
		...
		final int alternator = -1;

		// ICBM coordinates for my office
		final double lat = 36.595517;
		...
		final String networkModeString = systemProperties.getProperty("networkMode"); // unicast or multicast or
																						// broadcast
		...
		try {
			...
			for (int idx = 0; idx < NUMBER_TO_SEND; idx++) {
				...
				// just stuff idx into the timestamp field to get something that is
				// monotonically
				// increasing.

				// Note that timestamp is used to detect duplicate and out of order packets.
				// That means if you DON'T change the timestamp, many implementations will
				// simply
				...
				final int ts = disTime.getDisAbsoluteTimestamp();
				...
				final double direction = Math.pow((-1.0), (idx));
				lon = lon + (direction * 0.00006);
				System.out.println(lon);

				final double disCoordinates[] = CoordinateConversions.getXYZfromLatLonDegrees(lat, lon, 1.0);
				...
				System.out.println(
						"DIS coord:" + disCoordinates[0] + ", " + disCoordinates[1] + ", " + disCoordinates[2]);
				...
				final byte[] fireArray = fire.marshal();
				...
				final Iterator it = bcastAddresses.iterator();
				while (it.hasNext()) {
					final InetAddress bcast = (InetAddress) it.next();
				...
				System.out.println("Espdu #" + idx + " EID=[" + eid.getSite() + "," + eid.getApplication() + ","
						+ eid.getEntity() + "]");
				System.out.println(" DIS coordinates location=[" + location.getX() + "," + location.getY() + ","
						+ location.getZ() + "]");
				final double c[] = { location.getX(), location.getY(), location.getZ() };
				final double lla[] = CoordinateConversions.xyzToLatLonDegrees(c);
