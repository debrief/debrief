 ...
 * <p>
 *
 * There are several major reference frames that may be useful in various
 * contexts:
 * <p>
 *
...
public class RangeCoordinates {
	public static void main(final String args[]) {
		...
		final RangeCoordinates primeMeridian = new RangeCoordinates(0.0, 0.0, 0.0);
		...
		final RangeCoordinates northPole = new RangeCoordinates(0.0, 180.0, 0.0); // north pole
		northPole.DISCoordFromLocalFlat(0.0, 0.0, 0.0);

		// y-axis: equator, 90 deg east. x and z should be near-zero
		final RangeCoordinates yAxis = new RangeCoordinates(90.0, 0.0, 0.0); // y axis
		yAxis.DISCoordFromLocalFlat(0.0, 0.0, 0.0);

		// Move west a bit from the equator/prime meridian
		final RangeCoordinates westALittle = new RangeCoordinates(0.0, -1.0, 0.0);
	...
	/**
	 * A DIS reference frame, with a euclidian coordinate system with origin at the
	 * center of of the earth. Coordinates, in (x, y, z), in meters. This is the
	 * reference frame used by many DIS fields on the wire. The technical term for
	 * this would be geocentric. Z is through the north pole, x out the prime
	 * meridian at the equator, and y out the equator at 90 deg east.
	 */
	SRF_Celestiocentric disCoordinateReferenceFrame;

	/**
	 * A local, flat, euclidian reference frame. This is tangent to a (lat, lon,
	 * altitudeOrigin) on an earth that is supplied by the user in the constructor.
	 * This allows users to set up a local, relatively small area for moving things
	 * around without in the nuisance of worrying about curved earth. The technical
	 * term for this would be Local Tangent Euclidian, ie a plane tangent to the
	 * earth at the given (lat, lon, alt). Coordinate system is x east, y north, z
	 * up.
	 */
	...
	/**
	 * The latitude and longitude of the local, flat, euclidian coordinate system
	 * origin
	 */
	double latitudeOrigin, longitudeOrigin;

	/**
	 * The altitude of the local coordinate system origin, ie the distance above the
	 * ellipsoid, not distance above terrain
	 */
	 ...
	 */
	public RangeCoordinates(final double originLat, final double originLon, final double heightOffset) {
		...
		try {
			...
			final double latInRadians = Math.toRadians(originLat);
			final double lonInRadians = Math.toRadians(originLon);

			// Reference system for a local tangent euclidian space plane, tangent to the
			// lat/lon
			// at a give altitude.
			localTangentSurfaceReferenceFrame = new SRF_LocalTangentSpaceEuclidean(SRM_ORM_Code.ORMCOD_WGS_1984,
					SRM_RT_Code.RTCOD_WGS_1984_IDENTITY, lonInRadians, latInRadians, // Origin (note: lon, lat)
			...
			earthSurfaceReferenceFrameOrigin = earthSurfaceReferenceFrame.createCoordinate3D(latInRadians, lonInRadians,
					heightOffset);
	...
	public void c(final double lat, final double lon, final double alt, final double bank, final double pitch,
			final double head) {
		try {
			// The geocentric aircraft location
			final Coord3D gd_coord = earthSurfaceReferenceFrame.createCoordinate3D(Math.toRadians(lon),
					Math.toRadians(lat), alt);
			// The geocentric location to be computed.
			final Coord3D gc_coord = disCoordinateReferenceFrame.createCoordinate3D(0.0, 0.0, 0.0);
			...
			final OrientationTaitBryanAngles tbE2NED_ori = new OrientationTaitBryanAngles(Math.toRadians(bank),
					Math.toRadians(pitch), Math.toRadians(head)); // AirCraft
			...
			final OrientationMatrix mW2LTSE_ori = new OrientationMatrix(1, 0, 0, 0, 1, 0, 0, 0, 1); // result target
			earthSurfaceReferenceFrame.transformOrientation(gc_coord, mW2LTSE_ori, gd_coord, mW2LTSE_ori);

			final OrientationMatrix mNED2LTSE_ori = new OrientationMatrix(0, 1, 0, 1, 0, 0, 0, 0, -1);

			// The DIS orientation is the composition of 3 orientations: tbE2NED_ori and
			// mNED2LTSE_ori and mNED2LTP_ori
			...
			// tbDIS_ori = new
			// OrientationTaitBryanAngles(tbE2NED_ori.composeWith(tbE2NED_ori.composeWith(mW2LTSE_ori)).getTaitBryanAngles());
	...
	public void change(final double localX, final double localY, final double localZ, final double bank,
			final double noseUp, final double bearing) {

		// bearing, in degrees, clockwise from true north is positive
		// noseUp, in degrees, angle at which the body is WRT the local flat plane, zero
		// is level, positive is up angle
		...
		try {
			final Coord3D localLTSEPosition = localTangentSurfaceReferenceFrame.createCoordinate3D(localZ, localY,
					localZ);
			final Coord3D localLTSEOrigin = localTangentSurfaceReferenceFrame.createCoordinate3D(0.0, 0.0, 0.0);
			final Coord3D disPosition = disCoordinateReferenceFrame.createCoordinate3D();
			final Coord3D disOrigin = disCoordinateReferenceFrame.createCoordinate3D();
			// Coord3D geodeticPosition =
			// earthSurfaceReferenceFrame.createCoordinate3D(latitudeOrigin,
			// longitudeOrigin, altitudeOrigin);

			// changes the contents of disPosition to reflect the geocentric coordinates of
			// the LTSE position
			final SRM_Coordinate_Valid_Region_Code region = disCoordinateReferenceFrame
					.changeCoordinateSRF(localLTSEPosition, disPosition);
			...
			// disCoordinateReferenceFrame.changeCoordinateSRF(geodeticPosition,
			// disPosition);
			// disCoordinateReferenceFrame.getCoordinate3DValues(disPosition);
			// System.out.println("Geodetic position:" + geodeticPosition);

			final OrientationTaitBryanAngles taitBryanOrientation = new OrientationTaitBryanAngles(
					Math.toRadians(-180.0 + bank), Math.toRadians(noseUp), Math.toRadians(360.0 - bearing - 270.0));
			System.out.println("tb orientation:" + taitBryanOrientation);

			// Will hold output, the orientation in DIS reference frame
			final OrientationTaitBryanAngles taitBryanDis = new OrientationTaitBryanAngles();
	 ...
	 *
	 * @param localCoordinates Position in local euclidian coordinate system. Values
	 *                         are overwritten to the DIS earth-centric coordinate
	 *                         system on return
	 */
	public void changeVectorToDisCoordFromLocalFlat(final Vector3Double localCoordinates) {
		final Vector3Double vec = this.DISCoordFromLocalFlat(localCoordinates.getX(), localCoordinates.getY(),
	 ...
	 */
	public void changeVectorToLocalCoordFromDIS(final Vector3Double worldCoordinates) {
		final Vector3Double vec = this.localCoordFromDis(worldCoordinates.getX(), worldCoordinates.getY(),
	 ...
	 */
	public Vector3Double DISCoordFromLocalFlat(final double x, final double y, final double z) {
		final Vector3Double disCoordinates = new Vector3Double();
		try {
			// Holds coordinates in the local tangent plane
			final Coord3D localCoordinates = localTangentSurfaceReferenceFrame.createCoordinate3D(x, y, z);
			// holds coordinates in the DIS (geocentric) coordinate frame
			final Coord3D disCoord = disCoordinateReferenceFrame.createCoordinate3D(0.0, 0.0, 0.0);

			final SRM_Coordinate_Valid_Region_Code region = disCoordinateReferenceFrame
					.changeCoordinateSRF(localCoordinates, disCoord);

			// System.out.println(region);

			// convert from the local tanget plane coordinates to the DIS coordinate frame
			final double values[] = disCoordinateReferenceFrame.getCoordinate3DValues(disCoord);
			// System.out.println("DIS x:" + values[0] + " y:" + values[1] + " z:" +
			// values[2] );
	...
	public SRF_LococentricEuclidean3D getPlatformReferenceFrame(final Vector3Double rangePositionCoordinates) {
		try {
			// The x,y,z location of the platform in range coordinates (ie, the LTSE).
			final Coord3D lococenter = localTangentSurfaceReferenceFrame.createCoordinate3D(
					rangePositionCoordinates.getX(), rangePositionCoordinates.getY(), rangePositionCoordinates.getZ());

			// We also need two unit vectors to describe the orientation of the platform in
			// the LTSE

			// primary axis direction--x, along the long axis of the platform
			final double[] orientationX = new double[3];

			final Direction xAxis = localTangentSurfaceReferenceFrame.createDirection(localEuclidianOrigin,
					orientationX);
		} catch (final Exception e) {
			System.out.println(e);
		}
	 ...
	 * @param z
	 */
	public Vector3Double localCoordFromDis(final double x, final double y, final double z) {
		final Vector3Double local = new Vector3Double();

		try {
			// Holds position in the local tangent plane
			final Coord3D localCoordinates = localTangentSurfaceReferenceFrame.createCoordinate3D(0.0, 0.0, 0.0);
			// Holds position in the DIS reference frame
			final Coord3D disCoord = disCoordinateReferenceFrame.createCoordinate3D(x, y, z);

			final SRM_Coordinate_Valid_Region_Code region = localTangentSurfaceReferenceFrame
					.changeCoordinateSRF(disCoord, localCoordinates);

			// System.out.println("Region:" + region);

			// Get the position in the local tangent place (ie, range coordinates) from DIS
			// coordinates (ie, geocentric)
			final double values[] = localTangentSurfaceReferenceFrame.getCoordinate3DValues(localCoordinates);
			// System.out.println("-->Local x:" + values[0] + " y:" + values[1] + " z:" +
			// values[2] );
			local.setX(values[0]);
			local.setY(values[1]);
			local.setZ(values[2]);
	...
	/**
	 * Converts a roll, pitch, and heading/yaw in the local flat coordinate system
	 * to DIS euler angles. Input orientation is in units of radians. DIS uses euler
	 * angles to describe the orientation of an object, using an earth-centered
	 * coordinate system, with successive rotations about the original x, y, and z
	 * axes. You need to be careful here because there are all sorts of conventions
	 * for "euler angles" including the order in which the axes are rotated about.
	 * <p>
	 *
	 ...
	 * roll/pitch/heading, but also where in the local coordinate frame the object
	 * is. Also, the pitch/roll/heading are in the local coordinate system, NOT the
	 * coordinate system of the object.
	 * <p>
	 *
	 * @param pitchRollHeading
	 */
	public edu.nps.moves.dis.Orientation localRollPitchHeadingToDisEuler(
			final edu.nps.moves.dis.Orientation pitchRollHeading, final edu.nps.moves.dis.Vector3Double localPosition) {
		// Tait-Bryan angles is jargon/correct terminology for the roll, pitch, and yaw.
		// It refers
		// to rotation about the original x, y, and z axes of the reference frame in
		// use; it's also
		// called cardano angles or tait-bryan. DIS uses rotation about the
		// earth-centric origin. In
		// a body-centric reference frame we have roll, pitch, and yaw. In the local
		// frame of reference
		// it is rotation about x (phi), y (theta) and z (psi).

		// Recall that the local frame of reference has X pointing east, y pointing
		// north, and
		// z pointing up. We are NOT using a body-centric reference frame here; we are
		// using the
		// range reference frame.

		try {
			// DIS euler angles in open-dis object. This is returned by the method.
			final edu.nps.moves.dis.Orientation openDisOrientation = new edu.nps.moves.dis.Orientation();

			// Local coordinate system orientation, in tait-bryan
			final OrientationTaitBryanAngles localOrientation = new OrientationTaitBryanAngles(
					pitchRollHeading.getPhi(), pitchRollHeading.getTheta(), pitchRollHeading.getPsi());

			// Local frame of reference position
			final Coord3D localSRMPosition = localTangentSurfaceReferenceFrame.createCoordinate3D(localPosition.getX(),
					localPosition.getY(), localPosition.getZ());

			// DIS frame of reference position
			final Vector3Double disLocation = this.DISCoordFromLocalFlat(localPosition.getX(), localPosition.getY(),
					localPosition.getZ());
			// Coord3D disCoord = disCoordinateFrame.createCoordinate3D(disLocation.getX(),
			// disLocation.getY(), disLocation.getZ());
			final Coord3D disCoord = disCoordinateReferenceFrame.createCoordinate3D(6378137.0, 0.0, 0.0);
			// double[] pos = disCoord.getValues();
			// System.out.println("DIS position for orienation change:" + pos[0] + "," +
			// pos[1] + "," + pos[2]);

			// An empty object passed into the method call below. When returned it's filled
			// out with
			// the DIS euler angles.
			final SRM.Orientation disOrientation = new OrientationTaitBryanAngles();
			...
			final SRM_Tait_Bryan_Angles_Params tait = disOrientation.getTaitBryanAngles();
