...
public class TransmitterPdu extends RadioCommunicationsFamilyPdu implements Serializable {
	...
	public int getAntennaPatternCount() {
		return antennaPatternList.size();
	}
	...
	public int getMarshalledSize() {
		...
		for (int idx = 0; idx < modulationParametersList.size(); idx++) {
			final Vector3Float listElement = modulationParametersList.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
		for (int idx = 0; idx < antennaPatternList.size(); idx++) {
			final Vector3Float listElement = antennaPatternList.get(idx);
			marshalSize = marshalSize + listElement.getMarshalledSize();
		}
	...
	public void marshal(final DataOutputStream dos) {
		...
		try {
			...
			for (int idx = 0; idx < modulationParametersList.size(); idx++) {
				final Vector3Float aVector3Float = modulationParametersList.get(idx);
				aVector3Float.marshal(dos);
			} // end of list marshalling

			for (int idx = 0; idx < antennaPatternList.size(); idx++) {
				final Vector3Float aVector3Float = antennaPatternList.get(idx);
				aVector3Float.marshal(dos);
			} // end of list marshalling
	...
	public void marshal(final java.nio.ByteBuffer buff) {
		...
		buff.putLong(frequency);
		buff.putFloat(transmitFrequencyBandwidth);
		buff.putFloat(power);
	 ...
	 * getantennaPatternCount method will also be based on the actual list length
	 * rather than this value. The method is simply here for java bean completeness.
	 */
	public void setAntennaPatternCount(final int pAntennaPatternCount) {
		antennaPatternCount = pAntennaPatternCount;
	}

	public void setAntennaPatternList(final List<Vector3Float> pAntennaPatternList) {
		antennaPatternList = pAntennaPatternList;
	}

	public void setAntennaPatternType(final int pAntennaPatternType) {
		antennaPatternType = pAntennaPatternType;
	}

	public void setCryptoKeyId(final int pCryptoKeyId) {
		cryptoKeyId = pCryptoKeyId;
	}

	public void setCryptoSystem(final int pCryptoSystem) {
		cryptoSystem = pCryptoSystem;
	}

	public void setFrequency(final long pFrequency) {
		frequency = pFrequency;
	}

	public void setInputSource(final short pInputSource) {
	 ...
	 * completeness.
	 */
	public void setModulationParameterCount(final short pModulationParameterCount) {
		modulationParameterCount = pModulationParameterCount;
	}

	public void setModulationParametersList(final List<Vector3Float> pModulationParametersList) {
		modulationParametersList = pModulationParametersList;
	}

	public void setModulationType(final ModulationType pModulationType) {
	...
	public void setPadding3(final short pPadding3) {
		padding3 = pPadding3;
	}

	public void setPower(final float pPower) {
		power = pPower;
	}

	public void setRadioEntityType(final EntityType pRadioEntityType) {
	...
	public void setRadioReferenceID(final EntityID pRadioReferenceID) {
	...
	public void setTransmitFrequencyBandwidth(final float pTransmitFrequencyBandwidth) {
		transmitFrequencyBandwidth = pTransmitFrequencyBandwidth;
	}

	public void setTransmitState(final short pTransmitState) {
		transmitState = pTransmitState;
	}

	public void setVariableTransmitterParameterCount(final int pVariableTransmitterParameterCount) {
	...
	public void unmarshal(final DataInputStream dis) {
		...
		try {
			...
			variableTransmitterParameterCount = dis.readUnsignedShort();
	...
	public void unmarshal(final java.nio.ByteBuffer buff) {
		...
		variableTransmitterParameterCount = buff.getShort() & 0xFFFF;
		antennaLocation.unmarshal(buff);
		relativeAntennaLocation.unmarshal(buff);
		antennaPatternType = buff.getShort() & 0xFFFF;
		antennaPatternCount = buff.getShort() & 0xFFFF;
		frequency = buff.getLong();
		transmitFrequencyBandwidth = buff.getFloat();
		power = buff.getFloat();
		modulationType.unmarshal(buff);
		cryptoSystem = buff.getShort() & 0xFFFF;
		cryptoKeyId = buff.getShort() & 0xFFFF;
