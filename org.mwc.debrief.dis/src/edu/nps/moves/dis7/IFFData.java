...
public class IFFData extends Object implements Serializable {
	...
	public boolean equalsImpl(final Object obj) {
		...
		for (int idx = 0; idx < iffData.size(); idx++) {
			if (!(iffData.get(idx).equals(rhs.iffData.get(idx))))
				ivarsEqual = false;
		}
	...
	public int getMarshalledSize() {
		...
		for (int idx = 0; idx < iffData.size(); idx++) {
			final OneByteChunk listElement = iffData.get(idx);
	...
	public void marshal(final DataOutputStream dos) {
		try {
			...
			for (int idx = 0; idx < iffData.size(); idx++) {
				final OneByteChunk aOneByteChunk = iffData.get(idx);
	...
	public void setIffData(final List<OneByteChunk> pIffData) {
		iffData = pIffData;
	}
