...
/**
 * Logs DIS packets to XML format. You can easily receive DIS packets off the
 * wire faster than they can be logged to XML, so you need to be careful about
 * that. With a 2 GHz core duo and OSX, on a macbook pro laptop drive, you can
 * log roughly 1,000 packets per second.
 * <p>
 *
...
public class DisLogger implements Runnable {
	...
	/**
	 * Maximum PDU size. Theoretically this can be 8K or more, but all our stuff is
	 * smaller
	 */
	...
	public static void main(final String args[]) {
		...
		try {
			...
			final DisLogger logger = new DisLogger(loggerProperties);
			final Thread loggerThread = new Thread(logger);
	 ...
	 */
	public DisLogger(final Properties loggerProperties) {
		final String stringMulticastGroup = loggerProperties.getProperty("multicastGroup");
		final String stringPort = loggerProperties.getProperty("port");
		exerciseName = loggerProperties.getProperty("exerciseName");
		final String stringPdusPerFile = loggerProperties.getProperty("pdusPerFile");

		System.out.println(
				"Multicast group: " + stringMulticastGroup + " port:" + stringPort + " exerciseName: " + exerciseName);
	...
	/**
	 * We may have broken out of the netowrk read loop, but the writer task is still
	 * involved in flushing out the already captured PDUs to disk. This gives us a
	 * test until that is done.
	 */
	...
	/**
	 * Run the logger. Create a new factory to decode IEEE format PDUs, and start
	 * reading into the buffer. When the buffer fills we do a quick-change and write
	 * the buffer to a file in another thread, so we don't drop (too many) packets.
	 */
	@Override
	public void run() {
		...
		if (logWriter == null) {
			...
			final Thread writerThread = new Thread(logWriter);
		...
		while (done == false) {
			...
			try {
				...
				if (timedout == false) {
					...
					final byte pduBytes[] = packet.getData();
				...
				if ((currentPduList.size() > pdusPerFile) || (done == true)) {
					final List<Pdu> pdusToLog = currentPduList;
