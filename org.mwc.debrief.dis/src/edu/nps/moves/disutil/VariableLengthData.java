...
/**
 * DIS has a frequent problem of shifting between units when describing the data
 * length of some fields. This is an attempt to fix that once, in hand-written
 * code, rather than generate source code to do it.
 * <p>
 *
 * The field has a length, a units descriptor (bytes or bits) and a descriptor
 * that tells us whether the record as a whole should end on a 8, 16, 32, or 64
 * bit word boundary, including the length field.
 *
 * @author DMcG
 */
public class VariableLengthData {
	...
	/**
	 * What units does the length field use? Typically bytes (octet) but also often
	 * bits. bits, bytes, 16 bit words, 32 bit words, 64 bit words.
	 */
	public enum DataLengthUnits {
		L_BIT, L_OCTET
	};

	/**
	 * We want the record as a whole to end on a byte, 16 bit word, 32 bit word, or
	 * 64 bit word boundary, so we pad to reach that.
	 */
	public enum PadToBoundary {
		BOUNDARY_8, BOUNDARY_16, BOUNDARY_32, BOUNDARY_64
	};
	...
	private int getPaddingBytes(final int writeLengthInBytes) {
		int paddingBytes = 0;
	...
	public void marshal(final DataOutputStream dos) {
		try {
			...
			if (this.dataLengthUnits == DataLengthUnits.L_BIT) {
				...
				final int remainder = (int) (dataLength % 8);
				if (remainder != 0) {
					writeLengthInBytes++;
				}
			} else {
	...
	/**
	 * Set the data to write. The assumption is that if your data is not written to
	 * a byte boundary (eg, 17 bits) you have padded the data array yourself to at
	 * least a byte boundary (eg, 24 bits or three bytes in the byte array). The
	 * code will handle padding the rest to a given byte, short, int, or 64 bit
	 * record boundary.
	 *
	 * @param data
	 */
	...
	public void unmarshal(final DataInputStream dis) {
		try {
			...
			final int paddingBytes = this.getPaddingBytes(data.length);
			for (int idx = 0; idx < paddingBytes; idx++) {
				dis.readByte();
			}
