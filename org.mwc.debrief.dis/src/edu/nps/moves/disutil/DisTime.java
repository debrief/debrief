...
public class DisTime {
	...
	private int getDisTimeUnitsSinceTopOfHour() {
		...
		final long currentTime = System.currentTimeMillis(); // UTC milliseconds since 1970
		cal.setTimeInMillis(currentTime);

		// Set cal to top of the hour, then compute what the cal object says was
		// milliseconds since 1970
		// at the top of the hour
		cal.set(Calendar.MINUTE, 0);
		cal.set(Calendar.SECOND, 0);
		cal.set(Calendar.MILLISECOND, 0);
		final long topOfHour = cal.getTimeInMillis();

		// Milliseconds since the top of the hour
		final long diff = currentTime - topOfHour;

		// It turns out that Integer.MAX_VALUE is 2^31-1, which is the time unit value,
		// ie there are
		// 2^31-1 DIS time units in an hour. 3600 sec/hr X 1000 msec/sec divided into
		// the number of
		// msec since the start of the hour gives the percentage of DIS time units in
		// the hour, times
		// the number of DIS time units per hour, equals the time value
		final double val = ((diff) / (3600.0 * 1000.0)) * Integer.MAX_VALUE;
		final int ts = (int) val;
	 ...
	 * year. This effectively eliminates the need for receivers to handle timestamp
	 * rollover, as long as you're not working on New Year's Eve.
	 *
	...
	public long getNpsTimestamp() {
		...
		cal.set(Calendar.SECOND, 0);
		cal.set(Calendar.MILLISECOND, 0);
		final long startOfYear = cal.getTimeInMillis();
