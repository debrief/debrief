...
public class PduMulticastReceiver extends UdpServer {
	...
	public static class Event extends java.util.EventObject {
		 ...
		 */
		public Event(final PduMulticastReceiver src) {
			super(src);
		}
	...
	private final Collection<PduMulticastReceiver.Listener> listeners = new LinkedList<PduMulticastReceiver.Listener>(); // Event
																															// listeners
	private final PduMulticastReceiver.Event event = new PduMulticastReceiver.Event(this);
	 ...
	 */
	public synchronized void addPduMulticastReceiverListener(final PduMulticastReceiver.Listener l) {
		listeners.add(l);
	}
	...
	protected synchronized void firePduReceived() {
		...
		final PduMulticastReceiver.Listener[] ll = listeners
				.toArray(new PduMulticastReceiver.Listener[listeners.size()]);
		for (final PduMulticastReceiver.Listener l : ll) {
			try {
				l.pduReceived(event);
	 ...
	 * @param l the UdpServer.Listener
	 */
	public synchronized void removePduMulticastReceiverListener(final PduMulticastReceiver.Listener l) {
		listeners.remove(l);
	}
