...
public class DisListenerView extends ViewPart {
	...
	/**
	 * we need to access the setting of live updates from outside the UI thread, so
	 * store it here.
	 */
	private boolean _doLiveUpdates = true;

	/**
	 * we need to access the setting of new plots from outside the UI thread, so
	 * store it here.
	 */
	private boolean _newPlotPerReplication = false;
	/**
	 * we need to access the setting of fit to data from outside the UI thread, so
	 * store it here.
	 */
	...
	private void addDropSupport() {
		...
		final DropTarget target = new DropTarget(pathText, operations);

		final Transfer[] types = new Transfer[] { ResourceTransfer.getInstance() };
	...
	private void contributeToActionBars() {
		...
		fitToDataAction.setToolTipText("Zoom the selected plot out to show the full data");
		fitToDataAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/fit_to_win.png"));
		...
		logMessagesAction.setToolTipText("Log DIS Message in System Log");
		logMessagesAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/list.png"));
		...
		final Action showConsole = new Action() {
			@Override
			public void run() {
				try {
					...
					DisActivator.log(IStatus.ERROR, "While showing console view", e);
				}
			}
		};
		showConsole.setText("Show console");
		showConsole.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/console.png"));
	...
	private void createEntityID() {
		_ourID = new EntityID();
		_ourID.setApplication((short) DisActivator.getDefault().getPreferenceStore().getInt(DisActivator.APP_ID));
		_ourID.setSite((short) DisActivator.getDefault().getPreferenceStore().getInt(DisActivator.SITE_ID));
	}
	...
	public void createPartControl(final Composite parent) {
		...
		final Composite topRow = new Composite(composite, SWT.NONE);
		...
		final Label iconLbl = new Label(topRow, SWT.CENTER);
		iconLbl.setImage(AbstractUIPlugin.imageDescriptorFromPlugin("org.mwc.debrief.dis", "icons/50px/dis_icon.png")
				.createImage());
		final GridData gd3 = new GridData(SWT.CENTER, SWT.CENTER, false, false);
		gd3.widthHint = col1Width;
		iconLbl.setLayoutData(gd3);

		final Group localGroup = new Group(topRow, SWT.NONE);
		...
		final Label label = new Label(localGroup, SWT.NONE);
		...
		pathText.addModifyListener(new ModifyListener() {
			...
			public void modifyText(final ModifyEvent e) {
				final String newText = pathText.getText();
				if (newText != null) {
					final IPreferenceStore store = DisActivator.getDefault().getPreferenceStore();
		...
		browseButton.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(final SelectionEvent e) {
				final FileDialog dialog = new FileDialog(getSite().getShell(), SWT.SINGLE);
		...
		final Composite controlHolder = new Composite(composite, SWT.NONE);
		gd = new GridData(SWT.FILL, SWT.FILL, true, false);
		...
		// SETTINGS
		// /////////////////////////
		final Group settingsPanel = new Group(composite, SWT.NONE);
		...
		link.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(final SelectionEvent e) {
				final PreferenceDialog dialog = PreferencesUtil.createPreferenceDialogOn(link.getShell(), DisPrefs.ID,
						null, null);
				dialog.open();
			}
		});
		...
		// CHART
		// /////////////////////////
		final Composite chartPanel = new Composite(composite, SWT.BORDER);
		...
		final JFreeChart theChart = ChartFactory.createTimeSeriesChart("Line Chart", "Time", "Hertz", dataset);
		chartComposite = new ChartComposite(chartPanel, SWT.NONE, theChart, 400, 600, 300, 200, 1800, 1800, true, true,
				true, true, true, true) {
			@Override
	...
	private void doConnect() {
		// collate the prefs
		final String app = DisActivator.getDefault().getPreferenceStore().getString(DisActivator.APP_FILTER);
		final String site = DisActivator.getDefault().getPreferenceStore().getString(DisActivator.SITE_FILTER);
		final String ex = DisActivator.getDefault().getPreferenceStore().getString(DisActivator.EXERCISE_FILTER);
	...
	private void doLaunch() {
		final IPreferenceStore store = DisActivator.getDefault().getPreferenceStore();
		final String pText = store.getString(DisActivator.PATH_TO_INPUT_FILE);
		doLaunch(pText);
	}
	 ...
	 * @param inputPath
	 */
	public void doLaunch(final String inputPath) {
	...
	private void fixChartComposite() {
		final Class<ChartComposite> clazz = ChartComposite.class;
		try {
			final Field field = clazz.getDeclaredField("canvas");
			field.setAccessible(true);
			final Object object = field.get(chartComposite);
	...
	private void handleStopMessage(final long time, final int appId, final short eid, final short reason,
			final long numRuns) {
		final Runnable theR = new Runnable() {

			@Override
			public void run() {
				final short ourAppId = (short) DisActivator.getDefault().getPreferenceStore()
						.getInt(DisActivator.APP_ID);
				...
				case IDISStopListener.PDU_STOP:
					...
					break;
				default:
					CorePlugin.logError(IStatus.WARNING, "Unknown DIS stop reason received:" + reason, null);
				}
	...
	private void initModule() {
		...
		final IDISNetworkPrefs netPrefs = new DebriefDISNetPrefs();

		// get the network data source
		final NetworkDISProvider prov = new NetworkDISProvider(netPrefs, new NetworkDISProvider.LogInterface() {
		...
		// sort out the part listneer. Note - we have to do this before we can setup the
		// DIS listeners
		// (below)
		...
		_context = new DISContext(_myPartMonitor) {
			...
			@Override
			public void setNewTime(final long time) {
				if (_doLiveUpdates) {
						...
						final Runnable theR = new Runnable() {
							...
							public void run() {
								...
								if (win != null) {
									...
									if (editor != null) {
										ct = editor.getAdapter(ControllableTime.class);
										tp = editor.getAdapter(TimeProvider.class);
									}
								}
							}
						};
					...
					if (ct != null) {
						...
						if (tp != null) {
							if (tp instanceof TimeManager) {
								final TimeManager tm = (TimeManager) tp;
								final TimePeriod curPeriod = tm.getPeriod();
								if (curPeriod != null) {
									final TimePeriod newPeriod = new TimePeriod.BaseTimePeriod(curPeriod.getStartDTG(),
											new HiResDate(time));
				...
				final Runnable doUpdate = new Runnable() {
					...
					public void run() {
						...
						if (win != null) {
							...
							final IEditorPart editor = page.getActiveEditor();
							if (editor != null) {
								final CanvasType canvas = editor.getAdapter(CanvasType.class);
								if (canvas != null) {
									canvas.rescale();
								}
							}
	...
	private void setupListeners(final IDISModule module) {
		...
		module.addGeneralPDUListener(new IDISGeneralPDUListener() {
			...
			public void logPDU(final Pdu pdu) {
				final long newTime = module.convertTime(pdu.getTimestamp());
		...
		final LoggingFileWriter logWriter = new LoggingFileWriter() {
			...
			public void writeThis(final String dType, final String header, final String output) {
				if (_logMessagesToStore) {
					DisActivator.log(IStatus.INFO, dType + "\n" + header + "\n" + output, null);
				}
			}
		};
		...
		module.addStartResumeListener(new StartFileListener("", true, false, logWriter));
