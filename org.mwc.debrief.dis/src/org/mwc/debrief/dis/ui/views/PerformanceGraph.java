...
public class PerformanceGraph implements IDISGeneralPDUListener, IDISScenarioListener {
	 ...
	 */
	public PerformanceGraph(final ChartComposite chartComposite) {
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.mwc.debrief.dis.listeners.IDISGeneralPDUListener#complete(java.lang.
	 * String)
	 */
	@Override
	 ...
	 *
	 * @see
	 * org.mwc.debrief.dis.listeners.IDISGeneralPDUListener#logPDU(edu.nps.moves.dis
	 * .Pdu)
	 */
	@Override
			...
			public void run() {
				final Runnable theR = new Runnable() {
					...
					public void run() {
						...
						final TimeSeriesCollection data = (TimeSeriesCollection) _chart.getChart().getXYPlot()
								.getDataset();
				...
				while (!_terminate) {
					final Runnable theR2 = new Runnable() {
						private void doThisQueue(final PerformanceQueue queue, final String name) {
							// store the new time in the queue
							// queue.add(new Date().getTime());

							final double freq = queue.freqAt(new Date().getTime()) * 1000d;
						...
						public void run() {
							...
							// clear them out?
							final DateAxis tAxis = (DateAxis) _chart.getChart().getXYPlot().getDomainAxis();
							tAxis.setRange(new Date(new Date().getTime() - GRAPH_PERIOD), new Date());
