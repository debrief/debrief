...
public class DISFilters {
	 ...
	 * @return
	 */
	public boolean accepts(final Pdu pdu) {
		if (hasFilter) {
			if (pdu instanceof SimulationManagementFamilyPdu) {
				final SimulationManagementFamilyPdu sim = (SimulationManagementFamilyPdu) pdu;
				if (siteId != null && siteId != sim.getOriginatingEntityID().getSite()) {
					return false;
				}
				if (appId != null && appId != sim.getOriginatingEntityID().getApplication()) {
					return false;
				}
				...
				final EntityStatePdu sd = (EntityStatePdu) pdu;
				...
				final DetonationPdu sd = (DetonationPdu) pdu;
				...
				final CollisionPdu sd = (CollisionPdu) pdu;
