...
public class VTracker implements DisposeListener {
	...
	static void addTopLevelPanel(final VPanel panel) {
		final VTracker tracker = instance();
	...
	public static VControl getVControl(Widget widget) {
		if (widget instanceof Shell) {
			final Control[] ca = ((Shell) widget).getTabList();
			if (ca.length > 0) {
				widget = ca[0];
			}
		}
		final Object o = widget.getData("cwt_vcontrol");
	...
	public static boolean isFocusControl(final Control control) {
		final VControl focusControl = instance().focusControl;
	...
	private static boolean setFocusFromNext(final Control control) {
		Control c = null;
		final Composite parent = control.getParent();
		if (parent == null) {
			c = control;
		} else {
			final Control[] ca = parent.getTabList();
	...
	private static boolean setFocusFromPrev(final Control control) {
		Control c = null;
		final Composite parent = control.getParent();
		if (parent == null) {
			c = control;
	...
	private static void setFocusToNext(final Composite comp) {
		if (comp != null) {
			final Composite parent = comp.getParent();
			final Control[] controls = parent.getTabList();
	...
	private static void setFocusToPrev(final Composite comp) {
		if (comp != null) {
			final Composite parent = comp.getParent();
			final Control[] controls = parent.getTabList();
			if (parent instanceof Shell) {
		...
		public void handleEvent(final Event event) {
			...
			case SWT.Traverse:
				...
				if (SWT.TRAVERSE_TAB_NEXT == event.detail || SWT.TRAVERSE_TAB_PREVIOUS == event.detail) {
					if (focusControl != null) {
						...
						if (event.doit) {
							final Composite comp = focusControl.getWidget();
			...
			case SWT.MouseMove:
				if (panels.containsKey(event.widget)) {
					final VControl vcontrol = panels.get(event.widget).getControl(event.x, event.y, true);
	...
	void activate(final VControl vcontrol) {
	...
	void deactivate(final VControl vcontrol) {
		if (vcontrol != null) {
	...
	private VControl getNewFocus(final VPanel panel) {
		for (final VControl child : panel.getChildren()) {
			if (!child.hasStyle(SWT.NO_FOCUS)) {
				if (child instanceof VPanel) {
					final VControl newFocus = getNewFocus((VPanel) child);
					if (newFocus != null) {
						return newFocus;
					}
				} else {
	...
	private void notifyWidgetFocusListeners(final VControl newFocus, final VControl oldFocus) {
		if (newFocus != null && !newFocus.isSameWidgetAs(oldFocus)) {
			final Widget widget = newFocus.getWidget();
		...
		if (oldFocus != null && !oldFocus.isSameWidgetAs(newFocus)) {
			final Widget widget = oldFocus.getWidget();
			if (widget.getData("cwt_focus") != null) { //$NON-NLS-1$
	...
	boolean setFocusControl(final VControl control) {
		...
		try {
			...
			final VControl oldFocus = focusControl;
	...
	public void widgetDisposed(final DisposeEvent e) {
		...
		if (tracker.panels != null && tracker.panels.containsKey(e.widget)) {
			...
			if (tracker.panels.isEmpty()) {
				...
				if (activeControl != null) {
					final Control control = activeControl.getControl();
