...
/**
 * An SvgShape is a graphical svg element which can be directly applied to a
 * given graphics context.<br>
 * Shapes consist of:
 * <ul>
 * <li>circle</li>
 * <li>ellipse</li>
 ...
 * <p>
 * See also: <a href=
 * "http://www.w3.org/TR/SVG/shapes.html">http://www.w3.org/TR/SVG/shapes.html</a>
 * </p>
 */
public class SvgShape extends SvgGraphic {
	...
	SvgShape(final SvgContainer container, final String id) {
		super(container, id);
	}
	 ...
	 */
	public boolean contains(final float x, final float y, final GC gc, final boolean outline) {
	...
	private void doApply(final GC gc) {
		gc.setAntialias(SWT.ON);

		final SvgFill derivedFill = getFill();
		final SvgStroke derivedStroke = getStroke();
		final Transform derivedTransform = getTransform(gc);
	...
	private void doFill(final GC gc) {
		if (path != null) {
			gc.fillPath(path);
		} else if (pathData.points.length == 4) {
			final int w = (int) (2 * pathData.points[2]);
			final int h = (int) (2 * pathData.points[3]);
			if (w > 0 && h > 0) {
				final int x = (int) (pathData.points[0] - pathData.points[2]);
				final int y = (int) (pathData.points[1] - pathData.points[3]);
				gc.fillOval(x, y, w, h);
			}
		} else if (pathData.points.length == 6) {
			final int w = (int) pathData.points[2];
			final int h = (int) pathData.points[3];
			if (w > 0 && h > 0) {
				final int x = (int) pathData.points[0];
				final int y = (int) pathData.points[1];
				final int rx = (int) pathData.points[4];
				final int ry = (int) pathData.points[5];
				if (rx > 0 || ry > 0) {
	...
	private void doStroke(final GC gc) {
		if (path != null) {
			gc.drawPath(path);
		} else if (pathData.points.length == 4) {
			final int w = (int) (2 * pathData.points[2]);
