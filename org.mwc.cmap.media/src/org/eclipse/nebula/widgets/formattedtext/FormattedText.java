 ...
 * <p>
 *
 * Formatting is delegated to formatter objects implementing the
 * <code>ITextFormatter</code> interface. Each formatter class manages a base
 * class of values (date, number...).<br>
 * Formatters are associated by 2 different means :
 * <ul>
 * <li>By the <code>setFormatter()</code> method.</li>
 * <li>When <code>setValue()</code> is called and there is currently no
 * formatter, a new one is automatically created based on the class of the
 * value.</li>
 * </ul>
 *
 * <h4>Styles:</h4> <blockquote> CENTER, LEFT, RIGHT, READ_ONLY </blockquote>
 */
public class FormattedText {
	...
	/**
	 * Set the beep sound to ON or OFF for all the FormattedText fields. Beep sound
	 * are produced by formatters on every input error. Formatter implementation
	 * must check this flag on before to emit a beep sound.
	 *
	 * @param beepSound true to emit beep sound on errors, else false
	 */
	public static void setBeepSound(final boolean beepSound) {
	 ...
	 */
	public FormattedText(final Composite parent) {
	 ...
	 */
	public FormattedText(final Composite parent, final int style) {
	 ...
	 */
	public FormattedText(final Text t) {
		text = t;
	 ...
	 *
	 * @return Formatter, or <code>null</code> if no formatter is currently
	 *         associated
	 */
	 ...
	 * <p>
	 *
	 * The returned value is provided by the formatter and is of the type managed by
	 * the formatter. For example a <code>DateFormatter</code> will return a
	 ...
	 * FormattedText handles, i.e. returns in {@link #getValue()}.
	 *
	 ...
	 * <code>false</code>.<br>
	 * An empty value depends of the formatter applied on the Text widget and is not
	 * just an empty String in the widget. Formatters can use special formatting
	 * characters to format the value. These characters are not always considered as
	 * part of the value. For example, in a DateFormatter the pattern uses "/"
	 * separator and always displays it in the input field.
	 *
	 ...
	 * <p>
	 *
	 * If no formatter is currently associated to he widget, a new one is created by
	 * the factory based on the value's class.<br>
