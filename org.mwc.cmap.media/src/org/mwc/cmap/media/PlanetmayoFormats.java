...
public class PlanetmayoFormats {
	...
	public String encodeDateInFileName(final Date date, final String extension) {
		if (date == null) {
	...
	public boolean isSupportedImage(String imageName) {
		...
		for (final String supportedFormat : supportedImageFormats) {
	...
	public Date parseDateFromFileName(String fileName) {
		...
		try {
			final StringBuilder fileNameFinal = new StringBuilder();
			final String[] dateTime = fileName.split("_");
			if (dateTime.length > 2) {
				fileNameFinal.append(dateTime[0]).append("_").append(dateTime[1]);
			} else {
				fileNameFinal.append(fileName);
			}
			final Date result = getFilenameFormat().parse(fileNameFinal.toString());
