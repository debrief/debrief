...
public class OpenVideoPlayerUtil {
	...
	public static void openVideoPlayer(final String fileName, final Date scenarioStart) {
		...
		if (start == null) {
			...
			final long startTime = PlatformUI.getPreferenceStore().getLong(new File(fileName).getName());
			if (startTime > 0) {
	...
	private static void showVideoPlayer(final String fileName, final Date start) {
		final IViewPart view = CorePlugin.openSecondaryView(CorePlugin.VIDEO_PLAYER_VIEW, fileNamePartOf(fileName),
				IWorkbenchPage.VIEW_ACTIVATE);
		if (view instanceof VideoPlayerView) {
			final VideoPlayerView videoView = (VideoPlayerView) view;
