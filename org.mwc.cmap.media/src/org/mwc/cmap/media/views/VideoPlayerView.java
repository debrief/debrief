...
public class VideoPlayerView extends ViewPart {
	private class ModifiedAdapter extends PlayerAdapter {
		...
		public void onPlaying(final XugglePlayer player, final long milli) {
			...
			if ((getViewSite().getPage().getActivePart() == VideoPlayerView.this && fireNewTime)
					|| player.isPlaying()) {
		...
		private void update(final XugglePlayer player, final long milli) {
			...
			if (_timeProvider != null && _timeProvider.getPeriod() != null && player.isOpened()) {
				final Date start = new Date(startTime.getTime());
	...
	private final PropertyChangeListener _propertyChangeListener = new PropertyChangeListener() {
		...
		public void propertyChange(final PropertyChangeEvent evt) {
			if (!player.isOpened() || _firingNewTime || _timeProvider == null || _timeProvider.getPeriod() == null
					|| _timeProvider.getTime() == null || !player.isOpened()) {
				return;
			}
	...
	private void createActions() {
		open = new Action() {
			...
			public void run() {
				fireNewTime = true;
				final FileDialog fd = new FileDialog(getViewSite().getShell(), SWT.OPEN);
	...
	private Button createPlayBtn(final Composite composite, final Composite parent) {
		final Button playBtn = new Button(parent, SWT.NONE);
	...
	private Scale createScaleControl(final Composite parent) {
	...
	private Button createStopBtn(final Composite composite, final Composite parent) {
		final Button stopBtn = new Button(parent, SWT.TRANSPARENT);
		...
		target.addDropListener(new DropTargetAdapter() {
			...
			public void drop(final DropTargetEvent e) {
				...
				if (e.data instanceof String[]) {
					...
					if (fileNames.length > 0 && fileNames[0] != null && !fileNames[0].isEmpty()) {
						final String fileName = fileNames[0];

						final int index = fileName.lastIndexOf(".");
						if (index >= 0 && new File(fileName).isFile()) {
							final String extension = fileName.substring(index + 1);
							final String[] supportedExtensions = { "avi", "vob", "mp4", "mov", "mpeg", "flv", "mp3",
									"wma", "wav" };
	...
	private void initializePlayer(final String fileName) {
		...
		final Date start = PlanetmayoFormats.getInstance().parseDateFromFileName(new File(fileName).getName());
		if (start == null) {
			// try to get the start time from last video start time.
			final long lastStartTime;
			if (_currentFilename != null) {
				lastStartTime = PlatformUI.getPreferenceStore().getLong(_currentFilename);
			} else {
				lastStartTime = 0;
			}
			...
			Display.getCurrent().asyncExec(new Runnable() {
				...
				public void run() {
					if (!player.open(currentFilename)) {
						...
						final MessageBox message = new MessageBox(getSite().getShell(), SWT.ICON_WARNING | SWT.OK);
	...
	private void openEditStartTimeDialog() {
		final VideoPlayerStartTimeDialog startTimeDialog = new VideoPlayerStartTimeDialog(getViewSite().getShell());
		...
		if (memento.getString(STATE_VIDEO_FILE) != null) {
			...
			if (player.open(memento.getString(STATE_VIDEO_FILE))) {
				...
				try {
					...
					startTime = (new Date(Long.parseLong(memento.getString(STATE_START_TIME))));
		...
		_myPartMonitor.addPartListener(TimeProvider.class, PartMonitor.ACTIVATED, new PartMonitor.ICallback() {
			...
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				if (_timeProvider != part) {
					...
					_timeProvider = tp;
					_timeProvider.addListener(_propertyChangeListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
				}
			}
	...
	public void setVideoStartTime(final Date date) {
		startTime = date;
		PlatformUI.getPreferenceStore().setValue(_currentFilename, date.getTime());
