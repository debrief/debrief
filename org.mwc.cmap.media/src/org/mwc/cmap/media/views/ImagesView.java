...
public class ImagesView extends ViewPart {
	...
	private void createActions() {
		open = new Action() {
			...
			@Override
			public void run() {
				final DirectoryDialog dialog = new DirectoryDialog(getSite().getShell());
	...
	private void createMainWindow(final Composite parent) {
		...
		gallery.setDefaultImage(
				PlanetmayoImages.UNKNOWN.getImage().createImage(gallery.getMainComposite().getDisplay()));
		...
		gallery.addElementMouseListener(new MouseListener() {
			...
			public void mouseUp(final MouseEvent event) {
				final ImageGallery<ImageMetaData, ThumbnailPackage>.ImageLabel label = (ImageGallery<ImageMetaData, ThumbnailPackage>.ImageLabel) event.data;
			...
			public void drop(final DropTargetEvent e) {
				...
				if (e.data instanceof String[]) {
					...
					if (fileNames.length > 0 && fileNames[0] != null && !fileNames[0].isEmpty()) {
						...
						if (!fileName.equals(openedFolder) && new File(fileName).isDirectory()) {
							openFolder(fileName);
						}
	...
	public boolean openFolder(final String folderName) {
		...
		for (final File child : childs) {
			...
			final Date date = PlanetmayoFormats.getInstance().parseDateFromFileName(childName);
			if (date != null) {
