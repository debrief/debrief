...
public class TimeBar implements IEventEntry {
	/** TimeBar start */
	private final Calendar _start = Calendar.getInstance(TimeZone.getTimeZone("GMT+0"), Locale.UK);
	/** TimeBar end */
	private final Calendar _end = Calendar.getInstance(TimeZone.getTimeZone("GMT+0"), Locale.UK);
	/** TimeBar caption */
	private final String _eventName;
	private Color _color = null;

	private final Object _source;
	private final List<IEventEntry> _children = new ArrayList<IEventEntry>();

	protected TimeBar(final BaseLayer tacticalItems, final boolean collapseChildren) {
		...
		while (enumer.hasMoreElements()) {
			...
			if (sensor instanceof TacticalDataWrapper) {
				...
				if (item.getStartDTG() != null && item.getEndDTG() != null) {
					...
					final TimePeriod thisP = new TimePeriod.BaseTimePeriod(item.getStartDTG(), item.getEndDTG());
