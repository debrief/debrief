...
public class TimeBarView extends ViewPart implements TimeBarPrefs {
	...
	void clearTimeListener() {
		if (_timeProvider != null) {
			_timeProvider.removeListener(_temporalListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
	...
	public void init(final IViewSite site, final IMemento memento) throws PartInitException {
		...
		if (memento != null) {
			...
			_defaultCollapseSegments = Activator.getDefault().getPreferenceStore().getBoolean(COLLAPSE_SEGMENTS);
			_defaultCollapseSensors = Activator.getDefault().getPreferenceStore().getBoolean(COLLAPSE_SENSORS);
		}
	}
	...
	private void listenToMyParts() {
		...
		_myPartMonitor.addPartListener(Layers.class, PartMonitor.OPENED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				processNewLayers(part);
			}
		...
		_myPartMonitor.addPartListener(TimeController.class, PartMonitor.ACTIVATED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				_timeProvider.addListener(_temporalListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
			}
		});
		_myPartMonitor.addPartListener(TimeController.class, PartMonitor.OPENED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				final TimeProvider provider = ((TimeController) part).getTimeProvider();
				if (provider != null) {
		...
		_myPartMonitor.addPartListener(TimeController.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				final TimeProvider provider = ((TimeController) part).getTimeProvider();
				if (provider != null && provider.equals(_timeProvider)) {
				...
				if (part != _viewer) {
					...
					if (!iS.equals(_selectionProvider)) {
						...
						if (_selectionChangeListener != null) {
							_selectionProvider.addSelectionChangedListener(_selectionChangeListener);
						}
					}
				}
			}
		...
		_myPartMonitor.addPartListener(ISelectionProvider.class, PartMonitor.DEACTIVATED, new PartMonitor.ICallback() {
			...
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				// aah, just check it's not is
				if (!part.equals(_viewer) && _selectionProvider != null && _selectionChangeListener != null) {
					_selectionProvider.removeSelectionChangedListener(_selectionChangeListener);
				}
			}
		});
		...
		_myPartMonitor.addPartListener(TimeBarView.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {

			@Override
			public void eventTriggered(final String type, final Object instance, final IWorkbenchPart parentPart) {
				Activator.getDefault().getPreferenceStore().setValue(COLLAPSE_SEGMENTS, _collapseSegments.isChecked());
				Activator.getDefault().getPreferenceStore().setValue(COLLAPSE_SENSORS, _collapseSensors.isChecked());
			}
		});
	...
	private void makeActions() {
		...
		_zoomInAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/zoomin.png"));
		...
		_zoomOutAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/zoomout.png"));
		...
		_fitToWindowAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/fit_to_win.png"));
		...
		_collapseSensors.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/16/sensor.png"));
		...
		_collapseSegments.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/16/tma_segment.png"));
	...
	void processNewData(final Layers theData, final Editable newItem, final HasEditables parentLayer) {
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				if (newItem != null) {
					// wrap the plottable
					final EditableWrapper parentWrapper = new EditableWrapper((Editable) parentLayer, null, theData);
