    ...
    RightClickContextItemGenerator
{
      ...
      Layer[] parentLayers, Editable[] subjects)
  {
    ...
    for (int i = 0; i < subjects.length; i++)
    {
          ...
          || (thisItem instanceof SATC_Solution))
      {
        ...
        if (thisItem instanceof ContributionWrapper)
        {
          ...
          period =
              new TimePeriod.BaseTimePeriod(new HiResDate(theCont
                  .getStartDate()), new HiResDate(theCont.getFinishDate()));
        ...
        else
        {
          ...
          if (solution.getSolver().getProblemSpace().getStartDate() != null)
            period =
                new TimePeriod.BaseTimePeriod(solution.getStartDTG(), solution
                    .getEndDTG());
        }
    ...
    if (validCuts != null)
    {
      ...
      DoIt wizardItem =
          new DoIt("Create new scenario from these bearings",
              new BearingMeasurementContributionFromCuts(null, title,
                  theLayers, validCuts)
              {
      ...
      CoreSolutionFromCuts
  {
    ...
    protected BearingMeasurementContribution
        createContribution(String contName)
    {
      // ok, now collate the contriubtion
      final BearingMeasurementContribution bmc =
          new BearingMeasurementContribution();
      ...
      Collection<Editable> hostFixes =
          host.getItemsBetween(new HiResDate(bmc.getStartDate()),
      ...
      ArrayList<SensorContactWrapper> validCuts)
  {
    ...
    // ok, now collate the contriubtion
    final Range1959ForecastContribution bmc =
        new Range1959ForecastContribution();
    ...
    while (iter.hasNext())
    {
      ...
      final FrequencyMeasurement thisM =
          new FrequencyMeasurement(date,
      ...
      ForecastContributionFromCuts
  {
    ...
    protected BaseContribution getContribution()
    {
      ...
      while (iter.hasNext())
      {
        ...
        RangeForecastContribution.ROrigin newR =
            new ROrigin(loc, scw.getDTG().getDate());
        rfc.addThis(newR);
  ...
  private class StraightLegWizardFromCuts extends ForecastContributionFromCuts
  {
    ...
    public IStatus execute(IProgressMonitor monitor, IAdaptable info)
    {
      ...
      WizardDialog wd =
          new WizardDialog(Display.getCurrent().getActiveShell(), _wizard);
  ...
  private abstract class CoreSolutionFromCuts extends CMAPOperation
  {
        ...
        Layers theLayers, BaseContribution existingContribution)
    {
      ...
      thePeriod =
          new TimePeriod.BaseTimePeriod(new HiResDate(existingContribution
              .getStartDate()), new HiResDate(existingContribution
              .getFinishDate()));
    ...
    public IStatus execute(IProgressMonitor monitor, IAdaptable info)
    {
      ...
      ISolversManager solversManager =
          SATC_Activator.getDefault().getService(ISolversManager.class, false);
      ...
      CoreSolutionFromCuts
  {
    ...
    {
      // ok, now collate the contriubtion
      final FrequencyMeasurementContribution fmc =
          new FrequencyMeasurementContribution();
      ...
      while (iter.hasNext())
      {
        ...
        if (scw.getHasFrequency())
        {
          ...
          if (statList.length > 0)
          {
            ...
            double spdMs =
                new WorldSpeed(stat.getSpeed(), WorldSpeed.Kts)
