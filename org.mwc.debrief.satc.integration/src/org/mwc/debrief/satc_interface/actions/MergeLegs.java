...
public class MergeLegs implements RightClickContextItemGenerator {
	...
	private static class MergeLegsOperation extends CMAPOperation {
		...
		public MergeLegsOperation(final String title, final Editable target, final Layers theLayers,
				final SATC_Solution parentLayer, final Editable[] subjects) {
			super(title);
			_target = target;
			_layers = theLayers;
			_parent = parentLayer;
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			for (int i = 0; i < _subjects.length; i++) {
				...
				final StraightLegWrapper thisS = (StraightLegWrapper) thisE;
				final StraightLegForecastContribution thisC = (StraightLegForecastContribution) thisS.getContribution();
	...
	public void generate(final IMenuManager parent, final Layers theLayers, final Layer[] parentLayers,
		...
		if (validItems >= 2) {
			...
			final Action doMergeInPlace = new Action(mergeTitle) {
				@Override
				public void run() {
					final IUndoableOperation theAction = new MergeLegsOperation(mergeTitle, editable, theLayers,
							solution, subjects);
