...
public class SATC_Solution extends BaseLayer implements NeedsToBeInformedOfRemove, NeedsToKnowAboutLayers,
		WatchableList, BaseLayer.ProvidesRange, ISecondaryTrack, NonColoredWatchable {
	...
	private static class DoPaint implements RouteStepper {
		...
		public void legComplete(final CoreRoute thisRoute) {
			...
			if (thisRoute.getType() == LegType.STRAIGHT) {
				...
				final State lastState = thisRoute.getStates().get(thisRoute.getStates().size() - 1);
				...
				final WorldLocation firstLoc = conversions.toLocation(firstState.getLocation().getCoordinate());
				final WorldLocation lastLoc = conversions.toLocation(lastState.getLocation().getCoordinate());

				CanvasTypeUtilities.drawLabelOnLine(_dest, thisRoute.getName(), theFont, theColor, firstLoc, lastLoc,
						LEG_LABEL_CLIPPING_THRESHOLD, true);

				final String vectorDescription = String.format("%.1f",
						new WorldSpeed(straight.getSpeed(), WorldSpeed.M_sec).getValueIn(WorldSpeed.Kts)) + " kts "
						+ String.format("%.0f", Math.toDegrees(MathUtils.normalizeAngle(straight.getCourse())))
						+ "\u00B0";

				CanvasTypeUtilities.drawLabelOnLine(_dest, vectorDescription, theFont, theColor, firstLoc, lastLoc,
						LEG_LABEL_CLIPPING_THRESHOLD, false);
			}
	...
	public class SATC_Info extends Editable.EditorType implements Serializable {
		...
		public final MethodDescriptor[] getMethodDescriptors() {
			...
			final MethodDescriptor[] mds = { method(c, "convertToLegs", null, "Convert To Manual TMA Track"),
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = {
						...
						prop("Color", "the color to display this solution", EditorType.FORMAT),
	...
	/**
	 * we remember the most recent score, to decide if it should be presented to the
	 * user
	 *
	 */
	...
	public void beingRemoved() {
		// get the manager
		final ISolversManager mgr = SATC_Activator.getDefault().getService(ISolversManager.class, true);
	...
	public void convertToLegs() {
		...
		if ((_newRoutes == null) || (_newRoutes.length == 0)) {
			CorePlugin.errorDialog("Convert solution to track", "Sorry, this solution contains no generated routes");
		} else {
			...
			for (int i = 0; i < _newRoutes.length; i++) {
				...
				while (legs.hasNext()) {
					...
					if (thisLeg instanceof StraightRoute) {
						...
						final WorldSpeed speed = new WorldSpeed(straight.getSpeed(), WorldSpeed.M_sec);
						final WorldLocation origin = conversions.toLocation(straight.getStartPoint().getCoordinate());
						final HiResDate startTime = new HiResDate(straight.getStartTime().getTime());
						final HiResDate endTime = new HiResDate(straight.getEndTime().getTime());
						...
						final AbsoluteTMASegment abs = new AbsoluteTMASegment(courseDegs, speed, origin, startTime,
								endTime, stamps);
						...
						if (pendingAlteration != null) {
							...
							final DynamicInfillSegment infill = new DynamicInfillSegment(pendingAlteration, abs);
						...
						// COMMENTED OUT THIS NEXT BLOCK - WE'RE NOT GOING TO USE THE DERIVED
						// ALTERATION,
						// WE'LL JUST USE OUR DYNAMIC INFILL SEGMENTS

					} else
						DebriefPlugin.logError(IStatus.ERROR, "Unexpected type of route encountered:" + thisLeg, null);
				}
			...
			for (int i = 0; i < _newRoutes.length; i++) {
				...
				while (legs.hasNext()) {
					...
					while (iter.hasNext()) {
						...
						final WorldLocation theLoc = conversions.toLocation(state.getLocation().getCoordinate());
						final double theCourse = state.getCourse();
						final double theSpeed = new WorldSpeed(state.getSpeed(), WorldSpeed.M_sec)
								.getValueIn(WorldSpeed.ft_sec / 3);
						final Fix newF = new Fix(new HiResDate(state.getTime().getTime()), theLoc, theCourse, theSpeed);
	...
	protected void finalize() throws Throwable {
		...
		_mySolver.getContributions().removeContributionsChangedListener(_contributionsListener);
		_mySolver.getBoundsManager().removeConstrainSpaceListener(_constrainListener);
	...
	public WorldArea getBounds() {
		...
		if (_newRoutes != null) {
			...
			for (final Iterator<CoreRoute> iterator = legs.iterator(); iterator.hasNext();) {
						...
						if (state.getTime().getTime() >= time) {
							...
							if (getInterpolatePoints()) {
								...
								if (before != null) {
									...
									wrapped = FixWrapper.interpolateFix(beforeWrapped, thisWrapped, DTG);
								} else {
	...
	private void listenToSolver(final ISolver solver) {
		...
		_contributionsListener = new IContributionsChangedListener() {
			...
			public void removed(final BaseContribution contribution) {
				...
				if (toBeRemoved != null) {
					...
					// .log(
					// IStatus.ERROR,
					// "We were asked to remove a contribution, but we didn't have it stored in the
					// Layer",
					// null);
				}
		...
		_constrainListener = new IConstrainSpaceListener() {
			...
			public void statesBounded(final IBoundsManager boundsManager) {
				...
				for (final BaseContribution cont : conts) {
					...
					if (cont instanceof BearingMeasurementContribution) {
						final BearingMeasurementContribution bmc = (BearingMeasurementContribution) cont;
		...
		solver.getContributions().addContributionsChangedListener(_contributionsListener);
	...
	private void paintThese(final CanvasType dest, final Collection<BoundedState> states) {
		// keep track of the leg name of the previous leg - we
		// use it to track which leg we're in.
		String lastName = null;

		// work through the location bounds
		for (final Iterator<BoundedState> iterator = states.iterator(); iterator.hasNext();) {
		...
		if ((_newRoutes == null) || (_newRoutes.length == 0)) {
			...
			while (legs.hasNext()) {
				...
				if (thisLeg instanceof StraightRoute) {
					...
					ts = new AbsoluteTMASegment(courseDegs, speed, origin, startTime, endTime, thisStamps);
	...
	public void selfScan() {
		...
		for (final Iterator<BaseContribution> iterator = conts.iterator(); iterator.hasNext();) {
			...
			if (!baseC.getDataType().equals(ContributionDataType.ANALYSIS)) {
				if (baseC instanceof BearingMeasurementContribution) {
					...
					wrapped = new CourseForecastWrapper((CourseForecastContribution) baseC);
				} else {
	...
	private void walkRoute(final CompositeRoute[] routes, final RouteStepper stepper) {
