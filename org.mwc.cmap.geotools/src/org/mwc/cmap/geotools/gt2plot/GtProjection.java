...
public class GtProjection extends PlainProjection implements GeoToolsHandler {
	...
	public static class TestProj extends TestCase {
		public void testOne() throws NoSuchAuthorityCodeException, FactoryException, NoninvertibleTransformException {
		...
		public void testTwo() throws NoSuchAuthorityCodeException, FactoryException, NoninvertibleTransformException {
			final MapContent mc = new MapContent();
			...
			System.out.println(
					"test 2:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -4), null));
			System.out.println(
					"test 2:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(44, -4), null));
			System.out.println(
					"test 2:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(43, -4), null));
			System.out.println(
					"test 2:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(42, -4), null));
			System.out.println(
					"test 2:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(41, -4), null));

			// try with series of points
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -5), null));
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -4), null));
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -3), null));
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -2), null));
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, -1), null));
			System.out.println(
					"test 3:" + mc.getViewport().getWorldToScreen().transform(new DirectPosition2D(45, 0), null));
	...
	public GtProjection(final MapContent mapContent) {
		...
		try {
			...
			Application.logError2(-1, "Can't find the requested authority whilst trying to create CRS transform", e);
		} catch (final FactoryException e) {
			Application.logError2(-1, "Unexpected problem whilst trying to create CRS transform", e);
		}
	...
	private WorldArea convertToWorldArea(final Envelope2D newMapArea) throws MismatchedDimensionException,
			org.opengis.referencing.operation.NoninvertibleTransformException, TransformException {
	...
	private DirectPosition2D getDirectPositionOf(final WorldLocation x)
		...
		final DirectPosition2D mapPos = new DirectPosition2D(x.getLong(), x.getLat());
	...
	public boolean layersOverlapWith(final WorldArea area) {
		...
		try {
			...
			while (layers.hasNext()) {
				...
				if (layer.isVisible()) {
					...
					if (thisBounds != null) {
						...
						try {
							newBounds = thisBounds.transform(other.getCoordinateReferenceSystem(), false);
			...
			Application.logError2(1, "unknown dimension trouble with getting bounds", e);
		} catch (final TransformException e) {
			Application.logError2(1, "unknown transform trouble with getting bounds", e);
		} catch (final FactoryException e) {
			Application.logError2(-1, "unknown factory trouble with getting bounds", e);
		}
	...
	private void mySetDataArea(final WorldArea theArea) {
		...
		try {
			...
			Application.logError2(-1, "trouble with proj, probably zoomed out too far", e);
		} catch (final MismatchedDimensionException e) {
	...
	public void setScreenArea(final Dimension theArea) {
		...
		final java.awt.Rectangle screenArea = new java.awt.Rectangle(0, 0, theArea.width, theArea.height);
	...
	public Point toScreen(final WorldLocation val) {
		...
		try {
			...
			// output the results
			res = new Point((int) _workScreen.getCoordinate()[0], (int) _workScreen.getCoordinate()[1]);
		} catch (final MismatchedDimensionException e) {
			Application.logError2(-1, "Whilst trying to convert to screen coords", e);
		} catch (final TransformException e) {
			Application.logError2(-1, "Whilst trying to convert to screen coords", e);
		}
	...
	public WorldLocation toWorld(final Point val) {
		...
		try {
			...
			if (dArea.getWidth() > 0 || dArea.getHeight() > 0) {
				...
				res = new WorldLocation(_workDegs.getCoordinate()[1], _workDegs.getCoordinate()[0], 0);
			}
		} catch (final MismatchedDimensionException e) {
			Application.logError2(-1, "Whilst trying to set convert to world coords", e);
		} catch (final org.opengis.referencing.operation.NoninvertibleTransformException e) {
			Application.logError2(-1, "Unexpected non-invertable problem whilst performing screen to world", e);
		} catch (final TransformException e) {
			Application.logError2(-1, "Unexpected transform problem whilst performing screen to world", e);
		}
		return res;
	}
	...
	public void zoom(double scaleVal) {
		...
		if (dataArea != null) {
			...
			try {
				...
				final DirectPosition2D corner = new DirectPosition2D(mapM.getX() - 0.5d * paneArea.width / newScale,
						mapM.getY() + 0.5d * paneArea.height / newScale);
				...
				Application.logError2(-1, "Unexpected problem whilst performing zoom", e);
			} catch (final org.opengis.referencing.operation.NoninvertibleTransformException e) {
				Application.logError2(-1, "Unable to do inverse transform in zoom", e);
			} catch (final TransformException e) {
				Application.logError2(-1, "Unexpected problem whilst performing", e);
			}
	...
	public void zoom(final double scaleVal, final WorldArea area) {
		...
		try {
			...
			final DirectPosition2D desiredCenter = getDirectPositionOf(area.getCentre());
			final DirectPosition2D actualCenter = getDirectPositionOf(super.getDataArea().getCentre());
			...
			final DirectPosition2D corner = new DirectPosition2D(
					desiredCenter.getX() - 0.5d * paneArea.width / newScale,
					desiredCenter.getY() + 0.5d * paneArea.height / newScale);
			...
			// translate
			newMapArea.setFrameFromDiagonal(newMapArea.getBounds().x + deltaX * scaleVal,
					newMapArea.getBounds().y + deltaY * scaleVal, newMapArea.getBounds().x + width + deltaX * scaleVal,
