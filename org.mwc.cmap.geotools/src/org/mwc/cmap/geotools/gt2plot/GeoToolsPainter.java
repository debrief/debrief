...
public class GeoToolsPainter {
	 ...
	 * @return
	 */
	public static BufferedImage drawAwtImage(final int width, final int height, final GtProjection proj,
			final Color bkColor) {
		...
		final boolean linux = "nix".equalsIgnoreCase(OS_NAME) || "nux".equalsIgnoreCase(OS_NAME)
				|| "aix".equals(OS_NAME);
		if (!linux) {
		...
		final LabelCache labelCache = new LabelCacheImpl();
		rendererParams.put(StreamingRenderer.LABEL_CACHE_KEY, labelCache);
		renderer.setRendererHints(rendererParams);

		final BufferedImage baseImage = new BufferedImage(width + 1, height + 1, BufferedImage.TYPE_INT_ARGB);
