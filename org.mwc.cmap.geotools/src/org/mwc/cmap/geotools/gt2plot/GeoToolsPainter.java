...
public class GeoToolsPainter
{
   ...
   * @param height
   *          height of reuqired image
   * @param proj
   *          projection containing the MapContent
   * @param bkColor
   *          (optional) background color to fill the image
   * @return
   */
      ...
      final GtProjection proj, final Color bkColor)
  {
    ...
    final boolean linux = "nix".equalsIgnoreCase(OS_NAME) || "nux"
        .equalsIgnoreCase(OS_NAME) || "aix".equals(OS_NAME);
    if (!linux)
    {
