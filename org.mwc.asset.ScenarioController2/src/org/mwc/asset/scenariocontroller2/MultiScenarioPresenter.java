...
public class MultiScenarioPresenter extends CoreControllerPresenter {
	...
	public static class TestMe extends TestCase {
		...
		private static class RunnablePresenter extends MultiScenarioPresenter {
			...
			public RunnablePresenter(final MultiScenarioDisplay display, final MultiScenarioCore model) {
				super(display, model);
			}
		...
		public final void NtestHandleBothFiles() {
			...
			final String[] files = { controlPath, scenarioPath };
		...
		public final void NtestHandleControlFile() {
			...
			// ok, try for the scenario first
			final String[] files = { controlPath };
		...
		public final void NtestHandleScenarioFile() {
			...
			// ok, try for the scenario first
			final String[] files = { scenarioPath };
		...
		public void NtestRealDataMultiPart() {
			...
			doAnswer(new Answer<Object>() {
				...
				public Object answer(final InvocationOnMock invocation) {
					...
					final ScenarioType theScenario = (ScenarioType) model.getSimulations().firstElement();
			...
			pres.handleTheseFiles(new String[] { scenarioPath, control_multiP_Path });
		...
		public final void testRelativePathMethod() {
			...
			final MultiScenarioPresenter pres = new MultiScenarioPresenter(mockDisplay, null);

			super.assertEquals("failed to recognise drive", false, pres.isRelativePath(new File("c:\\test.rep")));
			super.assertEquals("failed to root designator", false, pres.isRelativePath(new File("\\test.rep")));
			super.assertEquals("failed to root designator", false, pres.isRelativePath(new File("\\\\test.rep")));
			super.assertEquals("failed to root designator", false, pres.isRelativePath(new File("//test.rep")));
			super.assertEquals("failed to root designator", false, pres.isRelativePath(new File("////test.rep")));
			super.assertEquals("failed to recognise absolute ref", true, pres.isRelativePath(new File("test.rep")));
			super.assertEquals("failed to recognise relative ref", true, pres.isRelativePath(new File("./test.rep")));
			super.assertEquals("failed to recognise parent ref", true, pres.isRelativePath(new File("../test.rep")));
		}
	...
	public MultiScenarioPresenter(final MultiScenarioDisplay display, final MultiScenarioCore model) {
		super(display);
	...
	protected void controllerAssigned(final String controlFile) {
		...
		try {
			...
			CorePlugin.logError(IStatus.ERROR, "Whilst loading " + controlFile, e1);
			return;
		}
		...
		if (controlType == StandaloneObserverListHandler.type) {
			...
			try {
				_scenarioController = ASSETReaderWriter.importThisControlFile(controlFile,
						new java.io.FileInputStream(controlFile));
			} catch (final FileNotFoundException e) {
				CorePlugin.logError(IStatus.ERROR, "failed whilst loading control file", e);
	...
	protected void doFinish() {
		...
		final InstanceWrapper instance = _myModel.getWrapperFor(_currentScen.getScenario());
	...
	protected void doGenerate() {
		...
		final JobWithProgress theJob = new JobWithProgress() {
			...
			public void run(final ASSETProgressMonitor montor) {
				...
				try {
					_myModel.prepareFiles(_controlFileName, _scenarioFileName, System.out, System.err, System.in,
							montor, _scenarioController.outputDirectory);
				} catch (final XPathExpressionException e1) {
					CorePlugin.logError(IStatus.ERROR, "Whilst performing scenario generation", e1);
					return;
				}
				...
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "failed whilst checking type of control file", e);
				}
	...
	protected void doInit() {
		if (_currentScen == null) {
			CorePlugin.logError(IStatus.ERROR, "Should not be able to play, no scenario selected", null);
			return;
		}
		...
		if (instance == null) {
			CorePlugin.logError(IStatus.ERROR, "Should not be able to play, no scenario not found", null);
			return;
		}
		...
		for (int i = 0; i < fileNames.length; i++) {
			...
			if (thisName != null) {
				...
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "Whilst loading " + thisName, e);
					return;
				}
