...
public abstract class CoreCoordinateRecorder {
	...
	public static class ExportDialogResult {
		...
		public void setScaleBarUnit(final String scaleBarUnit) {
			this.scaleBarUnit = scaleBarUnit;
		}

		public void setScaleBarVisible(final boolean scaleBarVisible) {
	...
	public CoreCoordinateRecorder(final Layers layers, final PlainProjection plainProjection,
			final long worldIntervalMillis, final long modelIntervalMillis, final String dateFormat) {
		_myLayers = layers;
	...
	private void calculateScaleWidth(final TrackData td, final PlainProjection proj, final String scaleBarUnit) {
	...
	private ExportResult exportFile(final String fileName, final String exportFile, final String masterTemplateFile,
			final long interval, final String scaleBarUnit, final boolean scaleVisible) {
	...
	public void newTime(final HiResDate timeNow) {
		...
		final OperateFunction outputIt = new OperateFunction() {
			...
			public void operateOn(final Editable item) {
				...
				if (items != null && items.length > 0 && items[0] != null && (!track.isSinglePointTrack()
						|| singlePointTrackIsStarting((FixWrapper) items[0], timeNow))) {
					...
					if (tp == null) {
						...
						final int waitingSteps = _times.size() - 1;
					...
					final TrackPoint trackPoint = new TrackPoint((float) (screenHeight - point.getY()),
							(float) point.getX(), (float) fix.getLocation().getDepth(), fix.getDTG().getDate(),
							_dateFormat.format(_times.get(_times.size() - 1).getDate()));
	...
	private synchronized void setupUnits() {
		...
		_unitsList.put(MWC.GUI.Properties.UnitsPropertyEditor.KM_UNITS, new UnitsConverter() {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
		...
		_unitsList.put(MWC.GUI.Properties.UnitsPropertyEditor.METRES_UNITS, new UnitsConverter() {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
		...
		_unitsList.put(MWC.GUI.Properties.UnitsPropertyEditor.NM_UNITS, new UnitsConverter() {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
		...
		_unitsList.put(MWC.GUI.Properties.UnitsPropertyEditor.YDS_UNITS, new UnitsConverter() {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
		...
		_unitsList.put(MWC.GUI.Properties.UnitsPropertyEditor.KYD_UNITS, new UnitsConverter() {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
	...
	private boolean singlePointTrackIsStarting(final FixWrapper point, final HiResDate timeNow) {
		return (point.getDTG().equals(timeNow) || (timeNow.greaterThan(point.getDTG()) && !_times.isEmpty()
				&& point.getDTG().greaterThan(_times.get(_times.size() - 1))));
	}
	...
	public void stopStepping(final HiResDate now) {
		...
		if (_tracks.values().size() > PlotTracks.MARKER_FOOTPRINT_DELTA) {
			...
			MWC.GUI.Dialogs.DialogFactory.showMessage("Export to PPTX Errors",
					"There are too many tracks. No more than " + PlotTracks.MARKER_FOOTPRINT_DELTA
			...
			if (dialogResult.getStatus()) {
				final ExportResult expResult = exportFile(dialogResult.fileName, dialogResult.selectedFile,
						dialogResult.masterTemplate, interval, dialogResult.getScaleBarUnit(),
						dialogResult.isScaleBarVisible());
	...
	private void storeNarrativesInto(final ArrayList<ExportNarrativeEntry> narrativeEntries, final Layers layers,
			final Map<String, Track> tracks, final long startTime) {
		...
		if (narratives != null) {
			...
			final TimePeriod period = new TimePeriod.BaseTimePeriod(new HiResDate(firstTime), new HiResDate(lastTime));
			...
			while (nIter.hasMoreElements()) {
				...
				if (period.contains(new HiResDate(entry.getDTG()))) {
					...
					final ExportNarrativeEntry newE = new ExportNarrativeEntry(entry.getEntry(), dateStr, elapsedStr,
							entry.getDTG().getDate());
