...
/**
 * embedded class providing customized SwingChart used to provide an overview
 * shot.
 *
 */
final class SwingOverviewChart extends SwingChart implements java.beans.PropertyChangeListener {
	...
	/**
	 * our own zoom in tool, which allows us to view a zoomed in portion of the
	 * plot.
	 */
	 ...
	 */
	public SwingOverviewChart(final Layers theLayers, final PlainChart theChart, final ToolParent theParent) {
		super(theLayers);
		...
		// we also want to listen out for the projection of the target chart being
		// changed
	...
	public final void mouseClicked(final MouseEvent p1) {
		if (p1.getClickCount() == 2) {
			...
			// use the pan tool to move the current view (and put the operation on the
			// clipboard)
