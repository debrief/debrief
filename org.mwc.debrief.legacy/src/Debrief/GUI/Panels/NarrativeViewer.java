...
public final class NarrativeViewer extends MWC.GUI.Properties.Swing.SwingCustomEditor
		implements MWC.GUI.StepperListener, MWC.GUI.Properties.NoEditorButtons {
	...
	final class myDTGRenderer implements TableCellRenderer {
		 ...
		 */
		@Override
		public final Component getTableCellRendererComponent(final JTable p1, final Object p2, final boolean isSelected,
				final boolean p4, final int row, final int column) {
	...
	final class myEntryRenderer implements TableCellRenderer {
		 ...
		 */
		@Override
		public final Component getTableCellRendererComponent(final JTable p1, final Object object,
				final boolean isSelected, final boolean p4, final int row, final int column) {
	...
	public static void main(final String[] args) {
		...
		for (int i = 0; i < 15; i++) {
			...
			nw.add(new NarrativeEntry("TRK", new HiResDate(2534234234l + i * 1222054300), ":" + ct++
					+ " some text2 some text2 some text2 some text2 some text2 some text2 some text2 some text2 some text2 some text2 "));
	...
	/**
	 * handle the close event, so that we can remove ourselves from being a stepper
	 * listener
	 */
	@Override
	...
	void doResize() {
		...
		for (int i = 0; i < _myTable.getRowCount(); i++) {
			...
			final Component c = entries.getCellRenderer().getTableCellRendererComponent(_myTable,
					_myTable.getValueAt(i, 2), false, false, i, 1);
	...
	private JComponent getForm() {
			...
			public void mouseClicked(final java.awt.event.MouseEvent e) {
				...
				if (e.getClickCount() == 2) {
					...
					if (oj instanceof MWC.TacticalData.NarrativeEntry) {
						final MWC.TacticalData.NarrativeEntry ne = (MWC.TacticalData.NarrativeEntry) oj;
		...
		// create the data model, over-riding it slightly to ensure no elements are
		// editable
		...
		// date format
		final MWC.GUI.Properties.DateFormatPropertyEditor.SwingDateFormatEditor sf = new MWC.GUI.Properties.DateFormatPropertyEditor.SwingDateFormatEditor() {
			/**
					 *
					 */
			private static final long serialVersionUID = 1L;
	...
	private void initForm() {
		...
		if (_theStepper != null) {
			...
			MWC.Utilities.Errors.Trace
					.trace("Sorry, the narrative wasn't loaded properly. It won't be able to follow the time step");
		}
	}
	...
	/**
	 * the user has double-clicked on a narrative entry, cause the time stepper to
	 * move to that point
	 *
	...
	void newDateFormat(final String format) {
		...
		// now resize, since the width of the first column may have changed, if we have
		// data
		if (_myTable != null)
			doResize();
	}
	...
	/**
	 * method to select the narrative entry nearest to the current time in the time
	 * stepper
	 *
	...
	void newTime(final HiResDate time) {
		...
		for (int i = 0; i < _myTable.getRowCount(); i++) {
			...
			/*
			 * // get the narrative entry component Component c =
			 * entries.getCellRenderer().getTableCellRendererComponent(_myTable,
			 * _myTable.getValueAt(i, 2), false, false, i, 1); // cast to the entry
	...
	/**
	 * ////////////////////////////////////////////////// ignore this really, we
	 * don't mind if the user has switched to snail mode
	 *
