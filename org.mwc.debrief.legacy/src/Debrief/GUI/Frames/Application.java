    ...
    FileDropSupport.FileDropListener
{
  ...
  /**
   * keep track of whether we have provided our "app props missing" report. we only need to report
   * it once.
   */
  ...
  /**
   * allow us to override the toolparent responsibilities of the application (particularly in CMAP);
   */
  ...
  /**
   * add the set of tools we normally use for an application (menu and top toolbar)
   */
  private void addTools()
  {
    ...
    _theMenus.addElement(new MenuItemInfo("File", null, "Open Plot",
        new OpenPlotXML(this, null, this), new MenuShortcut(KeyEvent.VK_O),
        ' '));

    _theMenus.addElement(new MenuItemInfo("File", null, null, null, null, ' '));
    _theMenus.addElement(new MenuItemInfo("File", null, "Import Replay",
    ...
    /*
     * hey, cut/copy/paste don't do anything! _theMenus.addElement(new MenuItemInfo("Edit", null,
     * "Cut", null, null, ' ')); _theMenus.addElement(new MenuItemInfo("Edit", null, "Copy", null,
     * null, ' ')); _theMenus.addElement(new MenuItemInfo("Edit", null, "Paste", null, null, ' '));
     */

    _theTools.addElement(new MenuItemInfo("File", null, "New", new NewSession(
        this), null, ' '));
    _theTools.addElement(new MenuItemInfo("File", null, "Open", new OpenPlotXML(
        this, null, this), null, ' '));
  ...
  /**
   * go through the list of tool buttons we have defined, and create the GUI element for each
   */
  ...
  /**
   * go through the list of menu elements we have defined, and create the GUI element for each
   */
  private void setupMenu()
  {
    ...
    while (iter.hasMoreElements())
    {
      ...
      {
        addMenuItem(thisItem.getMenuName(), thisItem.getMenuItemName(), thisItem
            .getTool(), thisItem.getShortCut());
      }
    }
  }
  ...
  protected abstract void addMenuItem(final String theMenu,
      final String theLabel, final Tool theTool,
      ...
      if (pv instanceof Debrief.GUI.Views.AnalysisView)
      {
        ...
        for (int i = 0; i < len; i++)
        {
          ...
          if (ly instanceof MWC.TacticalData.NarrativeWrapper)
          {
            ...
            final MWC.TacticalData.NarrativeWrapper nw =
                (MWC.TacticalData.NarrativeWrapper) ly;
  ...
  public final void actionPerformed(final java.awt.event.ActionEvent p1)
  {
    ...
    openFile(theFile);
    /*
     * // sort out the suffix of the file String theSuffix=null; int pos = fName.lastIndexOf(".");
     * theSuffix = fName.substring(pos, fName.length()); if(theSuffix.equalsIgnoreCase (".xml")) {
     * final Action op = new OpenPlot.OpenPlotAction(fName, this); op.execute(); }
     */
  ...
  public static void logError3(int status, String text, Exception e,
      boolean revealLog)
  {
    if (_substituteParent != null)
    {
   ...
   * @param e
   */
  public static void logError2(final int status, final String text,
      final Exception e)
  {
    if (_substituteParent != null)
    {
