...
public abstract class Application implements ToolParent, ActionListener, FileDropSupport.FileDropListener {
	 ...
	 */
	public static Map<String, String> getPropertiesLikeThis(final String pattern) {
	...
	private static String getStatusFor(final int status) {
	...
	private static void initProps(final ApplicationProperties appProps) {
	 ...
	 * @param e
	 */
	public static void logError2(final int status, final String text, final Exception e) {
		if (_substituteParent != null) {
	...
	public static void logError3(final int status, final String text, final Exception e, final boolean revealLog) {
		if (_substituteParent != null) {
	 ...
	 * @param e
	 */
	public static void logStack2(final int status, final String message) {
		if (_substituteParent != null) {
	...
	public Application() {
		...
		ImportManager.addImporter(new DebriefXMLReaderWriter(this));

		try {
			final String header = System.getProperty("line.separator") + "#Debrief 2003 Properties File";
			_appProps = new MWC.GUI.Dialogs.ApplicationProperties("d2ksettings.prp", header);
	 ...
	 *
	 * @param theMenu     the name of the menu to add the item to
	 * @param theLabel    the text to use as the menu item
	 * @param theTool     callback function for menu item to trigger
	 * @param theShortCut shortcut to be applied to the menu item
	 */
	protected abstract void addMenuItem(final String theMenu, final String theLabel, final Tool theTool,
			final MenuShortcut theShortCut);

	/**
	 * create a menu separator
	 *
	 * @param theMenu the menu to insert the separator into
	 */
	...
	private void addTools() {
		_theMenus.addElement(new MenuItemInfo("File", null, "New", new NewSession(this, false),
				new MenuShortcut(KeyEvent.VK_N), ' '));
		_theMenus.addElement(new MenuItemInfo("File", null, "New (default plot)", new NewSession(this, true),
				new MenuShortcut(KeyEvent.VK_D), ' '));

		_theMenus.addElement(new MenuItemInfo("File", null, null, null, null, ' '));
		_theMenus.addElement(new MenuItemInfo("File", null, "Open Plot", new OpenPlotXML(this, null, this),
				new MenuShortcut(KeyEvent.VK_O), ' '));

		_theMenus.addElement(new MenuItemInfo("File", null, null, null, null, ' '));
		_theMenus.addElement(new MenuItemInfo("File", null, "Import Replay", new ImportData2(this, this, null),
				new MenuShortcut(KeyEvent.VK_M), ' '));

		_theMenus.addElement(new MenuItemInfo("File", null, null, null, null, ' '));
		// this gap is for our MRU items
		_theMenus.addElement(new MenuItemInfo("File", null, null, null, null, ' '));
		_theMenus.addElement(new MenuItemInfo("File", null, "Close Session", new CloseSession(this),
				new MenuShortcut(KeyEvent.VK_F4), ' '));

		_theMenus.addElement(new MenuItemInfo("File", null, "Exit", new ExitApplication(this),
				new MenuShortcut(KeyEvent.VK_X), ' '));

		_theMenus.addElement(
				new MenuItemInfo("Edit", null, "Undo", new Undo(this), new MenuShortcut(KeyEvent.VK_Z), ' '));
		_theMenus.addElement(
				new MenuItemInfo("Edit", null, "Redo", new Redo(this), new MenuShortcut(KeyEvent.VK_Y), ' '));
		/*
		 * hey, cut/copy/paste don't do anything! _theMenus.addElement(new
		 * MenuItemInfo("Edit", null, "Cut", null, null, ' ')); _theMenus.addElement(new
		 * MenuItemInfo("Edit", null, "Copy", null, null, ' '));
		 * _theMenus.addElement(new MenuItemInfo("Edit", null, "Paste", null, null, '
		 * '));
		 */

		_theTools.addElement(new MenuItemInfo("File", null, "New", new NewSession(this), null, ' '));
		_theTools.addElement(new MenuItemInfo("File", null, "Open", new OpenPlotXML(this, null, this), null, ' '));
		_theTools.addElement(new MenuItemInfo("File", null, "Close Session", new CloseSession(this), null, ' '));
		_theTools.addElement(new MenuItemInfo("File", null, "Exit", new ExitApplication(this),
				new MenuShortcut(KeyEvent.VK_X), ' '));
	 ...
	 *
	 * @param theSession the Session to close
	 */
	...
	public final void FilesReceived(final java.util.Vector<File> files) {
		...
		try {
			...
			while (iter.hasMoreElements()) {
				final java.io.File file = iter.nextElement();
				openFile(file);
			}
	...
	private void fillToolbar() {
		...
		while (iter.hasMoreElements()) {
			...
			_theToolbar.addTool(thisItem.getTool(), thisItem.getShortCut(), thisItem.getMnemonic());
		}
	}
	 ...
	 */
	@Override
	public final Map<String, String> getPropertiesLike(final String pattern) {
	...
	public final void openFile(final File theFile) {
		try {
			...
			if (suff.equalsIgnoreCase(".DPL")) {
				MWC.GUI.Dialogs.DialogFactory.showMessage("Open File", "Sorry DPL file format no longer supported");
			} else {
				if ((suff.equalsIgnoreCase(".REP")) || (suff.equalsIgnoreCase(".DSF"))
						|| (suff.equalsIgnoreCase(".DTF"))) {
					newLayers = new Layers();

					BaseImportCaller caller = new BaseImportCaller(fList, newLayers) {
	...
	private void setupMenu() {
		...
		while (iter.hasMoreElements()) {
			...
			if (thisItem.getMenuItemName() == null) {
				...
				addMenuItem(thisItem.getMenuName(), thisItem.getMenuItemName(), thisItem.getTool(),
						thisItem.getShortCut());
			}
		}
	}
