...
abstract public class StepControl implements Editable, MWC.Utilities.Timer.TimerListener,
		java.beans.PropertyChangeListener, StepperListener.StepperController

{
	...
	///////////////////////////////////////////////////////////
	// property editor to let us add our extra (T-Zero related) formats to the dates
	/////////////////////////////////////////////////////////// list
	///////////////////////////////////////////////////////////
	...
	static public final class somePeriod {
		...
		public final void extend(final somePeriod other) {
			if (other != null && other._start != null && other._start.lessThan(_start))
				_start = other._start;
		...
		public final String toString() {
			final String res = " start:" + DebriefFormatDateTime.toStringHiRes(_start) + " end:"
	...
	public final class StepControlInfo extends Editable.EditorType {
		...
		public final MethodDescriptor[] getMethodDescriptors() {
			...
			final MethodDescriptor[] mds = { method(c, "editHighlighter", null, "Edit Highlighter"),
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { displayProp("StepSmall", "Small step", "the small step size"),
				...
				// right, special processing here. If we're in hi-res mode, we want to specify
				// the very
				// hi res timers.
	...
	/**
	 * the list of participants from which we produce our time period. This
	 * hashtable actually contains a list of valid time periods for our watchables,
	 * indexed by the watchables themselves. The values for time period (expressed
	 * as a somePeriod) for a participant are updated following a property change
	 * from that item
	 */
	 ...
	 * want to represent a date in a T-Zero style we store the text pattern (eg T+
	 * mm:ss) in this String, knowing it may be an invalid pattern
	 */
	...
	/**
	 * add a new exercise participant to the list we monitor, providing the start
	 * and stop times aswell
	 *
	 * @param participant
	 * @param start
	 * @param end
	...
	/**
	 * retrieve the default highlighter - used primarily to show secondary
	 * highlights
	 *
	...
	/**
	 * private method to produce a time string from the indicated DTG we've moved it
	 * out of the newTime box so that we can access it from a tester
	 */
	...
	/**
	 * method intended to be overwritten by concrete classes, to find out that the
	 * painter has been defined
	 */
	...
	/**
	 * indicate that we no longer have a time period (this implementation favours
	 * D-Lite, TimeManager provides equivalent support in full Debrief)
	 */
	public void reset() {
	...
	private boolean validTime(final long val) {
		...
		if ((_startTime != null && val >= _startTime.getMicros()) && (_endTime != null && val <= _endTime.getMicros()))
			res = true;
