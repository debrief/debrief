...
final class SnailDrawTrack {
	...
	/**
	 * helper class that assists in the production of a graduated colors that blend
	 * into the background
	 *
	 * @author ian
	 *
	...
	public static class PhasedColorDef {
		...
		public PhasedColorDef(final Color mainCol, final Color backColor, final int numShades) {
			...
			/**
			 * NOTE that we are working in floats for all of the color stuff - if we were to
			 * work in ints, then when we want more than 255 shades, the deltas become zero
			 * and the track dissappears. By working in floats we can provide very fine
			 * deltas, allowing very large numbers of points to be tidily plotted in the
			 * track
			 */
	...
	private static void drawDot(final Point loc, final java.awt.Graphics dest, final int size, final Rectangle area) {
	...
	// member functions
	//////////////////////////////////
	public final Rectangle drawMe(final MWC.Algorithms.PlainProjection proj, final java.awt.Graphics dest,
			final Watchable watch, final SnailPainter parent, final HiResDate dtg, final Color backColor) {
		...
		if (myList != null) {
			...
			if (trk instanceof LightweightTrackWrapper) {
				...
				dotPoints = track.getUnfilteredItems(new HiResDate(0, dtg.getMicros() - _trailLength),
			...
			if (dotPoints.size() > 0) {
				...
				while (iter.hasNext()) {
					...
					if (gw.getLabelShowing()) {
						// set the font to the current font for the fix (so that we get the metrics
						// right)
