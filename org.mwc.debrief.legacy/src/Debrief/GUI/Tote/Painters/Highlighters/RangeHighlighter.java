...
public final class RangeHighlighter implements PlotHighlighter {
	...
	public static final class RangeHighlightInfo extends Editable.EditorType {
		...
		public final java.beans.PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final java.beans.PropertyDescriptor[] res = { prop("Color", "Color to paint highlight"),
						prop("Radius", "Radius of outer ring (yds)"), prop("Arcs", "Angle of arcs each side (degs)"),
	...
	private void drawRangeRings(final WorldLocation worldCentre, final double worldRadius, final int rawAxis,
			final int arcs, final int rings, final int spoke_separation, final CanvasType dest) {
		...
		final WorldLocation worldEdge = worldCentre.add(new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(rawAxis),
		...
		final Point edge = new Point((int) (radius * Math.sin(axisRads)), -(int) (radius * Math.cos(axisRads)));
		...
		/**
		 * NOTE we only draw the stalks if we are NOT drawing arcs of 180 degrees either
		 * side (since in that case drawing it twice effectively hides it
		 */
		...
		if (arcs != 180) {
			...
			edge.setLocation((int) (radius * Math.sin(startRads)), -(int) (radius * Math.cos(startRads)));
			edge.translate(centrex, centrey);
			dest.drawLine(centrex, centrey, edge.x, edge.y);

			// and the end edge stalk
			edge.setLocation((int) (radius * Math.sin(endRads)), -(int) (radius * Math.cos(endRads)));
			edge.translate(centrex, centrey);
			dest.drawLine(centrex, centrey, edge.x, edge.y);
		}
		...
		while ((thisSpoke < arcs) && (spoke_separation != 0)) {
			...
			edge1.setLocation((int) (radius * Math.sin(spokeRads1)), -(int) (radius * Math.cos(spokeRads1)));
			edge2.setLocation((int) (radius * Math.sin(spokeRads2)), -(int) (radius * Math.cos(spokeRads2)));
		...
		for (int i = 0; i < rings; i++) {
			...
			if (dest instanceof ExtendedCanvasType) {
				...
				ed.semiFillArc(origin.x, origin.y, thisRadius * 2, thisRadius * 2, startAngle, angle);
			} else
	...
	private void drawRectangle(final MWC.GenericData.Watchable watch, final java.awt.Graphics dest,
			final MWC.Algorithms.PlainProjection proj, final int mySize) {
	...
	public final void highlightIt(final MWC.Algorithms.PlainProjection proj, final java.awt.Graphics dest,
		...
		if (isJustPlotPrimary() && isPrimary) {
			...
			final int header = (int) MWC.Algorithms.Conversions.Rads2Degs(watch.getCourse());
			final CanvasAdaptor can = new CanvasAdaptor(proj, dest);

			drawRangeRings(center, _radius, header, _arcs, _rings, _spoke_separation, can);
	 ...
	 * get the arcs either side of ownship heading
	 *
	 * @param val range of arcs in degrees
	 */
	...
	/**
	 * /////////////////////////// whether just a plain rectangle should be plotted
	 * (ignoring the rings)
	 *
	 */
	 ...
	 * rings
	 *
	 * @param col the colour, of course
	 */
	...
	/**
	 * ////////////////////////////// the number of rings between the centre and the
	 * outer spoke
	 *
	 ...
	 *
	 * @param val separation of arcs in degrees
	 */
