...
final class SnailDrawTrack2 {
	...
	public final Rectangle drawMe(final MWC.Algorithms.PlainProjection proj, final java.awt.Graphics dest,
			final Watchable watch, final TotePainter parent, final HiResDate dtg, final ColorFadeCalculator fader) {
		...
		if (myList != null) {
			...
			if (trk instanceof LightweightTrackWrapper) {
				...
				final HiResDate trailTime = new HiResDate(0, dtg.getMicros() - _trailLength);

				// Let's add at the beginning the tail if we are interpolating.
				dotPoints = new Vector<Editable>(0, 1);

				if (track instanceof TrackWrapper && ((TrackWrapper) track).getInterpolatePoints()) {
				...
				dotPoints.addAll(track.getUnfilteredItems(trailTime, new HiResDate(0, dtg.getMicros() + 2000)));
	...
	private java.awt.Rectangle drawTrailFromPoints(final MWC.Algorithms.PlainProjection proj,
			final java.awt.Graphics dest, final ColorFadeCalculator fader, java.awt.Rectangle thisR,
