 ...
 */
public class TotePainter implements StepperListener, CanvasType.PaintListener, Editable {
	...
	public static void extractFromNotWatchableList(final Layer thisLayer, final Vector<Plottable> res) {
	...
	public static void extractFromWatchableList(final Layer thisLayer, final Vector<Plottable> res) {
	 ...
	 *
	 * @param theData the layer to extract the watchables for
	 ...
	 *
	 * @param theData the layers to extract the watchables for
	 ...
	 */
	protected static final void highlightIt(final PlainProjection proj, final Graphics dest, final Watchable watch) {
	...
	/**
	 * the tote whose data we are plotting. We keep track of this to determine which
	 ...
	 * @param theData  the data we will read from (to find the list of things to
	 *                 plot, aswell as the nearest point to the indicated time)
	 * @param theTote  the Tote itself, which we use to determine which are the
	 *                 primary and secondary tracks to highlight
	 */
	public TotePainter(final PlainChart theChart, final Layers theData, final AnalysisTote theTote) {
	...
	public void createNewHighlights(final HiResDate newDTG, final WatchableList primaryTrack,
			final boolean plottingSymbols, final HashMap<Watchable, WatchableList> newHighlights,
			final Watchable newPrimary, final Graphics dest, final PlainProjection proj) {
		...
		while (newies.hasNext()) {
			...
			if (plottingSymbols)
				getCurrentPrimaryHighlighter().highlightIt(proj, dest, thisTrack, newWt, isPrimary);
			else
				getCurrentSecondaryHighlighter().highlightIt(proj, dest, thisTrack, newWt, isPrimary);
		}
	...
	public void listNewHighlights(final HiResDate newDTG, final HashMap<Watchable, WatchableList> newHighlights) {
		Watchable[] list;
	...
	public void newTime(final HiResDate oldDTG, final HiResDate newDTG, final CanvasType canvas) {
		...
		if (oldPrimary == null && oldDTG != null) {
			/**
			 * there is a chance that we already have an oldHighlights object - since there
			 * may be a primary track assigned, but that no points were visible, in which
			 * case we don't need to re-create the old highlights track
			 */
			if (oldHighlights == null)
				oldHighlights = new HashMap<Watchable, WatchableList>();

			final MWC.GenericData.Watchable[] list = primaryTrack.getNearestTo(oldDTG);
			if (list.length > 0)
				oldPrimary = list[0];
		}
		...
		if (theParticipants != null) {
			...
			while (iter.hasMoreElements()) {
				...
				if (oj instanceof WatchableList) {
					...
					// check if this watchable found is visible
					final MWC.GenericData.Watchable[] theseMatches = thisList.getNearestTo(newDTG);
					final Watchable wat = theseMatches.length > 0 ? theseMatches[0] : null;
		...
		// Get the graphics
		final CanvasType theCanvas = canvas != null ? canvas : _theChart.getCanvas();
		...
		createNewHighlights(newDTG, primaryTrack, plottingSymbols, newHighlights, newPrimary, dest, proj);
	...
	public void removeOldHighlights(final WatchableList primaryTrack, final boolean plottingSymbols,
			final CanvasType theCanvas, final Graphics dest, final PlainProjection proj) {
		...
		if (_doXOR) {
			...
			while (oldies.hasNext()) {
				...
				if (_firstStep) {
					...
					final boolean isPrimary = thisTrack == primaryTrack;
					if (plottingSymbols) {
						getCurrentPrimaryHighlighter().highlightIt(proj, dest, thisTrack, oldWt, isPrimary);
					} else
						getCurrentSecondaryHighlighter().highlightIt(proj, dest, thisTrack, oldWt, isPrimary);
				}
