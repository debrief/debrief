...
public final class ReformatFixes implements FilterOperation {
	...
	private static final class ReformatAction implements Action {
		 ...
		 *
		 */
		public ReformatAction(final String theName, final Object newValue, final Layers theLayers) {
			_myName = theName;
			...
			for (int i = 0; i < _myEdits.size(); i++) {
				...
				try {
					...
					MWC.Utilities.Errors.Trace.trace(e,
							"Failed whilst updating " + setterHolder._object + " to " + _newValue);
				}
			}
		...
		public void undo() {
			...
			for (int i = 0; i < _myEdits.size(); i++) {
				...
				try {
					...
					MWC.Utilities.Errors.Trace.trace(e,
							"Failed whilst undoing " + setterHolder._object + " to " + _newValue);
				}
			}
	...
	/**
	 * embedded class acting a structure to contain an object, and it's setter
	 * method
	 *
	 *
	 */
	private final static class SetterHolder {
		 ...
		 */
		public SetterHolder(final Editable object, final Method setter, final Method getter, final String propertyName,
	...
	static public final class testListOfProperties extends junit.framework.TestCase {
		...
		private Vector<WatchableList> getTestObjects(final int num) {
			...
			final Debrief.Wrappers.ShapeWrapper sw = new Debrief.Wrappers.ShapeWrapper("bingo", theShape,
			...
			// and a label
			final Debrief.Wrappers.LabelWrapper lw = new Debrief.Wrappers.LabelWrapper("here", theLocation,
		...
		public void NtestGetProperties() {
			...
			final HashMap<String, SetterHolder> props = getSuitablePropertiesFor(tw,
					ReformatFixes.getPropertyManager());
...
//      }
		...
		public void testMatchingAnnotations() {
			...
			final Vector<SetterHolder> theSetters = commonProps.get("Color");
		...
		for (int i = 0; i < theObjects.size(); i++) {
			...
			if (!propertiesForThisObject.isEmpty()) {
				...
				if (theProperties == null) {
					...
					for (final Iterator<String> iterator = propertiesForThisObject.keySet().iterator(); iterator
							.hasNext();) {
	...
	/**
	 * examine this object, and find a property editor for it (self-defined or
	 * auto-defined)
	 *
	 * @param thisProperty
	 * @return
	...
	static HashMap<String, SetterHolder> getSuitablePropertiesFor(final WatchableList thisList,
		...
		final WatchableList thisObject = thisList;
			...
			for (int j = 0; j < pd.length; j++) {
				...
				if (pe != null) {
					...
					if (thisEditorIsSuitable(pe)) {
						final SetterHolder sh = new SetterHolder(thisList, thisProperty.getWriteMethod(),
								thisProperty.getReadMethod(), thisProperty.getName(), pe);
	...
	public final String getDescription() {
		...
		res += _theSeparator + "====================";
		res += _theSeparator
				+ "This operations allow the appearance of annotations and track positions within specified periods to be changed";
		return res;
	}
	...
	private Vector<SetterHolder> getProperty(final HashMap<String, Vector<SetterHolder>> propertyList) {
		...
		final Object val = JOptionPane.showInputDialog(null, "Which property do you wish to edit?", "Reformat objects",
				JOptionPane.QUESTION_MESSAGE, null, resList, null);
	 ...
	 */
	private Object getSelectedNewValueFromList(final SetterHolder firstEditor, final String[] tags,
			final PropertyEditor pe) {
		Object newVal;
		// get the new value
		final String col = (String) JOptionPane.showInputDialog(null,
				"Which value do you want to use for " + firstEditor._propertyName + " ?", "Reformat fixes",
				JOptionPane.QUESTION_MESSAGE, null, tags, null);
	...
	private Object getSelectedNewValueFromText(final SetterHolder firstEditor, final PropertyEditor pe) {
		Object newVal;
		// get the new value
		final String col = JOptionPane.showInputDialog(null,
				"What value do you want to use for " + firstEditor._propertyName + " ?", "Reformat data",
	...
	private String getTypeToSelect() {
		...
		// find out which one the user wants to edit
		res = (String) JOptionPane.showInputDialog(null, "Which type of item do you wish to edit?", _myDescription,
				JOptionPane.QUESTION_MESSAGE, null, types, ALL_ITEMS);
	 ...
	 * @return
	 */
	ReformatAction performTheOperation(final SetterHolder sampleEditor, final Object newValue,
			final Vector<SetterHolder> theSetters, final String position_type) {
		...
		while (iter.hasMoreElements()) {
			...
			// pass through our items and check if editing them is applicable. If so, create
			// some
			...
			if (validItems != null) {
				...
				for (final Iterator<Editable> iterator = validItems.iterator(); iterator.hasNext();) {
					...
					res.submitNewEdit(
							new SetterHolder(editable, wl._setter, wl._getter, wl._propertyName, wl._propertyEditor));
