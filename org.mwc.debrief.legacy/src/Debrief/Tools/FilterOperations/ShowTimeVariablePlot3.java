...
public final class ShowTimeVariablePlot3 implements FilterOperation {
	static public class CalculationHolder {
		...
		public CalculationHolder(final toteCalculation theCalcVal, final formattingOperation theFormatterVal,
				final boolean isRelative, final double clipMax) {
			this(theCalcVal, theFormatterVal, isRelative, clipMax, null);
		}

		public CalculationHolder(final toteCalculation theCalcVal, final formattingOperation theFormatterVal,
				final boolean isRelative, final double clipMax, final CalculationWizard wizard) {
			_theCalc = theCalcVal;
		 ...
		 * case we do not need to plot the primary track, but we only plot calculated
		 * data where both primary and secondary data are present
		 *
		 * @return yes/no
		 */
	...
	static public final class MySwingPlot extends SwingPlot {
		 ...
		 * @param theParent
		 */
		public MySwingPlot(final JPanel thePlot, final PropertiesPanel theParent, final NewFormattedJFreeChart xyPlot) {
	...
	/**
	 * worker bee type class which allows us polymorphically to add either a number
	 * x-y pair to an x-y series, or to add a TimePeriod value to a BasicTimeSeries
	 *
	 * @see BasicTimeSeries
	 * @see XYSeries
	 */
	private static interface VersatileSeriesAdder {
		 ...
		 *
		 * @param thisSeries        ther destination for the point
		 * @param theTime           the DTG
		 * @param data              the data item
		 * @param thisColor         the colour of this point
		 * @param connectToPrevious whether to connect to the previous one
		 * @param provider          something to do something with.
		 * @param parentIsVisible   TODO
		 */
		void add(Series thisSeries, HiResDate theTime, double data, Color thisColor, boolean connectToPrevious,
	...
	/**
	 * interpolate to determine the time at which the course would pass through zero
	 * degrees
	 *
	 * @param last_time     the time of the previous course value
	 * @param current_time  the time of the current course value
	 * @param lastCourse    the last course value
	 * @param currentCourse the current course value
	 * @param clipMax
	 * @return the time at which the course would pass through zero
	 */
	private static final long calcZeroCrossingTime(final long last_time, final long current_time,
			final double lastCourse, final double currentCourse, final double clipMax) {
		long res = 0;
		double delta, range;
	...
	private static boolean createDataPoint(final toteCalculation theCalculation, final Watchable thisPrimary,
		...
		if (Double.isNaN(data)) {
			...
			theAdder.add(thisSeries, currentTime, data, thisColor, connectToPrev, provider, parentIsVis);
	 ...
	 *
	 * @param primaryTrack the primary track
	 * @param myOperation  the calculation we're making
	 * @param theTracks    the selected set of tracks
	 * @param start_time   the start time selected
	 * @param end_time     the end time selected
	 * @param provider     the provider of the time offset used when plotting
	 *                     time-zero data
	...
	public static AbstractSeriesDataset getDataSeries(final WatchableList primaryTrack,
		...
		while (iter.hasMoreElements()) {
			...
			//
			final Collection<Editable> secondaryItems = thisSecondaryTrack.getItemsBetween(start_time, end_time);
			...
			// at the end of this method to stop us adding empty data series to the
			// collection.
			if (secondaryItems == null) {
				Application.logError2(ToolParent.WARNING, "Insufficient points found in secondary track."
						+ "\nPlease check coverage of time controller bars", null);
				continue;
			} else {
				...
				if (myOperation.isARelativeCalculation()) {
					...
					// data)
					final boolean secSingleton = (thisSecondaryTrack instanceof LightweightTrackWrapper)
					...
					if (thisSecondaryTrack.getStartDTG() == null || secSingleton) {
						...
						if (primaryTrack.getStartDTG() == null
								|| primaryTrack.getStartDTG().equals(primaryTrack.getEndDTG())) {
							...
							if (primaryPoints != null) {
								...
								produceTwoPointDataSeries(theCalculation, thisPrimary, theSecondaryPoint, thisSeries,
							...
							if (primaryPoints != null) {
								...
								while (throughPrimary.hasNext()) {
									...
									// to)
									connectToPrevious = createDataPoint(theCalculation, thisPrimary, theSecondaryPoint,
					...
					if (thisSecondaryTrack.getStartDTG() == null) {
						...
						// and
						produceTwoPointDataSeries(theCalculation, null, thisSecondary, thisSeries, start_time, end_time,
								provider, theAdder);
	 ...
	 *
	 * @param lastSecondaryValue the last value calculated
	 * @param thisVal            this calculated value
	 * @param lastTime           the time of the last calculation
	 * @param currentTime        the time of the current calculation
	 * @param thisColor          the colour of this data point
	 * @param thisSeries         the data series we add our new point(s) to
	 * @param theAdder
	 * @param clipMax
	 * @param connectToPrevious  whether the next data point should connect to these
	 *
	 * @return whether the next line segment should connect to this one
	 */
	private static boolean insertWrappingPoints(final double lastSecondaryValue, final double thisVal,
			final HiResDate lastTime, final HiResDate currentTime, final Color thisColor, final Series thisSeries,
		...
		if ((!Double.isNaN(lastSecondaryValue))) {
			...
			if (beyondThreshold) {
				...
				zeroTime = ShowTimeVariablePlot3.calcZeroCrossingTime(startTime, endTime, startCourse, endCourse,
						clipMax);
				...
				try {
					...
					// one.
					theAdder.add(thisSeries, firstDate, firstCourse, thisColor, true, provider, false);

					// now that we use the inverse of the firstPoint value
					// to
					// indicate whether to join the point to it's previous
					// one.
					theAdder.add(thisSeries, secondDate, secondCourse, thisColor, false, provider, false);
	 ...
	 * @param end_time
	 */
	private static void produceTwoPointDataSeries(final toteCalculation theCalculation, final Watchable thisPrimary,
			final Watchable thisSecondary, final Series thisSeries, final HiResDate start_time,
			final HiResDate end_time, final ColouredDataItem.OffsetProvider provider,
			final VersatileSeriesAdder theAdder) {
		...
		// add the start point
		createDataPoint(theCalculation, thisPrimary, thisSecondary, start_time, false, thisColor, thisSeries, provider,
				theAdder);

		// add the end point
		createDataPoint(theCalculation, thisPrimary, thisSecondary, end_time, true, thisColor, thisSeries, provider,
				theAdder);
	}
	...
	/**
	 * Option used for Debrief Lite. User selects the option before running the XY
	 * calculation.
	 */
	...
	public ShowTimeVariablePlot3(final MWC.GUI.Properties.PropertiesPanel thePanel,
		...
		_theOperations.addElement(new CalculationHolder(new relBearingCalc(), theFormatter, true, 180));
		_theOperations.addElement(new CalculationHolder(new atbCalc(), theFormatter, true, 180));
	}
	...
	public final MWC.GUI.Tools.Action getData() {
		...
		if (theHolder != null) {
			...
			// get the data
			// ////////////////////////////////////////////////////
			final AbstractDataset theDataset = getDataSeries(thePrimary, theHolder, _theTracks, _start_time, _end_time,
					jChart.getTimeOffsetProvider());
	...
	private XYPlot getPlot(final RelativeDateAxis xAxis, final ValueAxis yAxis, final StepControl theStepper,
	 ...
	 *
	 * @param panel     the panel containing the plot
	 * @param theXYPlot the plot itself
	 */
	private void insertPanel(final JPanel panel, final NewFormattedJFreeChart theXYPlot) {
		// create the panel we are using
		if (_thePanel != null) {
			final SwingPlot mySwingPlot = new MySwingPlot(panel, _thePanel, theXYPlot);
			...
			if (_thePanel instanceof MWC.GUI.Properties.Swing.SwingPropertiesPanel) {
				...
				final MWC.GUI.Properties.Swing.SwingPropertiesPanel sp = (MWC.GUI.Properties.Swing.SwingPropertiesPanel) _thePanel;
	...
	public void setPreselectedOperation(final CalculationHolder _preselectedOperation) {
		this._preselectedOperation = _preselectedOperation;
	}

	public void setPreselectedPrimaryTrack(final WatchableList _preselectedPrimaryTrack) {
