...
public final class ArcBuilder extends PatternBuilderType {
	...
	public final class ArcInfo extends MWC.GUI.Editable.EditorType {
		...
		public ArcInfo(final ArcBuilder data, final String theName) {
			...
			try {
				final PropertyDescriptor[] myRes = {
						...
						displayProp("PatternOrientation", "Pattern orientation",
								"the orientation of the centre point of this arc (degs)"),
						displayProp("PatternRadius", "Pattern radius", "the radius of this arc"),
						displayProp("PatternArcs", "Pattern arcs",
								"the size of the arcs either size of the orientation (degs)"),
	...
	// Constructor
	//////////////////////////////////////////
	public ArcBuilder(final WorldLocation centre, final MWC.GUI.Properties.PropertiesPanel thePanel,
	...
	protected final void addBuoys(final Debrief.Wrappers.BuoyPatternWrapper pattern) {
		...
		double theta = 2 * _arcs / (numArcs - 1);
		...
		if (_clockwise) {
			...
			// we're anti-clockwise, so we start to the right of the orientation and travel
			// left
			currentAngle = orient_rads + arcs_rads;
			theta = -1.0 * theta;
		}
		...
		for (int i = 0; i < numArcs; i++) {
			...
			final Debrief.Wrappers.LabelWrapper lw = new Debrief.Wrappers.LabelWrapper("A" + (i + 1), thisLoc,
