...
public final class SensorContactWrapper extends SnailDrawTMAContact.PlottableWrapperWithTimeAndOverrideableColor
		implements Watchable, CanvasType.MultiLineTooltipProvider, Editable.DoNotHighlightMe, TimeStampedDataItem,
	...
	public static final class SensorContactInfo extends Griddable {
		...
		public PropertyDescriptor[] getGriddablePropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Label", "the label for this data item", FORMAT),
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Label", "the label for this data item", FORMAT),
						prop("Visible", "whether this sensor contact data is visible", FORMAT),
						displayProp("LabelVisible", "Label visible", "whether the label for this contact is visible",
								FORMAT),
						prop("Color", "the color for this sensor contact", FORMAT),
						displayProp("HasFrequency", "Has frequency", "whether this data item includes frequency data",
								OPTIONAL),
						displayProp("HasBearing", "Has bearing", "whether this data item includes a bearing line",
								OPTIONAL),
						displayProp("HasAmbiguousBearing", "Has ambiguous bearing",
								"whether this data item includes an ambiguous bearing line", OPTIONAL),
						prop("Frequency", "the frequency measurement for this data item", OPTIONAL),
						...
						displayLongProp("PutLabelAt", "Put label at", "whereabouts on the line to position the label",
						...
						displayProp("Comment", "Comment", "Comment for this entry", OPTIONAL),
		...
		public final SubjectAction[] getUndoableActions() {
			final SubjectAction[] res = new SubjectAction[] { new DitchAmbiguousBearing(true, "Keep port bearing"),
	...
	static public final class testSensorContact extends junit.framework.TestCase {
		...
		public final void testMyCode() {
			...
			final SensorContactWrapper ed = new SensorContactWrapper("blank track",
					new HiResDate(new java.util.Date().getTime()), new WorldDistance(1, WorldDistance.DEGS), 55d,
					origin, DebriefColors.RED, "my label", 1, "theSensorName");
			 ...
			 */
			final WorldVector test_other_vector = new WorldVector(Conversions.Degs2Rads(55), 1, 0);
	...
	/**
	 * if the sensor cut doesn't have a range, we plot it out to the height + width
	 * of the screen (to be sure it extends past the visible viewport) But, if we're
	 * looking at the whole globe, that may give a distance that goes off the globe.
	 * So we double-check that it's not greater than a Monster range value.
	 */
	 ...
	 *
	 * @param course  current course (Degs)
	 * @param bearing absolute bearing to target (degs)
	...
	/**
	 * the calculation to determine if the bearing is to the port is expensive, so
	 * cache the result
	 */
	 ...
	 * @param other
	 */
	public SensorContactWrapper(final SensorContactWrapper other) {
		this(other.getTrackName(), new HiResDate(other.getDTG()), other.getRange(), other.getBearing(),
				other.getAmbiguousBearing(), other.getFrequency(), other.getOrigin(), other.getColor(),
				other.getLabel(), other.getLineStyle(), other.getSensorName());
	}

	public SensorContactWrapper(final String theTrack, final HiResDate theDtg, final WorldDistance range,
			final Double brgDegs, final Double ambigBearingDegs, final Double freq, final WorldLocation origin,
			final Color theColor, final String labelStr, final int theStyle, final String sensorName) {
		this();
	 ...
	 * @param sensorName
	 */
	public SensorContactWrapper(final String trackName, final HiResDate dtg, final WorldDistance range,
			final Double bearingDegs, final WorldLocation origin, final java.awt.Color color, final String label,
			final int style, final String sensorName) {
		this(trackName, dtg, range, bearingDegs, null, null, origin, color, label, style, sensorName);
	}

	/**
	 * we cache whether the bearing is to Port or Stbd Occasionally we need to clear
	 * this value
	 */
		...
		if ((_calculatedOrigin == null)) {
			if (_absoluteOrigin != null) {
				...
				if (theParent != null) {
					...
					_calculatedOrigin = ArrayOffsetHelper.getArrayCentre(this.getSensor(), _DTG, null, parentTrack);
				}
			}
	...
	final private WorldLocation getEnd(final WorldArea outerEnvelope, final double bearing) {
		...
		if (_calculatedOrigin != null) {
			...
			// also do the far end
			res = _calculatedOrigin.add(new WorldVector(Conversions.Degs2Rads(bearing), rangeToUse, 0d));
	...
	public final void paint(final WatchableList track, final CanvasType dest, final boolean keep_simple,
		...
		if (origin != null) {
			...
			if (this.getHasBearing()) {
				...
				final WorldLocation theFarEnd = getFarEnd(dest.getProjection().getDataArea());
				...
				if (hasAmbig && this.getHasAmbiguousBearing()) {
					...
					final WorldLocation theOtherFarEnd = getAmbiguousFarEnd(dest.getProjection().getDataArea());
	...
	public final void setHasAmbiguousBearing(final boolean val) {
		...
		if (val && Double.isNaN(_bearingAmbig)) {
			...
			// bearing anyway
			Application.logError2(ToolParent.WARNING, "User tried to set un-ambig bearing as ambiguous, ignoring",
					null);
		} else {
			_hasAmbiguous = val;
