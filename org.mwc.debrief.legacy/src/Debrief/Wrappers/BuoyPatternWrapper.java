 ...
 */
public final class BuoyPatternWrapper extends PlainWrapper implements WatchableList, Layer, Editable.DoNotHighlightMe {
	...
	public static final class BuoyPatternAdaptor implements Watchable {
		...
		public BuoyPatternAdaptor(final WorldArea theArea, final HiResDate theTime, final Color theColor,
				final String theName, final BuoyPatternWrapper thePattern) {
			_theArea = theArea;
			_theTime = theTime;
			_theColor = theColor;
			_theName = theName;
	...
	public final class BuoyPatternInfo extends Editable.EditorType {
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			try {
				...
				res[9].setPropertyEditorClass(MWC.GUI.Shapes.Symbols.SymbolScalePropertyEditor.class);
	...
	static public final class testMe extends junit.framework.TestCase {
		...
		public final void testListManagement() {
			...
			final LabelWrapper lw = new LabelWrapper("here", w1, Color.red, new HiResDate(100), new HiResDate(200));
			bpw.add(lw);
			...
			final LabelWrapper lw2 = new LabelWrapper("here", w2, Color.red, new HiResDate(100), new HiResDate(200));
			bpw.add(lw2);
			...
			final LabelWrapper lw3 = new LabelWrapper("here", w3, Color.red, new HiResDate(180), new HiResDate(260));
			bpw.add(lw3);
		...
		/**
		 * put test for BaseTimePeriod in here, since we can't put testing code (main
		 * method) into TimePeriod interface
		 */
		public final void testPeriod() {
			final TimePeriod btp = new TimePeriod.BaseTimePeriod(new HiResDate(500), new HiResDate(600));
			assertEquals("contains valid time", btp.contains(new HiResDate(550)), true);
			assertEquals("doesn't contain invalid time", btp.contains(new HiResDate(450)), false);
			assertEquals("doesn't contain invalid time", btp.contains(new HiResDate(650)), false);
			btp.extend(new HiResDate(650));
			assertEquals("contains valid time", btp.contains(new HiResDate(550)), true);
			assertEquals("contains valid time", btp.contains(new HiResDate(650)), true);
			assertEquals("doesn't contain invalid time", btp.contains(new HiResDate(450)), false);
			assertEquals("doesn't contain invalid time", btp.contains(new HiResDate(750)), false);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(450), new HiResDate(550)), true);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(550), new HiResDate(750)), true);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(550), new HiResDate(580)), true);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(450), new HiResDate(780)), true);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(450), new HiResDate(480)), false);
			assertEquals("overlaps valid period", btp.overlaps(new HiResDate(650), new HiResDate(780)), false);
		}
	...
	private WorldLocation getAnchor(final int location) {
		...
		if (wa != null) {
			...
			case MWC.GUI.Properties.LocationPropertyEditor.TOP: {
				final WorldLocation res = new WorldLocation(wa.getTopLeft().getLat(), centre.getLong(), 0);
				loc = res;
				break;
			}
			case MWC.GUI.Properties.LocationPropertyEditor.BOTTOM: {
				final WorldLocation res = new WorldLocation(wa.getBottomRight().getLat(), centre.getLong(), 0);
				loc = res;
				break;
			}
			case MWC.GUI.Properties.LocationPropertyEditor.LEFT: {
				final WorldLocation res = new WorldLocation(centre.getLat(), wa.getTopLeft().getLong(), 0);
				loc = res;
				break;
			}
			case MWC.GUI.Properties.LocationPropertyEditor.RIGHT: {
				final WorldLocation res = new WorldLocation(centre.getLat(), wa.getBottomRight().getLong(), 0);
				loc = res;
				break;
			}
	...
	public MWC.GenericData.Watchable[] getNearestTo(final HiResDate DTG) {
		final BuoyPatternAdaptor ans = new BuoyPatternAdaptor(getBounds(), DTG, getColor(), getName(), this);
	 ...
	 * set the colour of this pattern label
	 *
	 * @param theVal the colour
	 */
	public void setBuoyLabelLocation(final Integer theVal) {
		setChildren("setLabelLocation", new Object[] { theVal }, new Class[] { Integer.class });
	...
	public void setBuoyLabelVisible(final boolean theVal) {
		setChildren("setLabelVisible", new Object[] { new Boolean(theVal) }, new Class[] { Boolean.class });
	...
	public void setBuoySymbolSize(final double theVal) {
		setChildren("setSymbolSize", new Object[] { new Double(theVal) }, new Class[] { Double.class });
	...
	private void setChildren(final String methodName, final Object[] args, final Class<?>[] parameters) {
		try {
			final java.lang.reflect.Method setter = LabelWrapper.class.getMethod(methodName, parameters);
			...
			MWC.Utilities.Errors.Trace.trace(nm, "Buoy setter unable to find method:" + methodName);
		} catch (final java.lang.reflect.InvocationTargetException te) {
			MWC.Utilities.Errors.Trace.trace(te, "Buoy setter unable execute method:" + methodName);
		} catch (final java.lang.IllegalAccessException ie) {
			MWC.Utilities.Errors.Trace.trace(ie, "Buoy setter illegally accessing method:" + methodName);
		}
	}
	 ...
	 *
	 * @param theName the name as a String
	 */
	@Override
	@FireReformatted
	 ...
	 * the relative location of the label
	 *
	 * @param val the relative location
	 */
