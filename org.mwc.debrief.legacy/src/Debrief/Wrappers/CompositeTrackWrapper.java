...
public class CompositeTrackWrapper extends TrackWrapper
		implements GriddableSeriesMarker, NeedsToKnowAboutLayers, ExportLayerAsSingleItem {

	/**
	 * class containing editable details of a track
	 */
	public final class CompositeTrackInfo extends Editable.EditorType implements Editable.DynamicDescriptors {
		...
		public final MethodDescriptor[] getMethodDescriptors() {
			...
			final MethodDescriptor[] mds = { method(c, "addLeg", null, "Add New Leg"),
					method(c, "addClosingLeg", null, "Add Closing Leg"), method(c, "exportThis", null, "Export Shape"),
					method(c, "appendReverse", null, "Append reversed segments"), };
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { expertProp("Origin", "where this track starts", FORMAT),
						displayExpertProp("StartDate", "Start date", "the time this track starts", FORMAT),
						...
						expertProp("Name", "the track name", FORMAT), expertProp("Color", "the track color", FORMAT),
						displayExpertLongProp("SymbolType", "Symbol type", "the type of symbol plotted for this label",
								FORMAT, MWC.GUI.Shapes.Symbols.SymbolFactoryPropertyEditor.class),
	...
	private static class FromRangeTime extends PlanningCalc {
		...
		double getMinuteDelta(final PlanningSegment seg) {
			...
			final double metresPerSec = seg.getDistance().getValueIn(WorldDistance.METRES) / travelSecs;
			...
			final WorldSpeed speedKTs = new WorldSpeed(speedMtrs.getValueIn(WorldSpeed.Kts), WorldSpeed.Kts);
	...
	private abstract static class PlanningCalc {
		private WorldLocation addFix(final PlanningSegment seg, WorldLocation theOrigin, final double courseRads,
				final WorldVector vec, final long tNow) {
			final HiResDate thisDtg = new HiResDate(tNow);

			// ok, do this fix
			final Fix thisF = new Fix(thisDtg, theOrigin, courseRads, seg.getSpeed().getValueIn(WorldSpeed.ft_sec) / 3);
		...
		void construct(final PlanningSegment seg, final WorldLocation origin, final HiResDate date) {
			...
			WorldVector vec = new WorldVector(courseRads, new WorldDistance(distPerStep, WorldDistance.METRES), null);

			long tNow = timeMillis;
			while (tNow <= (timeMillis + timeTravelledMillis)) {
				if (timeStepMillis > timeTravelledMillis) {
					final double remaining = timeTravelledMillis;
					distPerStep = distPerMinute * ((remaining) / ONE_MIN);
					vec = new WorldVector(courseRads, new WorldDistance(distPerStep, WorldDistance.METRES), null);
				}
				theOrigin = addFix(seg, theOrigin, courseRads, vec, tNow);
				final double remaining = timeMillis + timeTravelledMillis - tNow;
	...
	public void add(final Editable point) {
		if (point instanceof PlanningSegment) {
			...
			final PlanningSegment segment = (PlanningSegment) point;
			...
			if (resetFormatting) {
				...
				while (iter.hasMoreElements()) {
					final FixWrapper thisFix = (FixWrapper) iter.nextElement();
	...
	public void addClosingLeg() {
		this.add(new ClosingSegment("Closing segment", 45, new WorldSpeed(12, WorldSpeed.Kts),
	...
	public void append(final Layer other) {
		throw new RuntimeException("You can't add another layer to this composite track");
	}
		...
		while (numer.hasMoreElements()) {
			...
			if (thisSeg.getVisible()) {
				...
				if (endFix != null) {
					...
					final WorldLocation fixLocation = new WorldLocation(endFix.getLocation()) {
						private static final long serialVersionUID = 1L;
						...
						public void addToMe(final WorldVector delta) {
							...
							final double brgRads = super.bearingFrom(thisSeg.first().getBounds().getCentre());
							...
							final double newBearing = m * ANGLE_DELTA;
