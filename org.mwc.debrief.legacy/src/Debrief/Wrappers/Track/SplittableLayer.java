	...
	public void AutoSplitTracks() {
		...
		while (iter.hasMoreElements()) {
			...
			while (theseCuts.hasMoreElements()) {
				final SensorContactWrapper thisCut = (SensorContactWrapper) theseCuts.nextElement();

				// do we have a last one
				boolean newTrack = false;
				if (lastCut != null) {
					// what's the delta
					final long delta = thisCut.getTime().getDate().getTime() - lastCut.getTime().getDate().getTime();
