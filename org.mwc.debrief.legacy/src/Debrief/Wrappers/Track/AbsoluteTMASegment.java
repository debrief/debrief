...
/**
 * extension of track segment that represents a single TMA solution as a series of fixes - based at
 * a fixed origin
 * 
 * @author Ian Mayo
 * 
...
public class AbsoluteTMASegment extends CoreTMASegment
{
    ...
    public final PropertyDescriptor[] getPropertyDescriptors()
    {
      ...
      try
      {
        final PropertyDescriptor[] res =
            {
                displayExpertProp("DTG_Start", "Start time",
   ...
   */
  private void
      createDataFrom(final HiResDate startTime, final HiResDate endTime)
  {
  ...
  protected FixWrapper createFixAt(final long theTime, final long startTime)
  {
    ...
    final double distM =
        _speed.getValueIn(WorldSpeed.M_sec) * (theTime - startTime) / 1000;
    final double courseRads = MWC.Algorithms.Conversions.Degs2Rads(_courseDegs);

    final WorldDistance theDist =
        new WorldDistance(distM, WorldDistance.METRES);
    final WorldLocation newLoc =
        this._origin.add(new WorldVector(courseRads, theDist,
            new WorldDistance(0, WorldDistance.METRES)));

    final Fix fix =
        new Fix(new HiResDate(theTime), newLoc, courseRads, _speed
  ...
  public void stretch(final double rngDegs, final WorldLocation origin)
  {
    ...
    else
    {
      ...
      final WorldVector newLeg =
          new WorldVector(thisLeg.getBearing(), rangDegs, thisLeg.getDepth());
