...
public class AbsoluteTMASegment extends CoreTMASegment {
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			...
			try {
				final PropertyDescriptor[] res = {
						displayExpertProp("DTG_Start", "Start time", "Start time for this TMA Solution", SOLUTION),
	 ...
	 * @param endTime
	 */
	public AbsoluteTMASegment(final AbsoluteTMASegment relevantSegment, final SortedSet<Editable> theItems,
			final WorldLocation location, final HiResDate startTime, final HiResDate endTime) {
	 ...
	 * @param origin
	 */
	public AbsoluteTMASegment(final double courseDegs, final WorldSpeed speed, final WorldLocation origin,
	 ...
	 */
	public AbsoluteTMASegment(final double courseDegs, final WorldSpeed speed, final WorldLocation origin,
			final HiResDate startTime, final HiResDate endTime, final long[] timeStamps) {
	 ...
	 * @param endTime
	 */
	private void createDataFrom(final HiResDate startTime, final HiResDate endTime, final long[] timeStamps) {
		// ditch any existing data
		this.removeAllElements();
	...
	protected FixWrapper createFixAt(final long theTime, final long startTime) {
		...
		final double distM = _speed.getValueIn(WorldSpeed.M_sec) * (theTime - startTime) / 1000;
		final double courseRads = MWC.Algorithms.Conversions.Degs2Rads(_courseDegs);

		final WorldDistance theDist = new WorldDistance(distM, WorldDistance.METRES);
		final WorldLocation newLoc = this._origin
				.add(new WorldVector(courseRads, theDist, new WorldDistance(0, WorldDistance.METRES)));

		final Fix fix = new Fix(new HiResDate(theTime), newLoc, courseRads, _speed.getValueIn(WorldSpeed.ft_sec) / 3);
	...
	public void stretch(final double rngDegs, final WorldLocation origin) {
		...
		if (first.getLocation().equals(origin)) {
			...
			final WorldVector newLeg = new WorldVector(thisLeg.getBearing(), rangDegs, thisLeg.getDepth());
