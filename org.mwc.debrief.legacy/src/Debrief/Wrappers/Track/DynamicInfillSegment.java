...
public class DynamicInfillSegment extends TrackSegment implements ColoredWatchable, DeleteWithCare {
	...
	public static class TestInterp extends TestCase {
		...
		public void testSimple() {
			...
			final UnivariateFunction longInterp = interpolator.interpolate(times, longs);
	 ...
	 *
	 * @param trackOne the segment to get the data from
	 * @param oneUse   how many elements to use
	 * @return the subset
	 */
	public static FixWrapper[] getFirstElementsFrom(final TrackSegment trackTwo, final int num) {
	 ...
	 *
	 * @param trackOne the segment to get the data from
	 * @param num      how many elements to use
	 * @return the subset
	 */
	public static FixWrapper[] getLastElementsFrom(final TrackSegment trackOne, final int num) {
		final FixWrapper[] res = new FixWrapper[num];
	 ...
	 * @param afterName
	 */
	public DynamicInfillSegment(final String beforeName, final String afterName, final Color trackColor) {
	 ...
	 * @param after
	 */
	public DynamicInfillSegment(final TrackSegment before, final TrackSegment after) {
		this(before.getName(), after.getName(),
				getColorStrategy(((FixWrapper) before.last()).getColor().darker().darker()));
	...
	private TrackSegment findSegment(final String name) {
		...
		if (res == null) {
			Application.logError2(ErrorLogger.ERROR, "Unable to find host segnent named:" + name, null);
		}
	...
	public void reconstruct() {
		...
		if (_before.isEmpty() || _after.isEmpty()) {
			Application.logError2(ToolParent.ERROR, "Can't reconstruct infill, before/after segment is missing data",
					null);
			return;
		}
		...
		final UnivariateFunction latInterp = interpolator.interpolate(times, lats);
		final UnivariateFunction longInterp = interpolator.interpolate(times, longs);
		final UnivariateFunction depthInterp = interpolator.interpolate(times, depths);
		...
		if (tDelta == 0)
			throw new RuntimeException("cannot generate infill when calculated step time is zero");
		...
		if (_myParent != null) {
			if (origin.getLocation() == null)
				_myParent.logError(ToolParent.INFO, "origin element has empty location", null);
		}
		...
		for (long tNow = tStart; tNow <= tEnd - tDelta; tNow += tDelta) {
			...
			final WorldLocation newLocation = new WorldLocation(thisLat, thisLong, thisDepth);
			...
			final double timeSecs = (nextTime - origin.getTime().getDate().getTime()) / 1000;
			...
			// create the fix
			final Fix newFix = new Fix(new HiResDate(tNow), newLocation, thisCourseRads, speedYps);
			...
			// note "silent" usage in line above, as optimisation to reduce calls to
			// SimpleDateFormat
			// constructor
		...
		// sort out our name
		if (getName() == null) {
			final String name = "infill_" + FormatRNDateTime.toShortString(new Date().getTime());
			this.setName(name);
		}
	...
	private void startWatching(final TrackSegment segment) {
		...
		segment.addPropertyChangeListener(BaseItemLayer.WRAPPER_CHANGED, _wrapperListener);
		...
		if (segment instanceof RelativeTMASegment) {
			...
			if (sensor != null) {
				sensor.addPropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _moveListener);
			}
		}
	}
	...
	private void stopWatching(final TrackSegment segment) {
		...
		if (segment != null) {
			segment.removePropertyChangeListener(CoreTMASegment.ADJUSTED, _moveListener);
			segment.removePropertyChangeListener(BaseItemLayer.WRAPPER_CHANGED, _wrapperListener);
			...
			if (segment instanceof RelativeTMASegment) {
				...
				final SensorWrapper sensor = rel.getReferenceSensor();
				if (sensor != null) {
					sensor.removePropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _moveListener);
				}
			}
