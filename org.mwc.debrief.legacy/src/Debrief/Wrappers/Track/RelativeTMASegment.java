...
public class RelativeTMASegment extends CoreTMASegment implements NeedsToKnowAboutLayers {
	...
	public static class TestMe extends TestCase {
		public final void testMyParams() {
			RelativeTMASegment rs = new RelativeTMASegment(0, new WorldSpeed(12, WorldSpeed.Kts),
	...
	public final class TMASegmentInfo extends TrackSegmentInfo {
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			...
			try {
				final PropertyDescriptor[] res = { expertProp("Course", "Course of this TMA Solution", SOLUTION),
						...
						displayExpertProp("DTG_Start", "DTG Start", "Start time for this TMA Solution", SOLUTION),
						displayExpertProp("DTG_End", "DTG End", "End time for this TMA Solution", SOLUTION) };
				mine = res;
			} catch (final IntrospectionException e) {
				Application.logError2(ToolParent.ERROR, "Failed to create properties for RelativeTMASegment", e);
				return super.getPropertyDescriptors();
			}

			// now combine them.
			final PropertyDescriptor[] bigRes = new PropertyDescriptor[parent.length + mine.length];
	...
	/**
	 * preference name for the cut-off value for shading error plot (when REL TMA is
	 * being dragged)
	 *
	 */
	 ...
	 * object pointer until after file-load is complete. It's also important that
	 * reference track and reference sensor objects are transient - so they don't
	 * get byte-by-byte duplication on copy-item operation. Doing this for very
	 * large blue track caused out of memory.
	 *
	 * @return
	 */
	...
	/**
	 * name of the sensor we're going to use as our origin. See justification for
	 * this string reference in the reference track.
	 *
	 */
	...
	/**
	 * the sensor that we're using as an origin (particularly relevant for an offset
	 * sensor like a towed array
	 */
	 ...
	 */
	public RelativeTMASegment(final double courseDegs, final WorldSpeed speed, final WorldVector offset,
			final Layers theLayers, final String trackName, final String sensorName) {
	...
	/**
	 * kind of copy constructor. take the settings from the segment, but store the
	 * supplied cuts
	 *
	 ...
	 * @param theOffset
	 */
	public RelativeTMASegment(final RelativeTMASegment relevantSegment, final SortedSet<Editable> theItems,
			final WorldVector theOffset) {
		...
		this(relevantSegment._courseDegs, relevantSegment._speed, theOffset, relevantSegment._theLayers,
				relevantSegment._referenceTrackName, relevantSegment._referenceSensorName);
	 ...
	 *
	 * @param observations create a single position for the DTG of each solution
	 * @param offset       the range/brg from the host's position at the DTG of the
	 *                     first observation
	 * @param speed        the initial target speed estimate
	 * @param courseDegs   the initial target course estimate
	 * @param theLayers
	 * @param override     optional, whether to force usage of a track color. If
	 *                     null, the relevant sensor cut color will be used
	 */
	public RelativeTMASegment(final SensorContactWrapper[] observations, final WorldVector offset,
			final WorldSpeed speed, final double courseDegs, final Layers theLayers, final Color override) {
	 ...
	 * @param endTime
	 */
	public RelativeTMASegment(final SensorWrapper sensor, final WorldVector offset, final WorldSpeed speed,
			final double course, final Collection<Editable> dataPoints, final Layers layers, final TrackWrapper track,
			final HiResDate startTime, final HiResDate endTime) {
		this(course, speed, offset, layers, sensor.getHost().getName(), sensor.getName());

		// ok, now generate the points
		setTrack(sensor.getHost());

		for (final Iterator<Editable> iterator = dataPoints.iterator(); iterator.hasNext();) {
			final FixWrapper thisF = (FixWrapper) iterator.next();
			final FixWrapper newF = createFixAt(thisF.getDateTimeGroup().getDate().getTime());
	...
	private void addFix(final FixWrapper theLoc, final long thisT) {
		...
		final Fix newFix = new Fix(newTime, newLoc, MWC.Algorithms.Conversions.Degs2Rads(this.getCourse()),
				MWC.Algorithms.Conversions.Kts2Yps(this.getSpeed().getValueIn(WorldSpeed.Kts)));
	...
	/**
	 * create the movement listener, if we need to. We need to be able to regenerate
	 * the listener for instances where the segment has been copy/pated between
	 * plots
	 */
	 ...
	 */
	private void createPointsFrom(final SensorContactWrapper[] observations, final Color override) {
	...
	public void fanStretch(final WorldVector vector) {
		...
		final WorldLocation newStart = hisStart.add(new WorldVector(startBrg, currSensorDist + theRange, 0));
		...
		final WorldLocation outerEnd = outerStart.add(new WorldVector(endBrg, _offset.getRange(), 0));
	...
	public WorldLocation getHostLocation() {
		...
		if (this.isEmpty()) {
			Application.logError2(ErrorLogger.ERROR, "Empty track, can't find host location", null);
			return null;
		}
			...
			if (_referenceSensor != null) {
				...
				if (items.length > 0) {
					...
					if (host != null) {
						...
						final FixWrapper hostFix = host.getBacktraceTo(startDTG, _referenceSensor.getSensorOffset(),
								_referenceSensor.getWormInHole());
						if (hostFix != null) {
	...
	public String getHostName() {
		final String res = _referenceTrack != null ? _referenceTrack.getName() : _referenceTrackName;
	...
	/**
	 * provide access to the layers object (necessary in support of moving tracks
	 * between layers)
	 *
	 * @return
	 */
	...
	public String getSensorName() {
		final String res = _referenceSensor != null ? _referenceSensor.getName() : _referenceSensorName;
	...
	private void identifyReferenceTrack() {
		...
		if (this.getWrapper() != null && this.getWrapper().getName().equals(_referenceTrackName)) {
			Application.logError2(ErrorLogger.ERROR,
					"Data error - this TMA is trying to reference itself:" + _referenceTrackName, null);

			// ok - something bad has gone down. Drop out.
			return;
		}

		final TrackWrapper theTrack = (TrackWrapper) _theLayers.findLayer(_referenceTrackName);
		...
		if (theTrack != null) {
			final TimePeriod hisPeriod = new TimePeriod.BaseTimePeriod(theTrack.getStartDTG(), theTrack.getEndDTG());
			final HiResDate myStart = getDTG_Start();
			if (myStart == null) {
				Application.logError2(ErrorLogger.ERROR, "This track contains no fixes", null);
			} else {
				if (myStart.lessThan(hisPeriod.getStartDTG()) || getDTG_End().greaterThan(hisPeriod.getEndDTG())) {
					if (this.getWrapper() != null) {
						...
						_myParent.logError(ToolParent.WARNING,
								"Deleting leg " + this.getName() + ",  host data not available", null);
					}
				} else {
	...
	public void rotate(final double brg, final WorldLocation origin) {
		...
		if (distFromOrigin < distFromEnd) {
			...
			this.setOffsetBearing(MWC.Algorithms.Conversions.Rads2Degs(offset.getBearing()));
			this.setOffsetRange(new WorldDistance(offset.getRange(), WorldDistance.DEGS));
		...
		final Double courseVal = new Double(MWC.Algorithms.Conversions.Degs2Rads(this._courseDegs));
	...
	public void setDetectionBearing(final double detectionBearing) {
		_offset = new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(detectionBearing),
	...
	public void setDTG_End(final HiResDate newEnd) {
		...
		if (delta < 0) {
			...
			// and ditch them
			for (final Iterator<FixWrapper> iterator = onesToRemove.iterator(); iterator.hasNext();) {
	...
	public void setDTG_Start(final HiResDate newStart) {
		...
		final double distDegs = this.getSpeed().getValueIn(WorldSpeed.Kts) * deltaHrs / 60;

		final double theDirection = MWC.Algorithms.Conversions.Degs2Rads(this.getCourse());
		...
		final WorldLocation newOrigin = currentStart.add(new WorldVector(theDirection, distDegs, 0));

		// and what's the point on the host track
		@SuppressWarnings("deprecation")
		final FixWrapper hostFix = this._referenceTrack.getBacktraceTo(theNewStart, _referenceSensor.getSensorOffset(),
				_referenceSensor.getWormInHole());
		...
		if (theNewStart.lessThan(startDTG())) {
			...
			if (this.getReferenceSensor() != null) {
				final SensorWrapper ref = this.getReferenceSensor();
				final Collection<Editable> cuts = ref.getItemsBetween(theNewStart, startDTG());
				if (cuts != null) {
					for (final Editable t : cuts) {
						final SensorContactWrapper cut = (SensorContactWrapper) t;
						addFix(theLoc, cut.getDTG().getDate().getTime());
	...
	public void setOffsetBearing(final double offsetBearing) {
		_offset.setValues(MWC.Algorithms.Conversions.Degs2Rads(offsetBearing), _offset.getRange(), _offset.getDepth());
	}
	...
	public void setOffsetRange(final WorldDistance offsetRange) {
		_offset.setValues(_offset.getBearing(), offsetRange.getValueIn(WorldDistance.DEGS), _offset.getDepth());
	}
	...
	private void setTrack(final TrackWrapper newTrack) {
		...
		if (newTrack != null) {
			...
			final Enumeration<Editable> sensors = _referenceTrack.getSensors().elements();
			while (sensors.hasMoreElements()) {
			...
			// did it work?
			if (_referenceSensor == null) {
				Application.logError2(ErrorLogger.ERROR, "Unable to find host sensor named:" + _referenceSensorName,
						null);
			}
	...
	public void shear(final WorldLocation cursor, final WorldLocation origin) {
		...
		if (distFromOrigin < distFromEnd) {
			...
			// update the offset to the new start location
			this.setOffsetBearing(MWC.Algorithms.Conversions.Rads2Degs(newOffset.getBearing()));
			this.setOffsetRange(new WorldDistance(newOffset.getRange(), WorldDistance.DEGS));
		}

		// how long do we have for the travel?
		final long periodMillis = this.endDTG().getDate().getTime() - this.startDTG().getDate().getTime();
		...
		final Double newCourseRads = new Double(MWC.Algorithms.Conversions.Degs2Rads(newCourse));
	...
	public void stretch(final double rngDegs, final WorldLocation origin) {
		...
		if (first.getLocation().equals(origin)) {
			...
			final WorldVector newLeg = new WorldVector(thisLeg.getBearing(), theRngDegs, thisLeg.getDepth());
	 ...
	 *
	 * @param courseVal the (optional) course to update
	 * @param speedVal  the (optional) speed to update
	 */
