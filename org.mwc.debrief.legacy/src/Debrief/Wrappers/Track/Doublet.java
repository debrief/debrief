...
public class Doublet implements Comparable<Doublet> {
	...
	static public final class testCalc extends junit.framework.TestCase {
		...
		private double convertAndTest(final double myCrseDegs, final double bearingDegs, final double mySpeedKts,
			...
			return FrequencyCalcs.calcDopplerComponent(bearingRads, myCrseRads, mySpeedKts, observedFreq);
		}
		...
		public void testTwoWay() {
			final SensorWrapper rxSensor = new SensorWrapper("Receiver_Sensor");
			final SensorContactWrapper receiver = new SensorContactWrapper("host", new HiResDate(1000), null, 90d, null,
					Color.red, "label", 0, "Receiver");
			receiver.setOrigin(new WorldLocation(0.1, 0.1, 0d));
			receiver.setSensor(rxSensor);
			final double targetCourse = 120;
			final double targetSpeed = 15;
			final Fix targetFix = new Fix(new HiResDate(2000), new WorldLocation(2, 2, 2d), targetCourse, targetSpeed);
			final FixWrapper targetFixW = new FixWrapper(targetFix);
			final WorldLocation targetLocation = new WorldLocation(0, 0, 0d);
			final HiResDate startTime = new HiResDate(1000);
			final HiResDate endTime = new HiResDate(1000000);
			final TrackSegment parent = new AbsoluteTMASegment(34, new WorldSpeed(12, WorldSpeed.Kts), targetLocation,
					startTime, endTime);
			final double hostCourse = 90d;
			final double hostSpeed = 10d;
			final Fix hostFix = new Fix(new HiResDate(4000), new WorldLocation(1, 1, 1d), hostCourse, hostSpeed);
			final FixWrapper hostFixW = new FixWrapper(hostFix);
			final double speed_of_sound = FrequencyCalcs.SpeedOfSoundKts;
			final SensorWrapper transmitter = new SensorWrapper("Transmitter");
			transmitter.setBaseFrequency(300d);
			final Doublet dub = new Doublet(receiver, targetFixW, parent, hostFixW) {

				@Override
				protected FixWrapper getTransmitterFix(final SensorWrapper source) {
					return targetFixW;
				}

			};
			final double freq = dub.getPredictedMultistaticFrequency(speed_of_sound, transmitter);
	 ...
	 * @return
	 */
	public static double getDopplerShift(final double SpeedOfSound, final double fNought, final Fix host,
			final Fix tgt) {
		...
		// done, go for it.
		return FrequencyCalcs.calcPredictedFreqSI(SpeedOfSound, osHeadingRads, tgtHeadingRads, osSpeed, tgtSpeed,
				offset.getBearing(), fNought);
	}
	...
	// constructor
	// ////////////////////////////////////////////////
	public Doublet(final SensorContactWrapper sensor, final FixWrapper targetFix, final TrackSegment parent,
	...
	public int compareTo(final Doublet o) {
		...
		if (_hostFix == null || o._hostFix == null || _hostFix.getDTG().equals(o._hostFix.getDTG())) {
			...
			if (_targetFix != null && o._targetFix != null) {
				...
				final int timeDiff = _targetFix.getDTG().compareTo(o._targetFix.getDTG());
				if (timeDiff != 0) {
					res = timeDiff;
				} else {
					final int sensorDiff = _sensor.getSensor().compareTo(o._sensor.getSensor());
	...
	public double getCalculatedBearing(final WorldVector sensorOffset, final WorldVector targetOffset) {
		final WorldLocation sensorOrigin = _sensor.getCalculatedOrigin(null);
		return getCalculatedBearing(sensorOffset, targetOffset, sensorOrigin, _targetFix.getLocation());
	}

	public double getCalculatedBearing(final WorldVector sensorOffset, final WorldVector targetOffset,
			final WorldLocation rxLoc, final WorldLocation txLoc) {
		// copy our locations
	...
	public double getCorrectedFrequency() {
		...
		final double dopplerComponent = FrequencyCalcs.calcDopplerComponent(theBearingRads, myCourseRads, mySpeedKts,
				observedFreq);
	...
	public double getPredictedFrequency(final double speedOfSoundKts) {
		...
		if (_targetTrack instanceof CoreTMASegment) {
			...
			predictedFreq = FrequencyCalcs.getPredictedFreq(baseFreq, speedOfSoundKts, mySpeedKts, myCourseDegs,
					hisSpeedKts, hisCourseDegs, theBearingDegs);
		}
	...
	public double getPredictedMultistaticFrequency(final double speedOfSoundKts, final SensorWrapper source) {
		...
		if (_targetTrack instanceof CoreTMASegment) {
			...
			predictedFreq = FrequencyCalcs.getPredictedFreq(baseFreq, speedOfSoundKts, rxSpeedKts, rxCourseDegs,
					txSpeedKts, txCourseDegs, theBearingDegs);
			...
			predictedFreq = FrequencyCalcs.getPredictedFreq(predictedFreq, speedOfSoundKts, rxSpeedKts, rxCourseDegs,
					txSpeedKts, txCourseDegs, theBearingDegs);
	...
	protected FixWrapper getTransmitterFix(final SensorWrapper source) {
		final Watchable[] nearest = source.getHost().getNearestTo(_hostFix.getDateTimeGroup(), false);
		final Watchable res;
