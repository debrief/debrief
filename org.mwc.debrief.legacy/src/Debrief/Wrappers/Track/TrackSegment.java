 ...
 *
 */
public class TrackSegment extends BaseItemLayer
		implements DraggableItem, GriddableSeriesMarker, NeedsWrappingInLayerManager, ICantContainLayers {
	...
	public static class testListMgt extends TestCase {
		...
		private FixWrapper getFix(final long dtg, final double course, final double speed) {
			final Fix theFix = new Fix(new HiResDate(dtg), new WorldLocation(2, 2, 2), course, speed);
		...
		public void testDeleteNotVisible() {
			...
			// hide the first
			final Enumeration<Editable> ele = ts.elements();
			final FixWrapper first = (FixWrapper) ele.nextElement();
			final FixWrapper second = (FixWrapper) ele.nextElement();
		...
		public void testResample() {
			...
			final TrackSegment t1 = (TrackSegment) tw.getSegments().elements().nextElement();
			...
			final AbsoluteTMASegment as = new AbsoluteTMASegment(12, new WorldSpeed(4, WorldSpeed.Kts),
					new WorldLocation(2, 2, 2), new HiResDate(100000), new HiResDate(1000000));
			tw.add(as);
		...
		public void testTrim() {
			...
			TimePeriod newP = new TimePeriod.BaseTimePeriod(new HiResDate(30000), new HiResDate(40000));
			assertEquals("correct len", 4, ts0.size());
			ts0.trimTo(newP);
			assertEquals("correct new len", 2, ts0.size());

			ts0 = getDummyList();
			newP = new TimePeriod.BaseTimePeriod(new HiResDate(35000), new HiResDate(40000));
			assertEquals("correct len", 4, ts0.size());
			ts0.trimTo(newP);
			assertEquals("correct new len", 1, ts0.size());

			ts0 = getDummyList();
			newP = new TimePeriod.BaseTimePeriod(new HiResDate(15000), new HiResDate(40000));
			assertEquals("correct len", 4, ts0.size());
			ts0.trimTo(newP);
			assertEquals("correct new len", 3, ts0.size());

			ts0 = getDummyList();
			newP = new TimePeriod.BaseTimePeriod(new HiResDate(45000), new HiResDate(50000));
			assertEquals("correct len", 4, ts0.size());
			ts0.trimTo(newP);
	...
	public class TrackSegmentInfo extends Editable.EditorType {
		...
		public final MethodDescriptor[] getMethodDescriptors() {
			...
			MethodDescriptor[] newMeds = { method(c, "revealAllPositions", null, "Reveal All Positions"),
			...
			if (mds != null) {
				final MethodDescriptor resMeds[] = new MethodDescriptor[mds.length + newMeds.length];
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { expertProp("Visible", "whether this layer is visible", FORMAT),
	...
	private static void copyFormattingFrom(final FixWrapper firstFix, final FixWrapper storeMe) {
	...
	public static void decimatePointsTrack(final HiResDate theVal, final LightweightTrackWrapper parentTrack,
			final long theStartTime, final Vector<FixWrapper> newItems, final boolean fixRelative) {
		...
		if (!parentTrack.getPositionIterator().hasMoreElements()) {
			Application.logError2(ToolParent.INFO, "Can't resample empty track", null);
			return;
		}
		...
		while (iter.hasMoreElements()) {
			...
			final long thisTime = currentPosition.getDateTimeGroup().getDate().getTime();
			...
			if (previousPosition == null) {
				...
				while (thisTime >= requiredTime) {
					...
					if (thisTime == requiredTime) {
						...
						newPos = FixWrapper.interpolateFix(previousPosition, currentPosition,
					...
					if (fixRelative) {
						...
						final WorldVector offset = newPos.getLocation().subtract(previousPosition.getLocation());
						...
						final double timeSecs = (requiredTime - previousPosition.getTime().getDate().getTime()) / 1000d;
	...
	public void addFixSilent(final FixWrapper fix) {
		...
		if (_myTrack != null) {
			...
			fix.addPropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _myTrack.getLocationListener());
		}
	}
	...
	public void append(final Layer other) {
		...
		if (this.getPlotRelative()) {
			...
			final double timeSecs = (first.getTime().getDate().getTime() - myLast.getTime().getDate().getTime()) / 1000;
			// start off with the course
	...
	public void decimate(final HiResDate theVal, final TrackWrapper parentTrack, final long startTime) {
		...
		for (final Iterator<FixWrapper> iterator = newItems.iterator(); iterator.hasNext();) {
	...
	private void decimateAbsolute(final HiResDate theVal, final TrackWrapper parentTrack, final long startTime,
	...
	private void decimateRelative(final HiResDate theVal, final TrackWrapper parentTrack, final long startTime,
			final Vector<FixWrapper> newItems) {
	...
	private void decimateRelativeTMA(final HiResDate theVal, final Vector<FixWrapper> newItems,
		...
		for (tNow = startMicros; tNow <= endDTG().getMicros(); tNow += theVal.getMicros()) {
			final Fix theFix = new Fix(new HiResDate(0, tNow), new WorldLocation(myStartLocation), courseRads,
					speedYps);
	...
	public void deleteNonVisiblePositions() {
		...
		if (!toDelete.isEmpty()) {
			for (final Editable fix : toDelete) {
				removeElement(fix);
			}
		}
	}
	...
	protected void drawMyStalk(final CanvasType dest, final Point lastPoint, final Point thisPoint,
		...
		final Point backPoint = new Point(lastPoint.x + (int) (myLen * gradient), lastPoint.y + myLen);
		dest.setLineStyle(2);
	 ...
	 *
	 * @param brg    angle to rotate through (Radians)
	 * @param origin origin of rotation, probably one end of the track
	 */
	public void rotate(final double brg, final WorldLocation origin) {
		...
		for (final Iterator<Editable> iterator = items.iterator(); iterator.hasNext();) {
			...
			if (thisFix.getLocation() == origin) {
				...
				final WorldLocation newLoc = thisFix.getLocation().rotatePoint(origin, brg);
	...
	public void setWrapper(final TrackWrapper wrapper) {
		...
		if (_myTrack != null) {
			...
			for (final Iterator<Editable> iterator = items.iterator(); iterator.hasNext();) {
				...
				fix.removePropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _myTrack.getLocationListener());
			}
		}
		...
		if (wrapper != null) {
			...
			for (final Iterator<Editable> iterator = items.iterator(); iterator.hasNext();) {
				...
				fix.addPropertyChangeListener(PlainWrapper.LOCATION_CHANGED, wrapper.getLocationListener());
			}
		}
	}
	...
	public void shift(final WorldVector vector) {
		...
		for (final Iterator<Editable> iterator = items.iterator(); iterator.hasNext();) {
			...
			final WorldLocation copiedLoc = new WorldLocation(thisFix.getFix().getLocation());
	 ...
	 */
	public final void shiftTrack(final Enumeration<Editable> theEnum, final WorldVector offset) {
		...
		while (enumA.hasMoreElements()) {
			...
			if (thisO instanceof FixWrapper) {
				...
				final WorldLocation copiedLoc = new WorldLocation(fw.getFix().getLocation());
	...
	@Deprecated
	/**
	 * we've deprecated this, since it gets very compuationally expensive when
	 * processing a very long track
	 *
	 * @param theVal
	 * @param parentTrack
	 * @param theStartTime
	 * @param newItems
	 */
	private void usingNearest(final HiResDate theVal, final TrackWrapper parentTrack, final long theStartTime,
			final Vector<FixWrapper> newItems) {
		long tNow;
		...
		//
		// right - sort out what time period we're working through
		for (tNow = theStartTime; tNow <= endDTG().getMicros(); tNow += theVal.getMicros()) {
			...
			if (matches.length > 0) {
				...
				if (lastPositionStored != null) {
					// start off with the course
					final WorldVector offset = currentPosition.getLocation().subtract(lastPositionStored.getLocation());
	 ...
	 *
	 * @param fw     the leg we're looking at
	 * @param period how long it's travelling for (millis)
	 * @return a vector representing the subject
	 */
	public WorldVector vectorFor(final long period, final double speedKts, final double courseRads) {
