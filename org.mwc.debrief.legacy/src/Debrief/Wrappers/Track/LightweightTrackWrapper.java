...
public class LightweightTrackWrapper extends PlainWrapper
		implements WatchableList, Plottable, Layer, IsTransientForChildren, ICantContainLayers {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] _coreDescriptors = {
						...
						displayExpertProp("SymbolColor", "Symbol color", "the color of the symbol (when used)", FORMAT),
						prop("Visible", "the Layer visibility", VISIBILITY),
						prop("Name", "the name of the track", FORMAT),
						displayExpertProp("NameVisible", "Name Visible", "show the name of the track", FORMAT),
						...
						displayExpertLongProp("LineStyle", "Line style", "the line style used to join track points",
								TEMPORAL, LineStylePropertyEditor.class) };
	...
	public static class TestLightweight extends TestCase {
		...
		private static FixWrapper create(final long date, final double lat, final double lon) {
			return new FixWrapper(new Fix(new HiResDate(date), new WorldLocation(lat, lon, 0), 0d, 0d));
		}

		private static int doCount(final LightweightTrackWrapper track, final IsValid aTest) {
			int ctr = 0;
	 ...
	 * @param subjects
	 */
	public static void groupTracks(final LightweightTrackWrapper target, final Layers layers,
			final Editable[] subjects) {
		// ok, loop through the subjects, adding them onto the target
		for (int i = 0; i < subjects.length; i++) {
			final LightweightTrackWrapper source = (LightweightTrackWrapper) subjects[i];
			if (source != target) {
	...
	protected HiResDate _lastDataFrequency = new HiResDate(0, TimeFrequencyPropertyEditor.SHOW_ALL_FREQUENCY);
	 ...
	 */
	protected MWC.GUI.Shapes.Symbols.PlainSymbol _theSnailShape;
	...
	public LightweightTrackWrapper(final String name, final boolean visible, final boolean nameVisible,
			final Color color, final int lineStyle) {
		this();
	...
	public void append(final Layer other) {
		// is it a track?
		if ((other instanceof LightweightTrackWrapper) || (other instanceof LightweightTrackWrapper)) {
			// yes, break it down.
	...
	public HiResDate getEndDTG() {
		final HiResDate res = _thePositions.isEmpty() ? null : ((FixWrapper) _thePositions.last()).getTime();
		return res;
	...
	public Watchable[] getNearestTo(final HiResDate DTG) {
		...
		if (DTG.lessThan(myFirst.getTime()) || DTG.greaterThan(myLast.getTime())) {
			...
			while (fIter.hasNext()) {
				final FixWrapper fix = fIter.next();
				final long dist = Math.abs(fix.getDateTimeGroup().getDate().getTime() - dtg);
				if (dist < distance) {
					nearest = fix;
					distance = dist;
				}
	...
	public HiResDate getStartDTG() {
		final HiResDate res = _thePositions.isEmpty() ? null : ((FixWrapper) _thePositions.first()).getTime();
		return res;
	}
	 ...
	 * filtered, and which have valid depths. The isVisible flag indicates whether a
	 * track has been filtered or not. We also have the getVisibleFixesBetween
	 * method (below) which decides if a fix is visible if it is set to Visible, and
	 * it's label or symbol are visible.
	 * <p/>
	 * We don't have to worry about a valid depth, since 3d doesn't show points with
	...
	public void reconnectChildObjects(final Object clonedObject) {
		final LightweightTrackWrapper clonedTrack = (LightweightTrackWrapper) clonedObject;
	 ...
	 *
	 * @param theVal frequency in seconds
	 */
	...
	public void setResampleDataAt(final HiResDate theVal) {
		...
		if (theVal.getDate().getTime() <= 0) {
			...
			TrackSegment.decimatePointsTrack(theVal, this, this.getStartDTG().getDate().getTime(), newItems, false);
	...
	public final void setSymbolType(final String val) {
		...
		if (val.equals(_theSnailShape.getType())) {
			...
			final PlainSymbol shape = MWC.GUI.Shapes.Symbols.SymbolFactory.createSymbol(val);
			if (shape != null) {
