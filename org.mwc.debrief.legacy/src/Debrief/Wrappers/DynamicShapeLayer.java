...
public class DynamicShapeLayer extends BaseLayer implements DynamicPlottable, WatchableList {

	/**
	 * class containing editable details of a layer
	 */
	public final class DynamicShapeInfo extends LayerInfo implements Editable.DynamicDescriptors {
		...
		public final PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] sres = super.getPropertyDescriptors();
				final PropertyDescriptor[] res = new PropertyDescriptor[sres.length + 1];
				System.arraycopy(sres, 0, res, 0, sres.length);
				res[sres.length] = displayExpertProp("PlotAllShapes", "Plot all shapes", "Plot All Shapes", VISIBILITY);
				return res;
	...
	public void filterListTo(final HiResDate start, final HiResDate end) {
		...
		while (ele.hasMoreElements()) {
			final IDynamicShapeWrapper wrapper = (IDynamicShapeWrapper) ele.nextElement();
	...
	public Color getColor() {
		...
		if (!isEmpty()) {
			final Editable first = this.elements().nextElement();
			final IDynamicShapeWrapper dsw = (IDynamicShapeWrapper) first;
			res = dsw.getColor();
		}
		return res;
	}
	...
	public Collection<Editable> getItemsBetween(final HiResDate start, final HiResDate end) {
		final Vector<Editable> list = new Vector<Editable>();
		...
		if (!isEmpty()) {
			if (((ShapeWrapper) first()).getStartDTG().lessThanOrEqualTo(end)) {
				...
				final ShapeWrapper endT = (ShapeWrapper) last();
				...
				if (inRange) {
					...
					while (ele.hasMoreElements()) {
						...
						final IDynamicShapeWrapper dsw = (IDynamicShapeWrapper) editable;
	...
	public Watchable[] getNearestTo(final HiResDate DTG) {
		...
		if (!isEmpty()) {
			...
			if (firstS.getStartDTG().lessThanOrEqualTo(DTG)) {
				boolean inRange = false;
				// does the last one have and end time
				final ShapeWrapper end = (ShapeWrapper) last();
	...
	private void internalPaint(final CanvasType dest, final long time) {
		final Watchable[] nearest = this.getNearestTo(new HiResDate(time));
	...
	public void setPlotAllShapes(final boolean plotAllShapes) {
