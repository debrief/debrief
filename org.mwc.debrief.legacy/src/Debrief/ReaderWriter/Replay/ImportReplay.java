...
public class ImportReplay extends PlainImporterBase {
	...
	static public final class testImport extends junit.framework.TestCase {
		...
		private final static String shape_file = "../org.mwc.cmap.combined.feature/root_installs/sample_data/shapes.rep";

		private final static String boat_file = "../org.mwc.cmap.combined.feature/root_installs/sample_data/boat1.rep";
		...
		private void assertElementsInLayers(final Layer layer, final int count) {
			...
			final Enumeration<Editable> elements = layer.elements();
			int lineCount = 0;
		...
		private final void doReadRep(final String mode, final Long freq, final int LAYER_COUNT, final int NUM_FIXES,
				final boolean checkArea) {
			File testFile = null;
			...
			if (checkArea) {
				...
				super.assertEquals("tl lat of first layer", area.getTopLeft().getLat(), 11.92276, 0.01);
				super.assertEquals("tl long of first layer", area.getTopLeft().getLong(), -11.59394, 0.01);
				super.assertEquals("tl depth of first layer", area.getTopLeft().getDepth(), 0, 0.00001);

				super.assertEquals("br lat of first layer", area.getBottomRight().getLat(), 11.89421, 0.001);
				super.assertEquals("br long of first layer", area.getBottomRight().getLong(), -11.59376, 0.00001);
				super.assertEquals("br depth of first layer", area.getBottomRight().getDepth(), 0, 0.00001);
			}

			// check those narrative lines got read in
			final NarrativeWrapper narratives = (NarrativeWrapper) _theLayers.elementAt(2);
		...
		public final void testParseSymbologyColor() {
			...
			final LabelWrapper label1 = (LabelWrapper) new ImportLabel().readThisLine(test1);
			...
			final LabelWrapper label2 = (LabelWrapper) new ImportLabel().readThisLine(test2);
		...
		public final void testPasteRepDynamicShapes() {
			final String textToPaste = ";DYNAMIC_RECT: @A \"Dynamic A\" 951212 051000.000 22 00 0 N 21 00 0 W 21 50 0 N 20 50 0 W dynamic A rect 1\r\n"
					+ ";DYNAMIC_CIRCLE: @A \"Dynamic A\" 951212 052100.000 21 00 0 N 20 53 0 W 2000 dynamic A circ 12\r\n"
					+ ";DYNAMIC_POLY: @A \"Dynamic A\" 951212 052600.000 20 35 0 N 21 02 0 W 20 35 0 N 20 55 0 W 20 42 0 N 20 52 0 W 20 45 0 N 21 00 0 W  dynamic A POLY 170";
			final ImportReplay testImporter = new ImportReplay();
			final Layers tmpLayers = new Layers();
			final Layers dest = new Layers();
			testImporter.setLayers(tmpLayers);
			final int totalLines = textToPaste.split("\\r?\\n").length;
			testImporter.importThis(textToPaste, totalLines);
			ImportReplay.injectContent(tmpLayers, dest, true);
			assertElementsInLayers(tmpLayers.findLayer("Dynamic A"), 3);
			ImportReplay.injectContent(dest, tmpLayers, false);
			assertNull((tmpLayers.findLayer("Dynamic A")));

		}

		// test if previously loaded layers remains after pasting new content
		public void testPasteRepExistingFile() {
			final String textToPaste = ";LINE: @B 20 50 0 N 21 10 0 W 22 0 0 N 21 10 0 W test line\r\n"
					+ ";VECTOR: @C 21.6 12 0 N 21.5 11 0 W 5000 270 test vector\r\n"
					+ ";CIRCLE: @D    21.8 0 0 N 21.0 0 0 W 2000 test circle\r\n"
					+ ";TEXT: @E 21.7 0 0 N 21.5 0 0 W test text\r\n"
					+ ";TEXT: WB 21.72 0 0 N 21.52 0 0 W wreck symbol\r\n"
			...
			try {
				...
				assertNotNull(tLayers.findLayer("NELSON"));
				final Layers tmpLayers = new Layers();
				...
				assertNotNull(tLayers.findLayer("NELSON"));
				assertElementsInLayers(tLayers.findLayer(ANNOTATION_LAYER), 8);
				...
				// verify undo
				assertNull((tLayers.findLayer(ANNOTATION_LAYER)));
		...
		public final void testPasteRepShapes() {
			...
			testImporter.importThis(textToPaste, totalLines);
			ImportReplay.injectContent(tmpLayers, dest, true);
			assertElementsInLayers(tmpLayers.findLayer(ANNOTATION_LAYER), 8);
			...
			// if they are added to the correct layer.
			// verify undo
			assertNull((tmpLayers.findLayer(ANNOTATION_LAYER)));
	...
	private final static void addElementsToExistingLayer(final Layer layerToAddTo, final Editable newItems) {
		// special handling. If we're adding a track to a track, we'll add the
		// individual
		// elements, not the segments
		if (layerToAddTo instanceof LightweightTrackWrapper && newItems instanceof LightweightTrackWrapper) {
			...
			while (tempElements.hasMoreElements()) {
				final Editable elem = tempElements.nextElement();
	 ...
	 * @param importers
	 */
	public static void addExtraImporters(final List<ExtensibleLineImporter> importers) {
		// do we have a holder?
	...
	/**
	 * utility method to extract a label from a composite end of line block that
	 * optionally includes a comment ;SENSOR2: 20090722 041434.000 NONSUCH @B NULL
	 * 59.3 300.8 49.96 NULL LABEL // COMMENT
	 */
	...
	public final static void injectContent(final Layers from, final Layers destination, final boolean performAdd) {
		final Enumeration<Editable> tempElements = from.elements();
		// now add to the plot's layers object
		while (tempElements.hasMoreElements()) {
			final Editable l = tempElements.nextElement();
			final Layer existingLayer = destination.findLayer(l.getName());
			if (existingLayer == null) {
				final Layer layerToAdd = (Layer) l;
				if (performAdd) {
				...
				if (performAdd) {
					...
					if (l instanceof Layer) {
						// dont directly delete a layer, may be there were some elements previously
						// existing
						final Enumeration<Editable> elements = ((Layer) l).elements();
						final Layer destLayer = destination.findLayer(l.getName());
						while (elements.hasMoreElements()) {
	...
	final public static boolean isContentImportable(final String content) {
		...
		int lineCount = 0;
		final int maxLines = Math.min(6, lines.length);
		while (lineCount < maxLines && proceed) {
			final String line = lines[lineCount];
			if (line.startsWith(";") && !line.startsWith(";;")) {
				final StringTokenizer lineTokens = new StringTokenizer(line);
				if (lineTokens.hasMoreTokens()) {
					final String firstWord = lineTokens.nextToken();
					final String regex = "^;[A-Z1-9_]{3,40}+:$";
					final Pattern pattern = Pattern.compile(regex);
					final Matcher match = pattern.matcher(firstWord);
					if (!match.matches()) {
						proceed = false;
					}
				}
	 ...
	 *
	 */
	private final HashMap<TrackWrapper, Vector<SensorWrapper>> _pendingSensors = new HashMap<TrackWrapper, Vector<SensorWrapper>>();

	/**
	 * a list of any exiting tracks that got modified (so we can tell people they've
	 * moved at the end of hte operation
	 */
	private final List<TrackWrapper> _existingTracksThatMoved = new ArrayList<TrackWrapper>();
	...
	private final Map<String, Long> _lastImportedItem = new HashMap<String, Long>();
	...
	public final void exportThis(final String val) {
		if (val != null) {
			...
			final java.awt.datatransfer.StringSelection ss = new java.awt.datatransfer.StringSelection(val);
			cl.setContents(ss, ss);
		}
	}

	/**
	 * some importers may need to finalize, if there is end of import processing to
	 * conduct. Trigger that here
	 */
		...
		if (_extensionImporters != null) {
			for (final ExtensibleLineImporter importer : _extensionImporters) {
				if (importer instanceof PlainLineImporter.ImportRequiresFinalisation) {
					final PlainLineImporter.ImportRequiresFinalisation fin = (ImportRequiresFinalisation) importer;
					fin.finalise();
				}
			}
		}
	...
	final private String getThisSymProperty(final String symbology, final String property_name) {
		final int indexFirstBracket = symbology.indexOf('[');
		final int indexLastBracket = symbology.indexOf(']');
		if (indexFirstBracket < 0 || indexLastBracket < 0) {
			return null;
		}
		final String regexp = "(?<NAME>.*?)\\=(?<VALUE>[^,]*),?";
		final Matcher m = Pattern.compile(regexp).matcher(symbology.substring(indexFirstBracket + 1, indexLastBracket));
	...
	private void handleException(final Exception e, final int lineCount, final String line, final String fName,
	...
	private final void importRep(final String fName, final InputStream is, final int numLines,
		...
		try {
			...
			br = provider == null ? new ReaderMonitor(reader, numLines, nameToUse)
					: new ReaderMonitor(reader, numLines, nameToUse, provider);
			...
			if (is.available() > 0) {
				...
				final Iterator<INewItemListener> newIiter = getLayers().getNewItemListeners().iterator();
				...
				final Iterator<TrackWrapper> tIter = _existingTracksThatMoved.iterator();
				while (tIter.hasNext()) {
	...
	@Override
	public final void importThis(final String fName, final InputStream is, final MonitorProvider provider) {
		final int numLines = countLinesFor(fName);
	...
	private void proccessShapeWrapper(final PlainLineImporter thisOne, final Object thisObject) {
		...
		if (symbology != null && !symbology.isEmpty() && symbology.length() > 2) {
			shapeWrapper.getShape().setLineStyle(ImportReplay.replayLineStyleFor(symbology.substring(2)));
			if (symbology.length() > 3) {
				shapeWrapper.getShape().setLineWidth(ImportReplay.replayLineThicknesFor(symbology.substring(3)));
			}
	...
	private HiResDate processDynamicShapeWrapper(final DynamicTrackShapeWrapper sw) {
		...
		if (val == null || !(val instanceof TrackWrapper)) {
			MWC.Utilities.Errors.Trace.trace(
					"Dynamic shapes can only be added to top level tracks. " + "Dynamic data ignored: " + sensorName);
			return res;
		}
	...
	private HiResDate processReplayFix(final ReplayFix rf) {
		...
		final String targetLayer = targetLayerFor(rf.theSymbology);

		if (targetLayer != null) {
			// does this exist?
			final Layer target = getLayerFor(targetLayer, true);
			...
			final Editable found = folder.find(rf.theTrackName);
			...
			if (_importSettings != null) {
				...
				if (ImportReplay.IMPORT_AS_OTG.equals(_importSettings.importMode)) {
					...
					if (lastTime == null || (!isMax && res.getDate().getTime() >= lastTime + sampleFreq)) {
						// ok, carry on
					} else {
						return null;
					}
	...
	public HiResDate readLine(final String theLine) throws IOException, ParseException {
		...
		if (thisObject instanceof ReplayFix) {
			...
			final PlanningSegment ps = (PlanningSegment) thisObject;

			final String trackName = ps.getParentName();
			final Layer track = getLayerFor(trackName, false);
			if (track == null) {
	...
	public void storePendingSensors() {
		...
		while (tIter.hasNext()) {
			final TrackWrapper trackWrapper = tIter.next();
			final Iterator<SensorWrapper> iter = _pendingSensors.get(trackWrapper).iterator();
			while (iter.hasNext()) {
				...
				if (parent != null) {
					...
					Application.logError2(ToolParent.INFO, "Not storing sensor, track not found", null);
				}
			}
		}
	...
	/**
	 * examine the sybmology, to see if a target layer is specified. If it isn't
	 * just put it into the annotations layer
	 *
	 * @param thisOne
	 * @return the name of the layer to use
	 */
