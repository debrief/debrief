 ...
 */
final class ImportPlanningLegSpeedTime extends AbstractPlainLineImporter {
	/**
	...
	public final String getYourType() {
		return _myTypePlanning_spd_time;
	}
	...
	public final Object readThisLine(final String theLine) throws ParseException {
		...
		// ;PLANNING_SPEED_TIME: AAAAAA BBBBBB SSS TTT
		...
		final WorldSpeed speed = new WorldSpeed(MWCXMLReader.readThisDouble(st.nextToken()), WorldSpeed.Kts);
		...
		if (closing) {
			res = new PlanningSegment.ClosingSegment(theLeg, course, speed, null);
		} else {
			res = new PlanningSegment(theLeg, course, speed, null);
		}

		res.setDuration(time);
		res.setParentName(theTrack);
		res.setCalculation(PlanningLegCalcModelPropertyEditor.SPEED_TIME);
