...
abstract public class EllipseHandler extends ShapeHandler implements PlottableExporter {
	...
	public final void exportThisPlottable(final MWC.GUI.Plottable plottable, final org.w3c.dom.Element parent,
		...
		if (ps instanceof MWC.GUI.Shapes.EllipseShape) {
			...
			WorldDistanceHandler.exportDistance("Maxima", cs.getMaxima(), ePlottable, doc);
			WorldDistanceHandler.exportDistance("Minima", cs.getMinima(), ePlottable, doc);
		} else {
	...
	public final MWC.GUI.Shapes.PlainShape getShape() {
		...
		if (_maxDist != null) {
			...
			ls = new MWC.GUI.Shapes.EllipseShape(_centre, _orient, new WorldDistance(max_degs, WorldDistance.DEGS),
					new WorldDistance(min_degs, WorldDistance.DEGS));
		}
		if (_filled != null)
