...
abstract public class TMAContactHandler extends MWC.Utilities.ReaderWriter.XML.MWCXMLReader {
			...
			try {
				...
				while (solutions.hasMoreElements()) {
					...
					while (contacts.hasMoreElements()) {
						...
						assertEquals("correct label loc", new Integer(MWC.GUI.Properties.LocationPropertyEditor.RIGHT),
	...
	public static void exportSolution(final Debrief.Wrappers.TMAContactWrapper contact, final Element parent,
			final Document doc) {
		/*
		 *
		 * <!ELEMENT tma_solution (colour?, centre?)> <!ATTLIST tma_solution Dtg CDATA
		 * #REQUIRED Bearing CDATA #IMPLIED Range CDATA #IMPLIED Visible (TRUE | FALSE)
		 * "TRUE" Label CDATA #REQUIRED LabelShowing (TRUE | FALSE) "TRUE" LineShowing
		 * (TRUE | FALSE) "TRUE" EllipseShowing (TRUE | FALSE) "TRUE" SymbolShowing
		 * (TRUE | FALSE) "TRUE" LabelLocation (Top | Left | Bottom | Centre | Right)
		 * "Left" Course CDATA #REQUIRED Speed CDATA #REQUIRED Depth CDATA #REQUIRED
		 * Orientation CDATA #REQUIRED Maxima CDATA #REQUIRED Minima CDATA #REQUIRED >
		 */
