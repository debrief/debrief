...
public class TrackHandler extends MWC.Utilities.ReaderWriter.XML.MWCXMLReader {
	 ...
	 */
	private static final NullableLocationPropertyEditor lp = new NullableLocationPropertyEditor();

	// private MWC.GUI.Layer _myLayer;

	private static void exportThisTrackSegment(final org.w3c.dom.Document doc, final Element trk,
			final TrackSegment segment) {
		// right, sort out what type it is
		if (segment instanceof RelativeTMASegment) {
			RelativeTMASegmentHandler.exportThisTMASegment(doc, trk, (RelativeTMASegment) segment);
		} else if (segment instanceof AbsoluteTMASegment) {
			AbsoluteTMASegmentHandler.exportThisTMASegment(doc, trk, (AbsoluteTMASegment) segment);
		} else if (segment instanceof DynamicInfillSegment) {
			DynamicInfillSegmentHandler.exportThisSegment(doc, trk, (DynamicInfillSegment) segment);
		} else {
	...
	protected static void exportTrackObject(final Debrief.Wrappers.TrackWrapper track, final org.w3c.dom.Element trk,
			final org.w3c.dom.Document doc) {
		...
		trk.setAttribute(SENSORS_VISIBLE, writeThis(track.getSensors().getVisible()));
		trk.setAttribute(SOLUTIONS_VISIBLE, writeThis(track.getSolutions().getVisible()));
		...
		if (sensors != null) {
			while (sensors.hasMoreElements()) {
				final Debrief.Wrappers.SensorWrapper thisS = (SensorWrapper) sensors.nextElement();
		...
		if (shapeSet != null) {
			while (shapeSet.hasMoreElements()) {
				final DynamicTrackShapeSetWrapper thisS = (DynamicTrackShapeSetWrapper) shapeSet.nextElement();
	...
	public void elementClosed() {
		...
		if (existing != null) {
			Application.logError2(ErrorLogger.WARNING,
					"Renaming track:" + _myTrack.getName() + ",  name already in use", null);
