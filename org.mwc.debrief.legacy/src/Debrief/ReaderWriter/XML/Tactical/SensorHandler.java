...
abstract public class SensorHandler extends MWC.Utilities.ReaderWriter.XML.MWCXMLReader {
	...
	public static void exportSensor(final Debrief.Wrappers.SensorWrapper sensor, final org.w3c.dom.Element parent,
		...
		if (theOffset != null) {
			...
			if (theOffset.getValue() != 0)
				MWC.Utilities.ReaderWriter.XML.Util.WorldDistanceHandler.exportDistance(OFFSET, theOffset, trk, doc);
		}

		// and the worm in the hole indicator?
		final ArrayCentreMode mode = sensor.getArrayCentreMode();
		...
		addAttributeHandler(new HandleAttribute(ARRAY_MODE) {
			...
			public void setValue(final String name, final String value) {
				...
				if (LegacyArrayOffsetModes.PLAIN.asString().equals(value)) {
					...
					mode = new ArrayOffsetHelper.DeferredDatasetArrayMode(value);
				}
				_mySensor.setArrayCentreMode(mode);
			}
		});
