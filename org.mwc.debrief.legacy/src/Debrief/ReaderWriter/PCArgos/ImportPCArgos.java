...
final class ImportPCArgos extends MWC.Utilities.ReaderWriter.PlainImporterBase {
	...
	/**
	 * the 'last' points loaded for each track (used to calculate course and speed
	 * from last point, and the time of next valid point).
	 */
	 ...
	 */
	public final void importThis(final Layers theData, final String fName, final java.io.InputStream is,
			final WorldLocation origin, final long DTG, final long freq) {
			...
			if (thisObject != null) {
				...
				if (thisObject instanceof Debrief.ReaderWriter.Replay.ReplayFix) {
					...
					if (oj != null) {
						...
						final double secsDelta = ((microsDelta)) / 1000000.0;
					...
					if (trkWrapper == null) {
						...
						final Color thisCol = colors.elementAt(track_counter);
