...
public class PlotTracks {
	...
	final public static int INITIAL_FOOTPRINT_ID = INITIAL_MARKER_ID + MARKER_FOOTPRINT_DELTA;

	private void addAnimationFootPrintsMarkerAppearing(final Element time_anim_tag_first,
			final Element anim_insertion_tag_upper, final TrackData trackData, final int intervalDuration_in,
			final int initialFootprintId_in, final ArrayList<Element> marker_appearing) {
		final int intervalDuration = intervalDuration_in;
		...
		for (final Element appearing : marker_appearing) {
			anim_insertion_tag_upper.insertChildren(0, appearing);
		}
	}

	private void addAnimationObjects(final ArrayList<ArrayList<Element>> all_animation_objs,
			final Element anim_tag_upper, final Element anim_insertion_tag_upper, final TrackData trackData,
		...
		for (final ArrayList<Element> track_anim_objs : all_animation_objs) {
			...
			anim_tag_upper_temp.selectFirst("p|cond").attr("delay",
					trackData.getTracks().get(track_num - 1).getStepsToSkip() * intervalDuration + "");
			anim_tag_upper_temp.selectFirst("p|cTn").attr("nodeType", "withEffect");
			anim_tag_upper_temp.selectFirst("p|cTn").attr("grpId", "1");
			final Element parent_temp = anim_tag_upper_temp.selectFirst("p|animMotion").parent();
	...
	private void addShapeMarkerFootPrintsObjects(final Element spTreeobj, final ArrayList<Element> arrow_objs,
	...
	private float[] arrowCoordinates(final Element temp_arrow_tag) {
		final float arrow_off_x = Float.parseFloat(temp_arrow_tag.selectFirst("a|off").attr("x"));
		final float arrow_off_y = Float.parseFloat(temp_arrow_tag.selectFirst("a|off").attr("y"));
		final float arrow_ext_cx = Float.parseFloat(temp_arrow_tag.selectFirst("a|ext").attr("cx"));
		final float arrow_ext_cy = Float.parseFloat(temp_arrow_tag.selectFirst("a|ext").attr("cy"));

		return new float[] { arrow_off_x, arrow_off_y, arrow_ext_cx, arrow_ext_cy };
	}
	 ...
	 */
	public float[] coordinateTransformation(final float x_in, final float y_in, final float dimensionWidth,
			final float dimensionHeight, final float rectX, final float rectY, final float rectWidth,
	...
	private int[] coordinateTransformation(final int x_in, final int y_in, final int dimensionWidth,
			final int dimensionHeight, final int rectX, final int rectY, final int rectWidth, final int rectHeight,
			final int invertY) {
	...
	/**
	 * Given the track data, the slide path and the temporary unpack folder path, it
	 * creates the pptx file and returns the path
	 *
	 ...
	 * @throws DebriefException
	 */
	private String createPptxFromTrackData(final TrackData trackData, final String slide_path,
			final String temp_unpack_path, final String output_filename)
			throws IOException, ZipException, DebriefException {
		Application.logError2(ToolParent.INFO, "Number of tracks::: " + trackData.getTracks().size(), null);

		// Get slide size from presentation.xml file
		final String[] slideDimen = new ParsePresentation().retrieveDimensions(temp_unpack_path);
		...
		final HashMap<String, String> mapDetails = FindMap.getMapDetails(temp_unpack_path);
		...
		float[] tl_tmp = coordinateTransformation(mapX, mapY, Float.parseFloat(slide_dimen_x),
				Float.parseFloat(slide_dimen_y), 0, 0, 1, 1, 0);
		final float TLx = tl_tmp[0];
		final float TLy = tl_tmp[1];
		tl_tmp = coordinateTransformation(mapX + mapCX, mapY + mapCY, Float.parseFloat(slide_dimen_x),
				Float.parseFloat(slide_dimen_y), 0, 0, 1, 1, 0);
		...
		if ("".equals(trackData.getScaleUnit())) {
			...
			final int[] scale_size_tmp = coordinateTransformation(trackData.getScaleWidth(), 0, dimensionWidth,
					dimensionHeight, 0, 0, mapCX, mapCY, 1);
			scaleBarTag.selectFirst("a|xfrm").selectFirst("a|ext").attr("cx", scale_size_tmp[0] + "");

			String scaleName = trackData.getScaleUnit();
			if (trackData.getScaleAmount() > 1) {
				scaleName += "s";
			}
			scaleValueTag.selectFirst("a|t").text(trackData.getScaleAmount() + " " + scaleName);
		}
		...
		final int footprint_x_size = Integer.parseInt(footprint_tag.selectFirst("a|ext").attr("cx"));
		final int footprint_y_size = Integer.parseInt(footprint_tag.selectFirst("a|ext").attr("cy"));

		int trackCount = 0;
		int current_arrow_id = Integer.parseInt(arrow_tag.selectFirst("p|cNvPr").attr("id"));
		Application.logError2(ToolParent.INFO, "Last Arrow Id::::: " + current_arrow_id, null);

		final ArrayList<Integer> arrow_ids = new ArrayList<>();
		final ArrayList<Element> arrow_objs = new ArrayList<>();
		final ArrayList<ArrayList<Element>> marker_animation_objs = new ArrayList<>();
		...
		final int initialFootprintId = Integer.parseInt(footprint_tag.selectFirst("p|cNvPr").attr("id"));
		int footprint_count = initialFootprintId;

		for (final Track track : trackData.getTracks()) {
			// We add the animation to the marker.
			final Element temp_time_anim_for_marker = time_anim_tag_first.clone();
			temp_time_anim_for_marker.selectFirst("p|spTgt").attr("spid", current_arrow_id + "");
			temp_time_anim_for_marker.selectFirst("p|cond").attr("delay",
					track.getStepsToSkip() * intervalDuration + "");
			temp_time_anim_for_marker.selectFirst("p|cTn").attr("nodeType", "withEffect");
			...
			final int[] arrow_pointer_temp = getArrowPointerCoordinates(temp_arrow_tag);
			...
			// the absolute)
			final float TailXUnscalled = arrow_ext_cx * (float) (arrow_pointer_x / 100000.0);
			final float TailYUnscalled = arrow_ext_cy * (float) (arrow_pointer_y / 100000.0);

			float[] tempCoordinates = coordinateTransformation(TailXUnscalled, TailYUnscalled,
					Float.parseFloat(slide_dimen_x), Float.parseFloat(slide_dimen_y), 0, 0, 1, 1, 0);
			final float TailX = tempCoordinates[0];
			final float TailY = tempCoordinates[1];

			// Scaling centre coordinates of call out values to 0...1
			tempCoordinates = coordinateTransformation(arrow_center_x, arrow_center_y, Float.parseFloat(slide_dimen_x),
					Float.parseFloat(slide_dimen_y), 0, 0, 1, 1, 0);
			...
			temp_arrow_tag.selectFirst("p|txBody").selectFirst("a|p").selectFirst("a|r").selectFirst("a|t")
					.text(trackName);
			...
			tempCoordinates = coordinateTransformation(first_x, first_y, dimensionWidth, dimensionHeight, animX, animY,
					animCX, animCY, 1);
			...
			final float toSubX = prev_anim_x;
			final float toSubY = prev_anim_y;

			// We calculate the first position for the marker
			tempCoordinates = coordinateTransformation(first_x, first_y, dimensionWidth, dimensionHeight, mapX, mapY,
					mapCX, mapCY, 1);
			// Get Shape offsets and exts
			temp_arrow_tag.selectFirst("a|off").attr("x",
					(int) (tempCoordinates[0] - arrow_ext_cx / 2 - TailXUnscalled) + "");
			temp_arrow_tag.selectFirst("a|off").attr("y",
					(int) (tempCoordinates[1] - arrow_ext_cy / 2 - TailYUnscalled) + "");
			...
			for (final TrackPoint coordinate : coordinates) {
				...
				tempCoordinates = coordinateTransformation(x, y, dimensionWidth, dimensionHeight, animX, animY, animCX,
						animCY, 1);
				...
				animation_path = "M " + String.format("%.4f", (prev_anim_x - toSubX)) + " "
						...
						+ String.format("%.4f", (anim_x - toSubX)) + " " + String.format("%.4f", (anim_y - toSubY));
				prev_anim_x = anim_x;
				prev_anim_y = anim_y;

				temp_anim_tag.attr("path", animation_path);
				temp_anim_tag.selectFirst("p|spTgt").attr("spid", current_arrow_id + "");
				temp_anim_tag.selectFirst("p|cTn").attr("id",
						Integer.parseInt(temp_anim_tag.selectFirst("p|cTn").attr("id")) + trackCount + coord_count
								+ "");
				...
				final int[] tempCoordinatesInt = coordinateTransformation(x_int, y_int, dimensionWidth, dimensionHeight,
						mapX, mapY, mapCX, mapCY, 1);
				...
				temp_footprint_tag.selectFirst("a|off").attr("x", (x_int - footprint_x_size / 2) + "");
				temp_footprint_tag.selectFirst("a|off").attr("y", (y_int - footprint_y_size / 2) + "");
				...
				final Element coordinate_soup = Jsoup.parse("<a:pt x='" + x_int + "' y='" + y_int + "'/>", "",
						Parser.xmlParser());
				if (num_coordinate == 0) {
			...
			final Element solidFill = temp_arrow_tag.selectFirst("p|spPr").selectFirst("a|ln")
					.selectFirst("a|solidFill");
		...
		addAnimationObjects(marker_animation_objs, anim_tag_upper, anim_insertion_tag_upper, trackData,
				intervalDuration);
		addAnimationFootPrintsMarkerAppearing(time_anim_tag_first, anim_insertion_tag_upper, trackData,
				intervalDuration, initialFootprintId, marker_appearing);
		createTimeNarrativeShapes(spTreeobj, trackData, time_tag, time_anim_tag_first, anim_insertion_tag_upper,
				time_anim_tag_big, time_anim_tag_big_insertion, narrative_tag);
	 ...
	 * @param narrative_tag
	 */
	private void createTimeNarrativeShapes(final Element spTreeobj, final TrackData trackData, final Element time_tag,
			final Element time_anim_tag_first, final Element anim_insertion_tag_upper, final Element time_anim_tag_big,
		...
		for (final TrackPoint coordinate : coordinates) {
			...
			if (coord_num == 0) {
				...
				temp_time_anim.selectFirst("p|par").child(0).selectFirst("p|par").selectFirst("p|cond").attr("delay",
						intervalDuration + "");
		...
		Application.logError2(ToolParent.INFO, "Last Narrative Id::::: " + current_narrative_id, null);
		...
		for (final ExportNarrativeEntry narrative : trackData.getNarrativeEntries()) {
			...
			temp_narrative_tag.selectFirst("p|txBody").selectFirst("a|p").selectFirst("a|r").selectFirst("a|t")
					.text(time_str + " " + narrative.getText());
			...
			if (num_narrative == 0) {
				...
				temp_narrative_anim.selectFirst("p|par").child(0).selectFirst("p|par").selectFirst("p|cond")
						.attr("delay", intervalDuration + "");
				time_anim_tag_big_insertion.insertChildren(time_anim_tag_big_insertion.childNodeSize(),
						temp_narrative_anim);
			}
	...
	private Element[] findAnimationTagObjects(final Document soup) {
		...
		return new Element[] { anim_tag, anim_tag_upper, anim_insertion_tag_upper };
	}
	 ...
	 * @return
	 */
	private Element[] findTimeAnimationObjects(final Document soup, final Element time_tag) {
		final String time_id_original = time_tag.select("p|cNvPr").get(0).attr("id");
		...
		return new Element[] { time_anim_tag_first, time_anim_tag_big, time_anim_tag_big_insertion };
	}
	...
	private int[] getArrowPointerCoordinates(final Element temp_arrow_tag) {
		final Elements gds = temp_arrow_tag.select("p|spPr").select("a|prstGeom").select("a|avLst").select("a|gd");
		final int arrow_pointer_x = Integer.parseInt(gds.get(0).attr("fmla").substring(4));
		final int arrow_pointer_y = Integer.parseInt(gds.get(1).attr("fmla").substring(4));
	...
	private int[] getMapDimesions(final HashMap<String, String> mapDetails) {
		return new int[] { Integer.parseInt(mapDetails.get("x")), Integer.parseInt(mapDetails.get("y")),
				Integer.parseInt(mapDetails.get("cx")), Integer.parseInt(mapDetails.get("cy")) };
	}

	/**
	 * We return the track, marker, time and narrative, removing it from the soup
	 * document
	 *
	...
	private Element[] getShapes(final Document soup) {
		...
		final ArrayList<Element> toRemove = new ArrayList<>();
		...
		for (final Element toRemoveElement : toRemove) {
			toRemoveElement.remove();
		}
		return new Element[] { arrow_tag, time_tag, narrative_tag, footprint_tag, scaleBarTag, scaleValueTag };
	}

	public HashMap<String, String> retrieveMapProperties(final String donorTemplatePath)
			throws IOException, ZipException, DebriefException {

		final String unpackPath = checkPathandInitialization(donorTemplatePath)[1];
		final HashMap<String, String> answer = FindMap.getMapDetails(unpackPath);

		final int pointSize = 12700;
		final String[] coordinates = new String[] { "x", "y", "cx", "cy" };
		for (final String coordinate : coordinates) {
			answer.put(coordinate, (int) Math.round(Double.parseDouble(answer.get(coordinate)) / pointSize) + "");
		}
	...
	/**
	 * It returns null (for success) or a series of String messages for the invalid
	 * conditions.
	 *
	 * @param donorTemplatePath donor path
	 * @return null (for success) or a series of String messages for the invalid
	 *         conditions.
	 */
	public String validateDonorFile(final String donorTemplatePath) {
		...
		try {
			final String[] slideAndUnpackPath = checkPathandInitialization(donorTemplatePath);
			...
			final HashSet<String> allFiles = new HashSet<>();
			for (final File genFile : FileUtils.listFilesAndDirs(new File(temp_unpack_path), TrueFileFilter.INSTANCE,
					TrueFileFilter.INSTANCE)) {
				String fileName = genFile.getAbsolutePath().substring(temp_unpack_path.length());
				if (fileName.contains("\\")) {
					fileName = fileName.replaceAll("\\\\", "/");
				}
				allFiles.add(fileName);
			}
			final String[] mustContain = new String[] { "/[Content_Types].xml", "/_rels", "/ppt", "/ppt/slides",
					presentation_path, "/docProps", slide_path.substring(temp_unpack_path.length()) };
			...
			final ArrayList<String> slides = FindMap.getSlides(slides_base_path);
			if (slides.size() != 1) {
				returnValue = "It must contains exactly one slide";
			}
			final HashMap<String, String> map = FindMap.getMapDetails(temp_unpack_path);
			...
			final String updateTemplate = "(Please, update the version of the master template)";
			if (shapes[0] == null) {
			...
			try {
				final byte[] encodedPresentation = Files.readAllBytes(Paths.get(temp_unpack_path + presentation_path));
		...
		if (temp_unpack_path != null) {
			try {
				...
				Application.logError2(ToolParent.ERROR, e.getMessage(), e);
			}
		}
	...
	private void writeSoup(final String slide_path, final Document soup) throws DebriefException {
		try {
