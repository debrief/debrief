...
public class TrackParser {
	...
	/**
	 * Given a Soup Document and a Track, it retrieves the the dimensions, interval
	 * and name
	 *
	...
	private void parseNarratives(final TrackData trackData, final Document soup) {
		...
		if (!narrativeEntries.isEmpty()) {
			...
			for (final Element entry : entries) {
				final ExportNarrativeEntry entryInstance = new ExportNarrativeEntry(entry.attr("Text"),
						entry.attr("dateStr"), entry.attr("elapsed"), null);
	...
	private void parseTracks(final TrackData trackData, final Document soup) throws ParseException {
		...
		for (final Element track : tracks) {
			...
			for (final Element coordinate : track.select("trkpt")) {
				...
				final TrackPoint point = new TrackPoint(Float.parseFloat(coordinate.attr("lat")),
						Float.parseFloat(coordinate.attr("lon")),
						Float.parseFloat(coordinate.selectFirst("ele").text()), dateTime,
