...
public class ImportASWDataDocument {
	public static class TestImportWord extends TestCase {
		private class MyHelper implements ImportNarrativeDocument.QuestionHelper {
			...
			@Override
			public String askQuestion(final String title, final String message, final String defaultStr) {
		...
		private static WorldLocation loc(final double lat, final double lon, final double dep) {
		...
		public void testDates() {
			...
			assertEquals("aaa", yearStr + "0426 120000",
					DebriefFormatDateTime.toString(dateFor(d1, 0).getDate().getTime()));
			assertEquals("aaa", yearStr + "0426 232700",
					DebriefFormatDateTime.toString(dateFor(d2, 0).getDate().getTime()));
			assertEquals("aaa", yearStr + "0122 030000",
					DebriefFormatDateTime.toString(dateFor(d3, 0).getDate().getTime()));
			assertEquals("aaa", yearStr + "0401 234500",
					DebriefFormatDateTime.toString(dateFor(d4, 0).getDate().getTime()));
			assertEquals("aaa", yearStr + "1129 225800",
					DebriefFormatDateTime.toString(dateFor(d5, 0).getDate().getTime()));
		}
		...
		public void testParseDates() {
			...
			assertNull("wrong format", yearFor("TMPOS/290000Z/300200Z/APR/2016/APR2016"));
			...
			assertNull("too late", yearFor("TMPOS/290000Z/300200Z/APR/124316/APR2016"));
		}
		...
		public void testParseDocument() {
			...
			assertEquals("we should have stored the default str", DEFAULT_TRACK_MESSAGE, questionStore);

			assertTrue("We should have a track", layers.size() == 1);
			final TrackWrapper track = (TrackWrapper) layers.findLayer(NORWICH);

			// check it has some data
			final TrackSegment segment = (TrackSegment) track.getSegments().elements().nextElement();
			...
			assertEquals("we should been offered the new default", "NORWICH", questionStore);

			final List<String> lines2 = incrementTimes(lines);
			ImportASWDataDocument.setQuestionHelper(new MyHelper(NORWICH));

			iw.processThese(lines2);
			final TrackSegment segment2 = (TrackSegment) track.getSegments().elements().nextElement();
		...
		public void testParseDocument_mm_mm() {
			...
			final WorldArea area = track.getBounds();
			// System.out.println(area);
			assertEquals("correct area",
					" Area TL: 00\u00B010'45.00\"N 000\u00B034'45.00\"W  BR: 00\u00B001'00.00\"N 000\u00B021'00.00\"W ",
					area.toString());
		}
		...
		public void testParseDocument_mm_ss() {
			...
			// System.out.println(area);
			assertEquals("correct area",
					" Area TL: 00\u00B010'40.00\"N 000\u00B035'00.00\"W  BR: 00\u00B001'00.00\"N 000\u00B021'00.00\"W ",
					area.toString());
		}

		@SuppressWarnings("unused")
		public void testParseFixes() {
			final String d1 = "TMPOS/261200ZAPR/IN/12.23.34N-121.12.1E/o57T/04KTS/05.1M//";
			final String d2 = "TIMPOS/262327ZAPR/INS/13.14.15S-011.22.33W/000T/13KTS/0o1FT";
			final String d3 = "TMPOS/220000ZAPR/GPS/1230N/01215W";
			final String d4 = "TMPOS/230001ZAPR/GPS/22.33.44N/020.11.22W/006T/10KTS//";
			...
			assertEquals(new WorldDistance(1, WorldDistance.FT).getValueIn(WorldDistance.METRES), f2.getDepth());
			assertEquals("160426 232700", DebriefFormatDateTime.toStringHiRes(f2.getDateTimeGroup()));
			...
			assertEquals("980422 000000", DebriefFormatDateTime.toStringHiRes(f3.getDateTimeGroup()));
		}
		...
		public void testParseLocations() {
			...
			try {
				...
				assertEquals("Longitude out of limits:211.22.11W", ie.getMessage());
			}
			...
			try {
				...
				assertEquals("Latitude out of limits:94.02.01N", ie.getMessage());
			}
		...
		public void testReadRealDocument() throws IOException {
			...
			final String fName = "../org.mwc.cmap.combined.feature/root_installs/sample_data/other_formats/ASW Data Format.docx";
	...
	/**
	 * helper class that can ask the user a question populated via Dependency
	 * Injection
	 */
	...
	public static boolean canImport(final List<String> strings) {
		// run through strings, see if we have TMPOS in more than 5 lines in the first
		// 50
	...
	private static double courseFor(final String courseStr) {
		...
		final double courseDegs = Double.parseDouble(clean(courseStr.substring(0, tIndex)));
		return MWC.Algorithms.Conversions.Degs2Rads(courseDegs);
	}

	private static HiResDate dateFor(final String str, final int year) {
		if (!str.contains("Z")) {
			throw new IllegalArgumentException("Wrongly formatted date (expected 123245ZJAN):" + str);
		}
	...
	private static double depthFor(final String depth) {
		...
		if (components.length != 2) {
			throw new IllegalArgumentException("Depth not formatted correctly (expected 23M or 12FT):" + depth);
		}
	...
	private static double getDegreesFor(final String str, final boolean isDecimalMinutes) {
		...
		try {
			...
			if (str.contains(".")) {
				...
				if (comps.length == 3) {
					if (isDecimalMinutes) {
						...
						if (secs >= 60d) {
							throw new IllegalArgumentException(LOCATION_FORMAT_WRONG + str);
						}
			...
			throw new IllegalArgumentException("Couldn't extract location form:" + str);
		}
	}
	...
	private static double getHemiFor(final String str) {
		...
		case "W":
			...
			break;
		default:
			throw new IllegalArgumentException("Not a valid hemisphere:" + lastLetter + " from:" + cleaned);
		}
	...
	private static WorldLocation locationFor(final String str, final String depthStr) {
		...
		// Note: if the depth Str contains "ft", we will interpret the last two blocks
		// as
		...
		final double latVal = getDegreesFor(clean(latStr).substring(0, latStr.length() - 1), IsDecimalMinutes);
		...
		final double longVal = getDegreesFor(clean(longStr).substring(0, longStr.length() - 1), IsDecimalMinutes);
	...
	public static void setQuestionHelper(final ImportNarrativeDocument.QuestionHelper helper) {
	...
	private static String[] splitIntoValueAndUnits(final String string) {
		...
		final String[] components = cleaned.split("[^A-Z.0-9]+|(?<=[A-Z])(?=[0-9])|(?<=[0-9])(?=[A-Z])");
	...
	public void processThese(final List<String> strings) {
		...
		if (questionHelper == null) {
			throw new RuntimeException("ASW Data importer has not had a Question Helper assigned");
		}

		final String defaultTrack = _lastTrackName != null ? _lastTrackName : DEFAULT_TRACK_MESSAGE;

		final String trackName = questionHelper.askQuestion("Load ASW Track", "Name for this track:", defaultTrack);
		if (trackName == null) {
			return;
		} else {
