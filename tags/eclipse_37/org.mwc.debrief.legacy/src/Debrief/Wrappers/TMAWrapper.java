...
public final class TMAWrapper extends TacticalDataWrapper
{
   ...
   */
			...
			LinearInterpolator interp, long tNow)
	{
		...
		if (origin != null)
		{
			newS = new TMAContactWrapper(_last.getSolutionName(), _last
					.getTrackName(), new HiResDate(0, tNow), origin, courseDegs,
					speedKts, depthM, _last.getActualColor(), _last.getLabel(),
					theEllipse, _last.getSymbol());
		}
		else
		{
			newS = new TMAContactWrapper(_last.getSolutionName(), _last
					.getTrackName(), new HiResDate(0, tNow), rangeYds, bearingDegs,
					courseDegs, speedKts, depthM, _last.getActualColor(), _last
							.getLabel(), theEllipse, _last.getSymbol());
		}
	 ...
	 * of this processing from the similar method in TrackWrappper
	 * 
	 * @param DTG
	...
	public final Watchable[] getNearestTo(final HiResDate DTG)
	{
		...
		else
		{
			...
			final TMAContactWrapper theFirst = (TMAContactWrapper) _myContacts
					.first();
					...
					&& (DTG.lessThanOrEqualTo(theLast.getDTG())))
			{
				...
				// get the data..
				final java.util.Vector<TMAContactWrapper> list = new java.util.Vector<TMAContactWrapper>(
						0, 1);
	...
	static public final class testSolutions extends junit.framework.TestCase
	{
		...
		public final void testValues()
		{
			...
			solution.add(new TMAContactWrapper("tester", "tester", new HiResDate(cal
					.getTime().getTime()), 0, 0, 0, 0, 0, null, "sol:"
					+ solutionCounter++, null, null));
			...
			solution.add(new TMAContactWrapper("tester", "tester", new HiResDate(cal
					.getTime().getTime()), 0, 0, 0, 0, 0, null, null, null, null));
			...
			cal.set(2001, 10, 4, 4, 4, 05);
			Watchable[] list = solution.getNearestTo(new HiResDate(cal.getTime()
					.getTime()));
			...
			list = solution
					.getNearestTo(new HiResDate(cal_other.getTime().getTime()));
			nearest = (TMAContactWrapper) list[0];
			assertTrue("Nearest or greater than fix", (nearest.getDTG().getDate()
					.getTime() == cal.getTime().getTime()));
			...
			// ////////////////////////////////////////////////////////////////////////
			// do the filter
			solution.filterListTo(new HiResDate(cal.getTime().getTime()),
			...
			{
				final TMAContactWrapper contact = (TMAContactWrapper) iter
						.nextElement();
			...
			// //////////////////////////////////////////////////////
			// get items between
			java.util.Collection<Editable> res = solution.getItemsBetween(
			...
			res = solution.getItemsBetween(solution.getStartDTG(), solution
					.getEndDTG());
			...
			list = solution.getNearestTo(new HiResDate(cal.getTime().getTime()));
			nearest = (TMAContactWrapper) list[0];
			assertEquals("test mid way", nearest.getCentre(track), new WorldLocation(
					2.75, 2.0, 0.0));
			...
			list = solution.getNearestTo(new HiResDate(cal.getTime().getTime()));
			nearest = (TMAContactWrapper) list[0];
			assertEquals("test nearer first point", nearest.getCentre(track),
					(new WorldLocation(2.5, 2.0, 0.0)));

			// start point?
			cal.set(2001, 10, 4, 4, 4, 0);
			list = solution.getNearestTo(new HiResDate(cal.getTime().getTime()));
			nearest = (TMAContactWrapper) list[0];
			assertEquals("test start point", nearest.getCentre(track),
					new WorldLocation(2.0, 2.0, 0.0));

			// end point?
			cal.set(2001, 10, 4, 4, 4, 55);
			list = solution.getNearestTo(new HiResDate(cal.getTime().getTime()));
			nearest = (TMAContactWrapper) list[0];
			assertEquals("test end point", nearest.getCentre(track),
					new WorldLocation(2.25, 2.25, 0.0));
		...
		public final void testDuplicates()
		{
			...
			// and create the list of sensor contact data items
			cal.set(2001, 10, 4, 4, 4, 0);
			sensor.add(new TMAContactWrapper("tester", "tester", new HiResDate(cal
					.getTime().getTime()), -1, 0, 0, 0, 0, null, null, null, null));
			...
			cal.set(2001, 10, 4, 4, 4, 25);
			final Watchable[] list = sensor.getNearestTo(new HiResDate(cal.getTime()
					.getTime()));
