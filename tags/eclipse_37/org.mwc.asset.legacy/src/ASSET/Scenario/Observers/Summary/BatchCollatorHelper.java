...
public class BatchCollatorHelper
{
	 ...
	 * 
	 * @param perCase
	 *          whether to collate data on a per-case basis
	 * @param collationStrategy
	 *          the way in which we should collate data
	 */
	public BatchCollatorHelper(String name, boolean perCase,
			String collationStrategy, boolean _isActive, String units)
	{
		setActive(_isActive);
		if (getActive())
		{
	 ...
	 * 
	 * @param type
	 *          the type of collation required
	 * @param perCase
	 *          whether the user wants per-case stats
	 * @return
	 */
	...
	private FileWriter createOutputFileWriter()
	{
		...
		{
			System.err
					.println("Suspect file not found when writing batch collator results");
			e.printStackTrace();
		}
		return os;
	}
	...
	abstract private static class StatsCollator
	{
		 ...
		 *          the id for this case
		 * @param datum
		 *          the new value to store
		 */
		public void addDatum(String scenario_name, String caseIdentifier,
				double datum)
		{
		...
		/**
		 * ok, this hasn't got to be managed on a per-case basis, do the simple
		 * stuff
		 * 
		 * @param scenario_name
		 * @param datum
		 *          the new data value
		 */
		protected abstract void processNonCaseResult(String scenario_name,
				Object datum);
		 ...
		 * @param scenario_name
		 * @param thisCase
		 *          the current container for this data set
		 * @param datum
		 *          the current data item received
		 * @return the changed/updated container for the data set
		 */
		protected abstract Object processCaseResult(String scenario_name,
				Object thisCase, Object datum);
		...
		public String getResult(String name, String units)
		{
			...
			res = "Results for:" + name
					+ MWC.Utilities.TextFormatting.GeneralFormat.LINE_SEPARATOR;
	...
	private static class CountCollator extends StatsCollator
	{
		 ...
		 * @return the changed/updated container for the data set
		 */
		protected Object processCaseResult(String scenario_name, Object thisCase,
				Object datum)
		{
	...
	private static class AverageCollator extends StatsCollator
	{
				...
				Object datum)
		{
			...
			{
				Number thisNum = (Number) datum;
				thisCase = processCaseResult(scenario_name, thisCase, thisNum
						.doubleValue());
			}
	...
	private static class ItemisedListCollator extends ListCollator
	{
		...
		protected void processNonCaseResult(String scenario_name, Object datum)
		{
			super.processNonCaseResult(scenario_name, collatedResult(scenario_name,
					datum));
		}
		...
		protected Object processCaseResult(String scenario_name, Object thisCase,
				double datum)
		{
			return super.processCaseResult(scenario_name, thisCase, collatedResult(
					scenario_name, datum)); // To change body of overridden methods use
		...
		{
			super.processNonCaseResult(scenario_name, collatedResult(scenario_name,
					datum)); // To change body of overridden methods use File | Settings |
										// File Templates.
		}
	...
	static public class CollatorTest extends SupportTesting
	{
		...
		public void doCoreTest(StatsCollator nonPerCaseCC, StatsCollator perCaseCC,
				String name, String nonPerCaseString, String perCaseString, String units)
		{
			...
			assertTrue("count wrongly performed", thisResult
					.indexOf(nonPerCaseString) > 0);
			...
			assertTrue("results wrongly performed",
					newResult.indexOf(perCaseString) > 0);
		...
		public void testAverageCollator()
		{
			...
			doCoreTest(cc, cc2, "meanie", "Count:,4 ,Average:,2.25",
					"a,Count:,3 ,Average:,4", "feet");
		...
		public void testFrequencyListCollator()
		{
			...
			doCoreTest(cc, cc2, "lister", "(3.0: 2), (sddfgsdfs: 2),",
					"a, (sddfgsdfs: 2), (2.0: 1), (5.0: 2)", "feet");
		...
		{
			BatchCollatorHelper helper = new BatchCollatorHelper("lister", true,
					BatchCollator.LIST, true, "feet");
			...
			helper = new BatchCollatorHelper("lister", true, BatchCollator.LIST,
					false, "feet");
