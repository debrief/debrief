...
public class TurnAlgorithm implements MWCModel {
	...
	private double demCourse; // the requested course - in degs
	...
	public ASSET.Participants.Status doTurn(
		...
		// so, lets see if we are handling a vehicle which travels at default
		// speeds
		if (chars instanceof ClimbRateCharacteristics) {
			...
			if (demStatus != null) {
				...
				// ok - what's the delta
				double heightDelta = demHeight
						- (-status.getLocation().getDepth());
				...
				if (heightDelta != 0) {
					...
					if (heightDelta < 0) {
						// we're diving
						defaultSpeed = climber.getDefaultDiveSpeed()
								.getValueIn(WorldSpeed.M_sec);
					} else {
						// we're ascending
						defaultSpeed = climber.getDefaultClimbSpeed()
								.getValueIn(WorldSpeed.M_sec);
					}

					// how long will the depth change take?
					double HeightTime = calcHeightChangeTime(heightDelta,
							chars, secs);
					...
					res = doTurn2(status, demStatus, chars, status.getTime()
							+ HeightMillis);
	...
	public ASSET.Participants.Status doTurn2(
			final ASSET.Participants.Status status,
			final ASSET.Models.Movement.SimpleDemandedStatus demStatus,
			final ASSET.Models.Movement.MovementCharacteristics chars,
			final long newTime) {
		...
		curCourse_rads = MWC.Algorithms.Conversions.Degs2Rads(status
				.getCourse());
		...
		// check that we have a demanded status, else we'll continue in steady
		// state
		if (demStatus == null) {
			demSpeed_m_sec = status.getSpeed().getValueIn(WorldSpeed.M_sec);
			demCourse = status.getCourse();
			demCourseRads = MWC.Algorithms.Conversions.Degs2Rads(demCourse);
			demHeight = -status.getLocation().getDepth();
		} else {
			demSpeed_m_sec = demStatus.getSpeed();
			demCourse = demStatus.getCourse();
			demCourseRads = MWC.Algorithms.Conversions.Degs2Rads(demCourse);
		...
		final double accel_rate_m_sec_sec = chars.getAccelRate().getValueIn(
				WorldAcceleration.M_sec_sec); // value in m/sec/sec
		final double decel_rate_m_sec_sec = chars.getDecelRate().getValueIn(
				WorldAcceleration.M_sec_sec); // value in m/sec/sec

		final double max_speed_m_sec = chars.getMaxSpeed().getValueIn(
				WorldSpeed.M_sec);
		final double min_speed_m_sec = chars.getMinSpeed().getValueIn(
				WorldSpeed.M_sec);

		final double min_height_m = chars.getMinHeight().getValueIn(
				WorldDistance.METRES);
		final double max_height_m = chars.getMaxHeight().getValueIn(
				WorldDistance.METRES);
		...
		if (demHeight != curHeight) {
			...
			final double HeightChangeTime = calcHeightChangeTime(dHeight,
					chars, secs);
			...
			if (Math.abs(dHeight) < 0.001) {
				...
				if (dHeight > 0) {
					...
					changeRate = chars.getClimbRate().getValueIn(
							WorldSpeed.M_sec);
				} else {
					// oooh, diving, get dive rate
					changeRate = chars.getDiveRate().getValueIn(
							WorldSpeed.M_sec);
				}
				...
				// yes, it's a significant height change. Do it justice and give
				// it a proper
				// calculation
				if (dHeight > 0)
		...
		if (deltaSpeed > SPEED_DELTA) {
			...
			// ok, we're not as much as the speed delta, but we are still more
			// than zero - set
			// to demanded speed
		...
		speed_at_end_of_step = curSpeed_m_sec + accel_time
				* speed_change_rate_m_sec_sec * accelerating;
		...
		if (Math.abs(course_change) > COURSE_DELTA) {
			...
			turn_time = chars.calculateTurnTime(mean_speed,
					MWC.Algorithms.Conversions.Rads2Degs(course_change));
		...
		if ((accel_time == 0) && (turn_time == 0)) {
			...
			if (accel_time >= turn_time) {
				...
				if (turn_time > 0) {
					...
					turn_speed = curSpeed_m_sec
							+ (accelerating * turn_time
									* speed_change_rate_m_sec_sec / 2);

					// ok, now do the turn
					performConstantSpeedTurn(chars, turn_speed, turn_time,
							turn_direction);

					// update the current speed
					curSpeed_m_sec = (curSpeed_m_sec + accelerating * turn_time
							* speed_change_rate_m_sec_sec);
				...
				// acceleration
				accel_speed = curSpeed_m_sec + accelerating
						* (accel_time - turn_time)
						* speed_change_rate_m_sec_sec / 2;

				// do the travel whilst changing speed
				this.performStraightLineTravel(accel_time - turn_time,
						accel_speed);

				// update the current speed as a product of the straight line
				// acceleration
				curSpeed_m_sec = (curSpeed_m_sec + accelerating
						* (accel_time - turn_time)
						* speed_change_rate_m_sec_sec);

				// we've now completed the acceleration. do the straight line
				// portion
				...
				if (accel_time > 0) {
					...
					this.performConstantSpeedTurn(chars, turn_speed,
							accel_time, turn_direction);

					// and update the speed
					curSpeed_m_sec = (curSpeed_m_sec + (accelerating
							* accel_time * speed_change_rate_m_sec_sec));

				} // there was an acceleration in the turn

				// and now perform the steady speed turn
				this.performConstantSpeedTurn(chars, curSpeed_m_sec,
						(turn_time - accel_time), turn_direction);
		...
		final double used = rate * secs
				* MWC.Algorithms.Conversions.Mps2Kts(curSpeed_m_sec);
		fuelLevel = fuelLevel - used;

		// update the vectors
		// now produce rng & brg for the lat/long deltas
		final double dRng_m = Math.sqrt(latVal_m * latVal_m + longVal_m
				* longVal_m);
		final double dBrg = Math.atan2(longVal_m, latVal_m);

		// create a new location
		final WorldLocation newLoc = status.getLocation()
				.add(
						new WorldVector(dBrg, Conversions.m2Degs(dRng_m),
								-HeightDelta));
		...
		// minor prob, degs 2 rad conversion means identical courses not
		// assigned as such
		...
		// sometimes we never get onto the demanded course because of rads 2
		// degs not being commutative
	...
	private void performStraightLineTravel(final double straight_time,
			final double travelSpeed_m_sec) {
		if (straight_time > 0) {
	...
	private void performConstantSpeedTurn(final MovementCharacteristics chars,
			final double turn_speed, final double turn_time,
			final double turn_direction) {
		...
		if (turn_time > 0) {
			...
			turn_rate_rads = MWC.Algorithms.Conversions.Degs2Rads(chars
					.calculateTurnRate(curSpeed_m_sec));
			...
			// the turn
			longVal_m = (longVal_m + 2
					* TCircle
					* Math.sin(curCourse_rads + turn_direction * course_change
							/ 2) * Math.sin(course_change / 2));

			latVal_m = (latVal_m + 2
					* TCircle
					* Math.cos(curCourse_rads + turn_direction * course_change
							/ 2) * Math.sin(course_change / 2));
	...
	/**
	 * convenience method to return boolean depending on if we're accelerating
	 * or not
	 * 
	 * @param currentSpeed
	 *            current speed (m/sec)
	 ...
	 */
	public static boolean areWeAccelerating(double currentSpeed,
	 ...
	 *            whether we're accelerating or not
	 * @param moves
	 *            the movement characteristics of this vehicle
	 * @return the relevant accelerating/deceleration rate
	 */
	public static WorldAcceleration calcAccelRate(boolean isAccelerating,
	 ...
	 */
	private static WorldAcceleration calcAccelRate(boolean isAccelerating,
	...
	protected static double calcHeightChangeTime(double dHeight,
			MovementCharacteristics chars, double secs) {
		final double changeRate;
	...
	static public class TurnTest extends SupportTesting {
		...
		public void testHeloStraight() {
			...
			ASSET.Models.Movement.SimpleDemandedStatus dem = new ASSET.Models.Movement.SimpleDemandedStatus(
					0, dem_stat);
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.HeloMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate,
							fuel_usage_rate, maxSpeed, minSpeed,
							defaultClimbRate, defaultDiveRate, maxHeight,
							minHeight, myTurnRate, defaultClimbSpeed,
							defaultDiveSpeed);
			...
			super
					.assertEquals("Straight, const spd",
							MWC.Algorithms.Conversions.Degs2m(wv.getRange()),
							40, 0.001);
			...
			// so, how far should we have travelled: change of 25 m/s takes 1
			// secs at 25 m/sec/sec.
			// so we travel at 22.5 m/sec for 1 secs then 10 m/sec for remaining
			// 9 secs
			// total distance is thus 105m
			final double t1_expected_range = 112.5;
			super.assertEquals("Straight, decelerating", t1_expected_range,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			super.assertEquals("Straight, decelerating", res.getSpeed()
					.getValueIn(WorldSpeed.M_sec), 10, 0.001);
			super.assertEquals("Straight, decelerating", res.getCourse(), stat
					.getCourse(), 0.001);
			...
			wv = res.getLocation().subtract(origin);
			// so, how far should we have traveled? It will take 4 secs to slow
			// from 100 to 0 at 25 m/sec/sec
			// the step is only 1/2 this, so we will slow down to 50 kts -
			// travelling at an average of 75 m_sec through
			// the time step. So, we will travel at 75 m_sec for 2 secs for
			// total of 375m.
			final int t2_expected_range = 150;
			super.assertEquals("Straight, decelerating", t2_expected_range,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			super.assertEquals("Straight, decelerating", 50, res.getSpeed()
					.getValueIn(WorldSpeed.M_sec), 0.001);
			...
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			wv = res.getLocation().subtract(origin);
			// so, how far should we have traveled? It will take 1 second to
			// slow from
			// 20 kts to -5, leaving 9 seconds at -5 m/sec. So 1 at 2.5 (12.5
			// m), 9 at
			// -5 (-45 m), total is -37.5m
			final double t3_expected_range = 37.5;
			super.assertEquals(
					"Straight, decelerating - travelled correct distance",
					t3_expected_range, MWC.Algorithms.Conversions.Degs2m(wv
							.getRange()), 0.001);
			super.assertEquals("Straight, decelerating - travelled backwards",
					180, MWC.Algorithms.Conversions.Rads2Degs(wv.getBearing()),
					0.001);
			super.assertEquals("Straight, decelerating", -5, res.getSpeed()
					.getValueIn(WorldSpeed.M_sec), 0.001);
			...
			wv = res.getLocation().subtract(origin);
			// how far will we go?
			// it would take 10 secs to get to 100 from zero. We only have 5 -
			// so we will only get up to 50 m_sec.
			// it we accelerate linearly, we will travel at average of 25 m_secs
			// for 5 secs = 125m
			super.assertEquals("Straight, accelerating", 125,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			super.assertEquals("Straight, accelerating", 50, res.getSpeed()
					.getValueIn(WorldSpeed.M_sec), 0.001);
			...
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			wv = res.getLocation().subtract(origin);
			// how far? We will take 1 sec to get to 100 m_sec, so 95 m_sec for
			// 1 sec plus 100 m_sec for 9 secs, total
			// dist of 995m
			super.assertEquals("Straight, accelerating", 995,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
		}
		...
		public void testHeloTurnShape() {
			...
			final ASSET.Models.Movement.SimpleDemandedStatus dem = new ASSET.Models.Movement.SimpleDemandedStatus(
					0, dem_stat);
			...
			assertEquals("we're using correct turn diameter", turn_diam, chars
					.getTurningCircleDiameter(10), 0.01);
			...
			assertEquals("changed course by correct amount", 3,
					res.getCourse(), 0.001);
			assertEquals("maintained speed", 10, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0.001);

			// and again
			res = turner.doTurn(res, dem, chars, res.getTime() + 1000);
			// check we've changed course by the correct amount
			assertEquals("changed course by correct amount", 6,
					res.getCourse(), 0.001);
			assertEquals("maintained speed", 10, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0.001);
			...
			final WorldLocation thisEndPoint = stat.getLocation().add(
					new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(90),
							MWC.Algorithms.Conversions.m2Degs(turn_diam), 0));
		...
		public void testSSNStraight() {
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.SSMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate,
							fuel_usage_rate, maxSpeed, minSpeed,
							defaultClimbRate, defaultDiveRate, maxHeight,
			...
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			WorldVector wv = res.getLocation().subtract(origin);
			super.assertEquals("Straight, const spd", 40,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			...
			// if we decelerate at 0.2 m/sec/sec and we want to decelerate 7
			// m/s. This will take 35 secs. We've only
			// got 10 = so it's a uniform deceleration. After 10 secs will have
			// dec by 2 m/sec. Therefore mean speed is
			// (78 + 19)/2 = 18. So, 10 secs at 18 (180)
			super.assertEquals("Straight, decelerating", 180,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			super.assertEquals("Straight, decelerating", 17, res.getSpeed()
					.getValueIn(WorldSpeed.M_sec), 0.001);
			super.assertEquals("Straight, decelerating", res.getCourse(), stat
					.getCourse(), 0.001);
			...
			dem = new SimpleDemandedStatus(0, dem_stat);
			stat.setSpeed(new WorldSpeed(19, WorldSpeed.M_sec));
			// how far should we be? Drop to 17 m/sec will take 20 secs. Mean
			// speed through this
			// period will be 17 m/sec (340m). Leaving 20 secs at 15 m/sec
			// (300m).
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 40000);
			wv = res.getLocation().subtract(origin);
			super.assertEquals("Straight, decelerating", 640,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			...
			// how far? Accelerate will take 4 secs - average is 13 (52m),
			// leaving 6 secs at 14 m/sec (84)
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			wv = res.getLocation().subtract(origin);
			super.assertEquals("Straight, accelerating", 136,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			...
			stat.setSpeed(new WorldSpeed(12, WorldSpeed.M_sec));
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			// how far? We need to accelerate by 7 m/sec. This will take 14 secs
			// at 0.5 m/sec/sec. We will accelerate
			// for 10 secs, taking us to 17 m/sec - average is 14.5 m/sec for 10
			// secs. Distance is: 145m
			wv = res.getLocation().subtract(origin);
			super.assertEquals("Straight, accelerating", 145,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			...
			stat.setSpeed(new WorldSpeed(19, WorldSpeed.M_sec));
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 10000);
			// so, how far? Our max speed is 21. It will take 4 secs to do this
			// 2m/sec change (4 * 20 = 80), leaving
			// 6 secs at 21 m/sec (126).
			wv = res.getLocation().subtract(origin);
			super.assertEquals("Straight, accelerating", 206,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
		...
		public void testAlmostOnCourseTurn() {
			...
			final SimpleDemandedStatus dem = new SimpleDemandedStatus(0,
					dem_stat);
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.SSMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate,
							fuel_usage_rate, maxSpeed, minSpeed, turningCircle,
							defaultClimbRate, defaultDiveRate, maxHeight,
							minHeight);

			// so, how far should we turn in a second?
			// final double sec_turn =
			// chars.calculateTurnRate(stat.getSpeed().getValueIn(WorldSpeed.M_sec));

			stat = turner.doTurn(stat, dem, chars, stat.getTime() + 1000);

			assertEquals("not on demanded course", demCourse, stat.getCourse(),
					0);
		...
		public void testSSNSimpleTurn() {
			...
			// so, how far should we turn in a second?
			final double sec_turn = chars.calculateTurnRate(stat.getSpeed()
					.getValueIn(WorldSpeed.M_sec));
			...
			for (int i = 1; i < 30; i++) {
				...
				assertEquals("turned correct amount", i * sec_turn, stat
						.getCourse(), 0.001);
			}
		...
		public void testSSNTurn() {
			...
			assertEquals("Turn clock, steady speed", 200,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
							.getLat()
							- origin.getLat()), 0.01);
			assertEquals("Turn clock, steady speed", 200,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
			...
			dem = new SimpleDemandedStatus(0, dem_stat);
			res = turner.doTurn(stat, dem, chars, stat.getTime() + 78539);
			assertEquals("Turn anti-clock, steady speed", 200,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
							.getLat()
							- origin.getLat()), 0.01);
			assertEquals("Turn clock, steady speed", 200,
			...
			assertEquals("long turn, final speed", res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 4, 0.001);
			assertEquals("Long turn, final head", res.getCourse(), 179.999,
					0.001);
			assertEquals("Long turn vert travel", 0.000,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
							.getLat()
							- origin.getLat()), 0.01);
			assertEquals("Long turn horiz travel", 400,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
							.getLong()
							- origin.getLong()), 0.01);
			...
			assertEquals("long turn, final speed", res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 4, 0.001);
			assertEquals("Long turn, final head", res.getCourse(), 180.000,
					0.001);
			assertEquals("Long turn vert travel", 0.0000,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation()
							.getLat()
							- origin.getLat()), 0.01);
			assertEquals("Long turn horiz travel", -400,
		...
		public void testSSNDelayedTurn() {
			...
			assertEquals("travelled correct dist", 16,
					MWC.Algorithms.Conversions.Degs2m(vec.getRange()), 0.001);
		...
		public void testHeloHeightChange() {
			...
			bravo.setCategory(new Category(Category.Force.BLUE,
					Category.Environment.AIRBORNE, Category.Type.HELO));
			...
			// see how it looks
			assertEquals("decelerated", 70, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 30, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 40, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 50, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			assertEquals("dived", -80, res.getLocation().getDepth(), 0);
			...
			// see how it looks
			assertEquals("decelerated", 52.5, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 32.5, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 22.5, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 20, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			...
			// see how it looks
			assertEquals("decelerated", 100, res.getSpeed().getValueIn(
					WorldSpeed.M_sec), 0);
			assertEquals("dived", -80, res.getLocation().getDepth(), 0);
		...
		public void testZeroStepTime() {
			...
			Status res = turner.doTurn2(oldStat, oldDem, chars, oldStat
					.getTime());
	 ...
	 * 
	 * @param heightDelta
	 *            the height change we're after
	 * @param moves
	 *            the movement chars for this platform
	 * @return the speed we travel at
	 */
	public static WorldSpeed calcClimbSpeed(double heightDelta,
			ClimbRateCharacteristics moves) {
		WorldSpeed res;
	 ...
	 * 
	 * @param timeSecs
	 *            the time available for the height change
	 * @param moves
	 *            the climb characteristics for the vehicle
	 * @param heightDelta
	 *            the change in height
	 */
	public static double calcHeightChange(double timeSecs,
			MovementCharacteristics moves, double heightDelta) {
		double res = 0;
	...
	public static boolean areWeAccelerating(WorldSpeed speed,
			WorldSpeed climbSpeed) {
		return areWeAccelerating(speed.getValueIn(WorldSpeed.M_sec), climbSpeed
				.getValueIn(WorldSpeed.M_sec));
	}
