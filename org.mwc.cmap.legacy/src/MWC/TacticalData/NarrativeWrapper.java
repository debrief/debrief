...
public final class NarrativeWrapper extends MWC.GUI.PlainWrapper
		implements Layer, IRollingNarrativeProvider, GriddableSeriesMarker {
	...
	public static class TestMe extends TestCase {
		...
		/**
		 * note: the following test was produced to overcome a specific issue. The
		 * narrative wrapper was successfully ignoring duplicate entries, except after a
		 * file had been save/restored.
		 *
		...
		public void testDuplicates() throws ParserConfigurationException, TransformerException, SAXException {
			...
			final NarrativeEntry n1 = new NarrativeEntry("track", new HiResDate(3000), "some entry");
			narr.add(n1);
			...
			final NarrativeEntry n3 = new NarrativeEntry("track", new HiResDate(3100), "some entry");
			narr.add(n3);

			assertEquals("still has two", 2, narr.size());

			// make tiny change
			final NarrativeEntry n4 = new NarrativeEntry("track", new HiResDate(3100), "some entry.");
			narr.add(n4);
			...
			final InputStream stream = new ByteArrayInputStream(output.getBytes(StandardCharsets.UTF_8));
			...
			// get the contents
			final NarrativeWrapper narrLayer = (NarrativeWrapper) parent.findLayer(LayerHandler.NARRATIVE_LAYER);

			final NarrativeEntry theEntry = (NarrativeEntry) narrLayer.elements().nextElement();
		...
		public void testGridding() {
			...
			final NarrativeEntry entry = new NarrativeEntry("some track", new HiResDate(2000000), "Some entry text");
			...
			// we force the entry to be different, since identical copies get silently
			// dropped.
	...
	public static NarrativeWrapper createDummyData(final String title, final int len) {
		...
		final Date newDate = new Date(2005, 06, (int) (Math.random() * 12), (int) (Math.random() * 13), 33);
		for (int i = 0; i < len; i++) {
			...
			final NarrativeEntry ne = new NarrativeEntry(title, "type_" + (int) (Math.random() * 5),
				...
				if (evt.getPropertyName().equals(NarrativeEntry.DTG)) {
					if (evt.getSource() instanceof NarrativeEntry) {
						...
						if (oldValue instanceof GetHiResValue && newValue instanceof GetHiResValue) {
							 ...
							 * getting lost in the tree. So, what I am doing here is to store the old value
							 * again to find the item in the tree, removing it, and then adding it again.
							 * The idea is to resort the items.
							 *
							 * Saul
							 */
	...
	public final void add(final Editable editable) {
		...
		if (editable instanceof NarrativeEntry) {
			...
			if (_myListeners != null) {
				for (final Iterator<INarrativeListener> iter = _myListeners.iterator(); iter.hasNext();) {
	...
	public void doSave(final String message) {
		throw new IllegalArgumentException("should not have called manual save for Sensor Wrapper");
	}
	...
	public TimeStampedDataItem makeCopy(final TimeStampedDataItem item) {
		...
		final NarrativeEntry result = new NarrativeEntry(template.getTrackName(), template.getType(),
				new HiResDate(template.getDTG()), pending);
