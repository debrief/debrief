...
/**
 * class proving support for drag and drop of files from Windows to the
 * indicated Java (AWT or JFC) component
 *
 * @author Ian Mayo
 * @version 1
 */
public class FileDropSupport implements DropTargetListener {
	 ...
	 */
	/**
	 * interface to be supported by classes which want to listen out for files being
	 * dropped
	 */
		...
		if (tra.isDataFlavorSupported(DataFlavor.javaFileListFlavor)) {
			...
			try {
				...
				while (enumer.hasNext()) {
					final java.io.File file = enumer.next();
	...
	public void removeComponent(final java.awt.Component comp) {
		// get this target
		final DropTarget thisOne = targets.get(comp);
