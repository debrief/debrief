...
public class LocalGridPainter extends GridPainter {
	...
	protected WorldArea getOuterBounds(final CanvasType g, final Dimension screenArea, final double deltaDegs) {
		...
		workingVal = _myOrigin.getLong();
		final WorldVector longOffset = new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(270), deltaDegs, 0);
		...
		while (workingVal >= topLeft.getLong()) {
			if (getDelta().isAngular()) {
				...
				final WorldLocation loc = new WorldLocation(getOrigin().getLat(), workingVal, 0);
