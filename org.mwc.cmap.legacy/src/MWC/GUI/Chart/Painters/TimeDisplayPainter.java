 ...
 */
public class TimeDisplayPainter implements Plottable, DynamicPlottable, ExtendedEditable, Serializable,
		ExcludeFromRightClickEdit, NotInBaseLayer {
	...
	// info class
	// //////////////////////////////////////////////////////////
	public class TimeDisplayPainterInfo extends Editable.EditorType implements Serializable {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Color", "the Color to draw the time display", FORMAT),
						displayProp("Background", "Background color", "the Background color for the time display",
								FORMAT),
						...
						prop("Visible", "whether this time display is visible", VISIBILITY), longProp("Location",
								"the time display location", DiagonalLocationPropertyEditor.class, FORMAT), };

				PropertyDescriptor[] tmp;
				if (!_absolute) {
					tmp = new PropertyDescriptor[res.length + 3];
					System.arraycopy(res, 0, tmp, 0, res.length);
					tmp[res.length] = displayProp("Origin", "Time Origin", "the Time Origin for the time display",
							TEMPORAL);
	...
	private void formatInternal(final StringBuilder builder, final long value, final DecimalFormat fmt,
	...
	/**
	 * the area covered by the time display. It's null in this case, since the time
	 * display resizes to suit the data area.
	 *
	...
	public void paint(final CanvasType g) {
		...
		if (_absolute) {
			final String formattedDTG = FormatRNDateTime.toStringLikeThis(_DTG.getMicros() / 1000, _format);
			formattedString = (_prefix == null ? "" : _prefix) + formattedDTG + (_suffix == null ? "" : _suffix);
