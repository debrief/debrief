...
/**
 * interface which indicates that this class is capable of providing an integer value at the
 * indicated grid location .
 */
abstract public class SpatialRasterPainter extends BaseLayer implements
    Layer.BackgroundLayer, BathyProvider
{

  /**
   * embedded interface for classes capable of createing a device-specific colour (int) from the
   * three pigments
   * 
   */
  ...
  {
    /**
     * color converter which always produces a swing color. We need to call this one whether we have
     * SWT or Swing colors - since in SWT plotting we need to produce an SWT color for drawing the
     * image by hand, whereas we need to produce a Swing color when we're drawing the key (using
     * Debrief canvas type)
     * 
     * @author ian.mayo
     *
   ...
   */
  public static final class KeyLocationPropertyEditor extends
      PropertyEditorSupport
  {
    ...
    public final String[] getTags()
    {
      ...
      {
        _myTags = new String[]
        {"Not Shown", "Left", "Right", "Top", "Bottom"};
      }
      return _myTags;
    }
  ...
  abstract public static class PainterComponent implements ColorConverter
  {
    ...
    /**
     * always draw the key using AWT/SWING colors - since it plots rectangles via our CanvasType
     * plotting routines
     */
    public SwingColorConverter SWING_COLOR_CONVERTER =
        new SwingColorConverter();
     ...
     * @param y_coord
     */
    abstract protected void assignPixel(final int width, final int thisValue,
     ...
     */
    private double[] createRasterImage(final CanvasType dest,
        final SpatialRasterPainter parent, final boolean useNE)
    {
                                                                          ...
                                                                          // screen
      {
                                                                           ...
                                                                           // the screen
        {
          ...
          if (_workingLocation.isValid())
          {
            ...
            for (int y_a = -parent._gridRes / 2; (y_a < parent._gridRes) && (y
                + y_a < height); y_a++)
            {
              for (int x_a = -parent._gridRes / 2; (x_a < parent._gridRes) && (x
                  + x_a < width); x_a++)
              {
      ...
      if (parent._showBathy)
      {
        ...
        final int alpha;
        final String transparency = Defaults.getPreference(
            CreateTOPO.ETOPO_TRANSPARENCY);
        ...
        final SpatialRasterPainter parent, final boolean useNE)
    {
      ...
      // final int txtHt = 85;
      final int txtWid = dest.getStringWidth(_myFont, " " + (int) Math.max(
          min_height * -1, max_h));
      ...
      switch (keyLocation.intValue())
      {
        ...
        case (KeyLocationPropertyEditor.LEFT):
          ...
          BR = new Point(TL.x + txtWid, (int) (screen_size.height * (MIN_LEN
              + LEN)));
        ...
        case (KeyLocationPropertyEditor.RIGHT):
          TL = new Point((int) (screen_size.width - (screen_size.width
              * MIN_WID) - txtWid), (int) (screen_size.height * MIN_LEN));
          BR = new Point(TL.x + txtWid, (int) (screen_size.height * (MIN_LEN
              + LEN)));
          num_labels = (BR.y - TL.y) / txtHt;
          total_height = BR.y - TL.y;
        ...
        case (KeyLocationPropertyEditor.TOP):
          TL = new Point((int) (screen_size.width * MIN_WID),
              (int) (screen_size.height * MIN_LEN));
          BR = new Point((int) (screen_size.width * (MIN_LEN + LEN)), (TL.y
              + txtHt));
      ...
      // sort out how many shade steps we are going to produce (it depends on how big the text
      // string is)
      ...
      if (parent.isBathyVisible())
      {
        ...
        for (int i = 0; i < num_labels; i++)
        {
          ...
          // move forward
          thisPoint.move(TL.x + (int) (i * stepWidth), TL.y + (int) (i
              * stepHeight));

          // produce this new colour
          final int thisCol = parent.getColor(thisDepth, min_height, max_h,
              SWING_COLOR_CONVERTER, useNE);
          ...
          dest.fillRect(thisPoint.x, thisPoint.y, (int) (rectSize.getWidth()),
              (int) (rectSize.getHeight()));
        }
      }
      ...
      if (parent._showContours && (parent._contourDepths.length > 0))
      {
        ...
        final Dimension wholeSize = new Dimension((int) (stepWidth
            * num_labels), (int) (stepHeight * num_labels));
        ...
        for (int i = 0; i < parent._contourDepths.length; i++)
        {
          ...
          if ((depth > min_height) && (depth < max_h))
          {
            ...
            final double offset = 1 - ((depth - min_height) / (max_h
                - min_height));
            ...
            translatedPoint.translate((int) rectSize.getWidth(), (int) rectSize
                .getHeight());
            ...
            dest.fillRect(translatedPoint.x, translatedPoint.y, otherEnd.x
                - translatedPoint.x, otherEnd.y - translatedPoint.y);
          }
        }
      }
      ...
      for (int i = 0; i < num_labels; i++)
      {
        ...
        dest.drawText(_myFont, thisLabel, (int) (thisPoint.x + rectSize
            .getWidth() - thisTxtWid - 4), thisPoint.y + txtHt - 2);
      }
        ...
        final SpatialRasterPainter parent)
    {
      ...
      /*
       * todo - create a series of painted rectangles, not an image buffer - this will probably draw
       * to screen more quickly, but will definitely write to metafile more quickly. The rectangles
       * should be either sized to suit the resolution of the data, or the resolution requested by
       * the user
       */
      ...
      if (parent.isContoursVisible())
      {
        ...
        if (max_depth == 0)
          max_depth = Math.max(max_depth,
              parent._contourDepths[parent._contourDepths.length - 1]);
     ...
     * @param height
     */
    abstract protected void paintTheImage(final CanvasType dest,
     ...
     *          where we're painting to
     */
    abstract protected void updatePixelColors(SpatialRasterPainter parent,
        final int width, final int height, int min_height, int max_height,
        CanvasType dest, final boolean useNE);
  ...
  static public final class RasterPainterTest extends junit.framework.TestCase
  {
    ...
    public void NtestDepthAllocation()
    {
      ...
      assertEquals("depths output correctly", outputDepths,
          "-100, -20, -10, 0, 5");
    ...
    public void testRasterDrawing()
    {
      ...
      /**
       * represent a raster as a series of painted rectangles. Using the memory image (as we
       * originally did, the time to plot the image is directly proportional to the resolution (num
       * pixels) of the image. Using the series of rectangles, however, will be related to the
       * resolution of the data to be plotted into the image - and is therefore independent of the
       * screen resolution
       */
  ...
  public class SwingPainterComponent extends PainterComponent
  {
        ...
        final int max_height, final CanvasType dest, final boolean useNE)
    {
      ...
      for (int i = 0; i < width * height; i++)
      {
        ...
        final int thisCol = parent.getColor(thisHeight, min_height, max_height,
            this, useNE);
  ...
  {
    final int res = ((255 & 0xFF) << 24) | ((red & 0xFF) << 16) | ((green
        & 0xFF) << 8) | ((blue & 0xFF) << 0);
    return res;
  }
   ...
   */
  protected final double[] DEFAULT_CONTOUR_DEPTHS =
  {-1000, -500, -200, -100};
  ...
  public final void append(final Layer other)
  {
    if (other instanceof SpatialRasterPainter)
      MWC.GUI.Dialogs.DialogFactory.showMessage("Gridded dataset",
          "Gridded dataset already loaded");
    else
      MWC.Utilities.Errors.Trace.trace("Sorry, cannot add a layer (" + other
          + ") to a painter layer");
  }
   ...
   * @param contourIndex
   */
  public final void drawThisContour(final CanvasType dest, final double startX,
  ...
  /**
   * get the screen resolution of this grid. (i.e. how many screen pixels do we "blob-up" into one
   * data value)
   */
  ...
  public int getColor(final int elevation, final double lowerLimit,
      final double upperLimit, final ColorConverter converter,
      final boolean useNE)
  {
    int res;
  ...
  Color getContourColourFor(final int contourIndex)
  {
    ...
    else
    {
      ...
      final int blueCol = blueOffset + contourIndex * (255 - blueOffset)
          / _contourDepths.length;

      final int otherOffset = 20;
      // make it all a bit lighter than with bathy is shown
      final int otherCol = otherOffset + contourIndex * (255 - otherOffset)
          / _contourDepths.length;
    ...
    if (_showContours)
      if (paintContours())
      {
        ...
        if (_optimiseGridInterval)
        {
          ...
          final double wid = dest.getProjection().getVisibleDataArea()
              .getWidth();
  ...
  public final void setContourDepths(final String contourDepths)
  {
    ...
    if (sortedDepths.size() > 0)
    {
      ...
      for (final Iterator<Double> iterator = sortedDepths.iterator(); iterator
          .hasNext();)
      {
   ...
   *
   * @param useNE
   *          if we're using Natural Earth rendering shades
   * @return
   */
