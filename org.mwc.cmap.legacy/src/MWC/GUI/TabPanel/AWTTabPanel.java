 ...
 * each with a text label. Each tab is associated with a panel or component that
 * gets shown when the user clicks on the tab. The TabPanel automatically
 * manages swapping panels when a tab selected. It can be used directly or
 * extended. When extending from TabPanel be sure to super() during construction
 * and to super.handleEvent(evt) from handleEvent if you override it.
 *
...
public class AWTTabPanel extends BaseTabbedPanel implements MWC.GUI.CoreTabPanel {
	...
	/**
	 * Constructs a TabPanel with the tabs in the given position and having the
	 * specified look. Note that if the tabs are on top, then they always are
	 * rounded.
	 *
	...
	/**
	 * Adds a component to the end of this container. This is a standard Java AWT
	 * method which gets called to add a component to a container. The specified
	 * component is added to the end of this container.
	 * <p>
	 * If the tab label for this position has not been set it is given the name "tab
	 * - #", where # is the zero-relative index of its position.
	 *
	...
	/**
	 * Adds a component to the end of this container. This is a standard Java AWT
	 * method which gets called to add a component to a container. Typically, the
	 * specified component is added to this container at the given zero-relative
	 * position index. A position index of -1 would append the component to the end.
	 * <p>
	...
	/**
	 * Takes no action. This is a standard Java AWT method which gets called to add
	 * a component to a container. It is overridden here to do nothing, so the user
	 * cannot change the way this container works.
	 *
	...
	/**
	 * Appends a new tab and associated panel, which will be shown when the tab is
	 * selected. The tab/panel is added after the last existing panel.
	 *
	...
	/**
	 * Adds a new tab and associated panel. The tab/panel is added at the specified
	 * index.
	 *
	 * @param sLabel   the tab label
	 * @param bEnabled enable the tab or not
	...
	/**
	 * Conditionally enables a tab and its associated panel at the given index. The
	...
	/**
	 * Gets the current tab label associated with the current panels at the given
	 * index.
	 *
	 * @param int labelIndex an index in the array of tab labels for the current
	 *            panels.
	 * @return String the label of the panel at the given index. Null if the index
	 *         is out of range.
	 * @see setPanelLabel
	 */
	 ...
	 * <p>
	 * The returned size is large enough to display the biggest tab panel at its
	 * preferred size.
	 *
	 * @see #getMinimumSize
	 */
	 ...
	 */
	public synchronized void insertTabPanel(final String sLabel, final boolean bEnabled, final Component panel,
			final int index) {
		if ((index < 0) || (index >= vPanels.size()))
			return;
	 ...
	 *
	 * @return true if the tabs are at the bottom of the dialog, false if they are
	 *         at the top
	 * @see #setTabsOnBottom
	 */
	...
	/**
	 * Removes a tab and its associated panel at the given index. The currently
	 * active tab cannot be removed.
	 *
	 * @param index zero-relative index of the tab
	 */
	public synchronized void removeTabPanel(final int index) {
		...
		try {
			final Component p = vPanels.elementAt(index);
			super.remove(p);
	...
	/**
	 * Sets the tab labels associated with the panel positions. Note that the panels
	 * do not need to have been added yet for this method to work.
	 *
	...
	/**
	 * Replaces a tab and its associated panel at the index specified. If it is
	 * desired to only change the label, use the base class BaseTabbedPanel's method
	 * setTab(String sLabel, boolean bEnabled, int index).
	 *
	 ...
	 *                                  unacceptable
	 */
	public synchronized void setTabPanel(final String sLabel, final boolean bEnabled, final Component panel,
			final int index) throws PropertyVetoException {
		if ((index < 0) || (index >= vPanels.size()))
			return;
		...
		try {
			final Component oldPanel = vPanels.elementAt(index);
	 ...
	 *
	 * @param bTabsOnBottom if true the tabs are placed at the bottom of the dialog,
	 *                      if false on top
	 * @see #getTabsOnBottom
	 * @exception PropertyVetoException if the specified property value is
	 *                                  unacceptable
	...
	/**
	 * Selects the tab at the given index, showing it and its associated panel. The
	 * panel is activated, ready for user input. The tab position must be enabled.
	 *
...
//				else
	 ...
	 * provided in setPanelLabels(). It is not typically called directly.
	 *
	 * @see #setPanelLabels
	 */
