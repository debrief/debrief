...
/**
 * An abstract class used to implement special Symantec buttons. It supports 3-D
 * beveled borders, and the posting of continuous notification events while the
 * button is pressed.
 * <p>
 *
...
public abstract class ButtonBase extends Canvas {
	...
	class Action implements java.awt.event.ActionListener, java.io.Serializable {
		...
		// Implement ActionListener to catch ActionEvents sent by either the
		// notifyTimer.
		/**
	 ...
	 * BevelHeight property. Handles vetoing BevelHeights that are not valid.
	 */
	class BevelVeto implements java.beans.VetoableChangeListener, java.io.Serializable {
		...
		/**
		 * This method gets called when an attempt to change the constrained BevelHeight
		 * property is made. Ensures the given bevel size is valid for this button.
		 *
	...
	/**
	 * This is the PropertyChangeEvent handling inner class for the constrained
	 * Frame property. Handles vetoing Frame strings that are not valid.
	 */
	class FrameVeto implements java.beans.VetoableChangeListener, java.io.Serializable {
		...
		/**
		 * This method gets called when an attempt to change the constrained Frame
		 * property is made. Ensures the given Frame string is valid for this button.
		 *
		 * @param e a <code>PropertyChangeEvent</code> object describing the event
		 *          source and the property that has changed.
		 * @exception PropertyVetoException if the recipient wishes the property change
	...
	class Mouse extends java.awt.event.MouseAdapter implements java.io.Serializable {
		...
		/**
		 * Handles the Mouse Pressed events If the notifyWhilePressed flag is true the
		 * notification Timer is started
		 *
		 * @param e the MouseEvent
		...
		/**
		 * Handles the Mouse Released events If the notification timer is running it is
		 * stopped. If the mouse was pressed inside the button then fire an action
		 * event.
		 *
		 * @param e the MouseEvent
		 * @see #mousePressed
	...
	/**
	 * If true show the linkURL in the status area when the mouse is over the
	 * button. If the linkURL is null, nothing is displayed, regardless of this
	 * flag. This flag also controls erasing of the status area after the URL has
	 * been displayed.
	 */
	protected boolean showURLStatus;

	/**
	 * Keeps track of wheather or not the button is added to a container. Check
	 * before attempting to getFontMetrics() to avoid getting a null pointer.
	 */
	...
	/**
	 * Frame specifier for showing a URL document in a browser or applet viewer. It
	 ...
	 * <DT>all others show document in a new toplevel window with the given
	 * name</DT>
	 * </UL>
	 */
	...
	/**
	 * The color of the content of the button. The highlights are derived from this
	 * color.
	 */
	...
	/**
	 * Constructs a default ButtonBase. The defaults are no notifyWhilePressed, no
	 * offset, and a bevel height of 2.
	 */
	 ...
	 * this component. It simply returns the results of a call to preferedSize().
	 */
	@Override
	...
	/**
	 * Returns true if a button image has been set, but it is not the size of this
	 * component.
	 */
	...
	/**
	 * Returns whether this button will be shown as having the focus when the mouse
	 * enters.
	 *
	 * @see #setShowFocus
	 */
	 ...
	 * been displayed.
	 *
	 * @return true if the linkURL will be displayed in the status area when the
	 ...
	 * @return true if the given frame is acceptable, false if not. To be valid it
	 *         has to be null, or one of the four strings below:
	 * @see symantec.itools.util.GeneralUtils#frameTarget_self
	 * @see symantec.itools.util.GeneralUtils#frameTarget_parent
	 * @see symantec.itools.util.GeneralUtils#frameTarget_top
	 * @see symantec.itools.util.GeneralUtils#frameTarget_blank
	...
	protected boolean isValidFrame(final String string) {
		...
		if (string.equals(GeneralUtils.frameTarget_self) || string.equals(GeneralUtils.frameTarget_parent)
				|| string.equals(GeneralUtils.frameTarget_top) || string.equals(GeneralUtils.frameTarget_blank))
			return true;
		else
			return false;
	}
	...
	/**
	 * Tell the browser to show the document referenced by the linkURL. If the frame
	 * specifier is not null or empty, then tell the browser to open the document
	 * with that frame.
	 */
	 ...
	 *
	 * @param color the new button color. The highlights of the button are derived
	 *              from this color.
	 * @exception PropertyVetoException if the specified property value is
	 *                                  unacceptable
	 * @see #getButtonColor
	...
	/**
	 * Enables this component so that it will respond to user input. This is a
	 * standard Java AWT method which gets called to enable or disable this
	 * component. Once enabled this component will respond to user input.
	 *
	 * @param flag true if the component is to be enabled; false if it is to be
	 *             disabled.
	 ...
	 * @exception java.beans.PropertyVetoException if the specified property value
	 *                                             is unacceptable
	 * @see #getFrame
	 * @see symantec.itools.util.GeneralUtils#frameTarget_self
	 * @see symantec.itools.util.GeneralUtils#frameTarget_parent
	...
	/**
	 * Sets whether this button will be shown as having the focus when the mouse
	 * enters.
	 *
	 * @param flag true to show focus at mouse enter; false to not show at mouse
	 *             enter
	 * @exception PropertyVetoException if the specified property value is
	 *                                  unacceptable
	 * @see #isShowFocus
	 ...
	 * is over the button. This flag also controls erasing of the status area after
	 * the URL has been displayed.
	 *
	 * @param flag true if the linkURL will be displayed in the status area when the
	 *             mouse is over the button; false if not.
	 * @exception PropertyVetoException if the specified property value is
	 *                                  unacceptable
	 * @see #isShowURLStatus
	 */
	...
	/**
	 * Sets whether objects in the button will be offset down and to the right bevel
	 * height amount or not. This also impacts the way the button is drawn when it
	 * is pressed.
	 *
	 * @param flag true to have objects use the offset; false to have objects not
	 *             move.
	 * @exception PropertyVetoException if the specified property value is
	 *                                  unacceptable
	 * @see #isUseOffset
	...
	/**
	 * Handles redrawing of this component on the screen. This is a standard Java
	 * AWT method which gets called by the Java AWT (repaint()) to handle repainting
	 * this component on the screen. The graphics context clipping region is set to
	 * the bounding rectangle of this component and its [0,0] coordinate is this
	 * component's top-left corner. Typically this method paints the background
	 * color to clear the component's drawing space, sets graphics context to be the
	 * foreground color, and then calls paint() to draw the component.
	 *
	...
	/**
	 * Ensures that this component is laid out properly, as needed. This is a
	 * standard Java AWT method which gets called by the AWT to make sure this
	 * component and its subcomponents have a valid layout. If this component was
	 * made invalid with a call to invalidate(), then it is laid out again.
	 *
