...
public class ETOPOWrapper extends MWCETOPOLayer implements Runnable, BathyProvider {
	...
	public static int getColor(final short elevation, final double lowerLimit, final double upperLimit,
			final boolean showLand) {
		int res = 0;
	...
	/**
	 * the scale factors to convert a coordinate in degs to the position in the data
	 * array
	 */
	float _scx;
	 ...
	 *
	 */
	protected void drawKey(final CanvasType dest, final double min_height, final double max_height,
...
//    final float WID = 0.05f;
		...
		for (int i = 0; i < num_labels; i++) {
			...
			dest.drawText(thisLabel, (int) (thisPoint.x + rectSize.getWidth() - thisTxtWid - 1),
					thisPoint.y + txtHt - 2);
...
//      continue;
	...
	public void run() {
		...
		try {
			...
			_scy = bufferHeight / 180F;
			_scx = bufferWidth / 360F;
