...
abstract public class Conrec {
	...
	public Conrec(/* Render render */) {
		/*
		 * if (render == null){ throw new Exception ("Render null"); } this.render =
		 * render;
		 */
	}
	 ...
	 *
	 * It emits calls to a line drawing subroutine supplied by the user which draws
	 * a contour map corresponding to real*4data on a randomly spaced rectangular
	 * grid. The coordinates emitted are in the same units given in the x() and y()
	 * arrays.
	 *
	 ...
	 *
	 *                     The following two, one dimensional arrays (x and y)
	 *                     contain the horizontal and vertical coordinates of each
	 *                     sample points.
	 ...
	 *
	 */
	public void contour(final double[][] da, final int ilb, final int iub, final int jlb, final int jub,
			final double[] xa, final double[] ya, final int nc, final double[] z, final int gridInterval) {
		int m1;
		int m2;
		int m3;
		int case_value;
		...
		final int[][][] castab = { { { 0, 0, 8 }, { 0, 2, 5 }, { 7, 6, 9 } }, { { 0, 3, 4 }, { 1, 3, 1 }, { 4, 3, 0 } },
				{ { 9, 6, 7 }, { 5, 2, 0 }, { 8, 0, 0 } } };
...
//            for (i=ilb;i<=iub-1;i++) {
	...
	/**
	 * drawContour - interface for implementing the user supplied method to render
	 * the countours. Draws a line between the start and end coordinates.
	 *
