...
/**
 * GSHHStoCMAP A crude utility that converts an GSHHS data base into a Debrief
 * CMAP-readable .dat file. TODO: Add a switch to set a filtering level (to
 * exclude lakes, for instance).
 *
 * The GSHHS files are:
 *
 * gshhs Global Self-consistent Hierarchical High-resolution Shorelines
 * (including lakes) wdb_borders WDB political borders wdb_rivers WDB rivers
 * _f.b Full resolution data _h.b High resolution data (half of full along each
 * axis) _i.b Intermediate resolution data (half of high along each axis) _l.b
 * Low resolution data (half of intermediate along each axis) _c.b Crude
 * resolution data (half of low along each axis)
 *
 * The GSHHS file structure (since version 4) is a series of polygons, each
 * polygon consisting of a header followed by a series of points. The header
 * structure is:
 *
 * int id; Unique polygon id number, starting at 0 int n; Number of points in
 * this polygon int flag; level + (version << 8) + (greenwich << 16) + (source
 * << 24) level: 1 land, 2 lake, 3 island_in_lake, 4 pond_in_island_in_lake
 * version: Set to 6 for GSHHS version 1.6 (released in March 2008) greenwich: 1
 * if "Greenwich is crossed" (meaning the polygon straddles Greenwich) source: 0
 * = CIA WDBII, 1 = WVS int west, Western bound, in micro-degrees east, Eastern
 * bound south, Southern bound north; Northern bound int area; Area of polygon
 * in kiloares (0.1 km^2)
 *
 * The point structure is:
 *
 * int lon; Longitude east, in micro-degrees (0..360) int lat; Latitude north
 * (-90..+90)
 *
 * All the ints are four-byte Java integers, MSB.
 *
 * The polygons are sorted by polygon length (not area), and the w/e/s/n values
 * for each polygon are those based on the full resolution (they are copied to
 * the other resolutions). The decimated polygons will in general have a
 * different w/e/s/n set which you can find if you calculate the actual w/e/s/n
 * values.
 */
...
// static int              flag;
	...
	/**
	 * Convert (the first four bytes of) a byte array to an int. The conversion
	 * assumes MSB byte order.
	 *
	...
	/**
	 * Convert a byte array to an int starting from the given offset. The conversion
	 * assumes MSB byte order.
	 *
	 * @param b      The byte array
	...
	public static void main(final String[] args) {
		try {
			...
			if ((args.length < 2) || (args.length > 3)) {
				System.out.println(
						"+++ GSHHStoCMAP - Usage is 'GSHHStoCMAP Path\\gshhs_f.b Path2\\gshhsWorld.dat [Path3\\Output.log]'");
...
//                   flag = byteArrayToInt(gshhs_header, 8); //Offset 8 to reach the "flag" field
							...
							if (nbytes != tmpNumPoints) {
								throw new IOException("Could not read the required bytes. Expected:" + nbytes + " was:"
										+ tmpNumPoints);
							}

							// Skip anything but sea shore (level 2 would filter islands in lakes and ponds
							// in islands in lakes, etc.)
...
//                fwOutputLog.write(Integer.toString(count_skipped_polygons) + " skipped polygons" + CRLF);
	...
	/**
	 * Snips the current segment over the 180 degree meridian. The "previous" pair
	 * has already been written to fwCMAP. The "current" pair will be written to
	 * fwCMAP after snip concludes. All longitudes are in the 0..360 eastern span.
	 *
	 ...
	 */
	public static boolean snip(final FileWriter fwCMAP, final double lon_previous, final double lat_previous,
			final double lon, final double lat, final boolean isLast) throws IOException {
		...
		if ((lon_previous < 180.0) && (lon <= 180.0))
			return false;
		if ((lon_previous < 180.0) && (lon > 180.0)) {
			if ((lon_previous < 170.0) || (lon > 190.0))
				return false; // False alarm due to Greenwich (jump between ~0 and ~360)
		...
		if ((lon_previous > 180.0) && (lon < 180.0)) {
			if ((lon_previous > 190.0) || (lon < 170.0))
				return false; // False alarm due to Greenwich (jump between ~0 and ~360)
			lat_interp = lat_previous + (lat - lat_previous) * (lon_interp - lon_previous) / (lon - lon_previous);
			write_lonlat(fwCMAP, lon_interp - 360.0, lat_interp);
			write_break(fwCMAP);
