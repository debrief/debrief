...
public class S57Database {
	...
	private static class FeatureRecord {
		...
		Vector<FeatureToSpatialPointer> _spatialPointers = new Vector<FeatureToSpatialPointer>(0, 1);

		private final Vector<FeatureToFeaturePointer> _featurePointers = new Vector<FeatureToFeaturePointer>(0, 1);
	...
	private void closeFeature(final FeatureRecord feature) {
		if (_closers == null) {
			...
			_closers.add(new StorePoints(129, "Soundings", new Color(136, 152, 139), false,
					new PointFeature.DepthPainter()));
		...
		for (final Iterator<StoreFeature> iter = _closers.iterator(); iter.hasNext();) {
			final StoreFeature storer = iter.next();
...
//	}
	...
	private void loadATTF(final DDFField field) {
		...
		for (final Iterator<HashMap<String, Object>> iterator = fields.iterator(); iterator.hasNext();) {
			final HashMap<String, Object> res = iterator.next();
...
//	}
	...
	public void loadDatabase(final String filename, final boolean fspt_repeating) {
		...
		try {
			...
			while ((poRecord = oModule.readRecord()) != null) {
				...
				while (iter.hasNext()) {
					final DDFField nextField = iter.next();
	...
	private void loadDSSI(final DDFField field) {
		...
		for (final Iterator<HashMap<String, Object>> iterator = fields.iterator(); iterator.hasNext();) {
			...
			while (iter.hasNext()) {
				final String thisKey = iter.next();
	...
	private void loadFFPT(final DDFField field) {
		...
		for (final Iterator<HashMap<String, Object>> iterator = res.iterator(); iterator.hasNext();) {
			final HashMap<String, Object> nextFSPT = iterator.next();
	...
	private Vector<HashMap<String, Object>> loadFields(final DDFField field) {
		...
		for (int iRepeat = 0; iRepeat < repeatCount; iRepeat++) {
			...
			for (int iSF = 0; iSF < subfieldCount; iSF++) {
				...
				final int nBytesConsumed = loadSubfield(thisMap, poSFDefn, pachFieldData, nBytesRemaining);
	...
	protected int loadSubfield(final HashMap<String, Object> dest, final DDFSubfieldDefinition poSFDefn,
			final byte[] pachFieldData, final int nBytesRemaining) {
		...
		if (ddfdt == DDFDataType.DDFInt) {
			dest.put(fieldName, poSFDefn.extractIntData(pachFieldData, nBytesRemaining, nBytesConsumed));
		} else if (ddfdt == DDFDataType.DDFFloat) {
			dest.put(fieldName, poSFDefn.extractFloatData(pachFieldData, nBytesRemaining, nBytesConsumed));
		} else if (ddfdt == DDFDataType.DDFString) {
			dest.put(fieldName, poSFDefn.extractStringData(pachFieldData, nBytesRemaining, nBytesConsumed));
	...
	private void populateThisList(final Vector<WorldLocation> tgt, final VectorRecord host) {
		...
		if (pointers != null) {
			for (final Iterator<VectorRecord.VectorPointer> iter = pointers.iterator(); iter.hasNext();) {
				final VectorRecord.VectorPointer nextP = iter.next();
	...
	void storeLine(final FeatureRecord feature, final LineFeature cl) {
		...
		for (final Iterator<FeatureRecord.FeatureToSpatialPointer> iter = names.iterator(); iter.hasNext();) {
			final FeatureRecord.FeatureToSpatialPointer element = iter.next();
	...
	void storePoints(final FeatureRecord feature, final PointFeature myType) {
		...
		for (final Iterator<FeatureRecord.FeatureToSpatialPointer> iter = names.iterator(); iter.hasNext();) {
			...
			if (rec == null) {
				System.err.println("vector not found for + name:" + element._rcnm + " id:" + element._rcid);
			} else {
				final Vector<WorldLocation> theList = new Vector<WorldLocation>(0, 1);
