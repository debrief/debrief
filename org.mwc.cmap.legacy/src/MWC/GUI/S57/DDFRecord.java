...
/**
 * Contains instance data from one data record (DR). The data is contained as a
 * list of DDFField instances partitioning the raw data into fields. Class
 * contains one DR record from a file. We read into the same record object
 * repeatedly to ensure that repeated leaders can be easily preserved.
 */
public class DDFRecord implements DDFConstants {
	...
	/**
	 * Clear any information associated with the last header in preparation for
	 * reading a new header.
	 */
	public void clear() {
		if (paoFields != null) {
			paoFields = null;
	 ...
	 *
	 * @param pszName     The name of the field to fetch. The comparison is case
	 *                    insensitive.
	 * @param iFieldIndex The instance of this field to fetch. Use zero (the
	 *                    default) for the first instance.
	 *
	 * @return Pointer to the requested DDFField. This pointer is to an internal
	 *         object, and should not be freed. It remains valid until the next
	 *         record read.
	 */
	...
	/**
	 * Fetch the raw data for this record. The returned pointer is effectively to
	 * the data for the first field of the record, and is of size GetDataSize().
	 */
	public byte[] getData() {
		return pachData;
	}
	...
	/**
	 * Fetch value of a subfield as a float (double). This is a convenience function
	 * for fetching a subfield of a field within this record.
	 *
	 * @param pszField       The name of the field containing the subfield.
	 * @param iFieldIndex    The instance of this field within the record. Use zero
	 *                       for the first instance of this field.
	 * @param pszSubfield    The name of the subfield within the selected field.
	 * @param iSubfieldIndex The instance of this subfield within the record. Use
	 *                       zero for the first instance.
	 * @return The value of the subfield, or zero if it failed for some reason.
	 */
	public double getFloatSubfield(final String pszField, final int iFieldIndex, final String pszSubfield,
		...
		/* -------------------------------------------------------------------- */
		final DDFSubfieldDefinition poSFDefn = poField.getFieldDefn().findSubfieldDefn(pszSubfield);
		...
		final byte[] pachData1 = poField.getSubfieldData(poSFDefn, nBytesRemaining, iSubfieldIndex);
	...
	/**
	 * Fetch value of a subfield as an integer. This is a convenience function for
	 * fetching a subfield of a field within this record.
	 *
	 * @param pszField       The name of the field containing the subfield.
	 * @param iFieldIndex    The instance of this field within the record. Use zero
	 *                       for the first instance of this field.
	 ...
	 * @return The value of the subfield, or zero if it failed for some reason.
	 */
	public int getIntSubfield(final String pszField, final int iFieldIndex, final String pszSubfield,
			final int iSubfieldIndex) {
		DDFField poField;
	...
	/**
	 * Fetch value of a subfield as a string. This is a convenience function for
	 * fetching a subfield of a field within this record.
	 *
	 * @param pszField       The name of the field containing the subfield.
	 * @param iFieldIndex    The instance of this field within the record. Use zero
	 ...
	 * @param iSubfieldIndex The instance of this subfield within the record. Use
	 *                       zero for the first instance.
	 * @return The value of the subfield, or null if it failed for some reason. The
	 *         returned pointer is to internal data and should not be modified or
	 *         freed by the application.
	 */

	String getStringSubfield(final String pszField, final int iFieldIndex, final String pszSubfield,
			final int iSubfieldIndex) {
	...
	/**
	 * Read a record of data from the file, and parse the header to build a field
	 * list for the record (or reuse the existing one if reusing headers). It is
	 * expected that the file pointer will be positioned at the beginning of a data
	 * record. It is the DDFModule's responsibility to do so.
	 *
	...
	/**
	 * This perform the header reading and parsing job for the read() method. It
	 * reads the header, and builds a field list.
	 */
	protected boolean readHeader() {
		...
		if (nReadBytes == -1) {
			...
			Debug.error("DDFRecord.readHeader(): Leader is short on DDF file. Is " + nReadBytes + " should be "
					+ (int) DDF_LEADER_SIZE);
			return false;
		}
	 ...
	 *
	 * A variety of information about this record, and all it's fields and subfields
	 * is written to the given debugging file handle. Note that field definition
	 * information (ala DDFFieldDefn) isn't written.
	 */
	@Override
