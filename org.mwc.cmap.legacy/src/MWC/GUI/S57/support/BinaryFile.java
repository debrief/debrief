...
public class BinaryFile {
	...
	public static synchronized void closeClosable() {
		...
		for (int i = 0; i < closableList.size(); i++) {
			final Closable c = (closableList.elementAt(i)).get();
	 ...
	 * string, can be found by the current Java environment. Uses the same tests as
	 * BinaryFile constructor for tracking down a file.
	 *
	...
	public static synchronized void removeClosable(final Closable it) {
		for (int i = 0; i < closableList.size(); i++) {
			final Object o = closableList.elementAt(i).get();
			if ((o == it) || (o == null)) {
				closableList.removeElementAt(i);
	...
	/**
	 * Constructs a new BinaryFile with the specified file as the input. The default
	 * byte-order is LSB first. Reads start at the first byte of the file.
	 *
	 ...
	 * correct constructor as appropriate. If the string represents a file available
	 * locally, then the BinaryFile will be accessed with a FileInputReader using a
	 * RandomAccessFile. If it's only available as a resource, then a
	 * StreamInputReader will be used. The name should be a path to a file, or the
	 ...
	 *                                 match
	 * @exception InvalidCharException throws when the character read doesn't match
	 *                                 <code>expected</code> The .c member of the
	 *                                 thrown exception is the actual char read
	 ...
	 *
	 * @param msbfirst <code>true</code>= MSB first, <code>false</code>= LSB first
	 */
	...
	/**
	 * Get the inputReader used for accessing the file, for quering purposes. Don't
	 * use it to get data, or the file pointers may get messed up.
	 */
	 ...
	 * file. (Check the filename and see if it looks like "jarfile!jarfileentry".)
	 * If it is, it separates the path, and set the inputReader to a JarInputReader
