...
public class Layers implements Serializable, Plottable, PlottablesType {
	...
	public interface DataListener {
		 ...
		 *
		 * @param theData      the Layers containing the item of data which has been
		 *                     modified
		 * @param changedLayer a layer which has changed (or null if not known)
		 */
	...
	public interface DataListener2 extends DataListener {
		 ...
		 *
		 * @param theData the Layers which have had something edited
		 * @param newItem the item that has been added/edited/deleted (if just one)
		 * @param parent  the layer containing the item
		 */
	...
	public interface INewItemListener extends ExcludeFromRightClickEdit {
		 ...
		 *
		 * @param parent       the layer that has a new item
		 * @param item         the new item (null if this is actually just a new layer)
		 * @param theSymbology
		 */
	...
	public class LayersInfo extends Editable.EditorType {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Name", "the name for these layers"), };
	...
	/**
	 * marker for objects that must be wrapped before they are pasted into the layer
	 * manager
	 *
	 */
	...
	/**
	 * make the property listener class into an embedded class - because we were
	 * experiencing some not-serializable problems
	 *
	 * @author ian
	 *
	 */
	...
	/**
	 * if we don't have any data, we still want to be able to centre on a geographic
	 * location. use HMS Dolphin.
	 *
	 ...
	 *
	 * @param theOther layer to add to us
	 */
	 ...
	 *
	 * @param theLayer the layer to add
	 */
	public void addThisLayer(final Layer theLayer) {
		...
		// does it need to know about the layers, or that it has been added to the
		// layers?
	...
	public void addThisLayerDoNotResize(final Layer theLayer) {
		...
		if (theLayer instanceof SupportsPropertyListeners) {
			...
			pr.addPropertyChangeListener(SupportsPropertyListeners.EXTENDED, _extendedListener);
		}
	...
	/**
	 * create a version of the supplied layer name that doesn't exist, by appending
	 * a counter
	 *
	 * @param prefix
	 * @return
	 ...
	 *
	 * @param theLayerName the name of the layer to look for
	 * @return the layer
	 */
	...
	public WorldArea getBounds() {
		...
		if (res == null) {
			...
			final WorldLocation newTL = centre.add(new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(315),
					new WorldDistance(diagExtendNm, WorldDistance.NM).getValueIn(WorldDistance.DEGS), 0));
			final WorldLocation newBR = centre.add(new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(135),
					new WorldDistance(diagExtendNm, WorldDistance.NM).getValueIn(WorldDistance.DEGS), 0));
	 ...
	 *
	 * @param dest destination for the plotting
	 */
	@Override
	 ...
	 *
	 * @param theLayer the layer to remove
	 */
	public void removeThisLayer(final Layer theLayer) {
		...
		if (theLayer instanceof SupportsPropertyListeners) {
			...
			pr.removePropertyChangeListener(SupportsPropertyListeners.EXTENDED, _extendedListener);
		}

		// do we need to tell it that it's being removed?
		if (theLayer instanceof NeedsToBeInformedOfRemove) {
			final NeedsToBeInformedOfRemove rem = (NeedsToBeInformedOfRemove) theLayer;
			rem.beingRemoved();
		}
	 ...
	 * itself it doesn't have a constructor and there's no way of setting this data
	 * (other than via static data, but it needs to be unique for each operation).
	 */
	 ...
	 *
	 * @param val largely ignored by the people of New Guinea
	 */
	...
	private void walkThis(final BaseLayer layer, final Class<?> mustMatch, final OperateFunction function) {
		// walk the tree
	...
	public void walkVisibleItems(final Class<?> mustMatch, final OperateFunction function) {
		// walk the tree
