...
public class DateAxisEditor extends AbstractPropertyEditor {
	...
	public static class DatedRNFormatter extends GMTDateFormat {
		...
		/**
		 * cache the last date formatted. when this moves backwards, we know we're
		 * restarting - which helps us to detect the first item.
		 */
		private long _lastDate = Long.MIN_VALUE;

		/**
		 * pattern for the on-the-day values. Note: we currently ignore this, since we
		 * just prepend the date value as a 2-digit value
		 */
		 ...
		 * <b>Note:</b> Not all locales support SimpleDateFormat; for full generality,
		...
		public StringBuffer format(final Date arg0, final StringBuffer arg1, final FieldPosition arg2) {
			...
			if (((arg0.getHours() == 0) && (arg0.getMinutes() == 0) && (arg0.getSeconds() == 0)) || firstOne) {
				// ok, use the suffix
	...
	public static class MWCDateTickUnitWrapper {
		...
		public MWCDateTickUnitWrapper(final DateTickUnitType unit, final int count, final String formatter) {
			_unit = unit;
			_count = count;
	...
	public static class OptimisedDateTickUnit extends DateTickUnit {
		...
		public OptimisedDateTickUnit(final DateTickUnitType unitType, final int multiple, final DateFormat formatter) {
	...
	public static TickUnits createStandardDateTickUnitsAsTickUnits() {
		...
		// milliseconds
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MILLISECOND, 500, new GMTDateFormat("HH:mm:ss.SSS")));

		// seconds
		units.add(new OptimisedDateTickUnit(DateTickUnitType.SECOND, 1, new GMTDateFormat("HH:mm:ss")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.SECOND, 5, new GMTDateFormat("HH:mm:ss")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.SECOND, 10, new GMTDateFormat("HH:mm:ss")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.SECOND, 30, new GMTDateFormat("HH:mm:ss")));

		// minutes
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 1, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 2, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 5, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 10, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 15, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 20, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.MINUTE, 30, new GMTDateFormat("HH:mm")));

		// hours
		units.add(new OptimisedDateTickUnit(DateTickUnitType.HOUR, 1, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.HOUR, 2, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.HOUR, 4, new GMTDateFormat("HH:mm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.HOUR, 6, new GMTDateFormat("ddHHmm")));
		units.add(new OptimisedDateTickUnit(DateTickUnitType.HOUR, 12, new GMTDateFormat("ddHHmm")));

		// days
		units.add(new OptimisedDateTickUnit(DateTickUnitType.DAY, 1, new GMTDateFormat("d-MMM")));
