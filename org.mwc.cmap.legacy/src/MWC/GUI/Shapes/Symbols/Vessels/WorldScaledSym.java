...
public abstract class WorldScaledSym extends PlainSymbol {
	...
	public void paint(final CanvasType dest, final WorldLocation theLocation, final double direction) {
		...
		final AffineTransform scaleToBoat = AffineTransform.getScaleInstance(widFactor, lenFactor);
		...
		final WorldVector lenOffset = new WorldVector(Math.PI / 2, Conversions.m2Degs(1000), 0);
		final WorldLocation newEnd = theLocation.add(lenOffset);
		final Point lenScreen = dest.toScreen(newEnd);
		final double lenPixels = (origin.x - lenScreen.x) / 1000.0;

		final AffineTransform boatToScreen = AffineTransform.getScaleInstance(-lenPixels, -lenPixels);
		...
		while (iter.hasNext()) {
			...
			for (int i = 0; i < thisLine.length; i++) {
				...
				final Point newPos = new Point(origin.x + (int) postTurn.getX(), origin.y - (int) postTurn.getY());
