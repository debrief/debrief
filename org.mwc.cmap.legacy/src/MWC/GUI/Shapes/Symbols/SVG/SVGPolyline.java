...
public class SVGPolyline extends SVGElement {
	public static class SVGPolylineTest extends junit.framework.TestCase {
		...
		public void testBasicSVGPolyline() {
			try {
				final String contentToLoad = "<polyline fill=\"#000000\" fill-opacity=\"0\" fill-rule=\"nonzero\" stroke=\"#000000\" stroke-width=\"5\" points=\"5 76 20 47 35 76 50 47 65 76 80 47 95 76\"/>";
				...
				final SVGPolyline newPolyline = new SVGPolyline(doc.getDocumentElement());
	...
	public SVGPolyline(final Element dom) {
		super(dom);
		try {
			...
			final String[] pointsSplitted = points.split(" ");
			final double[] _points = new double[pointsSplitted.length];
			...
			for (int i = 0; i < _points.length; i += 2) {
				_originalCoordinates[i / 2] = new java.awt.geom.Point2D.Double(_points[i], _points[i + 1]);
			}
