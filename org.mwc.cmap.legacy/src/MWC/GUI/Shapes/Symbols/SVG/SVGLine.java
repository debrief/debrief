...
public class SVGLine extends SVGElement {
	...
	public static class SVGLineTest extends junit.framework.TestCase {
		...
		public void testBasicSVGLine() {
			try {
				final String contentToLoad = "<line style=\"stroke: rgb(0, 0, 0); fill: rgb(10, 0, 0); stroke-width: 6;\" x1=\"91.425\" y1=\"27.093\" x2=\"59.992\" y2=\"57.483\"/>";
				...
				final SVGLine newSVGLine = new SVGLine(doc.getDocumentElement());
	...
	public SVGLine(final Element dom) {
		super(dom);
		try {
			_originalCoordinates = new java.awt.geom.Point2D[2];

			_originalCoordinates[0] = new java.awt.geom.Point2D.Double(Double.parseDouble(getDom().getAttribute("x1")),
					Double.parseDouble(getDom().getAttribute("y1")));
			_originalCoordinates[1] = new java.awt.geom.Point2D.Double(Double.parseDouble(getDom().getAttribute("x2")),
					Double.parseDouble(getDom().getAttribute("y2")));
	...
	public void render(final CanvasType dest, final double sym_size, final java.awt.Point origin_coords,
		...
		dest.drawLine(_intX[0], _intY[0], _intX[1], _intY[1]);
	}
}
