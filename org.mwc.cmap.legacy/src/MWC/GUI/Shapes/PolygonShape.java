...
public class PolygonShape extends PlainShape implements Editable, HasDraggableComponents, Layer {
	...
	public class PolygonInfo extends Editable.EditorType {
		...
		public final MethodDescriptor[] getMethodDescriptors() {
			...
			final MethodDescriptor[] mds = { method(c, "addNode", null, "Add Node"), };
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Filled", "whether to fill the polygon", FORMAT),
	...
	public static class PolygonNode implements Editable, Plottable, CreateEditorForParent, Serializable {
		...
		public class PolygonNodeInfo extends Editable.EditorType {
			...
			public PropertyDescriptor[] getPropertyDescriptors() {
				try {
					final PropertyDescriptor[] res = { prop("Location", "the location of this node"), };
					return res;
	...
	public static class TestPolygonShape extends TestCase {
		...
		public void testRangeFromEmpty() {
			final Vector<PolygonNode> nodes = new Vector<PolygonNode>();
			final PolygonShape poly = new PolygonShape(nodes);
			assertEquals("returns duff range", Plottable.INVALID_RANGE, poly.rangeFrom(new WorldLocation(3, 3, 3d)),
					0.001);
		}
	 ...
	 * constructor
	 *
	 * @param vector the WorldLocation marking the centre of the polygon
	 */
	...
	public void calcPoints() {
		...
		if (_nodes.size() > 0) {
			...
			_theAnchor = new WorldLocation(lats / _nodes.size(), longs / _nodes.size(), 0);
		}
	}
	...
	public void removeElement(final Editable point) {
		if (point instanceof PolygonNode) {
			...
			System.err.println("tried to remove point from polygon that isn't a node!");
		}
	}
