...
public class TextLabel extends PlainShape implements Editable {
	...
	public void paint(final CanvasType dest) {
		...
		blockHeight += (int) ((lineHeight * 0.2d) * (numLines - 1));
	 ...
	 * @param theFont
	 */
	private void paintMultiLine(final CanvasType dest, final String theStr, final Font theFont, final Point thePoint) {
		int _numLines = 0;
		...
		final int HorizonalAlignment = SwingConstants.CENTER;
		...
		case SwingConstants.CENTER: {
			...
			if (_numLines > 1) {
				for (int i = 0; i < _numLines; i++) {
					final String subString = _subStrings.elementAt(i);
		...
		case SwingConstants.LEFT: {
			...
			if (_numLines > 1) {
				for (int i = 0; i < _numLines; i++) {
					String subString = _subStrings.elementAt(i);
