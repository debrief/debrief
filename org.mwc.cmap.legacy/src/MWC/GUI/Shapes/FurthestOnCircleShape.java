...
public class FurthestOnCircleShape extends PlainShape implements Editable {
	...
	static public class WheelTest extends junit.framework.TestCase {
		...
		public void testMyParams() {
			MWC.GUI.Editable ed = new FurthestOnCircleShape(new WorldLocation(2d, 2d, 2d), 3,
					new WorldSpeed(2, WorldSpeed.Kts), 5000, 45, 180);
			MWC.GUI.Editable.editableTesterSupport.testParams(ed, this);
			ed = null;
		}

		public void testWidthCalc() {
			final FurthestOnCircleShape ed = new FurthestOnCircleShape(new WorldLocation(2d, 2d, 2d), 3,
					new WorldSpeed(60, WorldSpeed.Kts), 60 * 60 * 1000, 45, 180);
	 ...
	 */
	public FurthestOnCircleShape(final WorldLocation theCentre, final int numRings, final WorldSpeed speed,
			final long interval, final int arcCentre, final int arcWidth) {
		super(0, "Range Ring");
	...
	public void paint(final CanvasType dest) {
		...
		for (int i = 0; i < _numRings; i++) {
			...
			if ((lLoc == ALL) || (lLoc == LEFT))
				dest.drawText(thisLabel, centre.x - strWidth - thisRadius, centre.y + strHeight / 2);
			if ((lLoc == ALL) || (lLoc == RIGHT))
				dest.drawText(thisLabel, (int) (centre.x + thisRadius + strWidth / 15d), centre.y + strHeight / 2);
