...
public class SwingPropertyEditor2 extends PlainPropertyEditor implements KeyListener {
	protected class dataCellEditor implements TableCellEditor {
		...
		@Override
		public Component getTableCellEditorComponent(final JTable p1, final Object p2, final boolean p3, final int p4,
				final int p5) {
	 ...
	 * edits it
	 */
	private static class HandleTextChange extends java.awt.event.FocusAdapter implements ActionListener {
	...
	public static class TickableComboBox extends JComboBox {
		...
		protected class TickableComboBoxRenderer extends JLabel implements javax.swing.ListCellRenderer {
			/**
			 *
			 */
			private static final long serialVersionUID = 1L;
			...
			private final javax.swing.border.Border emptyBorder = BorderFactory.createEmptyBorder(1, 1, 1, 1);
	...
	public static void checkPropertyEditors() {
		...
		if (_myPropertyManager == null) {
			...
			PropertyEditorManager.registerEditor(MWC.GenericData.HiResDate.class, SwingDatePropertyEditor.class);
			...
			PropertyEditorManager.registerEditor(SteppingBoundedInteger.class, SwingSteppingBoundedIntegerEditor.class);
			PropertyEditorManager.registerEditor(BoundedInteger.class, SwingBoundedIntegerEditor.class);
			PropertyEditorManager.registerEditor(MWC.GenericData.WorldDistance.class,
					SwingDistanceWithUnitsPropertyEditor.class);
			PropertyEditorManager.registerEditor(MWC.GenericData.Duration.class, SwingDurationPropertyEditor.class);
			PropertyEditorManager.registerEditor(MWC.GenericData.WorldSpeed.class, SwingWorldSpeedPropertyEditor.class);
	 ...
	 */
	public SwingPropertyEditor2(final MWC.GUI.Editable.EditorType info, final SwingPropertiesPanel parent,
			final Layers theLayers, final MWC.GUI.ToolParent toolParent, final Layer parentLayer,
	 ...
	 * panel
	 *
	 * @param op                      what we're editing
	 * @param thePanel                the panel to put ourselves into
	 * @param ourCustomEditorInstance any custom editor specified by the data object
	...
	/**
	 * the object we are listening to has fired a new report. Display it in our GUI
	 * if we want to
	 *
	 ...
	 *
	 * @param thePanel our parent properties panel
	 */
	@Override
	protected void initForm(final PropertiesPanel thePanel) {
		...
		if (super._theCustomEditor != null) {
			try {
				...
				ourCustomEditorInstance = createCustomEditor(op, thePanel, ourCustomEditorInstance);
			} catch (final Exception e) {
				MWC.Utilities.Errors.Trace.trace(e);
			}

		} // we have a custom editor
		else { // a custom editor was not provided, let's resort to introspection
			...
			_methodsPanel.setLayout(new WrapLayout());
		...
		/**
		 * check that this isn't one of our special editors which doesn't apply/reset
		 * buttons at it's foot
		 */
	...
	@Override
	public void notifyDateInconsistency(final String startDescription, final String endDescription) {
		JOptionPane.showMessageDialog(getPanel(),
				"Start Date must be before End Date. Please, check the following values [" + startDescription
						+ "] and [" + endDescription + "]. Changes haven't been saved..",
				"Date inconsistency", JOptionPane.ERROR_MESSAGE);
	}
			...
			for (int i = 0; i < _theMethods.length; i++) {
				...
				if (_methodsPanel != null) {
					...
					MWC.Utilities.Errors.Trace
							.trace("Trying to show methods when we don't have a 'methods' panel to host:"
	...
	private void showThis(final PropertyDescriptor p, final PropertyEditor pe) {
		...
		if (theComp != null) {
			...
			MWC.Utilities.Errors.Trace.trace("Failed to find editor for:" + p.getShortDescription());
		}
