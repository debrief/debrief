...
/**
 * A Metal Look and Feel implementation of ToolBarUI. This implementation is a
...
public class MyMetalToolBarUI extends MyBasicToolBarUI {
		...
		public void mousePressed(final MouseEvent e) {
			...
			if (toolBar.getSize().height <= toolBar.getSize().width) // horizontal
			{
				if (toolBar.getComponentOrientation().isLeftToRight()) {
					...
					bumpRect.setBounds(toolBar.getSize().width - 14, 0, 14, toolBar.getSize().height);
				}
			} else // vertical
			{
	...
	/**
	 * embedded interface for objects which are capable of owning a toolbar this is
	 * needed for MDI interfaces when it is not clear which document owns the
	 * various toolbars
	 */
	...
	private static Border rolloverBorder = new CompoundBorder(new MetalBorders.RolloverButtonBorder(),
			new BasicBorders.MarginBorder());
	private static Border nonRolloverBorder = new CompoundBorder(new MetalBorders.ButtonBorder(),
			new BasicBorders.MarginBorder());
