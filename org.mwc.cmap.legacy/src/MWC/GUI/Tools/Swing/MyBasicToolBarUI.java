...
/**
 * A Basic L&F implementation of ToolBarUI. This implementation is a "combined"
 * view/controller.
 * <p>
 *
...
public class MyBasicToolBarUI extends javax.swing.plaf.basic.BasicToolBarUI {
	 ...
	 * it only within subclasses of MyBasicToolBarUI.
	 */
		...
		public void setOrientation(final int o) {
			if (isShowing()) {
				...
				if (offset != null) {
					...
					} else if (o == SwingConstants.HORIZONTAL) {
	...
	/**
	 * As of Java 2 platform v1.3 this previously undocumented field is no longer
	 * used. Key bindings are now defined by the LookAndFeel, please refer to the
	...
	protected DragWindow createMyDragWindow(final JToolBar toolbar) {
		...
		if (toolBar != null) {
			Container p;
			for (p = toolBar.getParent(); p != null && !(p instanceof Frame); p = p.getParent()) {
	...
	protected JDialog createMyFloatingFrame(final JToolBar toolbar) {
		...
		final JInternalFrame internalFrame = (javax.swing.JInternalFrame) SwingUtilities
				.getAncestorOfClass(javax.swing.JInternalFrame.class, toolbar);
	...
	protected void dragTo(final Point position, final Point origin) {
		if (toolBar.isFloatable() == true) {
			try {
				...
				final Point global = new Point(origin.x + position.x, origin.y + position.y);
				final Point dragPoint = new Point(global.x - offset.x, global.y - offset.y);
				if (dockingSource == null)
					dockingSource = toolBar.getParent();

				final Point dockingPosition = dockingSource.getLocationOnScreen();
				final Point comparisonPoint = new Point(global.x - dockingPosition.x, global.y - dockingPosition.y);
				if (canDock(dockingSource, comparisonPoint)) {
					dragWindow.setBackground(getDockingColor());
					final String constraint = getDockingConstraint(dockingSource, comparisonPoint);
					...
					dragWindow.setOrientation(SwingConstants.HORIZONTAL);
...
//    return s;
	...
	protected void installKeyboardActions() {
		final InputMap km = getInputMap2(JComponent.WHEN_ANCESTOR_OF_FOCUSED_COMPONENT);
	...
	public void setFloating(final boolean b, final Point p) {
		if (toolBar.isFloatable() == true) {
			...
			if (b == true) {
				...
				final String constraint = getDockingConstraint(dockingSource, p);
				final int orientation = mapConstraintToOrientation(constraint);
