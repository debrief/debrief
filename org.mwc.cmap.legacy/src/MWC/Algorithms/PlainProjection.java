...
/**
 * interface describing how to transform from earth coordinates to screen
 * coordinates
 */
abstract public class PlainProjection implements Serializable, Editable {
	...
	public class PlainProjectionInfo extends Editable.EditorType {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { displayProp("DataBorder", "Data border",
	...
	/**
	 * the parent class which will give us the information necessary to produce a
	 * relative plot
	 */
		...
		if (_theVisibleDataArea == null) {
			...
			if (this.getDataArea() != null && this.getDataArea().getWidth() != 0) {
				...
				if (origin != null) {
					...
					final WorldLocation BR = new WorldLocation(toWorld(
							new Point((int) this.getScreenArea().getWidth(), (int) this.getScreenArea().getHeight())));
