...
public class BriefFormatLocation implements PlainFormatLocation {
	static public class brokenDown {
		// the resolution we are going to use when 'rounding' the product of the
		// subtraction
		...
		public void setData(final double val, final boolean isLat) {
			...
			sec = ((degVal) - (min / 60.0)) * 3600d;

			// there's a wierd circumstance where we get 60 minutes calculated, look out for
			// it
			if (sec >= 59.99) {
				...
				// yes, it says that there are 60 seconds here. set to zero and increment the
				// minutes
				sec = 0;
			}
		}
	...
	/**
	 * degree symbol, obviously. Taken from helpful website at
	...
	/**
	 * reduce object creation - have working copy of object used for breaking down
	 * decimal value into components
	 *
	 */
	...
	static public String toStringLat(final double val, final boolean includeDecimal) {
		...
		String res = " " + df2.format(_workingHolder.deg) + DEGREE_SYMBOL + df2.format(_workingHolder.min) + "\'"
	...
	static public String toStringLong(final double val, final boolean includeDecimal) {
		...
		String res = df3.format(_workingHolder.deg) + DEGREE_SYMBOL + df2.format(_workingHolder.min) + "\'"
				+ secFormat.format(_workingHolder.sec) + "\"";
