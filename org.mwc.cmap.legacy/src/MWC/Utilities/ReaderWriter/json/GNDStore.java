...
public class GNDStore {
	public static class TestDatabase extends TestCase {
		// TODO FIX-TEST
		public void NtestBulkPost1() throws JsonGenerationException, JsonMappingException, IOException {
			...
			for (int i = 0; i < 10; i++) {
				...
				final ObjectNode js = new GNDDocHandler().toJson(trk.getName(), trk, "tst_platform2", "test_plat_type",
						"test_sensor", "test_sensor_type", "test_put_1");
				tracks.add(js);
			}
			...
			for (final Iterator<ObjectNode> iterator = tracks.iterator(); iterator.hasNext();) {
		...
		// TODO FIX-TEST
		public void NtestBulkPost2() throws JsonGenerationException, JsonMappingException, IOException {
			final ArrayList<JsonNode> nodes = new ArrayList<JsonNode>();
			for (int i = 0; i < 10; i++) {
				// get a track
				final Track trk = GNDDocHandler.TestJSON.getTestTrack("test2-" + i);
				final ObjectNode js = new GNDDocHandler().toJson(trk.getName(), trk, "tst_platform2", "test_plat_type",
						"test_sensor", "test_sensor_type", "test_bulk_put");
				nodes.add(js);
			}
			final String url = "http://gnd.iriscouch.com";
			final String db = "tracks";
		...
		// TODO FIX-TEST
		public void NtestGet() throws JsonParseException, JsonMappingException, IOException, ParseException {
			final String url = "http://gnd.iriscouch.com";
			final String db = "tracks";
		...
		public void testPost() throws JsonGenerationException, JsonMappingException, IOException {
			...
			final GNDStore store = new GNDStore(url, db);
			final JsonNode theDoc = new GNDDocHandler().toJson(trk.getName(), trk, "tst_platform2", "test_plat_type",
					"test_sensor", "test_sensor_type", "test_trial");
			// store.put(theDoc);
		}
	...
	public void bulkPut(final ArrayList<JsonNode> theTracks, final int batchSize) throws IOException {
		...
		for (int i = 0; i < len; i += batchSize) {
			...
			final Post post = Http.post(THE_URL, theDoc).header("Content-type", "application/json");
			...
			if (post.responseCode() == 201) {
				...
				for (int k = 0; k < docs.size(); k++) {
					...
					if (reason != null)
						...
						MWC.GUI.LoggingService.INSTANCE().logError(ErrorLogger.INFO, logMsg, null);
					}
				}
			} else {
				MWC.GUI.LoggingService.INSTANCE().logError(ErrorLogger.ERROR, post.responseMessage(), null);
			}
	...
	public GNDDocHandler.GNDDocument get(final String name)
	...
	public String getAnId() throws JsonParseException, JsonMappingException, IOException {
		String res = null;
		...
		final ArrayList<LinkedHashMap<String, String>> rows = (ArrayList<LinkedHashMap<String, String>>) root
				.get("rows");
		if (rows.size() > 0) {
	...
	public void put(final JsonNode doc) throws JsonGenerationException, JsonMappingException, IOException {
		final String THE_URL = _url + "/" + _dbName;
		final String theDoc = GNDDocHandler.asString(doc);

		final Post post = Http.post(THE_URL, theDoc).header("Content-type", "application/json");
		System.out.println("post res:" + post.responseCode() + " " + post.responseMessage());
