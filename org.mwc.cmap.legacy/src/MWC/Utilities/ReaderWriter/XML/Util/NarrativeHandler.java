...
public final class NarrativeHandler extends MWCXMLReader {

	static abstract public class EntryHandler extends MWCXMLReader {

		private static final String _myType1 = "narrative_entry";

		public static void exportEntry(final NarrativeEntry Entry, final Element parent, final Document doc) {
		...
		public final void elementClosed() {
			...
			final NarrativeEntry ne = new NarrativeEntry(_track, _type, _dtg, _entry);
	...
	public static void exportNarrative(final NarrativeWrapper Narrative, final Element parent, final Document doc) {
	...
	private void addThis(final NarrativeEntry entry) {
		...
		final Layer thisL = _theLayers.findLayer(source);
		if (thisL != null && thisL instanceof WatchableList) {
			final WatchableList tw = (WatchableList) thisL;
	...
	public final void elementClosed() {
		...
		if (!_myNarrative.getName().equals(LayerHandler.NARRATIVE_LAYER)) {
			// yes, better put the narrative name into the type field, since the user won't
			// see it
