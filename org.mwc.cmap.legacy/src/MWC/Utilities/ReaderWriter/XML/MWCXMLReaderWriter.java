...
public class MWCXMLReaderWriter extends MWCXMLReader implements PlainImporter {
	protected class ModifiedProgressMonitorInputStream extends ProgressMonitorInputStream {
		...
		public ModifiedProgressMonitorInputStream(final Component parentComponent, final Object message,
	 ...
	 * @throws SAXException
	 */
	static public void importThis(final MWCXMLReader theHandler, final String name, final InputStream is)
			throws SAXException {
	...
	protected void doImport(final InputSource is, final MWCXMLReader theHandler) throws PlainImporter.ImportException {
		try {
			...
			throw new ImportException("Unknown trouble with SAX parsing (not recognised): " + sre.getMessage(), sre);
		} catch (final SAXNotSupportedException spe) {
			MWC.Utilities.Errors.Trace.trace(spe, "Unknown trouble with SAX parsing (not supported)");
			throw new ImportException("Unknown trouble with SAX parsing (not supported): " + spe.getMessage(), spe);
	 ...
	 */
	@Override
	public void importThis(final String fName, final InputStream is, final Layers theData) throws ImportException {
		if (theData == null) {
			importThis(fName, is);
		} else {
			// create a handler
	...
	public void importThis(final String fName, final InputStream is, final MWCXMLReader reader)
		...
		final ProgressMonitorInputStream po = new ModifiedProgressMonitorInputStream(null, "Opening " + fName, is);
