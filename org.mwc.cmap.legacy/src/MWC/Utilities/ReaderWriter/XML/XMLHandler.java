...
public class XMLHandler extends HandlerBase {
	 ...
	 */
	static protected java.text.DateFormat RNdateFormat = new GMTDateFormat("yyMMdd HHmmss.SSS");
	...
	protected void handleOurselves(final String name, final AttributeList attributes) {
		...
		while (enumer.hasMoreElements()) {
			...
			if (val != null) {
				...
				// let's not bother about parameters not being found, they're mostly optional
				// anyway
				// NO, KEEP IT IN! to give us some hints if somebody's application isn't working
				// too well.
				// MWC.Utilities.Errors.Trace.trace("parameter not found for:" + ha.myName + "
				// in element " + name);
			}
		}
	}
	 ...
	 */
	public void handleThis(final Parser parser, final DocumentHandler parent) {
		_theParent = parent;
		_theParser = parser;
	...
	/**
	 * a new element has started. if it is one of ours, process it, else see if one
	 * of our handlers can manage it for us, thanks
	 */
	@Override
	public void startElement(final String name, final AttributeList attributes) throws SAXException {
		...
		if (_myHandlers != null) {
			...
			while (enumer.hasMoreElements()) {
				final XMLHandler hand = enumer.nextElement();
