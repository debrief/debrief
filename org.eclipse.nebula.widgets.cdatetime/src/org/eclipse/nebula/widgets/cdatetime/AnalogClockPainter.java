...
class AnalogClockPainter implements IControlPainter {
	...
	private final AnalogTimePicker picker;
	private final boolean paintMinorTicks = true;
	private final boolean paintShadows = true;

	public AnalogClockPainter(final CDateTime cdt, final AnalogTimePicker picker) {
		this.cdt = cdt;
		this.picker = picker;
	}
	...
	public final void paintContent(final VControl control, final Event e) {
		...
		final float angleS = cal.get(Calendar.SECOND) * 6;
		final float angleM = cal.get(Calendar.MINUTE) * 6;
		float angleH;
		if (picker.is24Hour) {
		...
		final Transform o = new Transform(e.display);
		e.gc.getTransform(o);

		final int lwidth = e.gc.getLineWidth();
		e.gc.setLineWidth(1);

		final int lcap = e.gc.getLineCap();
	...
	protected void paintFace(final VPanel panel, final Event e) {
		final int dia = 2 * picker.dialRadius;
		final int x = picker.dialCenter.x - picker.dialRadius;
		final int y = picker.dialCenter.y - picker.dialRadius;

		final Color c1 = new Color(e.display, 220, 220, 225);
		final Color c2 = new Color(e.display, 200, 200, 200);

		final Pattern p = new Pattern(e.display, 0, y, 0, y + dia, e.display.getSystemColor(SWT.COLOR_WHITE), c1);
		...
		if (paintMinorTicks || picker.is24Hour) {
			...
			final int skip = picker.is24Hour ? 2 : 5;
			for (int i = 0; i < inc; i++) {
				if (i % skip != 0) {
					e.gc.setTransform(t);
		...
		final SimpleDateFormat sdf = new SimpleDateFormat(picker.is24Hour ? "H" : "h");
		e.gc.setForeground(e.display.getSystemColor(SWT.COLOR_DARK_BLUE));
		...
		inc = 12;
		for (int i = 0; i < inc; i++) {
			final int x2 = x + picker.dialRadius
					+ (int) ((picker.dialRadius - 25) * (Math.cos(2 * (double) i * Math.PI / inc - Math.PI / 2)));
			final int y2 = y + picker.dialRadius
					+ (int) ((picker.dialRadius - 25) * (Math.sin(2 * (double) i * Math.PI / inc - Math.PI / 2)));
			final String str = sdf.format(tmpcal.getTime());
			final Point ss = e.gc.stringExtent(str);
	...
	protected void paintHourHand(final VPanel panel, final Event e, final int angle) {
		e.gc.drawLine(-15, 0, picker.dialRadius - 35, 0);
	}

	protected void paintMinuteHand(final VPanel panel, final Event e, final int angle) {
		e.gc.drawLine(-15, 0, picker.dialRadius - 17, 0);
	}

	protected void paintSecondHand(final VPanel panel, final Event e, final int angle) {
		e.gc.drawLine(-15, 0, picker.dialRadius - 12, 0);
	}

	private void setTransform(final Event e, final float angle, final boolean shadow) {
		final Transform t = new Transform(e.display);
