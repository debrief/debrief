...
/**
 * Created by IntelliJ IDEA. User: Ian.Mayo Date: 19-Aug-2003 Time: 14:49:45 To
...
abstract public class WaypointVisitor implements MWCModel {
	...
	/**
	 * We've got a height change manoeuvre. Clip the time if necessary so that we
	 ...
	 */
	protected static long calcClippedHeightChangeTimeFor(final WorldSpeed speed, final double heightDelta,
			final MovementCharacteristics moves, final long heightChangeTimeMillis, final double distanceRemaining) {
		long res = 0;

		// so, what's our climb rate
		final ClimbRateCharacteristics climbChars = (ClimbRateCharacteristics) moves;
		final WorldSpeed climbSpeed = TurnAlgorithm.calcClimbSpeed(heightDelta, climbChars);

		// do we need to accelerate or decelerate?
		final boolean isAccelerating = TurnAlgorithm.areWeAccelerating(speed, climbSpeed);
		final WorldAcceleration accelRate = TurnAlgorithm.calcAccelRate(isAccelerating, moves);

		// and find out how far through the time change we allow ourselves to go
		// (without
	...
	protected static long calcTrimmedAccelTime(final WorldSpeed finalSpeed, final WorldSpeed currentSpeed,
			final WorldAcceleration accelRate, final long stepTimeMillis, final double distanceRemaining) {
		long res;
		...
		final long straightTimeMillis = stepTimeMillis - accelTimeMillis;
		final double distTravelled = accelTimeMillis / 1000d * (curSpd + (finalSpd - curSpd) / 2)
				+ straightTimeMillis / 1000d * finalSpd;
		...
		if (distTravelled > distanceRemaining) {
			...
			final double theTimeSecs = accelTimeMillis / 1000d
					+ (distanceRemaining - (accelTimeMillis / 1000d * (curSpd + (finalSpd - curSpd) / 2))) / finalSpd;
	 ...
	 */
	public static WaypointVisitor createVisitor(final String type) {
		...
		} else if (type.equals(OnTopWaypoint._myType)) {
	 ...
	 */
	protected static double getTurnRateFor(final MovementCharacteristics moves, double speed) {
		// just trim the speed (if it's zero - just give us a little forward motion
		if (speed == 0)
			speed = 1e-10;

		final double turnRadius = moves.getTurningCircleDiameter(speed) / 2;
		...
		final double turnRate = speed / turnRadius;

		return turnRate;
	}

	protected static Status processStraightCourse(final Double requiredDistance, Status current, final long endTime,
			final HighLevelDemandedStatus highLevelDemStat, final TurnAlgorithm turner,
			final MovementCharacteristics moves) {

		// How far do we still have to go?
		final double straightDistance = requiredDistance.doubleValue();

		// how far will we travel in this time step?

		// hey, how long of the time step is remaining?
		final long thisTimeStep = endTime - current.getTime();

		// do we have a demanded height change ('cos this really buggers things up)
		final double demHeight = -highLevelDemStat.getCurrentTarget().getDepth();
		final double curHeight = -current.getLocation().getDepth();

		final WorldLocation origin = new WorldLocation(current.getLocation());

		// take note of the original height. We may change height during a height
		// change, but wish to
		// return to the original
		// height on completion. Alternatively we may wish to change to a demanded speed
		// on completion
		...
		if (demHeight != curHeight) {
			...
			final double distanceRemaining = straightDistance - distanceTravelled;
			if (distanceRemaining > 0.1) {
				// yup, let's continue

				// yup, height change there is...
				final double heightDelta = demHeight - curHeight;

				// find out what the height change speed is
				final double stepTime = (endTime - current.getTime()) / 1000d;

				// and how long will the height change take?
				long heightChangeTimeMillis = (long) (TurnAlgorithm.calcHeightChangeTime(heightDelta, moves, stepTime)
						* 1000d);

				// WORKAROUND - a very small height change may require a time less than one
				// millisecond -
				// represented
				...
				if (moves instanceof ClimbRateCharacteristics) {
					...
					// ok, we're doing a height change with speed consequences. remember the current
					// speed, so
					...
					timeTakenMillis = WaypointVisitor.calcClippedHeightChangeTimeFor(current.getSpeed(), heightDelta,
							moves, heightChangeTimeMillis, distanceRemaining);

					// todo - double-check that height change is achievable in the time-step,
					// though the turn decision algorithm should only produce achievable height
					// changes
					...
					timeTakenMillis = (long) (1000 * straightDistance
		...
		// that's the height change done - we should be at our demanded height now. If
		// there's any time
		// left we will allow
		// a speed change
		...
		if (current.getTime() < endTime) {
			...
			if (distanceRemaining > 0.1) {
				...
				final WorldSpeed demSpeedObj = highLevelDemStat.getSpeed();
				if (demSpeedObj != null) {
					final double demSpeed = demSpeedObj.getValueIn(WorldSpeed.M_sec);
					final double curSpeed = current.getSpeed().getValueIn(WorldSpeed.M_sec);

					if (curSpeed != demSpeed) {
						// ok, process the speed change
						final long remainingTime = endTime - current.getTime();

						final boolean isAccelerating = TurnAlgorithm.areWeAccelerating(curSpeed, demSpeed);
						final WorldAcceleration accelRate = TurnAlgorithm.calcAccelRate(isAccelerating, moves);
						final long timeAllowed = WaypointVisitor.calcTrimmedAccelTime(demSpeedObj, current.getSpeed(),
					...
					if (originalSpeed != null) {
						...
						final boolean isAccelerating = TurnAlgorithm.areWeAccelerating(current.getSpeed(),
								originalSpeed);
						final WorldAcceleration accelRate = TurnAlgorithm.calcAccelRate(isAccelerating, moves);
						final long timeAllowed = WaypointVisitor.calcTrimmedAccelTime(current.getSpeed(),
								current.getSpeed(), accelRate, remainingTime, distanceRemaining);
		...
		if (current.getTime() < endTime) {
			...
			if (distanceRemaining > 0.1) {
				...
				// how long will it take to cover the remaining distance at this speed
				long timeTakenMillis = (long) (1000 * straightDistance
						/ current.getSpeed().getValueIn(WorldSpeed.M_sec));
	...
	/**
	 * if we get interrupted we remember the last location we were at. When resuming
	 * the behaviour we head for the interrupted location and head for it before
	 * resuming the track.
	 */
	 ...
	 * @param demandedHeight the demanded height (normally the same as the current
	 *                       height - to maintain height (and therefore speed) in a
	 *                       manoeuvre.
	 * @return
	 */
	protected Status doThisPartOfTheTurn(final double turnRequired, final MovementCharacteristics moves, Status current,
			final long endTime, final TurnAlgorithm turner, final double demandedHeight) {
		// what's our turn rate?
		double turnRate = WaypointVisitor.getTurnRateFor(moves, current.getSpeed().getValueIn(WorldSpeed.M_sec));
		...
		final long thisStep = endTime - current.getTime();
		turnTime = Math.min(turnTime, thisStep);

		// now, to make sure we turn the right way, we're going to turn 1/2 way first,
		// then the second
		// part
		final double semiTurn = turnRequired / 2;
	 ...
	 * the time/distance to the next waypoint these calculations may become invalid
	 * - we need to clear them in order that they be re-calculated next time we
	 * actually get called. On being interrupted we will remember the location so we
	 * can head back to it before getting back on our proper route
	 *
	 * @param currentStatus
	 */
	public void routeInterrupted(final Status currentStatus) {
		...
		if (_interruptedLocation == null) {
			...
			// we've already been interrupted. we want to head back to that original
			// location, not
			// another.
		}
	 ...
	 * @return
	 */
	public abstract Status step(HighLevelDemandedStatus highLevelDemStatus, Status current, long time,
			MovementCharacteristics moves, TurnAlgorithm turner);
