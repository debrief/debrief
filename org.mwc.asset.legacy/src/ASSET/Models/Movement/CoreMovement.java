...
/**
 * high level class which converts high level demanded statuses into the
 * detailed once necessary for the turn algorith, where applicable
 */
public class CoreMovement implements MovementType, java.io.Serializable {
	...
	static public class CoreMovementTest extends SupportTesting {
		...
		public void testAcceleratingHighLevel() {
			final CoreMovement cm = new CoreMovement();

			WorldLocation.setModel(new CompletelyFlatEarth());

			final OnTopWaypoint myVisitor = new OnTopWaypoint();
			final WorldPath myPath = new WorldPath();
			...
			final HighLevelDemandedStatus hls = new HighLevelDemandedStatus(300, 200, 0, myPath, myVisitor,
		...
		public void testHighLevel() {
			...
			myPath.addPoint(createLocation(400, 300));
			myPath.addPoint(createLocation(600, 800));
			final HighLevelDemandedStatus hls = new HighLevelDemandedStatus(300, 200, 0, myPath, myVisitor, null);
		...
		public void testSimple() {
			final CoreMovement cm = new CoreMovement();
			final Status stat = new Status(0, 100);
			stat.setLocation(new WorldLocation(0, 0, 0));
			stat.setSpeed(new WorldSpeed(12, WorldSpeed.Kts));
			final SimpleDemandedStatus simpleStat = new SimpleDemandedStatus(200, stat);
			...
			final Status res = cm.step(300, stat, simpleStat, chars);
	...
	public Status step(final long newTime, final Status currentStatus, final DemandedStatus demandedStatus,
		...
		if (demandedStatus instanceof HighLevelDemandedStatus) {
			...
			final HighLevelDemandedStatus hl = (HighLevelDemandedStatus) demandedStatus;

			// ok, see what the simple demanded status would be
			final WaypointVisitor visitType = hl.getVisitType();

			// and determine the path
			res = visitType.step(hl, currentStatus, newTime, moves, _turner);
		} else {
			...
			res = _turner.doTurn(currentStatus, simpleDem, moves, newTime);
		}
