...
public class TurnAlgorithm implements MWCModel {
	...
	static public class TurnTest extends SupportTesting {
		...
		public void testAlmostOnCourseTurn() {
			...
			final double demCourse = 307.2185479616559;
			final double curCourse = 307.2185479359266;
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.SSMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate, fuel_usage_rate, maxSpeed, minSpeed, turningCircle,
							defaultClimbRate, defaultDiveRate, maxHeight, minHeight);
		...
		public void testHeloHeightChange() {
			...
			final SimpleDemandedStatus sds = new SimpleDemandedStatus(1000, stat);
			sds.setHeight(80);
			sds.setSpeed(110);
		...
		public void testHeloStraight() {
			...
			ASSET.Models.Movement.SimpleDemandedStatus dem = new ASSET.Models.Movement.SimpleDemandedStatus(0,
					dem_stat);
			...
			super.assertEquals("Straight, decelerating - travelled correct distance", t3_expected_range,
					MWC.Algorithms.Conversions.Degs2m(wv.getRange()), 0.001);
			super.assertEquals("Straight, decelerating - travelled backwards", 180,
					MWC.Algorithms.Conversions.Rads2Degs(wv.getBearing()), 0.001);
		...
		public void testHeloTurnShape() {
			...
			final ASSET.Models.Movement.SimpleDemandedStatus dem = new ASSET.Models.Movement.SimpleDemandedStatus(0,
					dem_stat);
			...
			final WorldLocation thisEndPoint = stat.getLocation().add(new WorldVector(
					MWC.Algorithms.Conversions.Degs2Rads(90), MWC.Algorithms.Conversions.m2Degs(turn_diam), 0));
		...
		public void testSSNDelayedTurn() {
			...
			final CoreScenario dummyScenario = new CoreScenario();
		...
		public void testSSNStraight() {
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.SSMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate, fuel_usage_rate, maxSpeed, minSpeed, defaultClimbRate,
							defaultDiveRate, maxHeight, minHeight, turningCircle);
		...
		public void testSSNTurn() {
			...
			assertEquals("Turn clock, steady speed", 200,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation().getLat() - origin.getLat()), 0.01);
			assertEquals("Turn clock, steady speed", 200,
					MWC.Algorithms.Conversions.Degs2m(res.getLocation().getLong() - origin.getLong()), 0.01);
		...
		public void testZeroStepTime() {
			final TurnAlgorithm turner = new TurnAlgorithm();
			final Status oldStat = new Status(12, 12000);
			final SimpleDemandedStatus oldDem = null;
			final MovementCharacteristics chars = null;
			final Status res = turner.doTurn2(oldStat, oldDem, chars, oldStat.getTime());
	...
	/**
	 * convenience method to return boolean depending on if we're accelerating or
	 * not
	 *
	 ...
	 */
	public static boolean areWeAccelerating(final double currentSpeed, final double demandedSpeed) {
		boolean res;
	...
	public static boolean areWeAccelerating(final WorldSpeed speed, final WorldSpeed climbSpeed) {
	 ...
	 */
	public static WorldAcceleration calcAccelRate(final boolean isAccelerating, final MovementCharacteristics moves) {
		WorldAcceleration res = null;
		final WorldAcceleration accelRate = moves.getAccelRate();
		final WorldAcceleration decelRate = moves.getDecelRate();
	 ...
	 */
	private static WorldAcceleration calcAccelRate(final boolean isAccelerating, final WorldAcceleration accelRate,
			final WorldAcceleration decelRate) {
	 ...
	 */
	public static WorldSpeed calcClimbSpeed(final double heightDelta, final ClimbRateCharacteristics moves) {
		WorldSpeed res;
	 ...
	 */
	public static double calcHeightChange(final double timeSecs, final MovementCharacteristics moves,
	 ...
	 */
	protected static double calcHeightChangeTime(final double dHeight, final MovementCharacteristics chars,
			final double secs) {
	...
	public ASSET.Participants.Status doTurn(final ASSET.Participants.Status status,
			...
			final ASSET.Models.Movement.MovementCharacteristics chars, final long newTime) {
		Status res = null;

		// so, lets see if we are handling a vehicle which travels at default
		// speeds
		if (chars instanceof ClimbRateCharacteristics) {
			final ClimbRateCharacteristics climber = (ClimbRateCharacteristics) chars;
			...
			if (demStatus != null) {
				...
				// ok - what's the delta
				final double heightDelta = demHeight - (-status.getLocation().getDepth());

				// now sort out the default
				final double defaultSpeed;

				// how long is this step
				final double secs = ((double) newTime - status.getTime()) / 1000;
				...
				if (heightDelta != 0) {
					...
					final double HeightTime = calcHeightChangeTime(heightDelta, chars, secs);

					// convert to long millis
					final long HeightMillis = (long) (HeightTime * 1000d);

					// remember the current dem speed
					final double oldDemSpeed = demStatus.getSpeed();
	...
	public ASSET.Participants.Status doTurn2(final ASSET.Participants.Status status,
		...
		final double accel_rate_m_sec_sec = chars.getAccelRate().getValueIn(WorldAcceleration.M_sec_sec); // value in
																											// m/sec/sec
		final double decel_rate_m_sec_sec = chars.getDecelRate().getValueIn(WorldAcceleration.M_sec_sec); // value in
																											// m/sec/sec
		...
		if (Math.abs(course_change) > COURSE_DELTA) {
			...
			final double sinChange = Math.sin(course_change);
		...
		if ((accel_time == 0) && (turn_time == 0)) {
			...
			if (accel_time >= turn_time) {
				...
				if (turn_time > 0) {
					...
					turn_speed = curSpeed_m_sec + (accelerating * turn_time * speed_change_rate_m_sec_sec / 2);
				...
				// acceleration
				accel_speed = curSpeed_m_sec
						+ accelerating * (accel_time - turn_time) * speed_change_rate_m_sec_sec / 2;
				...
				// acceleration
				curSpeed_m_sec = (curSpeed_m_sec
						+ accelerating * (accel_time - turn_time) * speed_change_rate_m_sec_sec);
				...
				if (accel_time > 0) {
					...
					turn_speed = curSpeed_m_sec + accelerating * accel_time * speed_change_rate_m_sec_sec / 2;
		...
		final WorldLocation newLoc = status.getLocation()
				.add(new WorldVector(dBrg, Conversions.m2Degs(dRng_m), -HeightDelta));
		...
		final double courseDegs = MWC.Algorithms.Conversions.Rads2Degs(demCourseRads);
		final double courseDelta = Math.abs(res.getCourse() - courseDegs);
	...
	private void performConstantSpeedTurn(final MovementCharacteristics chars, final double turn_speed,
		...
		if (turn_time > 0) {
			...
			// the turn
			longVal_m = (longVal_m + 2 * TCircle * Math.sin(curCourse_rads + turn_direction * course_change / 2)
					* Math.sin(course_change / 2));

			latVal_m = (latVal_m + 2 * TCircle * Math.cos(curCourse_rads + turn_direction * course_change / 2)
					* Math.sin(course_change / 2));
	...
	private void performStraightLineTravel(final double straight_time, final double travelSpeed_m_sec) {
		if (straight_time > 0) {
			longVal_m = (longVal_m + travelSpeed_m_sec * straight_time * Math.sin(curCourse_rads));

			latVal_m = (latVal_m + travelSpeed_m_sec * straight_time * Math.cos(curCourse_rads));
		}
	}
