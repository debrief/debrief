...
public final class ExpandingSquareSearch extends CoreDecision implements Serializable {
	...
	static public class ExpandingSquareSearchInfo extends EditorType {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Name", "the name of this bearing trail model"),
	...
	public static final class ExpandingSquareSearchTest extends SupportTesting.EditableTesting {
		...
		public final void testCreateRoute() {
			...
			final ASSET.Models.Movement.MovementCharacteristics chars = ASSET.Models.Movement.SSMovementCharacteristics
					.generateDebug(myName, accelRate, decelRate, fuel_usage_rate, maxSpeed, minSpeed, turnCircle,
							defaultClimbRate, defaultDiveRate, maxDepth, minDepth);
			final double currentSpeed = 10;

			//////////////////////////////////////////////////
			WorldPath wp = createSearchRoute(maxLegs, chars, flyClockwise, origin, trackSpacing, initialTrack,
					currentSpeed);
			//////////////////////////////////////////////////
			...
			//////////////////////////////////////////////////
			wp = createSearchRoute(new Integer(maxLegs.intValue() + 1), chars, flyClockwise, origin, trackSpacing,
					initialTrack, currentSpeed);
			...
			//////////////////////////////////////////////////
			wp = createSearchRoute(new Integer(1), chars, !flyClockwise, origin, trackSpacing, initialTrack,
					currentSpeed);
			...
			//////////////////////////////////////////////////
			wp = createSearchRoute(new Integer(0), chars, !flyClockwise, origin, trackSpacing, initialTrack,
					currentSpeed);
		...
		public final void testFollowRoute() {
			...
			final double initialTrack = 0;
			final ExpandingSquareSearch ess = new ExpandingSquareSearch(flyClockwise, initialTrack, maxLegs, origin,
					trackSpacing, "the square");
			...
			// assertEquals("search now complete", cp.getActivity(),
			// CoreParticipant.INACTIVE_DESCRIPTOR);

			// check we've ended up to the north west
			// assertTrue("ended up further north", cp.getStatus().getLocation().getLat() >
			// 0);
			// assertTrue("ended up further east", cp.getStatus().getLocation().getLong() >
			// 0);
		}
		...
		public final void testUnattainablePoints() {
			...
			assertTrue("we didn't create all the destinations",
					ess._myRoute.getDestinations().size() < maxLegs.intValue());
	 ...
	 */
	protected static WorldPath createSearchRoute(final Integer maxLegs, final MovementCharacteristics chars,
			final boolean flyClockwise, final WorldLocation origin, final WorldDistance trackSpacing,
			final double initialTrack, final double currentSpeed) {
		...
		final double turn_circle_rad_degs = MWC.Algorithms.Conversions
				.m2Degs(chars.getTurningCircleDiameter(currentSpeed)) / 1.5;
		...
		for (int i = 1; i <= routeLen; i++) {
			...
			if (lastLocation != null) {
				final double rngToLast = currentLocation.rangeFrom(lastLocation);
	...
	public ExpandingSquareSearch(final boolean flyClockwise, final double initialTrack, final Integer maxLegs,
			final WorldLocation myOrigin, final WorldDistance worldDistance, final String name) {
		super(name);
	 ...
	 */
	@Override
	public final DemandedStatus decide(final Status status, final ASSET.Models.Movement.MovementCharacteristics chars,
		...
		if (!_finished) {
			...
			if (_myRoute == null) {
				...
				final double curSpeed = status.getSpeed().getValueIn(WorldSpeed.M_sec);

				// create the path to follow
				final WorldPath route = createSearchRoute(_maxLegs, chars, _flyClockwise, _myOrigin, _trackSpacing,
						_initialTrack, curSpeed);
	...
	public void setClockwise(final boolean flyClockwise) {
		this._flyClockwise = flyClockwise;
	}
