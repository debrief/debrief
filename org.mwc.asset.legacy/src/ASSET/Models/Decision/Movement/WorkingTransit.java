...
public class WorkingTransit extends Transit {
	...
	public static class TransitWorkingTest extends SupportTesting.EditableTesting {
		...
		protected WorkingTransit getSampleInstance() {
			final WorldLocation newLoc1 = new WorldLocation(0, 1, 2);
			final WorldLocation newLoc2 = new WorldLocation(0, 2, 2);
			final WorldLocation newLoc3 = new WorldLocation(0, 1, 3);
			final WorldPath newPath = new WorldPath(new WorldLocation[] { newLoc1, newLoc2, newLoc3 });
			final WorldSpeed transitSpeed = new WorldSpeed(12, WorldSpeed.Kts);
			final Sequence activity = new Sequence(true, "Sequence");
		...
		public void NtestWorking() {
			...
			final WorldSpeed transitSpeed = new WorldSpeed(12, WorldSpeed.Kts);
			final Sequence activity = new Sequence(true, "Sequence");
			final Wait doWait = new Wait(new Duration(12, Duration.SECONDS), "do a wait");
			activity.insertAtHead(doWait);
			final WorkingTransit transit = new WorkingTransit(activity, newPath, transitSpeed, false, 2);
			...
			final Status theStat = new Status(23, 0);
			WorldLocation theLocation = new WorldLocation(0, 0, 0);
			final WorldSpeed mySpeed = new WorldSpeed(12, WorldSpeed.Kts);
			final MovementCharacteristics chars = SurfaceMovementCharacteristics.getSampleChars();
			...
			assertEquals("correctly reported doing activity",
					"WorkingTransit:Performing activity:Sequence:do a wait:Still waiting", transit.getActivity());
			...
			// ok, check it worked
			assertNotNull("haven't produced demanded status", dem);
			assertEquals("correctly reported doing activity", "WorkingTransit:Transit heading for waypoint:2",
					transit.getActivity());
			...
			// ok, check it worked
			assertNotNull("haven't produced demanded status", dem);
			assertEquals("correctly reported doing activity", "WorkingTransit:Transit heading for waypoint:3",
					transit.getActivity());
			assertEquals("correctly moved onto next waypoint", 2, transit.getCurrentDestinationIndex());
	 ...
	 */
	public WorkingTransit(final Sequence myActivity, final WorldPath destinations, final WorldSpeed transit_speed,
			final boolean loop, final int numStops) {
		...
		if (numStops > 0) {
			...
			final WorldPath brokenDownPath = destinations.breakIntoStraightSections(numStops);
	 ...
	 * @param _numStops
	 */
	public void setNumStops(final int _numStops) {
	 ...
	 */
	public void setWorkingActivity(final Sequence myActivity) {
		this._myActivity = myActivity;
	}
