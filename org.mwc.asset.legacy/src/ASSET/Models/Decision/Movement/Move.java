...
/**
 * behaviour to travel at an indicated speed and course for an indicated
 * distance. An optional depth value is available
 */
public class Move extends CoreDecision implements java.io.Serializable {
	...
	static private class MoveInfo extends MWC.GUI.Editable.EditorType {
		...
		public java.beans.PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final java.beans.PropertyDescriptor[] res = { prop("Speed", "the speed we transit at (kts)"),
						prop("Course", "the course to travel at (degs)"), prop("Distance", "the distance we travel"),
	...
	public static class MoveTest extends SupportTesting.EditableTesting {
		...
		public void testTheMove() {
			...
			final WorldLocation locA = new WorldLocation(0, 0, 12);
			final WorldLocation locC = new WorldLocation(0, 3, 12);
			...
			final Status theStat = new Status(12, 12000);
	...
	public DemandedStatus decide(final Status status, final ASSET.Models.Movement.MovementCharacteristics chars,
		...
		final WorldLocation currentLoc = status.getLocation();

		// are we running towards a destination?
		if (_theDistance == null) {
			// ok, we're not heading towards a particular point, just put us onto the
			// correct course and
			// speed
			final double curSpeed = status.getSpeed().getValueIn(WorldSpeed.M_sec);
			final double curCourse = status.getCourse();
			final double curHeight = -status.getLocation().getDepth();
			...
			if (_theDestination == null) {
				...
				final WorldVector vector = new WorldVector(
				...
				final double rngDegs = currentLoc.subtract(_theDestination).getRange();

				// and in yards
				final double rngYds = MWC.Algorithms.Conversions.Degs2Yds(rngDegs);
			...
			final double brg_rads = _theDestination.subtract(currentLoc).getBearing();
	 ...
	 */
	public void setCourse(final Double theCourse) {
	 ...
	 * @param theHeight
	 */
	public void setHeight(final WorldDistance theHeight) {
