...
public class Trail extends CoreDecision implements java.io.Serializable {
	...
	public static class TrailTest extends SupportTesting.EditableTesting {
		...
		public void testTheTrail() {
			...
			final ASSET.Models.Detection.DetectionEvent det1 = new DetectionEvent(2000, host.getId(), null, sensor,
					null, null, null, null, null, tgt, null, null, new CoreParticipant(12));
			...
			final ASSET.Models.Detection.DetectionEvent det2 = new ASSET.Models.Detection.DetectionEvent(2000,
					host.getId(), stat.getLocation(), sensor, new WorldDistance(2000, WorldDistance.YARDS),
					new WorldDistance(2000, WorldDistance.YARDS), new Float(33), new Float(44), null, tgt2, null, null,
					new CoreParticipant(12));
	...
	private static boolean isThisValid(final DetectionEvent detection, final TargetType myTarget,
			final Integer requiredHost) {
		boolean res = false;
	...
	/**
	 * calculate the course (in degs) from our location to the target, allowing for
	 * a third party sensor
	 *
	 * @param myId
	 * @param myLocation
	 * @param detection
	 * @return
	 */
	protected double courseToTarget(final int myId, final WorldLocation myLocation, final DetectionEvent detection) {
		double res = 0;
		...
		if (detection.getHost() == myId) {
			...
			// target
			final WorldLocation sensorLocation = detection.getSensorLocation();

			// Work out the estimated target location
			final WorldVector sensorToTarget = new WorldVector(
					MWC.Algorithms.Conversions.Degs2Rads(detection.getBearing().doubleValue()),
					detection.getRange().getValueIn(WorldDistance.DEGS), 0);

			final WorldLocation targetLocation = sensorLocation.add(sensorToTarget);

			// what's the bearing to the target location?
			final WorldVector meToTarget = targetLocation.subtract(myLocation);
	...
	public ASSET.Participants.DemandedStatus decide(final ASSET.Participants.Status status,
		...
		if (validDetection == null) {
			...
			final WorldDistance rangeToTarget = validDetection.getRange();
	...
	protected DemandedStatus getDemanded(final long time, final ASSET.Participants.Status status,
		...
		if (Brg != null) {
			...
			if (rng != null) {
				...
				final WorldVector sensorToTarget = new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(brg),
						rng.getValueIn(WorldDistance.DEGS), 0);

				final WorldLocation tgtLocation = detection.getSensorLocation().add(sensorToTarget);

				// are we very far from this location?
				final WorldVector hostToTarget = tgtLocation.subtract(status.getLocation());

				// modify our course so we are heading for the target location
				final double brg_degs = MWC.Algorithms.Conversions.Rads2Degs(hostToTarget.getBearing());
				res.setCourse(brg_degs);

				// and what's this range in yards?
				final double rangeToTargetYds = MWC.Algorithms.Conversions.Degs2Yds(hostToTarget.getRange());

				// are we very far from our required range?
				final double rangeYds = _trailRange.getValueIn(WorldDistance.YARDS);
	 ...
	 * @return
	 */
	protected WorldDistance rangeToTarget(final int myId, final WorldLocation myLocation,
	...
	public void setAllowSpeedChange(final boolean allowSpeedChange) {
		_allowSpeedChange = allowSpeedChange;
	}

	public void setStayOnBearingTime(final Duration stayOnBearing) {
	...
	public void setTrailHeight(final WorldDistance trailHeight) {
