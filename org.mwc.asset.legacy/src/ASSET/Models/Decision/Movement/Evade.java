...
public class Evade extends CoreDecision implements java.io.Serializable {
	...
	public static class EvadeTest extends SupportTesting.EditableTesting {
		...
		public Editable getEditable() {
			return new Evade(null, new WorldSpeed(12, WorldSpeed.M_sec), new WorldDistance(12, WorldDistance.METRES),
					null);
		}
	 ...
	 * @return
	 */
	private static double calcBDot(final double brg, final double rng, final double oCrse, final double oSpd,
			final double tCrse, final double tSpd) {
	...
	public static void main(final String[] args) {
		...
		final double brg = MWC.Algorithms.Conversions.Degs2Rads(45);
		final double rng = 1000;// MWC.Algorithms.Conversions.Yds2Degs(1000);
		final double res2 = calcBDot(brg, rng, oCrse, oSpd, tCrse, tSpd);
		...
		if (getName().equals("CQD"))
			...
			if (time <= _finishedFleeing) {
				...
				if ((detections != null) && (detections.size() > 0)) {
					...
					final DetectionList matches = detections.getDetectionsOf(_myTargetType);

					if (matches != null) {

						// find the nearest detection (which may not be the one we are
						// currently tracking
						final DetectionEvent nearestDet = matches.getNearestDetection();
					...
					if (_previousCourse != DUFF_COURSE) {
						...
						final double courseDelta = Math.abs(status.getCourse() - _previousCourse);
						if (courseDelta > 5) {
	...
	public SimpleDemandedStatus decideCQD(final ASSET.Participants.Status status,
		...
		if (_evading) {
			...
			final long elapsed = time - _firstContact;
			...
			if (elapsed > TRACK_FOR) {
				...
				// get teh target
				final NetworkParticipant target = monitor.getThisParticipant(_myTarget.intValue());

				// what's his bearing rate?
				final WorldVector wv = target.getStatus().getLocation().subtract(status.getLocation());
				...
				final double tCrse = MWC.Algorithms.Conversions.Degs2Rads(target.getStatus().getCourse());
				final double tSpd = target.getStatus().getSpeed().getValueIn(WorldSpeed.Kts);
				final double oCrse = MWC.Algorithms.Conversions.Degs2Rads(status.getCourse());
				final double oSpd = status.getSpeed().getValueIn(WorldSpeed.Kts);

				final double bDot = calcBDot(brg, rng, oCrse, oSpd, tCrse, tSpd);
				...
				if (finalDemandedCourse != null) {
					...
					final double finalCourse = finalDemandedCourse.doubleValue();
