...
/**
 * Represents a Waterfall of prioritised behaviours. If a higher priority
 * behaviour is not applicable, it will return a NULL demanded status object,
 * and processing will fall to the behaviour with next lower priority, and so on
 * until a "catch-all" behaviour is supplied which will always return a demanded
 * course, or not, in which case the Waterfall returns null. Company:
 *
 * @author Ian Mayo
 * @version 1.0
 */
...
public class Waterfall extends CoreDecision implements java.io.Serializable, BehaviourList {
	...
	/**
	 * the index of the last decision model called. we keep track of the previous
	 * model so we can call it's interrupted method after a higher-priority
	 * behaviour interrupts it
	 *
	...
	public ASSET.Participants.DemandedStatus decide(final ASSET.Participants.Status status,
		...
		for (thisIndex = 0; thisIndex < _myDecisions.size(); thisIndex++) {
			final ASSET.Models.DecisionType dt = _myDecisions.elementAt(thisIndex);
		...
		if (_lastDecision != INVALID_DECISION) {
			...
			if (thisIndex != _lastDecision) {
				...
				if (thisIndex < _lastDecision) {
					...
					final ASSET.Models.DecisionType dt = _myDecisions.elementAt(_lastDecision);
					dt.interrupted(status);
				}
			}
		}

		// output the activity (even though it will include both the waterfall and
		// activity reports)
		super.setLastActivity(activity);

		// just check if none of our models fired. If not, we need to set this decision
		// as invalid
