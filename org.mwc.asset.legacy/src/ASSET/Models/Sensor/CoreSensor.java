    ...
    java.io.Serializable, MWCModel
{
  ...
  /**
   * the time of the last scan (which we remember so that we can correctly observe the _TBDO value
   */
  ...
  /**
   * ok, we keep track of new detections in each cycle. use a static list so that we don't forget
   * them
   */
  ...
  /**
   * remove any of our past detections from the list. Detections stay valid between TBDOs, to give
   * some degree of persistnce
   * 
   * @param existingDetections
  ...
  /**
   * convenience method to indicate if this type of sensor is capable of identifying a target
   * 
   * @return yes/no
   */
  ...
  /**
   * convenience method to indicate if this sensor produces a bearing from the sensor to the target.
   * Very few sensors do not produce bearing.
   * 
   * @return yes/no
   */
