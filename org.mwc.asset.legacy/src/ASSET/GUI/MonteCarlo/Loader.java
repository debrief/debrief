...
abstract public class Loader extends SwingCustomEditor
{
	 ...
	 */
	public Loader(CoreScenario myScenario, String scenarioFile,
			String controlFile, PlainChart theChart, ToolParent theParent,
	 ...
	 */
	public Loader(CoreScenario theScenario, PlainChart theChart,
			ToolParent theParent, PropertiesPanel theProperties)
	{
	...
	private void initForm()
	{
		...
		_scenarioCatcher = new JLabel("<HTML>" + SCENARIO_LABEL
				+ " [drop here]</html>");
				...
				public void drop(DropTargetDropEvent dtde)
				{
					...
					if (dtde.isDataFlavorSupported(DataFlavor.javaFileListFlavor))
					{
						try
						{
							...
							final List list = (List) tr
									.getTransferData(DataFlavor.javaFileListFlavor);
							if (list.size() > 1)
							{
								MWC.Utilities.Errors.Trace.trace("One file at a time please",
										true);
							}
							else
							{
		...
		_controllerCatcher = new JLabel("<HTML>" + CONTROL_LABEL
				+ " [drop here]</html>");
	...
	void setController(File thisFile)
	{
		...
		_controllerCatcher.setText("<html>" + CONTROL_LABEL + thisFile.getName()
				+ "</html>");
	...
	void setScenario(File thisFile)
	{
		...
		_scenarioCatcher.setText("<html>" + SCENARIO_LABEL + thisFile.getName()
				+ "</html>");
	}
