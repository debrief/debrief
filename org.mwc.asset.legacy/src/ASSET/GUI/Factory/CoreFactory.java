...
public class CoreFactory implements GeneticAlgorithm.GAProgressed, GeneticAlgorithm.GAStepped {
	...
	protected class CoreScenarioRunner implements ScenarioRunner, ASSET.Scenario.ScenarioRunningListener {
		...
		public ScenarioRunner.ScenarioOutcome runThis(final String scenario, final String name, final String desc) {
			...
			System.out.print(".");
			final ScenarioRunner.ScenarioOutcome res = new ScenarioOutcome();
			res.score = -1;
			res.summary = "";
			...
			for (int i = 0; i < _myObservers.size(); i++) {
				...
				if (observer instanceof ScenarioObserver.ScenarioReferee) {
					...
					final ScenarioRunner.ScenarioOutcome thisRes = ref.getOutcome();
					if (thisRes != null) {
	 ...
	 */
	private final ScenarioRunner _myRunner;
	...
	/**
	 * add the indicated observer to our list. The observer will receive setup and
	 * teardown calls for each new scenario
	 */
	...
	public void loadObserver(final String fName) {
		...
		try {
			...
			final MWC.Utilities.ReaderWriter.XML.MWCXMLReader reader = new ASSET.Util.XML.Control.FactoryHandler() {
				@Override
				public void setFactory(final Vector<ScenarioObserver> list, final int genes, final int stars,
						final boolean lowScoresHigh) {
	...
	/**
	 * store the filename containing the scenario, and check if we are ready to
	 * start
	 *
	...
	/**
	 * store the filename containing the variables, and check if we are ready to
	 * start
	 *
	...
	public void signalAllDataLoaded() {
		// don't really bother - other classes can over-ride this method to find out
		// when data is loaded
	}
