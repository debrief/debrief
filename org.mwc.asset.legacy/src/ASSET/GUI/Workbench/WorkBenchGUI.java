...
public class WorkBenchGUI extends CoreGUISwing implements ScenarioNoiseLevelPainter.StatusProvider {
	...
	public static void main(final String[] args) {
		...
		if (args.length == 1) {
			...
			try {
				final Vector<File> theFiles = new Vector<File>();
				for (int i = 0; i < args.length; i++) {
					final String thisFile = args[i];
				...
				MWC.GUI.Dialogs.DialogFactory.showMessage("Load files",
						"Sorry one of the files to load was not found" + e.getMessage());
			...
			final String scenarioFile = args[0];
			final String controlFile = args[1];
			...
			final SwingPropertiesPanel spp = (SwingPropertiesPanel) viewer.getProperties();
	...
	protected void addUniqueComponents(final CoreGUISwing.MyToolbarHolder assetHolder) {
		...
		getChart().getCanvas().getProjection().setDataArea(new MWC.GenericData.WorldArea(
				new MWC.GenericData.WorldLocation(0, 0, 0), new MWC.GenericData.WorldLocation(2, 2, 2)));
		...
		final FileList flBehaviour = new FileList("Behaviours", "Behaviours", null);
		...
		getChart().getCanvas().getProjection().setDataArea(
				new WorldArea(new MWC.GenericData.WorldLocation(0, 0, 0), new MWC.GenericData.WorldLocation(2, 2, 2)));

		// add the chart plotter
		_scenarioPlotter = new ASSET.GUI.Workbench.Plotters.ScenarioLayer();
	...
	protected void addUniqueTools() {
		...
		_theTools.addElement(
				new MenuItemInfo("ASSET", null, "Restart", new PlainTool(_theParent, "Restart", "images/VCRBegin.gif") {
					/**
					 *
					 */
					private static final long serialVersionUID = 1L;
		...
		_theTools.addElement(new MenuItemInfo("File", null, "Save",
				new ASSET.Util.XML.Tools.SaveScenarioXML(_theParent, _theScenario), null, 'S'));

		_theTools.addElement(new MenuItemInfo("File", null, "SaveAs",
				new ASSET.Util.XML.Tools.SaveScenarioAsXML(_theParent, _theScenario), null, 'A'));

		_theTools.addElement(
				new MenuItemInfo("File", null, "Monte Carlo importer", new ASSET.GUI.Tools.OpenMonteCarloPanel(
						_theParent, (SwingPropertiesPanel) _theProperties, getChart(), _theScenario), null, 'M'));
		_theTools.addElement(new MenuItemInfo(
				"Noise", null, "Noise Levels", new ASSET.GUI.Tools.CreateNoiseLevel(_theParent, _theProperties,
						_theData, bounds, _theScenario.getEnvironment(), this, EnvironmentType.BROADBAND_PASSIVE),
				null, 'N'));

		_theTools.addElement(new MenuItemInfo(
				"Noise", null, "Noise Source", new ASSET.GUI.Tools.CreateNoiseSource(_theParent, _theProperties,
						_theData, bounds, _theScenario.getEnvironment(), EnvironmentType.BROADBAND_PASSIVE),
				null, 'A'));

		_theTools.addElement(new MenuItemInfo("Noise", null, "Noise Excess",
				new ASSET.GUI.Tools.CreateNoiseExcess(_theParent, _theProperties, _theData, bounds), null, 'A'));
	...
	void participantDropped(final Vector<File> files, final Point point) {
		...
		while (ii.hasNext()) {
			final File file = ii.next();
			// read in this file

			try {
				final ASSET.ParticipantType newPart = ASSETReaderWriter.importParticipant(file.getName(),
