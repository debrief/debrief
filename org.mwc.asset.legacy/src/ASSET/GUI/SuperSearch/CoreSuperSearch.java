...
/**
 * Structure of SuperSearch: CoreSuperSearch: non-gui framework which handles: -
 * creation of server, scenario, participants - listening to participant
 * creation, movement, detections - maintaining stats of detections, movement,
 * performance - format of presentation of results
 *
 */
...
public class CoreSuperSearch implements ASSET.Scenario.ScenarioSteppedListener {
	...
	private void fireScenarioStepped(final ScenarioType scenario, final long time) {
		if (_stepListeners != null) {
			final java.util.Iterator<ScenarioSteppedListener> it = _stepListeners.iterator();
			while (it.hasNext()) {
				final ASSET.Scenario.ScenarioSteppedListener pcl = it.next();
	...
	// {
	// // todo: reinstate this test - using the correct monte carlo generator schema
	// files
	//
	//
	...
	// assertEquals("gui has heard all new creations",
	// css._guiSupport.getVector().size(), 100);
	//
	// // add the blue vessel
	// final ASSET.Participants.Status stat = new ASSET.Participants.Status(2, 12);
	// final MWC.GenericData.WorldLocation origin = new
	// MWC.GenericData.WorldLocation(0.1,0.1,1);
	// stat.setLocation(origin);
	// stat.setCourse(0);
	// stat.setSpeed(new WorldSpeed(3, WorldSpeed.M_sec));
	//
	// final ASSET.ParticipantType ssn = new ASSET.Models.Vessels.SSN(20, stat, new
	// SimpleDemandedStatus(0, stat), "BLUE_SSN");
	...
	// final SSGuiSupport.ParticipantListener pl =
	// (SSGuiSupport.ParticipantListener)
	// css._guiSupport.getVector().lastElement();
	//
	// // check location
	// final MWC.GenericData.WorldLocation newLocation = new
	// MWC.GenericData.WorldLocation(pl.getLocation());
	//// assertEquals("listener has correct location", newLocation.rangeFrom(new
	// MWC.GenericData.WorldLocation(0,0,0)), 0, 0.005 );
	//
	// // move, and see if it has moved
	// css._myScenario.step();
	// super.assertTrue("We've moved", (pl.getLocation() != newLocation));
	// System.out.println("range:" +
	// MWC.Algorithms.Conversions.Degs2Yds(pl.getLocation().rangeFrom(newLocation)));
	//
	//
	//
	// }
