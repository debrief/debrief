...
abstract public class CoreGUISwing extends CoreGUI implements MyMetalToolBarUI.ToolbarOwner {
	...
	public static class ASSETParent implements ToolParent {
		...
		public ASSETParent(final JFrame frame) {
			...
			try {
				final String header = System.getProperty("line.separator") + "#ASSET Properties File";
				_appProps = new MWC.GUI.Dialogs.ApplicationProperties("asset.prp", header);
	...
	public static void registerEditors() {
		PropertyEditorManager.registerEditor(Color.class, MWC.GUI.Properties.Swing.ColorPropertyEditor.class);

		// add our editors
		PropertyEditorManager.registerEditor(ASSET.Models.Decision.TargetType.class,
	...
	public static void showSplash(final Frame parent, final String imageName) {
		// do the splash
		new SplashScreen(parent, imageName, "Built:" + ASSET.GUI.VersionInfo.getVersion(), Color.LIGHT_GRAY);
	}
	 ...
	 */
	private final Vector<ScenarioObserver> _myObservers = new Vector<ScenarioObserver>(0, 1);
	...
	protected void addThisTool(final MWC.GUI.Tools.MenuItemInfo item) {
		...
		if (item.getToggleGroup() == null) {
			...
			thisToolbar.addToggleTool(item.getMenuName(), item.getTool(), item.getShortCut(), item.getMnemonic());
		}
	}
	 ...
	 */
	abstract protected void addUniqueComponents(CoreGUISwing.MyToolbarHolder assetHolder);
	...
	private void initForm() {
		...
		final SwingChart _theChart = new SwingChart(_theData, "images\\in_plot_logo.gif");
		...
		_theProperties = new SwingPropertiesPanel(_theData, getUndoBuffer(), super.getParent(), this);
		...
		_theTime.setHorizontalAlignment(SwingConstants.CENTER);
	...
	public void observerDropped(final Vector<File> files) {
		...
		while (ii.hasNext()) {
			...
			try {
				final Vector<ScenarioObserver> theObservers = ASSETReaderWriter.importThisObserverList(file.getName(),
						new java.io.FileInputStream(file));

				// check we have a layer for the observers
				final Layer observers = checkObserverHolder();
