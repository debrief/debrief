...
abstract public class CoreGUI {
	 ...
	 */
	private final ASSET.ServerType _theServer;
	...
	private void addTools() {
		...
		_theTools.addElement(new MenuItemInfo("File", null, "File",
				new ShowLayers(_theParent, "Show Outline View", _theProperties, _theData), null, ' '));

		// _theTools.addElement(new MenuItemInfo("View", null, "Repaint", new
		// Repaint(_theParent,
		// _theChart), null, 'R'));
		...
		_theTools.addElement(new MenuItemInfo("View", null, "Repaint",
				new MWC.GUI.Tools.Chart.Repaint(_theParent, _theChart), null, 'r'));
		...
		_theTools.addElement(new MenuItemInfo("Chart Features", null, "Create Scale",
				new CreateScale(_theParent, _theProperties, _theData, bounds), null, ' '));

		_theTools.addElement(new MenuItemInfo("Chart Features", null, "Create Grid",
				new CreateGrid(_theParent, _theProperties, _theData, bounds), null, ' '));

		_theTools.addElement(new MenuItemInfo("Chart Features", null, "Create VPF Layers",
				new CreateVPFLayers(_theParent, _theProperties, _theData, bounds), null, ' '));

		_theTools.addElement(new MenuItemInfo("Chart Features", null, "Create World Coastline",
				new CreateCoast(_theParent, _theProperties, _theData, bounds), null, ' '));

		_theTools.addElement(new MenuItemInfo("Chart Features", null, "Create ETOPO Bathy",
				new CreateTOPO(_theParent, _theProperties, _theData, bounds), null, ' '));
	...
	// {
	// // import this data
	// ASSETReaderWriter.importThis(_theScenario, thisFile.getName(), new
	// java.io.FileInputStream(thisFile));
	// }
	...
	/**
	 * <code>buildTheInterface</code> puts the bits together, after the gui has been
	 * built
	 */
	void buildTheInterface() {
		...
		while (enumer.hasMoreElements()) {
			final MenuItemInfo thisItem = enumer.nextElement();
		...
		// we also want to try to give these properties to the layers object, so that it
		// can be edited
	...
	void close() {
		...
		while (enumer.hasMoreElements()) {
			final MenuItemInfo mn = enumer.nextElement();
			mn.close();
		}
