...
public class BMRObserver extends RecordStatusToFileObserverType {
	...
	static public class BMRObserverInfo extends EditorType {
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Directory", "The directory to place Debrief data-files"),
	 ...
	 * @param formatHelpers
	 */
	public BMRObserver(final String directoryName, final String fileName, final TargetType subjectToTrack,
			final String observerName, final boolean isActive, final String subjectSensor) {
		super(directoryName, fileName, true, true, true, subjectToTrack, observerName, isActive);
	...
	protected void writeTheseDetectionDetails(final ParticipantType pt, final DetectionList detections,
		...
		while (iter.hasNext()) {
			...
			if ((_subjectSensor == null || _subjectSensor.equals(sensorName)) && Math.random() <= 0.1) {
				...
				final String FCS = "SR" + trimmedTgt + " AAAA 1936 BBB (Lost) B-" + (int) brg + " R-"
						+ ONE_DP.format(rng) + "kyds C-321 S-6kts  Classified AAAAAA BBBBBB AAAAAA";
				writeToFile(dtg, FCS);
			}
		}
	}
	...
	@Override
	protected void writeThesePositionDetails(final WorldLocation loc, final Status stat, final ParticipantType pt,
			final long newTime) {
		final double thisNum = Math.random();

		if (thisNum > 0.8) {
			writeToFile(newTime,
					"CSD Crse:" + (int) stat.getCourse() + " Speed:" + ONE_DP.format(stat.getSpeed().getValue()));
		}
		if (thisNum > 0.95) {
			writeToFile(newTime, "Weather. Speed:" + (int) stat.getSpeed().getValueIn(WorldSpeed.M_sec) + " Speed:"
					+ ONE_DP.format(stat.getCourse()));
		}
