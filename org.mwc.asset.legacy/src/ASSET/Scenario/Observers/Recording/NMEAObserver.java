...
public class NMEAObserver extends RecordStatusToFileObserverType {
	 ...
	 * @param formatHelpers
	 */
	public NMEAObserver(final String directoryName, final String fileName, final boolean recordDetections,
		...
		super(directoryName, fileName, recordDetections, false, recordPositions, subjectToTrack, observerName,
				isActive);
	...
	private String writeAISContact(final WorldLocation loc, final int target) {
		...
		final String res = "$POSL,AIS," + MMSI + "," + pack(loc.getLat(), true) + "," + pack(loc.getLong(), false)
				+ ",a,b,c,a,b,AIS1,c,d" + LB;
		return res;
	}
	...
	private String writeDTG(final long newTime) {
		final String res = "$POSL,DZA," + date.format(new Date(newTime)) + "," + time.format(new Date(newTime))
				+ ",a,b,c,d" + LB;
		return res;
	}
	...
	private String writeStatus(final WorldLocation loc) {
		...
		final String res = "$POSL,POS,GPS," + pack(dLat, true) + "," + pack(dLong, false) + ",a,b,c,d" + LB;
		return res;
	}
	...
	protected void writeThesePositionDetails(final WorldLocation loc, final Status stat, final ParticipantType pt,
		...
		// speed
		if (Math.random() <= 0.5) {
			final String res = writeSpeed(stat.getSpeed().getValueIn(WorldSpeed.M_sec));
			writeToFile(res);
		}
