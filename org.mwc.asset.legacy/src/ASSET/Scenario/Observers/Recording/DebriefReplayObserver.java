...
public class DebriefReplayObserver extends RecordStatusToFileObserverType
{
	...
	/**
	 * keep our own little register of symbols for participant types - the method
	 * to retreive the symbol for a participant type is a compleicated one
	 */
			...
			long newTime)
	{
		...
		String hisSymbol = (String) _mySymbolRegister.get(pt.getCategory()
				.getType());
		if (hisSymbol == null)
		{
	...
	/**
	 * note that we only output detections once some positions have been written
	 * to file, since Debrief likes to know about tracks before loading sensor
	 * data
	 * 
	 * @param loc
	 * @param dtg
			...
			WorldDistance range, String sensor_name, String label)
	{
		...
		String msg = ";SENSOR: " + dateStr + " " + wrapName(hostName) + " " + col
