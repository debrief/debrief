...
public class DebriefReplayObserver extends RecordStatusToFileObserverType {
	...
	private static void outputThisLocation(final WorldLocation loc, final java.io.OutputStreamWriter os,
			final String message) {
		final String locStr = MWC.Utilities.TextFormatting.DebriefFormatLocation.toString(loc);
		final String msg = ";TEXT: AA " + locStr + " " + message + System.getProperty("line.separator");
		try {
			os.write(msg);
	...
	static public String writeDetailsToBuffer(final MWC.GenericData.WorldLocation loc,
			final ASSET.Participants.Status stat, final NetworkParticipant pt, final long newTime,
	...
	/**
	 * list of last messages for each participant - only required if we're skipping
	 ...
	 * @param formatHelpers
	 */
	public DebriefReplayObserver(final String directoryName, final String fileName, final boolean recordDetections,
			final boolean recordDecisions, final boolean recordPositions, final TargetType subjectToTrack,
			final String observerName, final boolean isActive, final List<String> formatHelpers) {
		super(directoryName, fileName, recordDetections, recordDecisions, recordPositions, subjectToTrack, observerName,
				isActive);
	...
	public DebriefReplayObserver(final String directoryName, final String fileName, final boolean recordDetections,
		...
		this(directoryName, fileName, recordDetections, false, true, null, observerName, isActive, null);
	}
	...
	public void outputThisArea(final WorldArea area) {
		final String topLeft = MWC.Utilities.TextFormatting.DebriefFormatLocation.toString(area.getTopLeft());
		final String botRight = MWC.Utilities.TextFormatting.DebriefFormatLocation.toString(area.getBottomRight());
	...
	/**
	 * note that we only output detections once some positions have been written to
	 * file, since Debrief likes to know about tracks before loading sensor data
	 *
	 * @param loc
	 * @param dtg
	 * @param hostName
	 ...
	 * @param label
	 */
	private void outputThisDetection(final WorldLocation loc, final long dtg, final String hostName,
			final Category hostCategory, final Float bearing, final WorldDistance range, final String sensor_name,
			final String label) {
		...
		final String dateStr = MWC.Utilities.TextFormatting.DebriefFormatDateTime.toString(dtg);
		...
		final String msg = ";SENSOR: " + dateStr + " " + wrapName(hostName) + " " + col + " " + locStr + " " + brgTxt
				+ " " + rangeTxt + " " + sensor_name + " " + label + System.getProperty("line.separator");
	 ...
	 * @param label
	 */
	private void outputThisDetection2(final WorldLocation loc, final long dtg, final String hostName,
			final Category hostCategory, final Float bearing, final Float ambigBearing, final WorldDistance range,
		...
		if (bearing == null) {
			...
			brgTxt = MWC.Utilities.TextFormatting.GeneralFormat.formatTwoDecimalPlaces(bearing.floatValue());
		}
		...
		if (ambigBearing == null) {
			...
			ambigTxt = MWC.Utilities.TextFormatting.GeneralFormat.formatTwoDecimalPlaces(ambigBearing.floatValue());
		}
		...
		if (freq == null) {
			...
			freqTxt = MWC.Utilities.TextFormatting.GeneralFormat.formatThreeDecimalPlaces(freq.floatValue());
		}
		...
		if ("NULL".equals(ambigTxt)) {
			...
			// ;SENSOR3: YYMMDD HHMMSS.SSS AAAAAA @@ DD MM SS.SS H DDD MM SS.SS H BBB.B
			// CCC.C
			...
			msg = ";SENSOR3: " + dateStr + " " + wrapName(hostName) + " " + col + " " + locStr + " " + brgTxt
					+ " NULL  " + freqTxt + " NULL " + rangeTxt + " " + sensor_name + " " + label
					+ System.getProperty("line.separator");
		} else {
			// use 2
			msg = ";SENSOR2: " + dateStr + " " + wrapName(hostName) + " " + col + " " + locStr + " " + brgTxt + " "
					+ ambigTxt + " " + freqTxt + " " + rangeTxt + " " + sensor_name + " " + label
					+ System.getProperty("line.separator");
		}
	...
	protected void writeTheseDetectionDetails(final ParticipantType pt, final DetectionList detections,
		...
		while (iter.hasNext()) {
			...
			final SensorType sensor = pt.getSensorFit().getSensorWithId(de.getSensor());
			...
			if (_subjectSensor == null || _subjectSensor.equals(sensorName)) {
				...
				if (de.getFreq() != null || de.isAmbiguous()) {
					...
					outputThisDetection2(de.getSensorLocation(), de.getTime(), pt.getName(), pt.getCategory(),
							de.getBearing(), ambig, de.getRange(), safeSensorName, de.toString(), de.getFreq());

				} else {
					outputThisDetection(de.getSensorLocation(), de.getTime(), pt.getName(), pt.getCategory(),
							de.getBearing(), de.getRange(), safeSensorName, de.toString());
				}
			}
		}
	}
	...
	public void writeThesePositionDetails(final MWC.GenericData.WorldLocation loc, final ASSET.Participants.Status stat,
		...
		final String res = writeDetailsToBuffer(loc, stat, pt, newTime, _targetFolder);
		writeToFile(res);
	}
	...
	protected void writeThisDecisionDetail(final NetworkParticipant pt, final String activity, final long dtg) {
		...
		// Templates.
		final String msg = ";NARRATIVE2: " + MWC.Utilities.TextFormatting.DebriefFormatDateTime.toString(dtg) + " "
				+ wrapName(pt.getName()) + " DECISION " + activity + System.getProperty("line.separator");
