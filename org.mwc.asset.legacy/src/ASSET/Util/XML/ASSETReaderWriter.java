...
public class ASSETReaderWriter extends MWC.Utilities.ReaderWriter.XML.MWCXMLReaderWriter {
	 ...
	 */
	static public void exportThis(final ScenarioType scenario, final Layer theDecorations,
		...
		try {
			final DOMImplementation domI = javax.xml.parsers.DocumentBuilderFactory.newInstance().newDocumentBuilder()
					.getDOMImplementation();
			final Document doc = domI.createDocument(null, null, null);
			...
			// catch (IOException e)
			// {
			// MWC.Utilities.Errors.Trace.trace("Debrief failed to save this file correctly.
			// Please
			// investigate the trace file", true);
	...
	static public ASSET.ParticipantType importParticipant(final String fName, final java.io.InputStream is) {
	...
	static public void importThis(final ASSET.Models.Decision.BehaviourList theChain, final String fName,
	 ...
	 */
	static public void importThis(final ASSET.Scenario.CoreScenario theScenario, final Layers theLayers,
	 ...
	 * handle the import of XML data, creating a new session for it
	 */
	static public void importThis(final ASSET.Scenario.CoreScenario theScenario, final String fName,
			final java.io.InputStream is) {
		// create the new handler
		final ASSETReaderWriter xr = new ASSETReaderWriter();
		...
		if (backdropHolder.size() > 0) {
			final Layer firstLayer = backdropHolder.elementAt(0);
	...
	static public void importThis(final SensorList theSensorFit, final String fName, final java.io.InputStream is) {
		// create the new handler
		final ASSETReaderWriter xr = new ASSETReaderWriter();
	...
	static public ResultsContainer importThisControlFile(final String fName, final java.io.InputStream is) {
		...
		final MWCXMLReader handler = new ScenarioControllerHandler() {
			@Override
			public void setResults(final ResultsContainer results) {
				resultsHolder.add(results);
			}
		};

		// import the datafile into this set of layers
		final InputSource inputStream = new InputSource(is);
