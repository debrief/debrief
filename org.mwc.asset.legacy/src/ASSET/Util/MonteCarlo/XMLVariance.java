...
public final class XMLVariance {
	...
	public static class IllegalExpressionException extends Exception {
		 ...
		 * @param cause   a parent exception - if applicable
		 */
		public IllegalExpressionException(final String pattern, final Throwable cause) {
	...
	public static class MatchingException extends Exception {
		 ...
		 * @param cause   a parent exception - if applicable
		 */
		public MatchingException(final String pattern, final Throwable cause) {
	...
	public static class NamespaceContextProvider implements NamespaceContext {
		...
		public static XPathExpression createPath(final String theXPath) throws XPathExpressionException {
			...
			final NamespaceContextProvider myResolver = new NamespaceContextProvider(NAMESPACE_PREFIX,
					"http://www.mwc.org/asset");
			xp.setNamespaceContext(myResolver);

			// note, we've got to stick the as bit in front of any matching xpath
			// items
			final String myPath = myResolver.insertPrefixesTo(theXPath);
		...
		private NamespaceContextProvider(final String prefix, final String URI) {
			boundPrefix = prefix;
			boundURI = URI;
		}
		...
		/**
		 * we've got a no-namespace string here. We need to insert namespaces for the
		 * xpath resolver to match the items. stick them in after every / symbol
		 *
		 * @param theXPath the xpath regex received
		...
		private String insertPrefixesTo(final String theXPath) {
			...
			final String[] items = theRes.split("/");
			for (int i = 0; i < items.length; i++) {
				final String thisA = items[i];
				if (thisA.length() > 2) {
	...
	private static WorldLocation extractLongLocation(final Element topL) throws ParseException {
		final int latDeg = Integer.parseInt(topL.getAttribute("LatDeg"));
		final int longDeg = Integer.parseInt(topL.getAttribute("LongDeg"));
		final int latMin = Integer.parseInt(topL.getAttribute("LatMin"));
		final int longMin = Integer.parseInt(topL.getAttribute("LongMin"));

		final double latSec = MWCXMLReader.readThisDouble(topL.getAttribute("LatSec"));
		final double longSec = MWCXMLReader.readThisDouble(topL.getAttribute("LongSec"));

		final char latHemi = topL.getAttribute("LatHem").charAt(0);
		final char longHemi = topL.getAttribute("LongHem").charAt(0);

		final WorldLocation thisCorner = new WorldLocation(latDeg, latMin, latSec, latHemi, longDeg, longMin, longSec,
				longHemi, 0);

		return thisCorner;
	}

	private static WorldLocation extractRelativeLocation(final Element topL) throws ParseException {
		...
		for (int i = 0; i < thisDirection.getLength(); i++) {
			// first the top left
			final Element North = (Element) thisDirection.item(i);
			final String units = North.getAttribute(UNITS);
			final int theUnits = WorldDistance.getUnitIndexFor(units);
			final String val = North.getAttribute(VALUE);
		...
		final WorldLocation res = new WorldLocation.LocalLocation(north, east, 0);

		return res;
	}

	private static WorldLocation extractShortLocation(final Element topL) throws ParseException {
		final String llat = topL.getAttribute("Lat");
		final String llong = topL.getAttribute("Long");

		final WorldLocation thisCorner = new WorldLocation(MWCXMLReader.readThisDouble(llat),
	 ...
	 * @param numberModel number model to use
	 * @return new location
	 */
	public static WorldLocation generateRandomLocationInArea(final WorldArea area, final int numberModel) {
		WorldLocation res = null;
		final double latV = RandomGenerator.generateRandomNumber(area.getBottomRight().getLat(),
				area.getTopLeft().getLat(), numberModel);
		final double longV = RandomGenerator.generateRandomNumber(area.getTopLeft().getLong(),
				area.getBottomRight().getLong(), numberModel);
	 ...
	 *
	 * @param modelTxt the string to examine
	 * @return the model described
	 */
	public static int getModelFromString(final String modelTxt) {
		int res;
	 ...
	 *
	 * @param myElement the element to read the data from
	...
	public static WorldArea readInAreaFromXML(final Element myElement) throws ParseException {
		...
		final NodeList theShortLocation = myElement.getElementsByTagName("shortLocation");

		final int numShortLocation = theShortLocation.getLength();

		for (int i = 0; i < numShortLocation; i++) {
			// first the top left
			final Element topL = (Element) theShortLocation.item(i);

			// and extract this corner
			final WorldLocation thisCorner = extractShortLocation(topL);
		...
		for (int i = 0; i < numLongLocation; i++) {
			// first the top left
			final Element topL = (Element) theLongLocation.item(i);

			// and extract this corner
			final WorldLocation thisCorner = extractLongLocation(topL);
		...
		for (int i = 0; i < numLongLocation; i++) {
			...
			// and extract this corner
			final WorldLocation thisCorner = extractRelativeLocation(topL);
	 ...
	 * @throws ParseException
	 */
	public static WorldLocation readInLocationFromXML(Element myElement, final String locationName)
			throws ParseException {
		WorldLocation theRes = null;
		final NodeList theLocation = myElement.getElementsByTagName(locationName);
		...
		if (theLocation.getLength() > 0) {
			...
			if (theRes == null) {
				// try for the long location
				final NodeList theLongLocation = myElement.getElementsByTagName("longLocation");

				final int numLongLocation = theLongLocation.getLength();
			...
			if (theRes == null) {
				// try for the long location
				final NodeList theLongLocation = myElement.getElementsByTagName("relativeLocation");
	...
	public static int readRandomNumberModel(final Element element) {
		final String modelTxt = element.getAttribute("RandomModel");
		int res;
		// ok, get it
	...
	public final boolean equals(final Object arg0) {
		...
		final XMLVariance other = (XMLVariance) arg0;
		boolean res = true;
		...
		for (int i = 0; i < items.length; i++) {
			...
			if (thisA.length() > 2) {
				if (Character.isLetter(thisA.charAt(0))) {
					...
					// so, we prepend the search with a slash, and include that slash
					// with the replacement string
					theRes = theRes.replace("/" + thisA, "/" + NAMESPACE_PREFIX + ":" + thisA);
				}
			}
		}
	...
	public final String permutate(final String parentXPath, final Document document)
		...
		List<?> results = null;
		try {
			final String myPath = insertPrefixesTo(theXPath);
			final DOMXPath expression = new org.jaxen.dom.DOMXPath(myPath);
		...
		if (results != null) {
			if (results.size() > 0) {
				...
				while (iter.hasNext()) {
					final Element ele = (Element) iter.next();
