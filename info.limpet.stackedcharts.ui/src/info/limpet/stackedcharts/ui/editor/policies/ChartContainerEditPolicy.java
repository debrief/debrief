...
public class ChartContainerEditPolicy extends ContainerEditPolicy implements EditPolicy {
	...
	protected Command getAddCommand(final GroupRequest request) {
		...
		if (toAdd.size() > 0) {
			...
			if (first instanceof ChartEditPart) {
				...
				final List<Chart> charts = ((ChartsPanelEditPart) getHost().getParent()).getModel().getCharts();
				for (final Object o : toAdd) {
					if (o instanceof ChartEditPart) {
						...
						if (newindex < 0)
							newindex = 0;
						res.add(new MoveChartCommand(charts, chartEditPart.getModel(), newindex));
					}
				}
			}
		}
