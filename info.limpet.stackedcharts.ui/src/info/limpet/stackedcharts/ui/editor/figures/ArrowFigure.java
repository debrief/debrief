...
public class ArrowFigure extends Figure {
	...
	public ArrowFigure(final boolean horizontal) {
		this.setHorizontal(horizontal);
		final Color color = Display.getDefault().getSystemColor(SWT.COLOR_DARK_GRAY);
	...
	protected void paintFigure(final Graphics graphics) {
		...
		final Point top = clientArea.getTop();
		final Point right = clientArea.getRight();
		...
		if (isHorizontal()) {
			...
			points.addPoint(right.getCopy().translate(-ARROW_HEAD_LENGTH, -ARROW_HEAD_HALF_WIDTH));
			points.addPoint(right.getCopy().translate(-ARROW_HEAD_LENGTH, ARROW_HEAD_HALF_WIDTH));
		} else {
			graphics.drawLine(clientArea.getBottom(), top);
			points.addPoint(top);
			points.addPoint(top.getCopy().translate(ARROW_HEAD_HALF_WIDTH, ARROW_HEAD_LENGTH));
			points.addPoint(top.getCopy().translate(-ARROW_HEAD_HALF_WIDTH, ARROW_HEAD_LENGTH));
		}
