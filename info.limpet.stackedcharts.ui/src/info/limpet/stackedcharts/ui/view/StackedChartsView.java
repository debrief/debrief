...
public class StackedChartsView extends ViewPart
		implements ITabbedPropertySheetPageContributor, ISelectionProvider, DisposeListener, IStackedTimeListener {

	/**
	 * interface for external objects that are able to supply a date and resond to a
	 * new date
	 *
	 * @author ian
	 *
	...
	/**
	 * we don't want to process all new-time events, only the most recent one. So,
	 * take a note of the most recent one
	 */
	...
	protected void connectFileDropSupport(final Control compoent) {
		final DropTarget target = new DropTarget(compoent, DND.DROP_MOVE | DND.DROP_COPY | DND.DROP_DEFAULT);
				...
				if (fileTransfer.isSupportedType(event.currentDataType)) {
					...
					if (files.length == 1 && files[0].endsWith("stackedcharts")) {
						...
						final Resource resource = new ResourceSetImpl()
								.createResource(URI.createURI(file.toURI().toString()));
						try {
	...
	/**
	 * Creates an Chart Set with a single chart so that user would be able to drop
	 * datasets in it.
	 *
	 * @return
	 */
	private ChartSet createBlankModel() {
		final ChartSet chartSet = StackedchartsFactory.eINSTANCE.createChartSet();
		chartSet.getCharts().add(StackedchartsFactory.eINSTANCE.createChart());
		final IndependentAxis independentAxis = StackedchartsFactory.eINSTANCE.createIndependentAxis();
		independentAxis.setAxisType(StackedchartsFactory.eINSTANCE.createDateAxis());
	...
	protected Control createEditView() {
		...
		chartEditor.getViewer().addSelectionChangedListener(new ISelectionChangedListener() {
			@Override
	...
	public void createPartControl(final Composite parent) {
		...
		final String secondaryId = ((IViewSite) getSite()).getSecondaryId();
		if (secondaryId != null) {
			...
			final ChartSet blankModel = createBlankModel();
		...
		final boolean IS_LINUX_OS = System.getProperty("os.name").toLowerCase().indexOf("nux") >= 0;
		final Image[] compImage = new Image[2]; // stackedPane comp count
		parent.addDisposeListener(new DisposeListener() {

			@Override
			public void widgetDisposed(final DisposeEvent e) {
				for (final Image img : compImage) {
					if (img != null) {
						img.dispose();
					}
				}
		...
		transitionManager = new TransitionManager(transitionable = new Transitionable() {
			...
			public double getDirection(final int toIndex, final int fromIndex) {
				return toIndex == CHART_VIEW ? Transition.DIR_RIGHT : Transition.DIR_LEFT;
			}
			...
			@Override
			public void startTransition(final int fromIndex, final int toIndex, final double direction) {
				if (IS_LINUX_OS) {
					final Control from = transitionable.getControl(fromIndex);
					final Rectangle fromSize = from.getBounds();
					final Image imgFrom = new Image(from.getDisplay(), fromSize.width, fromSize.height);
					final GC gcfrom = new GC(from);
					from.update();
	...
	protected void fillLocalToolBar(final IToolBarManager manager) {
		final String actionText = stackedPane.getActiveControlKey() == CHART_VIEW ? "Edit" : "View";
		final Action toggleViewModeAction = new Action(actionText, SWT.TOGGLE) {
			@Override
			public void run() {
		...
		showTime.setImageDescriptor(AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/clock.png"));
		...
		showMarker.setImageDescriptor(
				AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/labels.png"));
		...
		final Action export = new Action("Export image to clipboard", SWT.PUSH) {
			...
			private void toWMF() {
				try {
					...
					clpbrd.setContents(new DrawableWMFTransfer(_chartComposite.getChart(), _chartComposite.getBounds()),
							null);
					MessageDialog.openInformation(Display.getCurrent().getActiveShell(), "Image Export",
		...
		export.setImageDescriptor(
				AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/export_wmf.png"));
		manager.add(export);
		...
		sizeDown.setImageDescriptor(
				AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/decrease.png"));
		...
		sizeUp.setImageDescriptor(
				AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/increase.png"));
		manager.add(sizeUp);
	...
	private void findTimeController() {
		...
		if (registry != null) {
			...
			final IExtensionPoint point = Platform.getExtensionRegistry().getExtensionPoint(Activator.PLUGIN_ID,
					TIME_PROVIDER_ID);
			...
			for (int i = 0; i < extensions.length; i++) {
				...
				for (final IConfigurationElement extension : confE) {
					try {
						final IStackedTimeProvider provider = (IStackedTimeProvider) extension
	...
	protected void fireSelectionChnaged() {
		...
		for (final ISelectionChangedListener listener : new ArrayList<>(selectionListeners)) {
	...
	public ISelection getSelection() {
		...
		// if chart view need to provide selection info via properties view, change
		// empty selection to
	...
	public void setModel(final ChartSet charts, final int mode) {
		this.charts = charts;
		...
		_chartComposite = new ChartComposite(chartHolder, SWT.NONE, jFreeChart, 400, 600, 300, 200, 1800, 1800, true,
