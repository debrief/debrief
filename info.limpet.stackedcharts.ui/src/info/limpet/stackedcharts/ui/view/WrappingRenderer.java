...
public class WrappingRenderer extends XYLineAndShapeRenderer {
	...
	*/
	...
	/**
	 *
	 * @param min       the minimum wrapping value
	 * @param max       the maximum wrapping value
	 * @param midOrigin whether the axis range should have zero at origin
	 */
	...
	protected void drawPrimaryLine(final XYItemRendererState state, final Graphics2D g2, final XYPlot plot,
		...
		if (overflowCondition.isOverflow(y0, x0, y1, x1)) {
			...
			if (overflowAtMax) {
				final PolynomialSplineFunction psf = interpolator.interpolate(new double[] { y0, y1 + (max - min) },
						new double[] { x0, x1 });
				final double xmid = psf.value(max);
				...
				final PolynomialSplineFunction psf = interpolator.interpolate(new double[] { y1 - (max - min), y0 },
						new double[] { x1, x0 });
				final double xmid = psf.value(min);
	...
	protected void drawPrimaryLineAsPath(final XYItemRendererState state, final Graphics2D g2, final XYPlot plot,
		...
		try {
			...
			if (overflowCondition.isOverflow(y0, x0, y1, x1)) {
				final boolean overflowAtMax = y1 < y0;
				if (overflowAtMax) {
					final PolynomialSplineFunction psf = interpolator.interpolate(new double[] { y0, y1 + range },
							new double[] { x0, x1 });
					...
					final PolynomialSplineFunction psf = interpolator.interpolate(new double[] { y1 - range, y0 },
							new double[] { x1, x0 });
					final double xmid = psf.value(min);
