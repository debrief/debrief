...
public class ChartBuilder {
	 ...
	 *
	 */
	private static class AngularUnitAxis extends CoreAngularAxis implements FancyFormattedAxis {
		...
		public AngularUnitAxis(final String axisName, final double min, final double max) {
	...
	private static class CoreAngularAxis extends NumberAxis {
		...
		public CoreAngularAxis(final String axisName, final double min, final double max) {
			super(axisName);
		...
		/**
		 * when the plot is resized, we want to return to the default auto range, not
		 * the freshly calculated one.
		 */
		@Override
		protected final void autoAdjustRange() {
			final Plot plot = getPlot();
			if (plot != null && plot instanceof ValueAxisPlot) {
				final Range r = getDefaultAutoRange();
	 ...
	 *
	 */
	private static class RedGreenAngularUnitAxis extends CoreAngularAxis implements FancyFormattedAxis {
		private class RedGreenAnglularWrappingFormatter extends NumberFormat implements ColorProvider {
			...
			@Override
			public Color getColorFor(final double val) {
				final Color res;
				if (val < 0) {
					res = Color.red;
				} else if (val > 0) {
		...
		public RedGreenAngularUnitAxis(final String axisName, final double min, final double max) {
			super(axisName, min, max);
	...
	/**
	 *
	 * @param subplot           target plot for annotation
	 * @param annotations       annotation list to be added to plot eg: Marker,Zone
	 * @param isRangeAnnotation is annotation added to Range or Domain
	 */

	private static void addAnnotationToPlot(final XYPlot subplot, final List<AbstractAnnotation> annotations,
		...
		for (final AbstractAnnotation annotation : annotations) {
			...
			if (annotation instanceof info.limpet.stackedcharts.model.Marker) {
				...
				for (final Datum datum : datums) {
					...
					if (colorToUse != null) {
						final Color transColor = new Color(colorToUse.getRed(), colorToUse.getGreen(),
	...
	/**
	 *
	 * @param helper      Helper object to map between axis types
	 * @param datasets    list of datasets to add to this axis
	 * @param collection  XYDataset need to be fill with Series
	 * @param renderer    axis renderer
	 * @param seriesIndex counter for current series being assigned
	 *
	 *                    build axis dataset via adding series to Series & config
	 *                    renderer for styles
	 */
			...
			if (styling != null) {
				...
				if (lineType != null) {
					...
					case DOTTED:
						...
						stroke = new BasicStroke(thickness, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 0, pattern,
								0);
						renderer.setSeriesStroke(seriesIndex, stroke);
						break;
					case DASHED:
				...
				if (marker != null) {
					...
					case NONE:
						...
						break;
					case SQUARE:
						renderer.setSeriesShape(seriesIndex, new Rectangle2D.Double(0, 0, size, size));
						break;
					case CIRCLE:
	...
	public static JFreeChart build(final Chart chart) {
		...
		final int aCnt = subplot.getRangeAxisCount();
		for (int ctr = 0; ctr < aCnt; ctr++) {
			final ValueAxis axis = subplot.getRangeAxis(ctr);
			if (axis != null) {
				axis.setLabel(null);
			}
		}
		...
		plot.setOrientation(chart.getParent().getOrientation() == Orientation.VERTICAL ? PlotOrientation.VERTICAL
	 ...
	 * @return
	 */
	public static JFreeChart build(final ChartSet chartsSet, final ControllableDate controllableDate) {
		...
		if (controllableDate != null) {
			// ok, initialise it
			final Date theTime = controllableDate.getDate();
			if (theTime != null) {
		...
		plot.setGap(5.0);
		plot.setOrientation(chartsSet.getOrientation() == Orientation.VERTICAL ? PlotOrientation.VERTICAL
				: PlotOrientation.HORIZONTAL);

		return new JFreeChart(plot);
	}

	protected static XYPlot createChart(final IndependentAxis sharedAxisModel, final Chart chart,
			final ChartHelper helper) {
		...
		if (sharedAxisModel != null) {
			...
			final EList<SelectiveAnnotation> selectiveAnnotations = sharedAxisModel.getAnnotations();
			...
			for (final SelectiveAnnotation selectiveAnnotation : selectiveAnnotations) {
				...
				// chart
				if (appearsIn == null || appearsIn.isEmpty() || appearsIn.contains(chart)) {
	 ...
	 */
	private static void createDependentAxis(final XYPlot subplot, final int indexAxis,
			final DependentAxis dependentAxis, final ChartHelper axeshelper) {
		...
		if (axisType instanceof info.limpet.stackedcharts.model.NumberAxis) {
			final info.limpet.stackedcharts.model.NumberAxis number = (info.limpet.stackedcharts.model.NumberAxis) axisType;
		...
		if (axisType instanceof AngleAxis) {
			final AngleAxis angle = (AngleAxis) axisType;
			...
			if (angle.isRedGreen()) {
				...
				final IntervalMarker greenM = new IntervalMarker(0, max);
				...
				final IntervalMarker im = new IntervalMarker(min, 0);
		...
		addDatasetToAxis(axeshelper, dependentAxis.getDatasets(), collection, renderer, indexSeries);

		final EList<AbstractAnnotation> annotations = dependentAxis.getAnnotations();
	...
	protected static void populateCollection(final ChartHelper helper, final Dataset dataset, final Series series) {
		helper.clear(series);
