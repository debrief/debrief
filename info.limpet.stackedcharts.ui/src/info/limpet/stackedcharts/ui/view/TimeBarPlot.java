...
/**
 * provide new version of domain plot, that is able to render a time bar on the
 * independent axis
 *
 * @author ian
 *
...
public class TimeBarPlot extends CombinedDomainXYPlot {
	/**
	 * strategy for classes that can provide an interpolated value for JFreeChart
	 * datasets
	 *
	 * @author ian
	 *
	...
	protected static interface AxisHelper {
		Double getValueAt(Dataset dataset, int index, double time, NumberAxis rangeA);
	}
	...
	protected static class CacheingDateHelper implements AxisHelper {
		...
		@Override
		public Double getValueAt(final Dataset dataset, final int seriesIndex, final double tNow,
			...
			final boolean outOfRange = pending || set.previousX > tNow || set.nextX < tNow;
			if (pending || outOfRange) {
				final int len = items.size();

				// try the index
				final long indexVal = items.get(set.index).getPeriod().getFirstMillisecond();
			...
			if (set.previousY != null && set.nextY != null) {
				...
				final double proportion = (tNow - set.previousX) / (set.nextX - set.previousX);
	...
	protected static class DateHelper implements AxisHelper {
		...
		@Override
		public Double getValueAt(final Dataset dataset, final int index, final double tNow, final NumberAxis rangeA) {
			final TimeSeriesCollection tsc = (TimeSeriesCollection) dataset;
			...
			if (previousY != null && nextY != null) {
				...
				if (rangeObj != null) {
					final double range = rangeObj.getUpperBound() - rangeObj.getLowerBound();
	...
	protected static void paintThisMarker(final Graphics2D g2, final String label, final Color color,
			final float markerX, final float markerY, final boolean vertical, final Rectangle2D thisPlotArea) {
		...
		g2.fill3DRect((int) yPos, (int) (xPos - bounds.getHeight()), 3 + (int) bounds.getWidth(),
	 ...
	 * <P>
	 * XYPlot relies on an XYItemRenderer to draw each item in the plot. This allows
	 * the visual representation of the data to be changed easily.
	 * <P>
	 ...
	 *                   should be drawn.
	 * @param renderInfo Collects chart drawing information (null permitted).
	 */
	@Override
	public final void draw(final Graphics2D g2, final Rectangle2D plotArea, final Point2D anchor, final PlotState state,
		...
		if (_currentTime != null) {
			...
			final boolean vertical = this.getOrientation() == PlotOrientation.VERTICAL;
			...
			if (domainAxis instanceof DateAxis) {
				...
				linePosition = dateAxis.dateToJava2D(new Date(theTime), dataArea, this.getDomainAxisEdge());
			...
			if (_showLine) {
				plotStepperLine(g2, dataArea, vertical, domainAxis, theTime, (int) trimmedLinePosition, clipped);
			}
			...
			if (_showLabels && !clipped) {
				...
				plotStepperMarkers(g2, dataArea, vertical, domainAxis, theTime, renderInfo, (int) trimmedLinePosition,
						_helper);
	 ...
	 * @param clipped
	 */
	protected void plotStepperLine(final Graphics2D g2, final Rectangle2D dataArea, final boolean vertical,
			final Axis axis, final long theTime, final int linePosition, final boolean clipped) {
		...
		if (vertical) {
			// draw the line
			g2.drawLine(linePosition - wid + 2, (int) dataArea.getY(), linePosition - wid + 2,
					(int) dataArea.getY() + (int) dataArea.getHeight());
		} else {
			// draw the line
			g2.drawLine((int) dataArea.getX() + 1, linePosition - 1,
					(int) dataArea.getX() + (int) dataArea.getWidth() - 1, linePosition - 1);
	...
	protected void plotStepperMarkers(final Graphics2D g2, final Rectangle2D dataArea, final boolean vertical,
			final Axis domainAxis, final long theTime, final PlotRenderingInfo info, final int linePosition,
		...
		final Map<XYItemRenderer, Integer> seriesCounter = new HashMap<XYItemRenderer, Integer>();
		...
		for (final XYPlot plot : plots) {
			...
			final Rectangle2D thisPlotArea = info.getSubplotInfo(plotCounter++).getPlotArea();
			...
			for (int i = 0; i < numC; i++) {
				...
				if (axisHelper != null && dataset != null) {
					...
					for (int j = 0; j < num; j++) {
						...
						final Double interpolated = axisHelper.getValueAt(dataset, j, tNow, rangeA);
						...
						if (interpolated != null) {
							...
							final float markerY = (float) rangeA.valueToJava2D(chartLocation, thisPlotArea,
							...
							final String label;
							final NumberFormat fancyFormat = rangeA.getNumberFormatOverride();
							if (fancyFormat != null) {
							...
							if (fancyFormat != null && fancyFormat instanceof ColorProvider) {
								final ColorProvider cp = (ColorProvider) fancyFormat;
