	...
	public final class RectangleHighlight implements SWTPlotHighlighter
	{
				...
				MWC.GenericData.Watchable watch, boolean isPrimary)
		{
			...
			try
			{
				...
				// just see if we've got sensor data, so we can plot the array centre
					...
					if (tw.getPlotArrayCentre())
					{
						...
						while (enumer.hasMoreElements())
						{
							...
							if (len != null)
							{
								if (len.getValue() != 0)
								{
									WorldLocation centre = tw.getBacktraceTo(fw.getTime(), len,
											sw.getWormInHole());
									...
									thisR = new Rectangle(pt.x - _mySize, pt.y - _mySize,
											_mySize, _mySize);
									_areaCovered.add(thisR);
									thisR = new Rectangle(pt.x + _mySize, pt.y - _mySize,
											_mySize, _mySize);
