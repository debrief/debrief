...
public class NarrativeViewerModel extends KTableDefaultModel {
	...
	private final Column[] myAllColumns;
	private final ArrayList<Column> myVisibleColumns = new ArrayList<Column>();

	private final FixedCellRenderer myHeaderCellRenderer = new FixedCellRenderer(FixedCellRenderer.STYLE_PUSH | SWT.BOLD | TextCellRenderer.INDICATION_FOCUS_ROW) {
		{
			setAlignment(SWTX.ALIGN_HORIZONTAL_CENTER | SWTX.ALIGN_VERTICAL_CENTER);
		}
	};

	private final LinkedList<IEntry> myVisibleRows = new LinkedList<IEntry>();
	private IEntry[] myAllEntries = null;

	private Collection<String> mySourceFilter = null;
	private Collection<String> myTypeFilter = null;
	...
	public void setVisibleColumnVisible(boolean visible) {
		myColumnVisible.setVisible(visible);
		updateColumnsVisibility();
	}

	public void setSourceColumnVisible(boolean visible) {
		myColumnSource.setVisible(visible);
		updateColumnsVisibility();
	}

	public void setTypeColumnVisible(boolean visible) {
		myColumnType.setVisible(visible);
		updateColumnsVisibility();
	}
	...
	private void updateFilters() {
		...
		for (IEntry entry : myAllEntries) {
			if ((mySourceFilter == null || mySourceFilter.contains(entry.getSource())) && (myTypeFilter == null || myTypeFilter.contains(entry.getType()))) {
	...
	private Column getVisibleColumn(int col) {
	...
	public KTableCellRenderer doGetCellRenderer(int col, int row) {
		return row == 0 ? myHeaderCellRenderer : getVisibleColumn(col).getCellRenderer();
	}
	...
	public Object doGetContentAt(int col, int row) {
		...
		return row == 0 ? column.getColumnName() : column.getProperty(myVisibleRows.get(row - 1));
	}
	...
	public void onColumnsResized(GC gc, boolean force) {
		int newEntryCellContentWidth = myColumnSizeCalculator.getColumnWidth(myVisibleColumns.indexOf(myColumnEntry)) - KTABLE_CELL_CONTENT_MARGIN_WIDTH * 2;
		...
		for (int row = 0; row < myVisibleRows.size(); row++) {
			...
			int newRowHeight = gc.textExtent(entry).y + KTABLE_CELL_CONTENT_MARGIN_HEIGHT * 2;
	...
	private static class ColumnVisible extends AbstractColumn {
		...
		@Override
		protected KTableCellRenderer createRenderer() {
			return new CheckableCellRenderer(CheckableCellRenderer.INDICATION_CLICKED | CheckableCellRenderer.INDICATION_FOCUS);
		}
	}
	...
	private static class ColumnEntry extends AbstractTextColumn {
		...
		@Override
		protected KTableCellRenderer createRenderer() {
			myWrappingRenderer = new TextCellRenderer(TextCellRenderer.INDICATION_FOCUS_ROW);
			updateWrapping();
		...
		private void updateWrapping() {
			...
			int alignment = SWTX.ALIGN_HORIZONTAL_LEFT | SWTX.ALIGN_VERTICAL_TOP;
			if (myIsWrapping) {
				alignment |= SWTX.WRAP;
			}
