...
public final class ImportFix implements PlainLineImporter
{
  ...
  public final Object readThisLine(String theLine)
  {
    ...
    // trouble - the track name may have been quoted, in which case we will pull in the remaining fields aswell
    ...
    theCourse = MWC.Algorithms.Conversions.Degs2Rads(Double.valueOf(st.nextToken()).doubleValue());
  ...
  public final String exportThis(MWC.GUI.Plottable theWrapper)
  {
    ...
    line += " " + MWC.Utilities.TextFormatting.GeneralFormat.formatOneDecimalPlace(Conversions.Rads2Degs(fix.getCourse()));
    line += " " + MWC.Utilities.TextFormatting.GeneralFormat.formatOneDecimalPlace(Conversions.Yps2Kts(fix.getSpeed()));
  ...
  /**
   * when importing the track name, it may turn out to be quoted, in which case we
   * consume the remaining tokens until we get another quote
   *
   * @param st the tokenised stream to read the name from
  ...
  static public String checkForQuotedTrackName(StringTokenizer st)
  {
    ...
    if (quoteIndex >= 0)
    {
      // aah, but, we may have just read in all of the item. just check if the token contains
      // both speech marks...
  ...
  static public class testImport extends junit.framework.TestCase
  {
    ...
    {
      String iLine = "700101 000000 CARPET   @C   22 10 53.54 N 21 45 14.20 W 239.9   2.0      0 ";
      ImportFix iff = new ImportFix();
      ReplayFix res = (ReplayFix) iff.readThisLine(iLine);
