...
public final class DebriefXMLReaderWriter extends MWC.Utilities.ReaderWriter.XML.MWCXMLReaderWriter
{
   ...
   */
  public DebriefXMLReaderWriter(final Debrief.GUI.Frames.Application theApplication)
  {
                               ...
                               final MWC.GUI.Layers theData)
  {
    ...
    else
    {
      ...
      // now, occasionally the user will try to load a complete plot file to an existing session.
      // Debrief will have trouble with this, since we can only add a "layers" object to an existing
      // session
      // ==
      // try to trap this occurence by trimming the file to just the contents of the "Layers" object
      // and importing that.
  ...
  // Don't bother with this way of testing.  It relies on opening up a Debrief session & where it compares output file with reference
  // version it's too sensitive.

  //    static public final class testImport extends junit.framework.TestCase
  //    {
  //
  //      // todo put in test to handle when a plot-file is dragged into existing plot
  //
  ...
  //
  //        Debrief.GUI.Frames.Swing.SwingApplication application = new Debrief.GUI.Frames.Swing.SwingApplication();
  //
  //        // add the XML importer
  //        MWC.Utilities.ReaderWriter.ImportManager.addImporter(new Debrief.ReaderWriter.XML.DebriefXMLReaderWriter(application, null));
  //
  //        // get our thread to import this
  //        MWC.Utilities.ReaderWriter.ImportManager.BaseImportCaller reader =
  //          new MWC.Utilities.ReaderWriter.ImportManager.BaseImportCaller(_theFiles, null)
  //          {
  //            // handle the completion of each file
  //            public void fileFinished(java.io.File fName, MWC.GUI.Layers newData)
  //            {
  //              //     MWC.Utilities.Errors.Trace.trace("file finished received for:" + fName.getPath());
  //            }
  //
  //            // handle completion of the full import process
  //            public void allFilesFinished(java.io.File[] fNames, MWC.GUI.Layers newData)
  //            {
  //              //     MWC.Utilities.Errors.Trace.trace("XML Reader:all files finished received!");
  //            }
  //          };
  //
  ...
  //
  //        super.assertEquals("tl lat of first layer", area.getTopLeft().getLat(), 12.142258, 0.001);
  //        super.assertEquals("tl long of first layer", area.getTopLeft().getLong(), -11.7342556, 0.00001);
  ...
  //
  //        // area of projection
  //        Debrief.GUI.Views.AnalysisView av = (Debrief.GUI.Views.AnalysisView) sess.getCurrentView();
  //        //   System.out.println("area:" + av.getChart().getCanvas().getProjection().getDataArea());
  //
  //
  //        try
  //        {
