...
public final class TrackWrapper extends MWC.GUI.PlainWrapper implements
	...
	public TrackWrapper() {
		...
		// start off with positions showing (although the default setting for a fix
	...
	public final Watchable[] getNearestTo(final HiResDate srchDTG) {
		...
		if ((srchDTG.equals(lastDTG)) && (lastFix != null)) {
			...
			if ((srchDTG.greaterThanOrEqualTo(theFirst.getTime()))
				...
				// right, we really should be filtering the list according to if the
				// points are visible.
				...
				// right, that's the first points on or before the indicated DTG. Are we
				// meant
				// to be interpolating?
				if (res != null)
					if (getInterpolatePoints()) {
						...
						if (!res.getTime().equals(srchDTG)) {
							...
							// after the first
							final SortedSet otherSet = _thePositions.headSet(nearestFix);
		...
		if (res != null)
			return new Debrief.Tools.Tote.Watchable[] { res };
		else
			return new Debrief.Tools.Tote.Watchable[] {};
	...
	// {
	// _fastData = new com.sun.java.util.collections.TreeSet(new compareFixes());
	//
