/*
 * Desciption: Observer which performs various processing depending on what's happened
 * User: administrator
 * Date: Nov 6, 2001
 * Time: 10:31:15 AM
 */
...
public class DetectionObserver extends WatchParticipantObserver implements
		ASSET.Participants.ParticipantDetectedListener, IAttribute
{
	/***************************************************************
	 * member variables
	 ***************************************************************/
			...
			for (int i = 0; i < detections.size(); i++)
			{
				...
				if (passedDetectionLevelTest)
				{
					...
					if (_targetVessel.matches(thisD.getTargetType()))
					{
						...
						// update our data
						if (_myEditor != null)
							_myEditor.fireChanged(this, "Removed", null, new Integer(
									_numDetected));
					}
				}
			}
	...
	/**
	 * method indicating that we are to stop listening to this participant
	 * instance
	 * 
	 * @param thisPart
	 */
	 ...
	 */
	public static class StopOnProximityDetectionObserver extends
			StopOnDetectionObserver
	{
		 ...
		 *          the type of vessel the monitored vessel is looking for,
		 * @param cutOffRange
		 *          the detection range at which we stop the scenario
		 * @param name
		 * @param detectionLevel
		 *          the level of detection required of for this test
		 * @param isActive
		 *          whether this observer is active
		 */
		public StopOnProximityDetectionObserver(final TargetType watchVessel,
				final TargetType targetVessel, final WorldDistance cutOffRange,
				final String name, final Integer detectionLevel, final boolean isActive)
		{
			super(watchVessel, targetVessel, name, detectionLevel, isActive);
		...
		static public class StopOnProximityDetectionObserverInfo extends EditorType
		{
			 ...
			 */
			public StopOnProximityDetectionObserverInfo(
					final StopOnProximityDetectionObserver data)
			{
		...
		public static class DetObserver3Test extends SupportTesting.EditableTesting
		{
			...
			public Editable getEditable()
			{
				return new StopOnProximityDetectionObserver(null, null, null, null,
						null, true);
			}
		}
