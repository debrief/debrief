...
public class Intercept extends CoreDecision implements java.io.Serializable
{
	 ...
	 */
	public static SimpleDemandedStatus calculateInterceptCourseFor(
			Status ownship, Status target, long time)
	{
			...
			ASSET.Scenario.ScenarioActivityMonitor monitor, final long time)
	{
		...
		{
			DetectionList validDets = detections.getDetectionsOf(_currentTarget,
					_targetType);
		...
		if (targetDetection != null)
		{
			...
			if (res != null)
			{
				...
				if (courseDelta > 2)
				{
					activity = "Switching to new intercept course on target:"
							+ pt.getName();
				}
				else
				{
		...
		public java.beans.PropertyDescriptor[] getPropertyDescriptors()
		{
			try
			{
				final java.beans.PropertyDescriptor[] res =
				{
						...
						prop("SpeedChangeAllowed",
								"whether to allow speed change to intercept target"), };
				return res;
			}
	...
	public static class InterceptTest extends SupportTesting.EditableTesting
	{
		...
		public void testCalc()
		{
			...
			WorldLocation locB = locA.add(new WorldVector(Math.toRadians(240),
					new WorldDistance(2200, WorldDistance.METRES), new WorldDistance(0,
			...
			SimpleDemandedStatus res = calculateInterceptCourseFor(myStat, hisStat,
					1200);
			assertEquals("wrong demanded status calculated", 255.90, res.getCourse(),
					0.1);
		...
		public void testWorking()
		{
			...
			searcher.setCategory(new Category(Category.Force.BLUE,
			...
			ffta.setName("FFTA");
			ffta.setCategory(new Category(Category.Force.RED,
					Category.Environment.SURFACE, Category.Type.FRIGATE));
			...
			UserControl userBehaviour = new UserControl(330, new WorldSpeed(3,
			...
			SimpleEnvironment simpleEnv = new SimpleEnvironment(
					EnvironmentType.CLEAR, 1, EnvironmentType.DAYLIGHT);
			...
			// zero tome.
			assertEquals("we haven't performed a step at zero time", 0, cs.getTime(),
					0);
			...
			assertNull("we shouldn't have produced dem status - not in contact",
					_demStat);
