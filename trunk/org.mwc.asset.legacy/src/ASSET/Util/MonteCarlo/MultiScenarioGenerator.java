...
public final class MultiScenarioGenerator
{
   ...
   * @param index
   */
  private void renameScenario(Document newDoc,
                              int index)
  {
   ...
   */
  private static void setScenarioCase(Document newDoc,
                                      String caseId)
  {
    // get the scenario object
    NodeList list = newDoc.getElementsByTagName(ScenarioHandler.SCENARIO_NAME);
  ...
  public static final class MultiScenarioGennyTest extends SupportTesting
  {
    ...
    public final void testLoadVariances()
    {
      ...
      try
      {
        document = ScenarioGenerator.readDocumentFrom(new FileInputStream(docPath + "test_variance1.xml"));
      }
      catch (SAXException e)
      {
        e.printStackTrace();  //To change body of catch statement use Options | File Templates.
    ...
    public final void testPerformVariances()
    {
      ...
      // store the original speed for ssn bravo - so that we can check it's changed
      ...
      try
      {
        ...
        assertTrue("correct name", Double.parseDouble(thisE.getAttribute("Value")) != originalSpeed);
      }
      catch (JaxenException e)
      {
        e.printStackTrace();  //To change body of catch statement use Options | File Templates.
    ...
    public final void testPerformForceProtectionVariances()
    {
      ...
      try
      {
        ...
        if (el != null)
        {
          MultiParticipantGenerator participantGenny = new MultiParticipantGenerator(thisVarianceDocument);
      ...
      try
      {
        ...
        bravoa = new DOMXPath("//StopOnProximityDetectionObserver/Target/TargetType/Type[1]");
        ...
        bravoa = new DOMXPath("//StopOnProximityDetectionObserver/Watch/TargetType/Type[1]");
        thisA = (Element) bravoa.selectSingleNode(thisVarianceDocument);
        val = thisA.getAttribute("Name");
        assertEquals("correct name", "FISHING_VESSEL", val);

        bravoa = new DOMXPath("//StopOnProximityDetectionObserver/Watch/TargetType/Type[2]");
        thisA = (Element) bravoa.selectSingleNode(thisVarianceDocument);
        val = thisA.getAttribute("Name");
