...
public class UserControlEditor extends
		...
		java.beans.PropertyChangeListener
{
	...
	public void buildGUI()
	{
		...
		_course = new DemandedSliders(0, 360, (int) _userControl.getCourse(),
				JSlider.HORIZONTAL, "Course")
		{
			/**
			 * 
			 */
		...
		_speed = new DemandedSliders(0, 40, (int) _userControl.getSpeed()
				.getValueIn(WorldSpeed.Kts), JSlider.HORIZONTAL, "Speed")
		{
			/**
			 * 
			 */
		...
		_depth = new DemandedSliders(0, 300, (int) _userControl.getDepth()
				.getValueIn(WorldDistance.METRES), JSlider.HORIZONTAL, "Depth")
		{
			/**
			 * 
			 */
	...
	public void propertyChange(final java.beans.PropertyChangeEvent pe)
	{
		...
		if (type == ASSET.Models.Decision.UserControl.UPDATED)
		{
			...
			_depth.setCurrent((int) _userControl.getDepth().getValueIn(
	...
	abstract private class DemandedSliders extends JPanel
	{
		 ...
		 * @param orientation
		 *          whether the slider is horiz or vertical
		 * @param title
		 *          the title for the value being edited
		 */
		public DemandedSliders(int min, int max, int current, int orientation,
				String title)
		{
			buildGUI(orientation, title, min, max, current);
		}

		private void buildGUI(int orientation, String title, int min, int max,
				int current)
		{
	...
	{
		UserControl control = new UserControl(270, new WorldSpeed(12,
				WorldSpeed.Kts), new WorldDistance(40, WorldDistance.METRES));
