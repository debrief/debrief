...
public class BeanUtil {
	...
	/**
	 * Reflectively invokes getter associated with given {@link
	 * GriddableItemDescriptor} on given subject {@link TimeStampedDataItem}
	 * 
	 * @param item
	 ...
	 */
	public static <T> T getItemValue(TimeStampedDataItem item, GriddableItemDescriptor descriptor, Class<T> resultType) {
		if (item == null) {
...
//			throw new IllegalArgumentException(//
//					"Descriptor: " + descriptor.getTitle() + //
//							" is not applicable to item: " + item.getClass() + ", expected type: " + descriptor.getType() + //
...
//		}
	...
	/**
	 * Reflectively invokes setter associated with given {@link
	 * GriddableItemDescriptor} on given subject {@link TimeStampedDataItem}
	 ...
	 * @throws IllegalArgumentException
	 * 		if descriptor is not applicable to given item or not configured
	 * 		properly (e.g, if setter is not accessible, or does not conform to
	 * 		Beans specification, etc)
	 * @throws ClassCastException
	 ...
	 * 		if given item is <code>null</code>
	 */
	static void setItemValue(TimeStampedDataItem item, GriddableItemDescriptor descriptor, Object value) {
		if (item == null) {
			throw new NullPointerException();
		}
		...
		if (value != null && !descriptor.getType().isAssignableFrom(value.getClass())) {
			//not that strong of a check, but we will fail later (on invocation) in any case
			boolean autoUnboxable = descriptor.getType().isPrimitive() && Number.class.isAssignableFrom(value.getClass());
			if (!autoUnboxable) {
				throw new ClassCastException(//
		...
		Method setter;
		try {
//			sort out why this setter is failing!
//			find another way of getting the getter method
	...
	public static Object getItemValue(TimeStampedDataItem item, GriddableItemDescriptor descriptor) {
