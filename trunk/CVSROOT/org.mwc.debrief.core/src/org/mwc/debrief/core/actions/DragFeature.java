...
public class DragFeature extends CoreDragAction
{
	...
	private TrackDataProvider _trkData;
	...
	protected void execute()
	{
		...
		try
		{
			...
			// hey, also store the track-data shift listener, if there is one
			CorePlotEditor editor = getEditor();
			_trkData = (TrackDataProvider) editor.getAdapter(TrackDataProvider.class);
		}
	...
	// }
	//
	// protected static void findNearestHotSpotIn(TrackWrapper host,
	// java.awt.Point cursorPt,
	// WorldLocation cursorLoc, LocationConstruct currentNearest)
	// {
	//
	// // initialise thisDist, since we're going to be over-writing it
				...
				final Layers theLayers, SWTCanvas theCanvas)
		{
			...
			else
			{
				...
				if (_hoverTarget != null)
				{
					...
					// hey - let's tell anybody that wants to know about the track being
					// shifted
					if (_trkData != null)
					{
						...
						{
							_trkData.fireTrackShift((TrackWrapper) _hoverTarget);
						}
					}
				}
			}
	...
	public static final class DragFeatureAction implements MWC.GUI.Tools.Action
	{
		...
		public void undo()
		{
			...
			reverseVector.setValues(_theOffset.getBearing() + Math.PI, _theOffset.getRange(),
					_theOffset.getBearing());
