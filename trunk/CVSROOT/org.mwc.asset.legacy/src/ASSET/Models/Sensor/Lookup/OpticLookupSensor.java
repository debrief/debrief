...
public class OpticLookupSensor extends LookupSensor
{
   ...
   */
  public OpticLookupSensor(int id, String myName,
  ...
  private OpticEnvironment getDefaultLookups()
  {
    ...
    IntegerLookup attenuation = new IntegerLookup(new int[]{EnvironmentType.VERY_CLEAR, EnvironmentType.CLEAR, EnvironmentType.LIGHT_HAZE, EnvironmentType.HAZE, EnvironmentType.MIST, EnvironmentType.FOG},
    ...
    IntegerTargetTypeLookup sea_states = new IntegerTargetTypeLookup(new int[]{0, 1, 2, 3, 4, 5, 6},
                                                                     new StringLookup[]{
    ...
    IntegerLookup lightLevel = new IntegerLookup(new int[]{EnvironmentType.DAYLIGHT, EnvironmentType.DUSK, EnvironmentType.MOON_NIGHT, EnvironmentType.DARK_NIGHT},
                                                 new double[]{1, 0.4, 0.3, 0.05});

    OpticEnvironment defaultEnv = new OpticEnvironment(attenuation, lightLevel,
                                                       "sample", sea_states, visibility);
                                                         ...
                                                         long time)
  {
    ...
    int currentAtten = environment.getAtmosphericAttentuationFor(time, ownship.getStatus().getLocation());
    ...
    int lightLevel = environment.getLightLevelFor(time, ownship.getStatus().getLocation());
                                      ...
                                      long time, LookupSensor.LastTargetContact params)
  {
    ...
    final double B_FACTOR = 0.8279; // taken from  from Bowditch, The Practical navigator (1995) table 12
    ...
    double RP_2 = Math.sqrt(2 * EARTH_RADIUS * myHeight / B_FACTOR) + Math.sqrt(2 * EARTH_RADIUS * hisHeight / B_FACTOR);
  ...
  public static class OpticEnvironment
  {
    ...
    public OpticEnvironment(IntegerLookup attenuation, IntegerLookup lightLevel, String name,
                            IntegerTargetTypeLookup sea_states, StringLookup visibility)
    {
  ...
  static public final class OpticLookupTest extends SupportTesting.EditableTesting
  {
    ...
    {
      return new OpticLookupSensor(12, "eyeball", 0.05, 10, 1.05, 0.8, new Duration(20, Duration.SECONDS), 0.2,
    ...
    public void testDefaultValues()
    {
      ...
      assertNotNull("didn't find result which we should have done");
      assertEquals("didn't find correct lookup value", 0.001d, res.doubleValue(), 0d);
      ...
      boolean seaThrown = false;

      try
      {
        res = env._sea_states.find(1, Category.Type.CARRIER);
        assertNotNull("didn't find result which we should have done");
        assertEquals("didn't find correct lookup value", 1d, res.doubleValue(), 0d);
      ...
      assertTrue("should have fired missing item for missing type in sea state test", seaThrown);
