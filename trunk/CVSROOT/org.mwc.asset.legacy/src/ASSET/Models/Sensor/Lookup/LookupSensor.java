...
public abstract class LookupSensor extends CoreSensor
{
                                      ...
                                      long time, ScenarioType scenario)
  {
    ...
    if (oldParameters == null)
    {
      ...
      WorldDistance RP = calculateRP(ownship, target, scenario, environment, time, newParameters);
    ...
    actualRange = calculateSlantRangeFor(ownship.getStatus().getLocation(), target.getStatus().getLocation());

    WorldVector offset = target.getStatus().getLocation().subtract(ownship.getStatus().getLocation());
    ...
    if (!actualRange.greaterThan(newParameters.getRI()))
    {
        ...
        case DetectionEvent.CLASSIFIED:
          {
            ...
            checkAndMakeTransition(actualRange, nextThreshold,
                                   newParameters, time,
                                   ITP,
      ...
      res = new DetectionEvent(time, ownship.getId(),
                               ownship.getStatus().getLocation(), this,
                               theRange, null, bearingVal, relBearingVal,
                               null, target.getCategory(), target.getStatus().getSpeed(),
                               new Float(target.getStatus().getCourse()), target,
                               newParameters.getDetectionState());
    ...
    else
    {
      ...
      else
      {
        ...
        // so. we're outside the instantaneous range, but if we're already in contact it can stretch
        ...
        {
          // hey - we're on the hairy edge of being in contact.  Things aren't going to get any
          // better here.  Create a detection with the new relative data, but maintaining the same
          // detection state
          res = new DetectionEvent(time, ownship.getId(), ownship.getStatus().getLocation(), this,
                                   actualRange, null, new Float(bearing),
                                   new Float(relBearing), null, target.getCategory(), target.getStatus().getSpeed(),
    ...
    if (newParameters != null)
    {
      ...
      if (_pSupport != null)
      {
        ...
        {
          // create the event
          LookupSensorComponentsEvent sev =
            new LookupSensorComponentsEvent(newParameters.getDetectionState(),
                                            newParameters.getRI(), newParameters.getRP(), actualRange, target.getName());
  ...
  protected WorldDistance calculateRI(WorldDistance RP, double VDR)
  {
    ...
    WorldDistance RI = new WorldDistance(res,
  ...
  private static void checkAndMakeTransition(WorldDistance actualRange,
                                             WorldDistance nextThreshold,
                                             LastTargetContact newParameters,
                                             long time,
                                             Duration waitingPeriod,
                                             int newState)
  {
    // ok.  see if we are within the classification range factor
    //    WorldDistance classRng = new WorldDistance(oldParameters.getRP().getValueIn(WorldDistance.METRES) * rangeFactor,
    //                                               WorldDistance.METRES);
  ...
  private static WorldDistance calculateSlantRangeFor(WorldLocation ownship, WorldLocation target)
  {
   ...
   */
  protected abstract LastTargetContact parametersFor(ParticipantType ownship, ParticipantType target,
   ...
   * @return
   */
  abstract protected WorldDistance calculateRP(ParticipantType ownship, ParticipantType target,
  ...
  public static class StringLookup extends BaseLookup
  {
     ...
     * @param defaultValue - the value to return if a matching one isn't found. Throws
     *                     runtime error if matching type isn't found and a default value isn's supplied
  ...
  ////////////////////////////////////////////////////////////
  // embedded class which provides a two-dimensional lookup table of string values against sea-state, returning
  // a double value for the matching string
  ////////////////////////////////////////////////////////////
  public static class IntegerTargetTypeLookup
  {
     ...
     * @param vals         list of double values
     * @param defaultValue the value to use if the target type hasn't been represented in the table
     */
    public IntegerTargetTypeLookup(int[] states, StringLookup[] vals, Double defaultValue)
    {
    ...
    public Double find(int index, String type)
    {
      ...
      while (iterator.hasNext() && res == null)
      {
        ...
        if (s.equals(indexVal))
        {
          StringLookup thisSet = (StringLookup) _myTable.get(indexVal);
          res = thisSet.find(type, _defaultValue);
          break;
        }
      }
  ...
  public static class LookupSensorComponentsEvent
  {
     ...
     */
    private DetectionEvent.DetectionStatePropertyEditor converter
     ...
     * constructor
     */
    public LookupSensorComponentsEvent(int state, WorldDistance RI,
                                       WorldDistance RP, WorldDistance actual,
                                       String tgtName)
    {
      _tgtName = tgtName;
  ...
  ////////////////////////////////////////////////////////////
  // embedded utility class which contains a set of lookup parameters for the lookup
  // sensor in question
  ////////////////////////////////////////////////////////////
  ...
  protected class LastTargetContact
  {
    ...
    /**
     * the time we made the transition to this state
     * (in order to allow the time-related transition to the next)
     */
    ...
    public boolean matchesThis(LastTargetContact otherParams)
    {
      ...
      // first do "idiot test", if list are of different length they must be different
  ...
  private static class TestSensor extends LookupSensor
  {
    ...
    public TestSensor(double VDR, long TBDO, double MRF,
                      double CRF, Duration CTP, double IRF,
                      Duration ITP,
                      double rp_m, double ri_m, double cr_m, double ir_m,
                      double max_m)
    {
    ...
    // allow an 'overview' test, just to check if it is worth all of the above processing
    protected boolean canDetectThisType(ParticipantType ownship, ParticipantType other, EnvironmentType env)
    {
      return true;  //To change body of implemented methods use File | Settings | File Templates.
    }

    protected LastTargetContact parametersFor(ParticipantType ownship, ParticipantType target, ScenarioType scenario,
                                              EnvironmentType environment, long time)
    {
  ...
  static public final class LookupSensorTest extends SupportTesting
  {
    ...
    private static TestSensor getTestSensor(double rp_m, double ri_m,
                                            double cr_m, double ir_m, double max_m)
    {
      return new TestSensor(0.05, 1000, 1.05, 0.8, new Duration(20, Duration.SECONDS), 0.2,
                            new Duration(30, Duration.SECONDS),
                            rp_m, ri_m, cr_m, ir_m, max_m);
    }
    ...
    public void testClearingOldDetections()
    {
      ...
      RadarLookupSensor rs =
        RadarLookupSensor.RadarLookupTest.getTestRadarSensor(4000, 3100, 2500, 1900, 4700, 600);
      ...
      charlie.setCategory(new Category(Category.Force.BLUE, Category.Environment.SURFACE, Category.Type.FISHING_VESSEL));
      ...
      // check that we have removed the old detections and that we still have the old detections
      assertTrue("still have last contact", dets.contains(d1));
      assertTrue("still have last contact", dets.contains(d2));
      assertEquals("should have kept contacts, havne't done next scan yet", 2, dets.size());

      // keep moving forward,
      curTime += 3000;
      sl.detects(env, dets, alpha, scenario, curTime += 3000);

      // right, passed time for next scan. check that old detections removed, and no new ones added.
    ...
    public final void testFirst()
    {
      ...
      time += 100;
      ts.detects(env, res, alpha, scenario, time);
      assertEquals("old detections should have been returned (because it's not time yet)", 1, res.size());
      ...
      assertEquals("detections returned", 1, res.size());
      assertEquals("only one detection produced", 1, res.size());
      assertEquals("check detected state", DetectionEvent.IDENTIFIED, ((LastTargetContact) ts._pastContacts.get(bravo)).getDetectionState());
    ...
    public void testMultipleContacts()
    {
      ...
      assertNotNull("detections returned", res);
      assertEquals("only one detection produced", 1, res.size());
      assertEquals("check detected state", DetectionEvent.DETECTED, ((LastTargetContact) ts._pastContacts.get(test2bravo)).getDetectionState());
      ...
      assertEquals("check detected state", DetectionEvent.DETECTED, ((LastTargetContact) ts._pastContacts.get(test2bravo)).getDetectionState());
      assertEquals("check detected state", DetectionEvent.DETECTED, ((LastTargetContact) ts._pastContacts.get(test2charlie)).getDetectionState());
      ...
      assertNotNull("detections returned", res);
      assertEquals("two detections produced", 2, res.size());
      assertEquals("check detected state", DetectionEvent.CLASSIFIED, ((LastTargetContact) ts._pastContacts.get(test2bravo)).getDetectionState());
      assertEquals("check detected state", DetectionEvent.DETECTED, ((LastTargetContact) ts._pastContacts.get(test2charlie)).getDetectionState());
      ...
      assertEquals("two detections produced", 2, res.size());
      assertEquals("check detected state", DetectionEvent.CLASSIFIED, ((LastTargetContact) ts._pastContacts.get(test2bravo)).getDetectionState());
      assertEquals("check detected state", DetectionEvent.CLASSIFIED, ((LastTargetContact) ts._pastContacts.get(test2charlie)).getDetectionState());
      ...
      assertEquals("two detections produced", 2, res.size());
      assertEquals("check detected state", DetectionEvent.DETECTED, ((LastTargetContact) ts._pastContacts.get(test2bravo)).getDetectionState());
      assertEquals("check detected state", DetectionEvent.IDENTIFIED, ((LastTargetContact) ts._pastContacts.get(test2charlie)).getDetectionState());
    ...
    public void testThresholds()
    {
        ...
        public void newDetections(DetectionList detections)
        {
          if (detections.size() > 0)
          {
            ...
            if (de.getDetectionState() != lastState)
            {
              ...
              WorldDistance wd = LookupSensor.calculateSlantRangeFor(alpha.getStatus().getLocation(), bravo.getStatus().getLocation());
      ...
      ASSET.Scenario.Observers.Recording.DebriefReplayObserver dr = new DebriefReplayObserver("c:/temp", null, true, "record file", true);
      dr.setup(scenario);
    ...
    public final void testPersistentData()
    {
      ...
      ;
      boolean isSame = newRI.getValueIn(WorldDistance.METRES) == lastRI.getValueIn(WorldDistance.METRES);
    ...
    public void testTutorialScenario()
    {
      ...
      DebriefReplayObserver dro = new DebriefReplayObserver("./", "test_tutorial.rep", true, "test observer", true);
      dro.setup(cs);

      ASSET.Scenario.Observers.TimeObserver to = new TimeObserver(0, 3, 0, 0, "Test time observer", true);
      to.setup(cs);
