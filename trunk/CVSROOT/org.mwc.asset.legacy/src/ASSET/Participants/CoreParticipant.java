...
public class CoreParticipant implements ParticipantType,
  java.io.Serializable
{
  ...
  /**
   * our list of third party detections (such as those from weapon in the water, sonar buoy)
   */
   ...
   * that we can fire a new decision message if the the activitiy wihtin the behaviour
   * has changed but the last behaviour hasn't
   */
      ...
      if (_myDemandedStatus != null)
      {
        ...
        if (theStates != null)
        {
          if (theStates.size() > 0)
          {
            ...
            {
              DemandedSensorStatus sensorStatus = (DemandedSensorStatus) theStates.elementAt(i);
  ...
  {
    double vesselRadiated = _radiatedChars.radiatedEnergyFor(medium, getStatus(), brg_degs);
  ...
  public void setStatus(final Status val)
  {
    ...
    {
      // just do a quick check to ensure that we're not working with a dummy status
  ...
  public static class ParticipantTest extends junit.framework.TestCase
  {
    ...
    public void testParticipantStepping()
    {
      ...
      part.setMovementChars(new SSMovementCharacteristics("here", 1, 1, 0, 20, 1, 200, 1, 1, 100, 1));
