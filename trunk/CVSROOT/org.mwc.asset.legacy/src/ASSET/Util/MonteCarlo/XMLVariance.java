...
public final class XMLVariance
{
   ...
   */
  public static WorldLocation readInLocationFromXML(Element myElement, String locationName)
  {
    ...
    if (theLocation.getLength() > 0)
    {
      ...
      {
        // try for the long location
        NodeList theLongLocation = myElement.getElementsByTagName("relativeLocation");
  ...
  private static WorldLocation extractShortLocation(Element topL)
  {
    ...
    WorldLocation thisCorner = new WorldLocation(Double.parseDouble(llat), Double.parseDouble(llong), 0);
    return thisCorner;
  }
  ...
  private static WorldLocation extractLongLocation(Element topL)
  {
    ...
    WorldLocation thisCorner = new WorldLocation(latDeg, latMin, latSec, latHemi, longDeg, longMin, longSec, longHemi, 0);
   ...
   * @return new location
   */
  public static WorldLocation generateRandomLocationInArea(WorldArea area, int numberModel)
  {
    WorldLocation res = null;
    double latV = RandomGenerator.generateRandomNumber(area.getBottomRight().getLat(),
                                                       area.getTopLeft().getLat(), numberModel);
    double longV = RandomGenerator.generateRandomNumber(area.getTopLeft().getLong(),
