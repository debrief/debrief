		...
		LayerHandler.exporter
{
	...
	public GridHandler(String theType)
  {
	...
	public void elementClosed()
	{
		...
		else
		{
			...
			if (_myUnits == null)
				...
				csp.setDelta(new MWC.GenericData.WorldDistance(_deltaDegs,
						WorldDistance.NM));
			else
			{
				// yes, we do - best use them
				csp.setDelta(new MWC.GenericData.WorldDistance(_deltaDegs,
						WorldDistance.getUnitIndexFor(_myUnits)));
			}
		}
			...
			MWC.GUI.Chart.Painters.GridPainter theGrid, Document doc)
	{
		...
		WorldDistanceHandler.exportDistance(DELTA, theGrid.getDelta(), gridElement,
				doc);
