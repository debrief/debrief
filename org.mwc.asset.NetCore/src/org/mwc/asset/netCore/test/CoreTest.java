...
public class CoreTest extends junit.framework.TestCase {
	...
	protected static class MyPart extends Surface {
		...
		public MyPart(final int id, final String force) {
			super(id);
			setName("p" + id);
			this.setCategory(new Category(force, Category.Environment.SURFACE, Category.Type.FRIGATE));
			...
			newStat.setLocation(new WorldLocation(11 + id / 100d, 11 - id / 100d, 11));
			...
			rangeDoublets.add(new TypedRangeDoublet(null, new WorldDistance(10, WorldDistance.NM)));
			// and a sensor
	...
	public void ntestStartup() throws InterruptedException, IOException {
		...
		assertTrue("correct client event", _events.elementAt(1).contains("Discovered server"));
		assertTrue("correct client event", _events.elementAt(2).contains("Connecting"));
		assertTrue("correct client event", _events.elementAt(3).contains("connected"));
		assertTrue("correct client event", _events.elementAt(4).contains("connected"));
	...
	public void ntestZConnect() throws InterruptedException, IOException {
		...
		assertTrue("correct client event", _events.elementAt(1).contains("Connecting: /128.3.3.3:54555/54778"));
