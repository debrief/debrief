...
public interface RemoteObject {
	 ...
	 *
	 * @param nonBlocking     If false, the invoking thread will wait for a return
	 *                        value or timeout (default). If true, the invoking
	 *                        thread will not wait for a response. The response can
	 *                        later be retrieved with {@link #waitForLastResponse()}
	 *                        or {@link #waitForResponse(byte)}. The method will
	 *                        return immediately and the return value should be
	 *                        ignored.
	 * @param ignoreResponses If false, then the response for non-blocking method
	 *                        invocations can be retrieved with
	 *                        {@link #waitForLastResponse()} or
	 *                        {@link #waitForResponse(byte)}. The responses will be
	 *                        stored until retrieved, so each method call should
	 *                        have a matching retrieve. If true, then the response
	 *                        to non-blocking method invocations will never be sent
	 *                        by the remote side of the connection and the response
	...
	/**
	 * Waits for the response to the last method invocation to be received or the
