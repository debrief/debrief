...
public class NumberFormatter extends AbstractFormatter {
	...
	/**
	 * Flag indicating that the buffer is modified and the Number value must be
	 * computed
	 */
	 ...
	 * @param loc locale
	 */
	public NumberFormatter(final Locale loc) {
		this(null, null, loc);
	}
	 ...
	 */
	public NumberFormatter(final String editPattern) {
	 ...
	 * @param loc         locale
	 */
	public NumberFormatter(final String editPattern, final Locale loc) {
	 ...
	 */
	public NumberFormatter(final String editPattern, final String displayPattern) {
	 ...
	 * @param loc            locale
	 */
	public NumberFormatter(final String editPattern, final String displayPattern, final Locale loc) {
		this.locale = loc;
		alwaysShowDec = false;
	...
	/**
	 * Clears a part of the edition cache. The start and len parameters are adjusted
	 * to avoid clearing in prefix and suffix parts of the cache.
	 *
	...
	protected void clearText(int start, int len) {
		...
		final int d = editValue.indexOf(EMPTY + symbols.getDecimalSeparator());
		final boolean decimal = d >= start && d < start + len;
		for (int i = 0; i < len; i++) {
			final char c = editValue.charAt(start + i);
			if (c >= '0' && c <= '9') {
				if (d < 0 || start + i < d) {
					intCount--;
				}
	 ...
	 * deletes excess decimal digits and add 0 to complete to the minimal length of
	 * int and decimal parts. The position of the cursor is preserved.
	 *
	 ...
	 * @return Edit pattern
	 */
	protected String getDefaultPattern(final Locale loc) {
		...
		if (edit == null) {
			...
			final int i = edit.indexOf(((DecimalFormat) nf).getDecimalFormatSymbols().getPatternSeparator());
			if (i >= 0) {
	...
	public String getDisplayString() {
		return editValue.substring(0, prefixLen) + ((getValue() != null ? nfDisplay.format(value) : EMPTY))
	 ...
	 * value returned is the content of the StringBuilder <code>editValue</code>
	 * used as cache.
	 *
	 * @return edit string
		...
		if (modified) {
			try {
				...
				if (zeroIntLen + zeroDecimalLen == 0
						&& (editValue.length() == 0 || editValue.charAt(0) == symbols.getDecimalSeparator())) {
					value = null;
				} else {
					value = new Integer(0);
				}
	 ...
	 */
	public void setDecimalSeparatorAlwaysShown(final boolean show) {
		alwaysShowDec = show;
		final int i = editValue.lastIndexOf(EMPTY + symbols.getDecimalSeparator());
		if (alwaysShowDec) {
			if (i == -1) {
	 ...
	 */
	public void setFixedLengths(final boolean fixedInt, final boolean fixedDec) {
	 ...
	 */
	protected void setPatterns(String edit, final String display, final Locale loc) {
		// Symbols
	...
	public void setValue(final Object value) {
		...
		if (value instanceof Number) {
			...
			for (int i = prefixLen; i < editValue.length() - suffixLen; i++) {
				final char c = editValue.charAt(i);
	...
	public void verifyText(final VerifyEvent e) {
		...
		if (e.keyCode == SWT.BS || e.keyCode == SWT.DEL) {
			...
			int d = editValue.indexOf(EMPTY + symbols.getDecimalSeparator()); // Decimal
																				// separator
																				// position
			for (int i = 0; i < e.text.length(); i++) {
				if (p < prefixLen || p > editValue.length() - suffixLen)
					break;
				final char c = e.text.charAt(i);
				if (c >= '0' && c <= '9') {
				...
				} else if (groupLen > 0 && (c == symbols.getGroupingSeparator() || (c == ' ' && nbspSeparator))) {
					/*
					 * Some locales (eg. French) return a no-break space as the grouping separator.
					 * This character is not natural to use for users. So we recognize too the
					 * simple space as the grouping separator.
					 *
					 * Java bug: 4510618
					 */
					if (d >= 0 && p > d) {
					...
					final int n = editValue.indexOf(EMPTY + symbols.getGroupingSeparator(), p);
					if (n > p) {
						p = n + 1;
					}
				} else if (c == symbols.getMinusSign()) {
				...
				} else if (c == symbols.getDecimalSeparator() && (decimalLen > 0 || !fixedDec)) {
					if (d >= 0) {
						if (d < p) {
							beep();
							break;
					...
					intCount = 0;
					for (int j = 0; j < d; j++) {
						final char c1 = editValue.charAt(j);
						if (c1 >= '0' && c1 <= '9') {
							intCount++;
						}
					}
