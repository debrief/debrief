...
public class NumberFormatter extends AbstractFormatter
{
	...
	protected int format(int curseur)
	{
		...
		if (groupLen > 0)
		{
			...
			for (; i < editValue.length() - suffixLen; i++)
			{
				...
				{
					if (i > 0
							&& editValue.charAt(i - 1) == symbols.getGroupingSeparator())
					{
	...
	public String getDisplayString()
	{
		return editValue.substring(0, prefixLen)
				+ ((getValue() != null ? nfDisplay.format(value) : EMPTY))
	...
	public Object getValue()
	{
		if (modified)
		{
			try
			{
				value = nfEdit.parse(editValue.substring(prefixLen, editValue.length()
						- suffixLen));
			}
			catch (ParseException e1)
			{
				if (zeroIntLen + zeroDecimalLen == 0
						&& (editValue.length() == 0 || editValue.charAt(0) == symbols
								.getDecimalSeparator()))
				{
					value = null;
				}
	 ...
	 * 
	 * @param prefix
	 *          prefix to display, or <code>null</code> to clear
	 */
	 ...
	 * 
	 * @param suffix
	 *          suffix to display, or <code>null</code> to clear
	 */
			...
			for (int i = 0; i < e.text.length(); i++)
			{
				...
				if (c >= '0' && c <= '9')
				{
					...
					editValue.insert(p++, c);
						...
						&& (c == symbols.getGroupingSeparator() || (c == ' ' && nbspSeparator)))
				{
					/*
