...
public class RCPIslandEvolution {
	private class Epoch implements Callable<List<EvaluatedCandidate<List<StraightRoute>>>> {
		...
		Epoch(final EvolutionEngine<List<StraightRoute>> island, final int populationSize, final int eliteCount,
				final List<List<StraightRoute>> seedCandidates, final TerminationCondition... terminationConditions) {
			this.island = island;
		...
		@Override
		public List<EvaluatedCandidate<List<StraightRoute>>> call() throws Exception {
	...
	private static List<List<StraightRoute>> toCandidateList(
			final List<EvaluatedCandidate<List<StraightRoute>>> evaluatedCandidates) {
		final List<List<StraightRoute>> candidates = new ArrayList<List<StraightRoute>>(evaluatedCandidates.size());
		for (final EvaluatedCandidate<List<StraightRoute>> evaluatedCandidate : evaluatedCandidates) {
	...
	private final Set<EvolutionEngine<List<StraightRoute>>> finishedIslands = Collections
	...
	public RCPIslandEvolution(final GASolutionGenerator solutionGenerator, final List<StraightLeg> straightLegs,
			final int islandCount, final Random rng) {
	...
	protected EvolutionEngine<List<StraightRoute>> createContinient() {
		final List<EvolutionaryOperator<List<StraightRoute>>> operators = new ArrayList<EvolutionaryOperator<List<StraightRoute>>>();
		final PointsCrossover pointsCrossover = new PointsCrossover(straightLegs, new Probability(1));
		final SplitEvolution<List<StraightRoute>> crossovers = new SplitEvolution<List<StraightRoute>>(
				new ListCrossover<StraightRoute>(), pointsCrossover, 0.4);
		final AdaptivePointMutation mutation = new AdaptivePointMutation(straightLegs, new Probability(0.25));
		...
		final GenerationalEvolutionEngine<List<StraightRoute>> continent = new GenerationalEvolutionEngine<List<StraightRoute>>(
				new RoutesCandidateFactory(straightLegs), new EvolutionPipeline<List<StraightRoute>>(operators),
				new RoutesFitnessEvaluator(straightLegs, solutionGenerator.getParameters().isUseAlteringLegs(),
						solutionGenerator.getContributions(), solutionGenerator.getProblemSpace()),
	 ...
	 */
	private List<Callable<List<EvaluatedCandidate<List<StraightRoute>>>>> createEpochTasks(final int populationSize,
			final int eliteCount, final int epochLength, final List<List<List<StraightRoute>>> populations) {
		final TerminationCondition condition = new GenerationCount(epochLength);
		final List<Callable<List<EvaluatedCandidate<List<StraightRoute>>>>> epochs = new ArrayList<Callable<List<EvaluatedCandidate<List<StraightRoute>>>>>(
				islands.size() + 1);

		epochs.add(new Epoch(continent, populationSize, eliteCount, populations.get(0), condition));
		for (int i = 0; i < islands.size(); i++) {
			final EvolutionEngine<List<StraightRoute>> island = islands.get(i);
			epochs.add(new Epoch(island, populationSize, eliteCount, populations.get(i + 1), condition,
					new Stagnation(epochLength - 1) {

						@Override
						public boolean shouldTerminate(final PopulationData<?> populationData) {
							final boolean result = super.shouldTerminate(populationData);
							if (result) {
	...
	protected EvolutionEngine<List<StraightRoute>> createIsland(final boolean useAlterings) {
		final List<EvolutionaryOperator<List<StraightRoute>>> operators = new ArrayList<EvolutionaryOperator<List<StraightRoute>>>();
		final PointsCrossover pointsCrossover = new PointsCrossover(straightLegs, new Probability(1));

		AdaptivePointMutation adaptiveMutation;
		final RoutesCandidateFactory factory = new RoutesCandidateFactory(straightLegs);
		final SplitEvolution<List<StraightRoute>> crossovers = new SplitEvolution<List<StraightRoute>>(
				new ListCrossover<StraightRoute>(), pointsCrossover, 0.4);
		final SplitEvolution<List<StraightRoute>> mutatuions = new SplitEvolution<List<StraightRoute>>(
				adaptiveMutation = new AdaptivePointMutation(straightLegs, new Probability(0.3)),
				new RandomMutation(straightLegs, new Probability(0.4), factory), 0.7);
		operators.add(crossovers);
	...
	public List<StraightRoute> evolve(final int populationSize, final int eliteCount, final int epochLength,
			final int migrantCount, final TerminationCondition... conditions) {
		// allocate one thread per island
		final ExecutorService threadPool = Executors.newFixedThreadPool(islands.size() + 1);

		// create the empty islands
		final List<List<List<StraightRoute>>> islandPopulations = new ArrayList<List<List<StraightRoute>>>(
				islands.size() + 1);
		...
		while (satisfiedConditions == null) {
			final List<Callable<List<EvaluatedCandidate<List<StraightRoute>>>>> epochs = createEpochTasks(
					populationSize, eliteCount, epochLength, islandPopulations);
			try {
				// get each island to do an evolution, in its own thread
				final List<Future<List<EvaluatedCandidate<List<StraightRoute>>>>> futures = threadPool
						.invokeAll(epochs);

				// create a placeholder for the finished population from each island
				final List<List<EvaluatedCandidate<List<StraightRoute>>>> evaluatedPopulations = new ArrayList<List<EvaluatedCandidate<List<StraightRoute>>>>(
						islands.size());

				// loop through the evolution tasks
				for (final Future<List<EvaluatedCandidate<List<StraightRoute>>>> future : futures) {
					//
					final List<EvaluatedCandidate<List<StraightRoute>>> evaluatedIslandPopulation = future.get();
				...
				data = EvolutionUtils.getPopulationData(evaluatedPopulations.get(0), false, eliteCount,
				...
				for (final List<EvaluatedCandidate<List<StraightRoute>>> evaluatedPopulation : evaluatedPopulations) {
				...
				final ArrayList<CompositeRoute> compositeRoutes = new ArrayList<CompositeRoute>();
				for (int i = 0; i < Math.min(40, islandPopulations.get(0).size()); i++) {
					compositeRoutes.add(solutionGenerator.solutionToRoute(islandPopulations.get(0).get(i), false));
				}
	...
	private void migrate(final List<List<List<StraightRoute>>> populations) {
		final List<List<StraightRoute>> elitePopulation = populations.get(0);
		int index = Math.min(70, elitePopulation.size()) - 1;
		for (int i = 1; i < populations.size(); i++) {
			final List<List<StraightRoute>> islandPopulation = populations.get(i);
	...
	private void renewIslands(final List<List<List<StraightRoute>>> populations) {
		for (final EvolutionEngine<List<StraightRoute>> island : finishedIslands) {
