...
public class GASolutionGenerator extends AbstractSolutionGenerator {
	...
	public GASolutionGenerator(final IContributions contributions, final IJobsManager jobsManager,
	 ...
	 *
	 * @param topRoutes the list of completed routes
	 * @param topScore  the score of the best performing route
	 */
	protected void fireIterationComputed(final List<CompositeRoute> topRoutes, final double topScore) {
		for (final IGenerateSolutionsListener listener : _readyListeners) {
			if (listener instanceof IGASolutionsListener) {
				((IGASolutionsListener) listener).iterationComputed(new ArrayList<CompositeRoute>(topRoutes), topScore);
			}
		}
	}
		...
		if (fullRerun || straightLegs == null) {
			previous = jobsManager.schedule(new Job<Void, Void>("Generate Legs", GA_GENERATOR_GROUP) {
				...
				protected <E> Void run(final IProgressMonitor monitor, final Job<Void, E> previous)
					...
					SATC_Activator.log(IStatus.INFO, "SATC GA - Generate Legs - Start", null);
					final List<CoreLeg> rawLegs = getTheLegs(problemSpaceView.states(), monitor);
					...
					SATC_Activator.log(IStatus.INFO, "SATC GA - Generate Legs - Complete", null);
					return null;
				}
			});
		}
	...
	private void runGA(final IProgressMonitor progressMonitor) throws InterruptedException {
		...
		final RCPIslandEvolution engine = new RCPIslandEvolution(this, straightLegs, 4, rng);
		final TerminationCondition progressMonitorCondition = new TerminationCondition() {
			@Override
		...
		final List<StraightRoute> solution = engine.evolve(parameters.getPopulationSize(), parameters.getElitizm(),
				parameters.getEpochLength(), 1, progressMonitorCondition, new ElapsedTime(parameters.getTimeout()),
	...
	protected CompositeRoute solutionToRoute(final List<StraightRoute> solution, final boolean createAltering) {
