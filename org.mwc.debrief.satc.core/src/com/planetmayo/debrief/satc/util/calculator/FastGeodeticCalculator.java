...
/**
 * Performs geodetic calculations on an {@linkplain Ellipsoid ellipsoid}. This
 * class encapsulates a generic ellipsoid and calculates the following
 * properties:
 * <p>
 * <ul>
 ...
 * <p>
 * <ul>
 * <li>The {@linkplain #setStartingPosition starting position}, which is always
 * considered valid. It is initially set at (0,0) and can only be changed to
 * another legitimate value.</li>
 ...
 * </ul>
 * The latest one set overrides the other and determines what will be
 * calculated.</li>
 * </ul>
 * <p>
 * Note: This class is not thread-safe. If geodetic calculations are needed in a
 * multi-threads environment, create one distinct instance of
 * {@code GeodeticCalculator} for each thread.
 *
 * @since 2.1
 *
 *
 ...
 */
public class FastGeodeticCalculator implements GeodeticCalculator {
	/**
	 * Tolerance factors from the strictest (<code>TOLERANCE_0</CODE>) to the most
	 * relax one (<code>TOLERANCE_3</CODE>).
	 */
	 ...
	 * @return The angle between between -{@linkplain Math#PI PI} and
	 *         {@linkplain Math#PI PI}.
	 */
	...
	/**
	 * Checks the azimuth validity. The argument {@code azimuth} should be greater
	 * or equal than -180 degrees and lower or equals than +180 degrees. As a
	 * convenience, this method returns the azimuth in radians.
	 *
	 * @param azimuth The azimuth value in <strong>decimal degrees</strong>.
	 * @return The azimuth value in <strong>radians</strong>.
	 * @throws IllegalArgumentException if {@code azimuth} is not between -180 and
	 *                                  +180 degrees.
	 */
	private static double checkAzimuth(final double azimuth) throws IllegalArgumentException {
		...
		throw new IllegalArgumentException(Errors.format(ErrorKeys.AZIMUTH_OUT_OF_RANGE_$1, new Longitude(azimuth)));
	}

	/**
	 * Checks the latidude validity. The argument {@code latidude} should be greater
	 * or equal than -90 degrees and lower or equals than +90 degrees. As a
	 * convenience, this method returns the latitude in radians.
	 *
	 * @param latitude The latitude value in <strong>decimal degrees</strong>.
	 * @return The latitude value in <strong>radians</strong>.
	 * @throws IllegalArgumentException if {@code latitude} is not between -90 and
	 *                                  +90 degrees.
	 */
	private static double checkLatitude(double latitude) throws IllegalArgumentException {
		// verify in-bounds
		if (latitude > Latitude.MAX_VALUE) {
			SATC_Activator.log(IStatus.WARNING, "Out of bounds lat requested:" + latitude, null);
	 ...
	 * @throws IllegalArgumentException if {@code longitude} is not between -180 and
	 *                                  +180 degrees.
	 */
	private static double checkLongitude(double longitude) throws IllegalArgumentException {
		...
		if (longitude > Longitude.MAX_VALUE) {
			SATC_Activator.log(IStatus.WARNING, "Out of bounds long requested:" + longitude, null);
	 ...
	 */
	private static void checkNumberOfPoints(final int numberOfPoints) throws IllegalArgumentException {
		if (numberOfPoints < 0) {
	...
	/**
	 * Format the specified coordinates using the specified formatter, which should
	 * be an instance of {@link CoordinateFormat}.
	 */
	...
	/**
	 * Returns the first two-dimensional geographic CRS using standard axis,
	 * creating one if needed.
	 */
	private static GeographicCRS getGeographicCRS(final CoordinateReferenceSystem crs) {
		if (crs instanceof GeographicCRS) {
			final CoordinateSystem cs = crs.getCoordinateSystem();
			if (cs.getDimension() == 2 && isStandard(cs.getAxis(0), AxisDirection.EAST)
	...
	/**
	 * Returns {@code true} if the specified axis is oriented toward the specified
	 * direction and uses decimal degrees units.
	 */
	...
	/**
	 * The transform from user coordinates to geodetic coordinates used for
	 * computation, or {@code null} if no transformations are required.
	 */
	private final TransformedDirectPosition userToGeodetic;

	/**
	 * The coordinate reference system for all methods working on {@link Position}
	 * objects. If {@code null}, will be created the first time
	 * {@link #getCoordinateReferenceSystem} is invoked.
	 */
	private CoordinateReferenceSystem coordinateReferenceSystem;

	/**
	 * The coordinate reference system for all methods working on {@link Point2D}
	 * objects. If {@code null}, will be created the first time
	 * {@link #getGeographicCRS} is invoked.
	 */
	...
	/*
	 * The maximum orthodromic distance that could be calculated onto the referenced
	 * ellipsoid.
	 */
	 ...
	 * <strong>in radians</strong>. This point is set by
	 * {@link #setStartingGeographicPoint}.
	 */
	...
	/**
	 * The (<var>latitude</var>, <var>longitude</var>) coordinate of the destination
	 * point <strong>in radians</strong>. This point is set by
	 * {@link #setDestinationGeographicPoint}.
	 */
	private double lat2, long2;

	/**
	 * The distance and azimuth (in radians) from the starting point
	 * ({@link #long1}, {@link #lat1}) to the destination point ({@link #long2},
	 * {@link #lat2}).
	 */
	private double distance, azimuth;

	/**
	 * Tell if the destination point is valid. {@code false} if {@link #long2} and
	 * {@link #lat2} need to be computed.
	 */
	private boolean destinationValid;

	/**
	 * Tell if the azimuth and the distance are valids. {@code false} if
	 * {@link #distance} and {@link #azimuth} need to be computed.
	 */
	private boolean directionValid;

	/**
	 * {@code true} if the source and destination points are almost antipodal. If
	 * {@code true}, then the distance and direction computed by
	 * {@link #computeDirection} are likely to be innacurate.
	 */
	 ...
	 */
	public FastGeodeticCalculator(final Ellipsoid ellipsoid) {
		this(ellipsoid, null);
	}
	 ...
	 */
	private FastGeodeticCalculator(final Ellipsoid ellipsoid, final CoordinateReferenceSystem crs) {
		if (ellipsoid == null) {
		...
		if (crs != null) {
			...
			/*
			 * Note: there is no need to set Hints.LENIENT_DATUM_SHIFT to Boolean.TRUE here
			 * since the target CRS computed by our internal getGeographicCRS(crs) method
			 * should returns a CRS using the same datum than the specified CRS. If the
			 * factory fails with a "Bursa-Wolf parameters required" error message, then we
	...
	/**
	 * Computes the orthodromic distance using the algorithm implemented in the
	 * Geotools's ellipsoid class (if available), and check if the error is smaller
	 * than some tolerance error.
	 */
	private boolean checkOrthodromicDistance() {
		if (ellipsoid instanceof DefaultEllipsoid) {
			...
			check = ellipsoid.orthodromicDistance(toDegrees(long1), toDegrees(lat1), toDegrees(long2), toDegrees(lat2));
	...
	/**
	 * Checks the orthodromic distance validity. Arguments
	 * {@code orthodromicDistance} should be greater or equal than 0 and lower or
	 * equals than the maximum orthodromic distance.
	 *
	 * @param distance The orthodromic distance value.
	 * @throws IllegalArgumentException if {@code orthodromic distance} is not
	 *                                  between 0 and the maximum orthodromic
	 *                                  distance.
	 */
	private void checkOrthodromicDistance(final double distance) throws IllegalArgumentException {
		if (!(distance >= 0.0 && distance <= maxOrthodromicDistance)) {
			throw new IllegalArgumentException(Errors.format(ErrorKeys.DISTANCE_OUT_OF_RANGE_$4, distance, 0.0,
					maxOrthodromicDistance, ellipsoid.getAxisUnit()));
		}
	}
	...
	/**
	 * Computes the destination point from the
	 * {@linkplain #getStartingGeographicPoint starting point}, the
	 * {@linkplain #getAzimuth azimuth} and the {@linkplain #getOrthodromicDistance
	 * orthodromic distance}.
	 *
	 * @throws IllegalStateException if the azimuth and the distance have not been
	 *                               set.
	 *
	...
	private void computeDestinationPoint() throws IllegalStateException {
		...
		/*
		 * Solution of the geodetic direct problem after T.Vincenty. Modified
		 * Rainsford's method with Helmert's elliptical terms. Effective in any azimuth
		 * and at any distance short of antipodal.
		 *
		 * Latitudes and longitudes in radians positive North and East. Forward azimuths
		 * at both points returned in radians from North.
		 *
		 * Programmed for CDC-6600 by LCDR L.Pfeifer NGS ROCKVILLE MD 18FEB75 Modified
		 * for IBM SYSTEM 360 by John G.Gergen NGS ROCKVILLE MD 7507 Ported from Fortran
		 * to Java by Daniele Franzoni.
		 *
		 * Source: ftp://ftp.ngs.noaa.gov/pub/pcsoft/for_inv.3d/source/forward.for
		 * subroutine DIRECT1
		 */
		double TU = fo * sin(lat1) / cos(lat1);
		final double SF = sin(azimuth);
		final double CF = cos(azimuth);
		double BAZ = (CF != 0) ? MathUtils.fast_atan2(TU, CF) * 2.0 : 0;
		final double CU = 1 / sqrt(TU * TU + 1.0);
		final double SU = TU * CU;
		final double SA = CU * SF;
		final double C2A = 1.0 - SA * SA;
		...
		// writer.write(SA + " : " + BAZ + "\n");
		// writer.flush();
		C = fo * MathUtils.calcAbsoluteValue(SA, BAZ);
		D = SU * CY + CU * SY * CF;
		lat2 = atan2(D, C);
		C = CU * CY - SU * SY * CF;
		X = MathUtils.fast_atan2(SY * SF, C);
	 ...
	 */
	public void computeDirection() throws IllegalStateException {
		if (!destinationValid) {
		...
		/*
		 * Computes the limit in longitude (alimit), it is equal to twice the distance
		 * from the equator to the pole, as measured along the equator.
		 */
		...
		final double u1 = MathUtils.fast_atan2(fo * sin(lat1), cos(lat1));
		final double u2 = MathUtils.fast_atan2(fo * sin(lat2), cos(lat2));
		...
		do {
			...
			// writer.write((slon*cu2) + " : " + (su2*cu1 - su1*cu2*clon) + "\n");
			// writer.flush();
			// ssig = hypot(slon*cu2, su2*cu1 - su1*cu2*clon);
			ssig = MathUtils.calcAbsoluteValue(slon * cu2, su2 * cu1 - su1 * cu2 * clon);
			sig = MathUtils.fast_atan2(ssig, csig);
		...
		if ((abs(su1) >= TOLERANCE_0) || (abs(su2) >= TOLERANCE_0)) {
			...
			az1 = MathUtils.fast_atan2(sina1, sina1 / tana1);
		}
		azimuth = castToAngleRange(az1);
		directionValid = true;
		return;
	 ...
	 * setDestinationGeographicPoint}(...)</code> has been invoked after. In this
	 * later case, the azimuth will be computed from the
	 * {@linkplain #getStartingGeographicPoint starting point} to the destination
	 * point.
	 *
	 * @return The azimuth, in decimal degrees from -180° to +180°.
	 * @throws IllegalStateException if the destination point has not been set.
	 *
	 * @todo Current implementation will provides an innacurate value for antipodal
	 *       points. For now a warning is logged in such case. In a future version
	 *       (if we have volunter time) we should provides a solution (search
	 *       Internet for "<cite>azimuth antipodal points</cite>").
	 */
	@Override
	public double getAzimuth() throws IllegalStateException {
		if (!directionValid) {
			computeDirection();
			/*
			 * if (antipodal) { Logging.getLogger(GeodeticCalculator.class).warning(
	 ...
	 * {@link Position} objects. This is the CRS specified at
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) construction
	 * time}.
	 *
	 ...
	 * setDestinationGeographicPoint}(...)</code> method, <strong>except</strong> if
	 * <code>{@linkplain #setDirection(double,double) setDirection}(...)</code> has
	 * been invoked after. In this later case, the destination point will be
	 * computed from the {@linkplain #getStartingGeographicPoint starting point} to
	 * the azimuth and distance specified.
	 *
	...
	/**
	 * Returns the destination position in user coordinates, which doesn't need to
	 * be geographic. The coordinate reference system is the one specified to the
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) constructor}.
	 *
	 * @return The destination position in user CRS.
	 * @throws TransformException if the position can't be transformed to user
	 *                            coordinates.
	 *
	 * @since 2.2
	 */
	 ...
	 * A curve in the ellipsoid is a path which points contain the longitude and
	 * latitude of the points in the geodetic curve. The geodetic curve is computed
	 * from the {@linkplain #getStartingGeographicPoint starting point} to the
	 * {@linkplain #getDestinationGeographicPoint destination point}.
	 *
	 ...
	 *                       <strong>NOTE:</strong> This argument is only a hint and
	 *                       may be ignored in future version (if we compute a real
	 *                       curve rather than a list of line segments).
	 * @return The path that represents the geodetic curve from the
	 *         {@linkplain #getStartingGeographicPoint starting point} to the
	 *         {@linkplain #getDestinationGeographicPoint destination point}.
	 *
	 * @todo We should check for cases where the path cross the 90°N, 90°S, 90°E or
	 *       90°W boundaries.
	 */
	public Shape getGeodeticCurve(final int numberOfPoints) {
		...
		final GeneralPath path = new GeneralPath(Path2D.WIND_EVEN_ODD, numberOfPoints + 1);
	 ...
	 * {@link Point2D} objects. This is inferred from the CRS specified at
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) construction
	 * time}.
	 *
	...
	/**
	 * Calculates the meridian arc length between two points in the same meridian in
	...
	private double getMeridianArcLengthRadians(final double P1, final double P2) {
		/*
		 * Latitudes P1 and P2 in radians positive North and East. Forward azimuths at
		 * both points returned in radians from North.
		 *
		 * Source: ftp://ftp.ngs.noaa.gov/pub/pcsoft/for_inv.3d/source/inverse.for
		 * subroutine GPNARC version 200005.26 written by Robert (Sid) Safford
		 *
		 * Ported from Fortran to Java by Daniele Franzoni.
		 */
		double S1 = abs(P1);
		double S2 = abs(P2);
		final double DA = (P2 - P1);
	...
	private String getNoConvergenceErrorMessage() {
		...
		return Errors.format(ErrorKeys.NO_CONVERGENCE_$2, format(cf, long1, lat1), format(cf, long2, lat2));
	}

	/**
	 * Returns the orthodromic distance (expressed in meters). This method returns
	 * the value set by the last call to
	 * <code>{@linkplain #setDirection(double,double) setDirection}(azimuth,distance)</code>,
	 * <strong>except</strong> if
	 * <code>{@linkplain #setDestinationGeographicPoint(double,double)
	 * setDestinationGeographicPoint}(...)</code> has been invoked after. In this
	 * later case, the distance will be computed from the
	 * {@linkplain #getStartingGeographicPoint starting point} to the destination
	 * point.
	 *
	...
	public double getOrthodromicDistance() throws IllegalStateException {
		if (!directionValid) {
			...
			if (antipodal) {
				...
				if (ellipsoid instanceof DefaultEllipsoid) {
					return ((DefaultEllipsoid) ellipsoid).orthodromicDistance(toDegrees(long1), toDegrees(lat1),
	...
	/**
	 * Returns the starting point in geographic coordinates. The <var>x</var> and
	 * <var>y</var> coordinates are the longitude and latitude in decimal degrees,
	 * respectively. If the starting point has never been set, then the default
	 * value is (0,0).
	 *
	...
	/**
	 * Returns the starting position in user coordinates, which doesn't need to be
	 * geographic. The coordinate reference system is the one specified to the
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) constructor}.
	 *
	...
	/**
	 * Set the destination point in geographic coordinates. The azimuth and distance
	 * values will be updated as a side effect of this call. They will be recomputed
	 * the next time {@link #getAzimuth()} or {@link #getOrthodromicDistance()} are
	 * invoked.
	 *
	 ...
	 * @throws IllegalArgumentException if the longitude or the latitude is out of
	 *                                  bounds.
	 *
	 * @since 2.3
	 */
	...
	/**
	 * Set the destination point in geographic coordinates. The <var>x</var> and
	 * <var>y</var> coordinates must be the longitude and latitude in decimal
	...
	/**
	 * Set the destination position in user coordinates, which doesn't need to be
	 * geographic. The coordinate reference system is the one specified to the
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) constructor}.
	 *
	 ...
	 * {@linkplain #getStartingGeographicPoint starting point}. The destination
	 * point will be updated as a side effect of this call. It will be recomputed
	 * the next time {@link #getDestinationGeographicPoint()} is invoked.
	 *
	 ...
	 * @throws IllegalArgumentException if the azimuth or the distance is out of
	 *                                  bounds.
	 *
	 * @see #getAzimuth
	 * @see #getOrthodromicDistance
	...
	/**
	 * Set the starting point in geographic coordinates. The
	 * {@linkplain #getAzimuth() azimuth}, the {@linkplain #getOrthodromicDistance()
	 * orthodromic distance} and the {@linkplain #getDestinationGeographicPoint()
	...
	/**
	 * Set the starting point in geographic coordinates. The <var>x</var> and
	 * <var>y</var> coordinates must be the longitude and latitude in decimal
	 * degrees, respectively.
	 *
	 * This is a convenience method for
	...
	/**
	 * Set the starting position in user coordinates, which doesn't need to be
	 * geographic. The coordinate reference system is the one specified to the
	 * {@linkplain #GeodeticCalculator(CoordinateReferenceSystem) constructor}.
	 *
	 * @param position The position in user coordinate reference system.
