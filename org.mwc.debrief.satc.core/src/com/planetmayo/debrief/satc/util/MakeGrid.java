...
public class MakeGrid {
	 ...
	 *
	 * @param p_geom      the area to cover (typically a polygon)
	 * @param numPoints   how many points to generate (roughly, see below)
	 * @param p_precision
	 * @return
	 * @throws Exception
	 *
	 *                   we aren't able reliable produce the correct amount. If you
	 *                   need an exact amount, the best I can suggest is to generate
	 *                   too many, and remove some - either at random, or from one
	 *                   end of the list
	 *
	 *                   This is a streamlined version of a class file provided in
	 *                   the JTS mailing list:
	...
	public static ArrayList<Point> ST_Tile(final Geometry p_geom, final int numPoints, final int p_precision) {
		...
		final String c_s_empty_geom = "Geometry must not be null or empty";
		final String c_s_unsupported = "Unsupported geometry type (*GTYPE*)";
		final double v_half_x = interval / 2.0, v_half_y = interval / 2.0;
		...
		if (!(p_geom.getGeometryType().equalsIgnoreCase("Point")
			...
			throw new RuntimeException(c_s_unsupported.replace("GTYPE", p_geom.getGeometryType()));
		}

		final PrecisionModel pm = new PrecisionModel();
		final GeometryFactory gf = new GeometryFactory(pm, p_geom.getSRID());
		...
		for (int v_col = v_loCol; v_col <= v_hiCol; v_col++) {
			for (int v_row = v_loRow; v_row <= v_hiRow; v_row++) {
				v_geometry = gf
						.createPoint(new Coordinate((v_col * interval) + v_half_x, (v_row * interval) + v_half_y));
				...
				final RelateOp ro = new RelateOp(v_clip_geom, p_geom);
				final IntersectionMatrix im = ro.getIntersectionMatrix();
