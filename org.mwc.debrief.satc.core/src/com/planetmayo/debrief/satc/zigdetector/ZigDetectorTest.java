...
public class ZigDetectorTest {
	...
	public static class LegStorer implements ILegStorer {
		...
		public void storeLeg(final String scenarioName, final long tStart, final long tEnd, final double rms) {
			System.out.println("Storing " + scenarioName + " : " + dateF.format(new Date(tStart)) + " - "
...
//			}
	...
	public static void main(final String[] args) throws Exception {
		...
		final NewValueListener newL = new NewValueListener() {
			...
			public void newValue(final String name, final double val) {
				...
				while (iterator.hasNext()) {
					...
					final String title = data._name + " Noise:" + numF.format(BRG_ERROR_SD) + " Conv:"
							+ expF.format(OPTIMISER_TOLERANCE) + " Zig:" + numF.format(RMS_ZIG_RATIO);
					...
					for (final Iterator<LegOfData> iterator2 = data.ownshipLegs.iterator(); iterator2.hasNext();) {
						...
						final List<Double> thisLegBearings = data.sensor.extractBearings(legStart, legEnd);
						final List<Long> thisLegTimes = data.sensor.extractTimes(legStart, legEnd);

						detector.sliceThis(SATC_Activator.getDefault().getLog(), SATC_Activator.PLUGIN_ID, name,
								legStart, legEnd, data.legStorer, null, RMS_ZIG_RATIO, OPTIMISER_TOLERANCE,
						...
						final TimeSeries thisSeries = new TimeSeries(thisLeg.getName() + " Slices");
						legResults.addSeries(thisSeries);
					}

					// plot the bearings
					Plotting.showBearings(data.bearingPlot, data.sensor.getTimes(), data.sensor.getBearings(),
							data.legStorer._rmsScores, data.legStorer.getLegs());
		...
		for (final Iterator<String> iterator = scenarios.iterator(); iterator.hasNext();) {
			...
			data.chartPanel = new ChartPanel(new JFreeChart("Results for " + name + " Tol:" + OPTIMISER_TOLERANCE,
		...
		while (iterator.hasNext()) {
			...
			data.ownshipLegs = legDetector.identifyOwnshipLegs(data.ownshipTrack.getDates(),
					data.ownshipTrack.getSpeeds(), data.ownshipTrack.getCourses(), 9);
			...
			data.ownshipPlot = Plotting.plotSingleVesselData(data._plot, "O/S", data.ownshipTrack, data.oShipColor,
					null, timeEnd);
...
//					data.ownshipTrack.getDates());
	...
	public static void main3(final String[] args) throws IOException {
		...
		// find the ownship legs
		final List<LegOfData> ownshipLegs = legDetector.identifyOwnshipLegs(ownshipTrack.getDates(),
				ownshipTrack.getSpeeds(), ownshipTrack.getCourses(), 5);
		// data.ownshipLegs = data.ownshipLegs.subList(2, 3);
		...
		final ILegStorer legStorer = new ILegStorer() {
			@Override
			public void storeLeg(final String scenarioName, final long tStart, final long tEnd, final double rms) {
				System.out.println("Leg identified from " + new Date(tStart) + " to " + new Date(tEnd));
			}
		};
		...
		for (final Iterator<LegOfData> iterator2 = ownshipLegs.iterator(); iterator2.hasNext();) {
			...
			legEnd = Math.min(legEnd, sensor.getTimes()[sensor.getTimes().length - 1]);

			final List<Double> thisLegBearings = sensor.extractBearings(legStart, legEnd);
			final List<Long> thisLegTimes = sensor.extractTimes(legStart, legEnd);

			detector.sliceThis(SATC_Activator.getDefault().getLog(), SATC_Activator.PLUGIN_ID, name, legStart, legEnd,
					legStorer, null, RMS_ZIG_RATIO, OPTIMISER_TOLERANCE, thisLegTimes, thisLegBearings);
	...
	public static String out(final Minimisation res) {
		...
		final String out = " B:" + numF.format(key[0]) + " P:" + numF.format(key[1]) + " Q:" + numF.format(key[2])
				+ " Sum:" + numF.format(res.getMinimum());
	...
	protected JPanel createControls(final NewValueListener newListener) {
		...
		// BRG_ERROR_SD));
		panel.add(createItem(OPTIMISER_THRESHOLD_STR, new double[] { 1e-4, 1e-5, 1e-6, 1e-7, 1e-8 }, newListener,
				expFormat, OPTIMISER_TOLERANCE));
		panel.add(createItem(RMS_ZIG_RATIO_STR, new double[] { 1.0, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3 }, newListener,
				decFormat, RMS_ZIG_RATIO));

		return panel;
	}

	protected JPanel createItem(final String label, final double[] values, final NewValueListener listener,
			final NumberFormat numberFormat, final double startValue) {
