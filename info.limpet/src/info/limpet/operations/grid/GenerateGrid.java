...
public class GenerateGrid implements IOperation {
	...
	private static interface DataProcessor {
		Document<?> getOutputDocument(final ICommand predecessor, final Unit<?> units);

		String outName();

		Dataset processGrid(final List<Double>[][] grid, final double[] oneBins, final double[] twoBins);
	}
	...
	public static class GenerateGridCommand extends AbstractCommand {
		...
		public GenerateGridCommand(final String title, final String description, final IStoreGroup store,
				final List<IStoreItem> inputs, final IContext context, final Triplet triplet, final double[] bins,
		...
		public double[] binsFor(final NumberDocument axis) {
			...
			if (axis.getUnits().equals(SampleData.DEGREE_ANGLE)) {
				...
				final DoubleDataset dd = (DoubleDataset) axis.getDataset();
				final double[] data = dd.getData();

				// ok, now bin the data
				final BinnedData binnedData = QuantityFrequencyBins.binTheseValues(data);

				// convert to array
				res = new double[binnedData.size()];
				int ctr = 0;
				for (final Bin d : binnedData) {
		...
		public void execute() {
			...
			final Document<?> nd = helper.getOutputDocument(this, triplet.measurements.getUnits());
		...
		@UIProperty(name = "BinSize", category = UIProperty.CATEGORY_CALCULATION, min = 1, max = 45)
		...
		public void performCalc() {
			...
			// get the output doc
			final Document<?> nd = super.getOutputs().get(0);
	...
	private class MeanHelper implements DataProcessor {
		...
		@Override
		public Document<?> getOutputDocument(final ICommand predecessor, final Unit<?> units) {
		...
		@Override
		public Dataset processGrid(final List<Double>[][] grid, final double[] oneBins, final double[] twoBins) {
	...
	public static List<Triplet> findPermutations(final List<IStoreItem> selection,
		...
		if (selection.size() == 3 && aTests.allEqualIndexed(selection)) {
			final Map<Unit<?>, ArrayList<NumberDocument>> matches = new HashMap<Unit<?>, ArrayList<NumberDocument>>();
			...
			for (final IStoreItem item : selection) {
				if (item instanceof NumberDocument) {
					...
					final double thisMin = doc.getIndexAt(0);
					final double thisMax = doc.getIndexAt(doc.size() - 1);
			...
			if (minIndexVal < maxIndexVal) {
				...
				if (matches.size() == 1) {
					...
					final ArrayList<NumberDocument> list = matches.get(matches.keySet().iterator().next());
					...
					for (final ArrayList<NumberDocument> thisL : matches.values()) {
						if (thisL.size() == 1) {
							...
							throw new IllegalArgumentException("Unable to organise inputs for gridding operation");
						}
					}
		...
		if (perms != null && perms.size() > 0) {
			...
			for (final Triplet thisP : perms) {
				...
				if (aUnits.equals(SampleData.DEGREE_ANGLE)) {
					...
					final String sampleTitle = "Collate 360 degree grid of samples of" + thisP.measurements
							+ " based on " + thisP.axisOne + " and " + thisP.axisTwo;
					final String meanTitle = "Collate 360 degree grid of mean of " + thisP.measurements + " based on "
							+ thisP.axisOne + " and " + thisP.axisTwo;

					// produce the description
					final String description = "Collate grid from data";

					res.add(new GenerateGridCommand(meanTitle, description, destination, selection, context, thisP,
							null, meanHelper));

					res.add(new GenerateGridCommand(sampleTitle, description, destination, selection, context, thisP,
							null, sampleHelper));
				} else {
					// produce the title
					final String sampleTitle = "Collate grid of samples of " + thisP.measurements + " based on "
							+ thisP.axisOne + " and " + thisP.axisTwo;
					final String meanTitle = "Collate grid of samples of " + thisP.measurements + " based on "
							+ thisP.axisOne + " and " + thisP.axisTwo;
	...
	protected Dataset doMeanGrid(final List<Double>[][] grid, final double[] oneBins, final double[] twoBins) {
		...
		final DoubleDataset ds = (DoubleDataset) DatasetFactory.createFromObject(means);

		return ds;
	}

	protected Dataset doSampleGrid(final List<Double>[][] grid, final double[] oneBins, final double[] twoBins) {
		final ObjectDataset ds = DatasetFactory.zeros(ObjectDataset.class,
				new int[] { oneBins.length, twoBins.length });
