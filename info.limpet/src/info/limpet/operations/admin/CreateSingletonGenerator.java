...
public class CreateSingletonGenerator implements IOperation {
	...
	public class CreateSingletonCommand extends AbstractCommand {
		...
		public CreateSingletonCommand(final String title, final StoreGroup group, final IStoreGroup store,
		...
		public void execute() {
			...
			final String name = getContext().getInput("New variable", "Enter name for variable", def_name);
			if (name == null || name.isEmpty()) {
				return;
			}

			final String valueStr = getContext().getInput("New variable", "Enter initial value for variable", "100");
			...
			try {
				...
				final String range = getContext().getInput("New variable",
						"Enter the range for variable (or cancel to leave un-ranged)", "0:100");

				final NumberDocument newData = generate(name, value, this);
				...
				if (range != null) {
					...
					if (tokens.length == 2) {
						try {
							final double lower = Double.parseDouble(tokens[0]);
							final double upper = Double.parseDouble(tokens[1]);
							newData.setRange(new Range(lower, upper));
						} catch (final NumberFormatException ndr) {
						}
					}
				}
	...
	public CreateSingletonGenerator(final String name, final Unit<?> unit) {
		_name = name;
		_unit = unit;
	}
		...
		if (appliesTo(selection)) {
			...
			if (selection.size() == 1) {
				...
				if (first instanceof StoreGroup) {
					final StoreGroup group = (StoreGroup) first;
	...
	private boolean appliesTo(final List<IStoreItem> selection) {
		...
		final boolean singleGroupSelected = getATests().exactNumber(selection, 1) && getATests().allGroups(selection);
		return getATests().exactNumber(selection, 0) || singleGroupSelected;
	}

	protected NumberDocument generate(final String name, final double value, final ICommand precedent) {
		final NumberDocumentBuilder builder = new NumberDocumentBuilder(name, _unit, precedent, null);
		builder.add(value);
