...
public abstract class AbstractCommand implements ICommand {
	...
	public AbstractCommand(final String title, final String description, final IStoreGroup store, final boolean canUndo,
			final boolean canRedo, final List<IStoreItem> inputs, final IContext context) {
		this.title = title;
	...
	public void collectionDeleted(final IStoreItem subject) {
		...
		final ArrayList<IStoreItem> toDelete2 = new ArrayList<IStoreItem>();
	...
	final protected LocationDocument locationsFor(final LocationDocument track1, final Document<?> times,
			final TimePeriod period) {
		// ok, get the time values
		final AxesMetadata axis = times.getDataset().getFirstMetadata(AxesMetadata.class);
	...
	final protected LocationDocument locationsFor(final LocationDocument track, final double[] times,
			final TimePeriod period) {
		// trim the times to the period
		final ArrayList<Double> dTimes = new ArrayList<Double>();
		for (final double thisT : times) {
			if (period.contains(thisT)) {
				dTimes.add(thisT);
			}
		}
		final DoubleDataset ds = (DoubleDataset) DatasetFactory.createFromObject(dTimes);
		...
		if (track.size() == 1) {
			ldb = new LocationDocumentBuilder("Interpolated locations", null, indexUnits);
			...
			final DoubleDataset latDataset = DatasetFactory.createFromObject(DoubleDataset.class, latVals);
			final DoubleDataset DoubleDataset = DatasetFactory.createFromObject(DoubleDataset.class, longVals);
			final DoubleDataset timeDataset = DatasetFactory.createFromObject(DoubleDataset.class, timeVals);

			final DoubleDataset latInterpolated = (DoubleDataset) Maths.interpolate(timeDataset, latDataset, ds, 0, 0);
			final DoubleDataset longInterpolated = (DoubleDataset) Maths.interpolate(timeDataset, DoubleDataset, ds, 0,
					0);

			// ok, now we need to re-create a locations document
			ldb = new LocationDocumentBuilder("Interpolated locations", null, indexUnits);
			for (int i = 0; i < ds.getSize(); i++) {
				final Point2D pt = track.getCalculator().createPoint(longInterpolated.getDouble(i),
	...
	final protected NumberDocument numbersFor(final NumberDocument document, final Document<?> times,
			final TimePeriod period) {
		// ok, get the time values
		final AxesMetadata axis = times.getDataset().getFirstMetadata(AxesMetadata.class);
		final DoubleDataset ds = (DoubleDataset) axis.getAxes()[0];
	...
	final protected NumberDocument numbersFor(final NumberDocument document, final double[] times,
			final TimePeriod period) {
		// trim the times to the period
		final ArrayList<Double> dTimes = new ArrayList<Double>();
		for (final double thisT : times) {
		...
		// we just keep re-using the same position
		if (headings.size() == 1) {
			ldb = new NumberDocumentBuilder("Interpolated headings", document.getUnits(), null, indexUnits);
			...
			final DoubleDataset hdgDataset = DatasetFactory.createFromObject(DoubleDataset.class, headings);
			final DoubleDataset timeDataset = DatasetFactory.createFromObject(DoubleDataset.class, timeVals);

			final DoubleDataset hdgInterpolated = (DoubleDataset) Maths.interpolate(timeDataset, hdgDataset, ds, 0, 0);
			final DoubleDataset timeInterpolated = (DoubleDataset) Maths.interpolate(timeDataset, timeDataset, ds, 0,
					0);

			// ok, now we need to re-create a locations document
			ldb = new NumberDocumentBuilder("Interpolated locations", document.getUnits(), null, indexUnits);
			for (int i = 0; i < ds.getSize(); i++) {
