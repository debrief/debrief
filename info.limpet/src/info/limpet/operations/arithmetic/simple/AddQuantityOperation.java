...
public class AddQuantityOperation extends BulkQuantityOperation {
	...
	public class AddQuantityValues extends BulkQuantityCommand {
		...
		public AddQuantityValues(final String name, final List<IStoreItem> selection, final IStoreGroup destination,
				final IContext context, final String outputPrefix, final IOperationPerformer operation) {
		...
		@Override
		protected String getBulkNameFor(final List<IStoreItem> items) {
			String res = "";
		...
		@Override
		protected Unit<?> getBulkOutputUnit(final List<Unit<?>> units) {
			return units.get(0);
		}

		@Override
		protected DoubleDataset getInitial(final int shape) {
	...
	private static class LogAdder implements InterpolatedMaths.IOperationPerformer {
		@Override
	...
	protected void addIndexedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (hasLogData(selection)) {
			...
			ICommand newC = new AddQuantityValues("Logarithmic Add for provided series (indexed)", selection,
					destination, context, "Log ", logAdder);
			res.add(newC);

			// just offer the log operation
			newC = new AddQuantityValues("Power Add for provided series (indexed)", selection, destination, context,
					"Power ", powerAdder);
			res.add(newC);

		} else {
			// just offer the log operation
			final ICommand newC = new AddQuantityValues("Add numeric values in provided series (indexed)", selection,
	...
	protected void addInterpolatedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (hasLogData(selection)) {
			...
			ICommand newC = new AddQuantityValues("Logarithmic Add for provided series (interpolated)", selection,
					destination, context, "Log ", logAdder);
			res.add(newC);

			newC = new AddQuantityValues("Power Add for provided series (interpolated)", selection, destination,
					context, "Power ", powerAdder);
			res.add(newC);

		} else {
			final ICommand newC = new AddQuantityValues("Add numeric values in provided series (interpolated)",
	...
	protected boolean appliesTo(final List<IStoreItem> selection) {
		...
		final boolean suitableLength = getATests().allEqualIndexedOrSingleton(selection)
		...
		return atLeastTwoItems && nonEmpty && allQuantity && suitableLength && equalDimensions && equalUnits;
	}
