...
public class MultiplyQuantityOperation extends BulkQuantityOperation {
	...
	public class MultiplyQuantityValues extends BulkQuantityCommand {
		public MultiplyQuantityValues(final String name, final List<IStoreItem> selection,
				final IStoreGroup destination, final IContext context) {
			super(name, "Multiply datasets", destination, false, false, selection, context);
		}
	...
	protected void addIndexedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		final ICommand newC = new MultiplyQuantityValues("Multiply numeric values in provided series (indexed)",
	...
	protected void addInterpolatedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
			final Collection<ICommand> res, final IContext context) {
		final ICommand newC = new MultiplyQuantityValues("Multiply numeric values in provided series (interpolated)",
				selection, destination, context);
		res.add(newC);
	}
