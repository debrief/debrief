...
public class DivideQuantityOperation extends BinaryQuantityOperation {
	...
	public class DivideQuantityValues extends BinaryQuantityCommand {
		public DivideQuantityValues(final String name, final List<IStoreItem> selection, final IStoreGroup store,
				final IContext context) {
			this(name, selection, store, null, context);
		}

		public DivideQuantityValues(final String name, final List<IStoreItem> selection, final IStoreGroup destination,
				final IDocument<?> timeProvider, final IContext context) {
			super(name, "Divide datasets", destination, false, false, selection, timeProvider, context);
		}
	...
	protected void addIndexedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		ICommand newC = new DivideQuantityValues("Divide " + doc1 + " by " + doc2 + " (indexed)", selection,
				destination, context);
		res.add(newC);
		newC = new DivideQuantityValues("Divide " + doc2 + " by " + doc1 + " (indexed)", reverse(selection),
				destination, context);
		res.add(newC);
	}
	...
	protected void addInterpolatedCommands(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (longest != null) {
			...
			ICommand newC = new DivideQuantityValues("Divide " + doc1 + " by " + doc2 + " (interpolated)", selection,
					destination, longest, context);

			res.add(newC);

			// provide a reversed list
			newC = new DivideQuantityValues("Divide " + doc2 + " by " + doc1 + " (interpolated)", reverse(selection),
					destination, longest, context);
