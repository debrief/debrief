...
public class RateOfChangeOperation implements IOperation {
	...
	public static class RateOfChangeCommand extends CoreQuantityCommand {
		public RateOfChangeCommand(final IStoreGroup store, final List<IStoreItem> inputs, final IContext context) {
			super("Rate of change", "Calculate rate of change across indexed value", store, false, false, inputs,
					context);
		}
		...
		protected String generateName() {
			...
			return "\u0394 (" + valueUnits + "/" + indexUnits + ") for " + doc.getName();
		}
		...
		protected Unit<?> getUnits() {
			...
			final Unit<? extends Quantity> outputUnits = valueUnits.divide(indexUnits);
		...
		protected IDataset performCalc() {
			...
			final NumberDocumentBuilder output = new NumberDocumentBuilder(generateName(), getUnits(), this,
					doc.getIndexUnits());
