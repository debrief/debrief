...
public abstract class BinaryQuantityOperation implements IOperation {
	...
	public abstract class BinaryQuantityCommand extends CoreQuantityCommand {
		...
		public BinaryQuantityCommand(final String title, final String description, final IStoreGroup store,
		...
		public BinaryQuantityCommand(final String title, final String description, final IStoreGroup store,
				final boolean canUndo, final boolean canRedo, final List<IStoreItem> inputs,
		 ...
		 * @return
		 */
		abstract protected Unit<?> getBinaryOutputUnit(Unit<?> first, Unit<?> second);
		...
		protected IDataset performCalc() {
			...
			final DoubleDataset in1 = (DoubleDataset) ((NumberDocument) getInputs().get(0)).getDataset();
			final DoubleDataset in2 = (DoubleDataset) ((NumberDocument) getInputs().get(1)).getDataset();
			...
			if (axis1 != null) {
				...
				final Monotonicity axis1Mono = Comparisons.findMonotonicity(axis1.getAxes()[0]);
				...
				if (axis1Mono.equals(Monotonicity.NOT_ORDERED)) {
					...
					if (axis2 == null || axis2.getAxes() == null || axis2.getAxes().length == 0) {
						// the axes don't match. We can't do interp
						doInterp = false;
						// we'll use A indices in the output
						outputIndices = (Dataset) axis1.getAxes()[0];
					} else {
						final Monotonicity axis2Mono = Comparisons.findMonotonicity(axis2.getAxes()[0]);
						...
						if (axis1.getAxes()[0].equals(axis2.getAxes()[0])) {
							...
							// ok, not ordered. we can't use it
							throw new IllegalArgumentException("Axes must be ordered. Cannot progress");
						} else {
				...
				// first dataset doesn't have units. We aren't going to be
				// interpolating.
	...
	protected IDocument<?> getLongestIndexedCollection(final List<IStoreItem> selection) {
