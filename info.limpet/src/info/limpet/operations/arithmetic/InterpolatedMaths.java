...
public class InterpolatedMaths extends Maths {
	 ...
	 * mathematical operation being conducted. This interface wraps the
	 * operation in a Command
	 *
	 */
	public static interface IOperationPerformer {
		/**
		 * perform some operation on datasets a and b. If an output dataset is
		 * provided, store the results in there.
		 *
		 * @param a left-hand operand
		 * @param b right-hand operand
		 * @param o (optional) results object
		 * @return a dataset product
		 */
		public Dataset perform(final Dataset a, final Dataset b, final Dataset o);
	}

	private static final String ONLY_1D_DATASETS = "Only 1d datasets are currently covered by this interpolation mechanism";
	private static final String DUPLICATE_INDEX_VALUES_NOT_ALLOWED = "Duplicate index values not allowed";
	private static final String AXES_MUST_ASCEND_IN_SAME_DIRECTION = "The axes must both ascend in the same direction to use interpolation";
	private static final String AXES_MUST_BE_MONOTONIC = "The dataset axes must both be monotonic to use interpolation";
	private static final String INDICES_DO_NOT_OVERLAP = "The indices of the dataset do not overlap";
	 ...
	 * @return result of operation, resampling data if necessary to synchronise
	 * AxesMetadata
	 */
	public static Dataset performWithInterpolation(final Dataset da, final Dataset db, final Dataset o,
		...
		if (axesA == null || axesB == null) {
			throw new IllegalArgumentException("Both datasets must have axes metadata");
			...
			if (needInterp) {
				...
				if (aMono.equals(Monotonicity.NOT_ORDERED) || bMono.equals(Monotonicity.NOT_ORDERED)) {
					throw new IllegalArgumentException(AXES_MUST_BE_MONOTONIC);
				} else if (aMono.equals(Monotonicity.NONDECREASING) || bMono.equals(Monotonicity.NONDECREASING)) {
					throw new IllegalArgumentException(DUPLICATE_INDEX_VALUES_NOT_ALLOWED);
				} else {
					// check they're in the same direction
					if (aMono != bMono) {
						throw new IllegalArgumentException(AXES_MUST_ASCEND_IN_SAME_DIRECTION);
					}

					// ok, they're both monotonic, in the same direction. we can
					// continue.
				}
				...
				if (aMax < bMin || aMin > bMax) {
					...
					final Dataset interpolatedValues = Maths.interpolate(aIndices, da, bIndices, null, null);
					...
					final Dataset interpolatedValues = Maths.interpolate(bIndices, db, aIndices, null, null);
					// remember the output axes, since we'll put them
					// into the results
					...
					final Dataset aReverse = aIndices.getSliceView(null, null, new int[] { -1 });
					final Dataset bReverse = bIndices.getSliceView(null, null, new int[] { -1 });
					...
					if (aMono == Monotonicity.STRICTLY_INCREASING) {
						...
						final int aStartIndex = DatasetUtils.findIndexGreaterThanOrEqualTo(aIndices, startPoint);

						// now retrieve the n-dimensional position that matches
						// this index
						aStartPosition = aIndices.getNDPosition(aStartIndex)[0];

						// note: we reduce index by one to allow for
						// zero-indexing
						final int aEndIndex = DatasetUtils.findIndexLessThanOrEqualTo(aReverse, endPoint);
						aEndPosition = aReverse.getSize() - 1 - aReverse.getNDPosition(aEndIndex)[0];

						final int bStartIndex = DatasetUtils.findIndexGreaterThanOrEqualTo(bIndices, startPoint);
						bStartPosition = bIndices.getNDPosition(bStartIndex)[0];

						final int bEndIndex = DatasetUtils.findIndexLessThanOrEqualTo(bReverse, endPoint);
						bEndPosition = bReverse.getSize() - 1 - bReverse.getNDPosition(bEndIndex)[0];
					} else {
						// ok - data descending
						// get the index of the first point in the region
						final int aStartIndex = DatasetUtils.findIndexLessThanOrEqualTo(aIndices, endPoint);
						...
						// note: we reduce index by one to allow for
						// zero-indexing
						final int aEndIndex = DatasetUtils.findIndexGreaterThanOrEqualTo(aReverse, startPoint);
						aEndPosition = aReverse.getSize() - 1 - aReverse.getNDPosition(aEndIndex)[0];

						final int bStartIndex = DatasetUtils.findIndexLessThanOrEqualTo(bIndices, endPoint);
						bStartPosition = bIndices.getNDPosition(bStartIndex)[0];

						final int bEndIndex = DatasetUtils.findIndexGreaterThanOrEqualTo(bReverse, startPoint);
						bEndPosition = bReverse.getSize() - 1 - bReverse.getNDPosition(bEndIndex)[0];
					}

					// ok, retrieve the index values that are within the
					// intersecting period
					final Dataset aIndicesTrimmed = aIndices.getSliceView(new int[] { aStartPosition },
							new int[] { aEndPosition + 1 }, null);
					final Dataset bIndicesTrimmed = bIndices.getSliceView(new int[] { bStartPosition },
							new int[] { bEndPosition + 1 }, null);

					// the interpolation strategy we're going to adopt is that
					// we're
					...
					// ////
					final Dataset interpolatedValues = Maths.interpolate(refIndices, refValues, lookupIndices, null,
							null);
					interpolatedValues.setName(lookupValues + "(interp)");

					// we can just extract the trimmed set of a values
					final Dataset values = lookupValues.getSliceView(new int[] { itemStartPos },
							new int[] { itemEndPos + 1 }, null);

					// clear the metadata from this, since we'll change the axis
					// length
					...
					if (interpInB) {
						...
						// note: swap them over, so they remain in the correct
						// order for
	...
	private static void storeMetadata(final Dataset indicesTrimmed, final Dataset datasetA, final Dataset datasetB) {
