...
public class SimpleMovingAverageOperation implements IOperation {
	public static class SimpleMovingAverageCommand extends AbstractCommand implements RangedEntity {
		...
		public SimpleMovingAverageCommand(final String operationName, final List<IStoreItem> selection,
				final IStoreGroup store, final int windowSize, final IContext context) {
			super(operationName, "Calculates a Simple Moving Average", store, false, false, selection, context);
			winSize = windowSize;
		}

		protected String getOutputName() {
			return getContext().getInput("Generate simple moving average", NEW_DATASET_MESSAGE,
					"Moving average of " + super.getSubjectList());
		}
		...
		//
		// protected NumberDocument getOutputFor(NumberDocument input, String
		// outName)
		// {
		...
		// /**
		// * wrap the actual operation. We're doing this since we need to
		...
		// {
		// // use temporal data
		// ITemporalQuantityCollection<?> outT =
		// (ITemporalQuantityCollection<?>) target;
		// ITemporalQuantityCollection<?> inT = (ITemporalQuantityCollection<?>)
		// input;
		//
	...
	public List<ICommand> actionsFor(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (appliesTo(selection)) {
			final ICommand newC = new SimpleMovingAverageCommand(SERIES_NAME_TEMPLATE, selection, destination,
					_windowSize, context);
			res.add(newC);
		}
