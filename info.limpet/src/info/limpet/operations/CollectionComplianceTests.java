...
public class CollectionComplianceTests {
	...
	public static class TimePeriod {
		...
		public boolean overlaps(final TimePeriod his) {
	 ...
	 * @return yes/no
	 */
	public boolean allChildrenAreTracks(final List<IStoreItem> selection) {
		boolean res = true;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
			if (storeItem instanceof IStoreGroup) {
				// ok, check the contents
				final IStoreGroup group = (IStoreGroup) storeItem;
	...
	public boolean allCollections(final List<IStoreItem> selection) {
		boolean res = true;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
	 ...
	 * @return true/false
	 */
	public boolean allEqualDimensions(final List<IStoreItem> selection) {
		...
		for (int i = 0; i < selection.size(); i++) {
			...
			if (thisI instanceof NumberDocument) {
				final NumberDocument thisC = (NumberDocument) thisI;
				if (thisC.getUnits() != null) {
					final Dimension thisD = thisC.getUnits().getDimension();
					if (theD == null) {
						theD = thisD;
					} else {
	 ...
	 * @return true/false
	 */
	public boolean allEqualDistanceUnits(final List<IStoreItem> selection) {
		...
		for (int i = 0; i < selection.size(); i++) {
			...
			if (thisI instanceof LocationDocument) {
				final LocationDocument doc = (LocationDocument) thisI;
				final Unit<?> hisUnits = doc.getUnits();
				if (distUnits == null) {
					// ok, store it
					distUnits = hisUnits;
				} else {
	 ...
	 * @return true/false
	 */
	public boolean allEqualIndexed(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
	 ...
	 * @return true/false
	 */
	public boolean allEqualIndexedOrSingleton(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
		Unit<?> _indexUnits = null;
	 ...
	 * @return true/false
	 */
	public boolean allEqualLength(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
		int size = -1;
	 ...
	 * @return true/false
	 */
	public boolean allEqualLengthOrSingleton(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
		int size = -1;

		for (int i = 0; i < selection.size(); i++) {
			final IStoreItem thisC = selection.get(i);

			if (thisC instanceof IDocument) {
				final IDocument<?> thisD = (IDocument<?>) thisC;
				final int thisSize = thisD.size();
	 ...
	 * @return true/false
	 */
	public boolean allEqualUnits(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
		Unit<?> theD = null;
		...
		for (int i = 0; i < selection.size(); i++) {
			...
			if (thisI instanceof NumberDocument) {
				final NumberDocument thisC = (NumberDocument) thisI;
				final Unit<?> thisD = thisC.getUnits();
				if (theD == null) {
					theD = thisD;
	...
	public boolean allGroups(final List<IStoreItem> selection) {
		boolean res = true;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
	...
	public boolean allHaveData(final List<IStoreItem> selection) {
		// are they all non location?
		boolean allValid = true;
	 ...
	 * @param dimension we're looking for
	 * @return yes/no
	 */
	public boolean allHaveDimension(final List<IStoreItem> kids, final Dimension dim) {
		boolean res = true;

		for (final IStoreItem sItem : kids) {
			if (sItem instanceof IDocument) {
				final IDocument<?> item = (IDocument<?>) sItem;

				if (item.isQuantity()) {
					final NumberDocument coll = (NumberDocument) item;
	 ...
	 * @return true/false
	 */
	public boolean allIndexed(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
	 ...
	 * @return true/false
	 */
	public boolean allIndexedOrSingleton(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = false;
	 ...
	 * @return true/false
	 */
	public boolean allLocation(final List<IStoreItem> selection) {
		// are they all non location?
		boolean allValid = true;
	 ...
	 * @return true/false
	 */
	public boolean allNonIndexed(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
	 ...
	 * @return true/false
	 */
	public boolean allNonLocation(final List<IStoreItem> selection) {
		// are they all non location?
		boolean allValid = true;
	 ...
	 * @return true/false
	 */
	public boolean allNonQuantity(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = true;
	 ...
	 * @return
	 */
	public boolean allOneDim(final List<IStoreItem> selection) {
	 ...
	 * @return true/false
	 */
	public boolean allQuantity(final List<IStoreItem> selection) {
		// are they all temporal?
	 ...
	 * @return
	 */
	public boolean allTwoDim(final List<IStoreItem> selection) {
		return checkDims(selection, 2);
	}

	private boolean checkDims(final List<IStoreItem> selection, final int requiredDims) {
		// are they all non location?
		boolean allValid = true;

		for (final IStoreItem thisI : selection) {
			if (thisI instanceof Document) {
				final Document<?> thisC = (Document<?>) thisI;
				final Dataset d = (Dataset) thisC.getDataset();
				final int[] dims = d.getShape();
	 ...
	 * @return yes/no
	 */
	public NumberDocument findCollectionWith(final Collection<IStoreItem> kids, final Dimension dimension,
			final boolean walkTree) {
		NumberDocument res = null;

		final Iterator<IStoreItem> iter = kids.iterator();
		while (iter.hasNext()) {
	...
	public TimePeriod getBoundingRange(final Collection<IStoreItem> items) {
		...
		final Iterator<IStoreItem> iter = items.iterator();
		while (iter.hasNext()) {
			final IDocument<?> iCollection = (IDocument<?>) iter.next();

			// allow for empty value. sometimes our logic allows null objects
			// for some
			// data types
			if (iCollection != null && iCollection.isIndexed() && iCollection.size() > 1) {
				final TimePeriod hisPeriod = getBoundsFor(iCollection);
	...
	public TimePeriod getBoundsFor(final IDocument<?> iCollection) {
		TimePeriod res = null;

		if (iCollection.isIndexed()) {
			final Iterator<Double> lIter = iCollection.getIndexIterator();
			Double start = null;
			Double end = null;
			while (lIter.hasNext()) {
				final double thisT = lIter.next();
				if (start == null) {
					start = thisT;
				}
	 ...
	 * @return yes/no
	 */
	public ArrayList<IStoreGroup> getChildTrackGroups(final List<IStoreItem> selection) {
		final ArrayList<IStoreGroup> res = new ArrayList<IStoreGroup>();
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
			if (storeItem instanceof IStoreGroup) {
				// ok, check the contents
				final IStoreGroup group = (IStoreGroup) storeItem;
				final Collection<IStoreItem> kids = group;

				final Iterator<IStoreItem> kIter = kids.iterator();
				while (kIter.hasNext()) {
					final IStoreItem storeItem2 = kIter.next();
	...
	public List<IDocument<?>> getDocumentsIn(final Collection<IStoreItem> selection) {
		final List<IDocument<?>> res = new ArrayList<IDocument<?>>();

		for (final IStoreItem sel : selection) {
	 ...
	 * @return true/false
	 */
	public LocationDocument getFirstLocation(final IStoreGroup track) {
		final Iterator<IStoreItem> iter = track.iterator();
		while (iter.hasNext()) {
			final IStoreItem thisI = iter.next();
	...
	public int getLongestCollectionLength(final List<IStoreItem> selection) {
	...
	public IDocument<?> getLongestIndexedCollection(final List<IStoreItem> selection) {
		// find the longest time series.
		final Iterator<IStoreItem> iter = selection.iterator();
		IDocument<?> longest = null;

		while (iter.hasNext()) {
			final IStoreItem thisC = iter.next();
			if (thisC instanceof IDocument) {
				final IDocument<?> thisD = (IDocument<?>) thisC;
				if (thisD.isIndexed() && (thisD.isQuantity() || thisC instanceof LocationDocument)) {
	...
	public int getNumberOfGroups(final List<IStoreItem> selection) {
		int res = 0;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
	 ...
	 * @return yes/no
	 */
	public int getNumberOfTracks(final List<IStoreItem> selection) {
		int count = 0;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
			...
			if (storeItem instanceof IStoreGroup) {
				...
				// special case: we can miss out course & speed if there's just
				// a single
	...
	/**
	 * find the best collection to use as a interpolation-base. Which collection
	 * has the most values within the specified range?
	 *
	 ...
	 */
	public IDocument<?> getOptimalIndex(final TimePeriod period, final Collection<IStoreItem> items) {
		IDocument<?> res = null;
		long resScore = 0;
		...
		while (iter.hasNext()) {
			...
			// occasionally we may store a null dataset, since it is optional in
			// some
			// circumstances
	 ...
	 * @return true/false
	 */
	public boolean hasIndexed(final List<IStoreItem> selection) {
		// are they all temporal?
		boolean allValid = false;
	 ...
	 * @return yes/no
	 */
	private boolean hasLocation(final Collection<IStoreItem> kids) {
		boolean res = false;
	 ...
	 * @return true/false
	 */
	public boolean hasSingleton(final List<IStoreItem> selection) {
		boolean res = false;
		for (int i = 0; i < selection.size(); i++) {
			final IStoreItem thisI = selection.get(i);
			if (thisI instanceof IDocument) {
	...
	private boolean hasSingletonLocation(final IStoreGroup group) {
		boolean res = false;
	 ...
	 * @return yes/no
	 */
	public boolean isATrack(final IStoreGroup group) {
	 ...
	 * @return yes/no
	 */
	public boolean isDimensionPresent(final Collection<IStoreItem> kids, final Dimension dim) {
		boolean res = false;
	...
	// /**
	// * retrieve the location at the specified time (even if it's a
	// non-temporal collection)
	// *
	// * @param iCollection
	 ...
	 *
	 * @param items to check
	 * @param units we're looking for
	 * @return yes/no
	 */
	public boolean isUnitPresent(final Collection<IStoreItem> kids, final Unit<?> dim) {
		boolean res = false;
	...
	public boolean minNumberOfGroups(final List<IStoreItem> selection, final int count) {
		int res = 0;
		final Iterator<IStoreItem> iter = selection.iterator();
		while (iter.hasNext()) {
			final IStoreItem storeItem = iter.next();
	 ...
	 * @return true/false
	 */
	public boolean nonEmpty(final List<IStoreItem> selection) {
		return selection.size() > 0;
	}

	private void processThis(final List<IDocument<?>> target, final IStoreGroup selection) {
		for (final IStoreItem sel : selection) {
			if (sel instanceof IDocument) {
	 ...
	 * @return yes/no
	 */
	public IDocument<?> someHaveLocation(final Collection<IStoreItem> kids) {
		IDocument<?> res = null;
	...
	/**
	 * determine if these datasets are suited to an indexed operation - where we
	 * interpolate time values
	 *
	 * @param selection
	 * @return
	 */
	public boolean suitableForIndexedInterpolation(final List<IStoreItem> selection) {
		// are suitable
			...
			for (int i = 0; i < selection.size(); i++) {
				...
				if (thisI instanceof Document) {
					...
					if (thisC.isQuantity() || thisC instanceof LocationDocument) {
						// note: we only consider it as suitable for
						// interpolation if it
						// has more than one value. With just one value we're
						// better
						// treating it as indexed
						if (thisC.isIndexed()) {
							final Unit<?> thisUnit = thisC.getIndexUnits();
							...
							// check the axis range, unless it's a singleton. If
							// it's
							// a singleton we ignore the index value, and assume
							// it's
							// persistent across the whole period
							if (thisC.size() > 1) {
								// NumberDocument nd = (NumberDocument) thisC;
								final AxesMetadata axes = thisC.getDataset().getFirstMetadata(AxesMetadata.class);
								final DoubleDataset axesDataset = (DoubleDataset) axes.getAxes()[0];
								final long thisStart = axesDataset.getLong(0);
								final long thisEnd = axesDataset.getLong(axesDataset.getSize() - 1);
	...
	/**
	 * retrieve the value at the specified index (even if it's a non-temporal
	 * collection)
	 *
	 ...
	 * @return
	 */
	public double valueAt(final IDocument<?> iCollection, final long thisTime, final Unit<?> requiredUnits) {
		Double res = null;

		// just check it's not an empty set, since we return zero for empty
		// dataset
		if (iCollection == null) {
			return 0;
		} else if (iCollection.isQuantity()) {
			final NumberDocument iQ = (NumberDocument) iCollection;
			...
			if (iCollection.isIndexed()) {
				final NumberDocument tQ = (NumberDocument) iCollection;
				res = tQ.interpolateValue(thisTime, InterpMethod.Linear);
			} else {
				final NumberDocument qC = (NumberDocument) iCollection;
				res = qC.getIterator().next();
			}

			if (res != null) {
				final UnitConverter converter = iQ.getUnits().getConverterTo(requiredUnits);
				final double doubleValue = res;
				final double result = converter.convert(doubleValue);
				return result;
			} else {
				return 0;
			}
