...
public class ProplossBetweenTwoTracksOperation extends TwoTrackOperation {
	...
	private final class ProplossBetweenOperation extends TwoTrackCommand {
		private ProplossBetweenOperation(final List<IStoreItem> selection, final IStoreGroup store, final String title,
				final String description, final IContext context) {
			this(selection, store, title, description, null, context);
		}

		ProplossBetweenOperation(final List<IStoreItem> selection, final IStoreGroup store, final String title,
				final String description, final IDocument<?> timeProvider, final IContext context) {
			super(selection, store, title, description, timeProvider, context, NonSI.DECIBEL);
		}
		...
		protected String getOutputName() {
			return getContext().getInput("Generate propagation loss", NEW_DATASET_MESSAGE,
	...
	public List<ICommand> actionsFor(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (appliesTo(collatedTracks)) {
			...
			if (getATests().suitableForIndexedInterpolation(collatedTracks)) {
				...
				final ICommand newC = new ProplossBetweenOperation(collatedTracks, destination,
						"Propagation loss between tracks (interpolated)",
						"Calculate prop loss between two tracks (interpolated)", timeProvider, context);
				res.add(newC);
			} else if (getATests().allEqualLengthOrSingleton(collatedTracks)) {
				// ok, provide an indexed action
				final ICommand newC = new ProplossBetweenOperation(collatedTracks, destination,
						"Propagation loss between tracks (indexed)", "Calculate loss between two tracks (indexed)",
						context);
				res.add(newC);
			}
		}
