...
public class DopplerShiftBetweenTracksOperation implements IOperation {
	public static class DopplerShiftOperation extends AbstractCommand {
		...
		protected static class CompositeTrackWrapper implements TrackProvider {
			...
			private final NumberDocument _course;
			private final NumberDocument _speed;
			private LocationDocument _location;

			public CompositeTrackWrapper(final IStoreGroup track, final String name) {
				_name = name;

				// assign the components
				_course = aTests.findCollectionWith(track, Angle.UNIT.getDimension(), false);
				_speed = aTests.findCollectionWith(track, Velocity.UNIT.getDimension(), false);
			 ...
			 * (non-Javadoc)
			 *
			 * @see info.limpet.data.operations.spatial.
			 * DopplerShiftBetweenTracksOperation
			 * .DSOperation.TrackProvider#addDependent(info.limpet.IOperation)
			 */
			@Override
			public void addDependent(final ICommand operation) {
			...
			@Override
			public double getCourseAt(final long time) {
				return aTests.valueAt(_course, time, RADIAN.asType(Angle.class));
			}

			@Override
			public Point2D getLocationAt(final long time) {
			...
			@Override
			public double getSpeedAt(final long time) {
				return aTests.valueAt(_speed, time, METRE.divide(SECOND).asType(Velocity.class));
			}
		...
		protected static class SingletonWrapper implements TrackProvider {
			...
			private final String _name;
			private final LocationDocument _dataset;

			public SingletonWrapper(final String name, final LocationDocument loc) {
				_name = name;
				_dataset = loc;
			}
		...
		/**
		 * way to make singleton locations (that don't have course/speed) look
		 * like tracks
		 *
		 * @author ian
		 *
		 ...
		 * @return
		 */
		private static double calcPredictedFreqSI(final double speedOfSound, final double osHeadingRads,
			...
			// angle along the bearing, not the angle back down the bearing
			// (ATB).
		 ...
		 * @return
		 */
		public static List<TrackProvider> getTracks(final IStoreGroup ignoreMe, final List<IStoreItem> selection,
				final CollectionComplianceTests aTests) {
			final List<TrackProvider> res = new ArrayList<TrackProvider>();
			...
			while (iter.hasNext()) {
				...
				if (item != ignoreMe) {
					if (item instanceof LocationDocument) {
						final LocationDocument loc = (LocationDocument) item;
						res.add(new SingletonWrapper(loc.getName(), loc));
					} else if (item instanceof IStoreGroup) {
						// CHECK IF IT'S SUITABLE AS A TRACK. IF NOT, SEE IF IT
						// JUST
						// CONTAINS LOCATIONS
						// - THEN ADD THEM ALL
						//
						final IStoreGroup grp = (IStoreGroup) item;
						...
						if (aTests.isATrack(grp)) {
							...
							final Iterator<IStoreItem> iter2 = grp.iterator();
							while (iter2.hasNext()) {
								final IStoreItem iStoreItem = iter2.next();
								if (iStoreItem instanceof IDocument) {
									final Document<?> coll = (Document<?>) iStoreItem;
									if (coll.size() == 1) {
		...
		public DopplerShiftOperation(final StoreGroup transmitter, final IStoreGroup store, final String title,
				final String description, final List<IStoreItem> selection, final IContext context) {
		...
		public void execute() {
			...
			for (final IStoreItem c : _data.values()) {
				final IDocument<?> doc = (IDocument<?>) c;
				doc.addDependent(this);
			}

			// and the tracks
			for (final TrackProvider c : _allTracks) {
				c.addDependent(this);
			}
			...
			for (final IDocument<?> o : getOutputs()) {
				getStore().add(o);
			}
		}

		private String getOutputNameFor(final String tx, final String rx) {
		...
		private void organiseData() {
			if (_data == null) {
				...
				// ok, transmitter data
				_data.put(TX + FREQ, tests.findCollectionWith(_tx, Frequency.UNIT.getDimension(), true));
				_data.put(TX + COURSE, tests.findCollectionWith(_tx, SI.RADIAN.getDimension(), true));
				_data.put(TX + SPEED, tests.findCollectionWith(_tx, METRE.divide(SECOND).getDimension(), true));
				_data.put(TX + LOC, tests.someHaveLocation(_tx));

				// and the sound speed
				_data.put(SOUND_SPEED,
						tests.findCollectionWith(getInputs(), METRE.divide(SECOND).getDimension(), false));
			}
		}
		...
		private void performCalc() {
			// just check we've been organised (if we've been loaded from
			// persistent
			// storage)
			organiseData();
			...
			final Iterator<TrackProvider> rIter = _allTracks.iterator();
			while (rIter.hasNext()) {
				final TrackProvider trackProvider = rIter.next();

				// find the relevant outputs dataset
				final String thisOutName = getOutputNameFor(_tx.getName(), trackProvider.getName());

				final NumberDocumentBuilder builder = new NumberDocumentBuilder(thisOutName,
						HERTZ.asType(Frequency.class), this, SampleData.MILLIS);
				...
				while (tIter.hasNext()) {
					...
					if (thisTime >= period.getStartTime() && thisTime <= period.getEndTime()) {
						...
						final LocationDocument txLocDoc = (LocationDocument) _data.get(TX + LOC);
						final Point2D txLoc = txLocDoc.locationAt(thisTime);

						final double txCourseRads = aTests.valueAt((IDocument<?>) _data.get(TX + COURSE),
								(long) thisTime, SI.RADIAN);

						final double txSpeedMSec = aTests.valueAt((IDocument<?>) _data.get(TX + SPEED), (long) thisTime,
								SI.METERS_PER_SECOND);

						final double freq = aTests.valueAt((IDocument<?>) _data.get(TX + FREQ), (long) thisTime,
								SI.HERTZ);

						final double soundSpeed = aTests.valueAt((IDocument<?>) _data.get(SOUND_SPEED), (long) thisTime,
								SI.METERS_PER_SECOND);
						...
						// check we have locations. During some property editing
						// we receive
						// recalc call
						// after old value is removed, and before new value is
						// added.
						if (txLoc != null && rxLoc != null) {
							...
							final double shifted = calcPredictedFreqSI(soundSpeed, txCourseRads, rxCourseRads,
				...
				// capture the output
				final NumberDocument outputData = builder.toDocument();

				if (outputData == null) {
					throw new RuntimeException("Didn't generate any data");
				}

				final NumberDocument targetDoc = _outputMap.get(trackProvider);
				if (targetDoc == null) {
					...
					final IDataset ds = outputData.getDataset();
		...
		protected void recalculate(final IStoreItem subject) {
			...
			// share the good news
			for (final Document<?> o : getOutputs()) {
				o.fireDataChanged();
			}
		}
	}
	...
	public List<ICommand> actionsFor(final List<IStoreItem> selection, final IStoreGroup destination,
		...
		if (appliesTo(selection)) {
			...
			final ArrayList<IStoreGroup> trackList = aTests.getChildTrackGroups(selection);

			// ok, loop through them
			final Iterator<IStoreGroup> iter = trackList.iterator();
			while (iter.hasNext()) {
				final StoreGroup thisG = (StoreGroup) iter.next();
				final boolean hasFrequency = aTests.findCollectionWith(thisG, Frequency.UNIT.getDimension(),
						true) != null;
				if (hasFrequency) {
					final ICommand newC = new DopplerShiftOperation(thisG, destination,
							"Doppler between tracks (from " + thisG.getName() + ")",
							"Calculate doppler between two tracks", selection, context);
					res.add(newC);
				}
	...
	protected boolean appliesTo(final List<IStoreItem> selection) {
		...
		final boolean someHaveFreq = aTests.findCollectionWith(selection, Frequency.UNIT.getDimension(), true) != null;
		final boolean topLevelSoundSpeed = aTests.findCollectionWith(selection, METRE.divide(SECOND).getDimension(),
				false) != null;
