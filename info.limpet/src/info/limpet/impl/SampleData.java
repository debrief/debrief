...
public class SampleData {
	private static class ObjectColl extends ArrayList<String> {
		...
		public ObjectColl(final String name, final ICommand predecessor) {
			_name = name;
			_predecessor = predecessor;
		}

		public IStoreItem toDocument() {
			final StringDataset dataset = (StringDataset) DatasetFactory.createFromObject(this);
			dataset.setName(_name);
			final IDocument<?> res = new StringDocument(dataset, _predecessor);
			return res;
		}
	}

	public static final Unit<?> MILLIS = SI.MILLI(SECOND).asType(Duration.class);
	...
	public StoreGroup getData(final long count) {
		...
		final NumberDocumentBuilder freq1 = new NumberDocumentBuilder(FREQ_ONE, HERTZ.asType(Frequency.class), null,
				null);
		final NumberDocumentBuilder angle1 = new NumberDocumentBuilder(ANGLE_ONE, DEGREE_ANGLE.asType(Angle.class),
				null, SampleData.MILLIS);
		final NumberDocumentBuilder speedSeries1b = new NumberDocumentBuilder(SPEED_ONE,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedSeries2b = new NumberDocumentBuilder(SPEED_TWO,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedSeries3 = new NumberDocumentBuilder(SPEED_THREE_LONGER,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedEarly1 = new NumberDocumentBuilder(SPEED_EARLY,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedIrregularB = new NumberDocumentBuilder(SPEED_IRREGULAR2,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedSeries4 = new NumberDocumentBuilder(SPEED_FOUR,
				METRE.divide(SECOND).asType(Velocity.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder speedRadNoise = new NumberDocumentBuilder(SPEED_ONE_RAD, NonSI.DECIBEL, null,
				SampleData.MILLIS);
		final NumberDocumentBuilder radNoiseLookup = new NumberDocumentBuilder(RAD_NOISE_LOOKUP, NonSI.DECIBEL, null,
				METRE.divide(SECOND).asType(Velocity.class));
		final NumberDocumentBuilder length1 = new NumberDocumentBuilder(LENGTH_ONE, METRE.asType(Length.class), null,
				null);
		final NumberDocumentBuilder length2 = new NumberDocumentBuilder(LENGTH_TWO, METRE.asType(Length.class), null,
				null);
		final ObjectColl string1 = new ObjectColl(STRING_ONE, null);
		final ObjectColl string2 = new ObjectColl(STRING_TWO, null);
		final NumberDocumentBuilder singleton1 = new NumberDocumentBuilder(FLOATING_POINT_FACTOR, Dimensionless.UNIT,
				null, null);
		final NumberDocumentBuilder singletonRange1b = new NumberDocumentBuilder(RANGED_SPEED_SINGLETON,
				METRE.divide(SECOND).asType(Velocity.class), null, null);
		final NumberDocumentBuilder singletonLength = new NumberDocumentBuilder(LENGTH_SINGLETON,
				METRE.asType(Length.class), null, null);
		final NumberDocumentBuilder timeIntervals = new NumberDocumentBuilder(TIME_INTERVALS,
				SECOND.asType(Duration.class), null, SampleData.MILLIS);
		final NumberDocumentBuilder timeStamps_1 = new NumberDocumentBuilder(TIME_STAMPS_1,
				SECOND.asType(Duration.class), null, null);
		final NumberDocumentBuilder timeStamps_2 = new NumberDocumentBuilder(TIME_STAMPS_2,
				SECOND.asType(Duration.class), null, null);
		final LocationDocumentBuilder track1 = new LocationDocumentBuilder(TRACK_ONE, null, SampleData.MILLIS);
		final LocationDocumentBuilder track2 = new LocationDocumentBuilder(TRACK_TWO, null, SampleData.MILLIS);
		final LocationDocumentBuilder singleLoc1 = new LocationDocumentBuilder(SINGLETON_LOC_1, null, null);
		final LocationDocumentBuilder singleLoc2 = new LocationDocumentBuilder(SINGLETON_LOC_2, null, null);
		final LocationDocumentBuilder track1_2D = new LocationDocumentBuilder(TRACK_2D_ONE, null, SampleData.MILLIS,
				SI.METER);
		final LocationDocumentBuilder track2_2D = new LocationDocumentBuilder(TRACK_2D_TWO, null, SampleData.MILLIS,
				SI.METER);
		...
		final Point2D pos2 = calc.createPoint(-4.2, 54.9);
		...
		for (int i = 1; i <= count; i++) {
			...
			if (i < (count) * 0.4) {
				if (i % 6 > 2) {
					timeStamps_1.add(thisTime - interval + (interval * 2d * Math.random()));
				}
			}
			if (i > (count) * 0.6) {
				if (i % 6 > 2) {
					timeStamps_2.add(thisTime - interval + (interval * 2d * Math.random()));
				}
			}

			// sort out the tracks
			final Point2D p1 = calc.calculatePoint(pos1, Math.toRadians(77 - (i * 4)), 554);

			final Point2D p2 = calc.calculatePoint(pos2, Math.toRadians(54 + (i * 5)), 133);
		...
		final double minR = 940d;
		final double maxR = 1050d;
		final Range speedRange = new Range(minR, maxR);
		...
		final StoreGroup group1 = new StoreGroup(SPEED_DATA_FOLDER);
		...
		final IStoreGroup factors = new StoreGroup("Factors");

		final NumberDocumentBuilder singletonRange2b = new NumberDocumentBuilder(RANGED_SPEED_SINGLETON,
				METRE.divide(SECOND).asType(Velocity.class), null, null);
		final double minR1 = 940d;
		final double maxR1 = 1050d;
		final Range speedRange1 = new Range(minR1, maxR1);
		singletonRange2b.setRange(speedRange1);
		singletonRange2b.add(998d);

		final NumberDocument singletonRange2 = singletonRange2b.toDocument();

		final NumberDocumentBuilder singletonSpeedb = new NumberDocumentBuilder("Ranged Speed", Velocity.UNIT, null,
				null);
		final double min = 0d;
		final double max = 100d;
		singletonSpeedb.add(54d);
		singletonSpeedb.setRange(new Range(min, max));
		final NumberDocument singletonSpeed = singletonSpeedb.toDocument();

		final NumberDocumentBuilder singletonLength1b = new NumberDocumentBuilder("Ranged Speed", Length.UNIT, null,
				null);
		singletonLength1b.add(134d);
		singletonLength1b.setRange(new Range(40d, 150d));

		final NumberDocument singletonLength1 = singletonLength1b.toDocument();
		...
		final IStoreGroup compositeTrack = new StoreGroup(COMPOSITE_ONE);
		...
		final Collection<ICommand> actionsA = new AddQuantityOperation().actionsFor(selection, list, context);
		final Iterator<ICommand> addIterA = actionsA.iterator();
		// addIter.next();
		final ICommand addActionA = addIterA.next();
		addActionA.execute();
		...
		final Collection<ICommand> actions = new AddQuantityOperation().actionsFor(selection, list, context);
		final Iterator<ICommand> addIter = actions.iterator();
		// addIter.next();
		...
		final Collection<ICommand> actions2 = new MultiplyQuantityOperation().actionsFor(selection, list, context);
		...
		final Collection<ICommand> actions3 = new MultiplyQuantityOperation().actionsFor(selection, list, context);
