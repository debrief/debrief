...
public class LocationDocumentBuilder extends CoreDocumentBuilder<Point2D, LocationDocument>
		implements IDocumentBuilder<Point2D> {
	final private Unit<?> _distanceUnits;

	public LocationDocumentBuilder(final String name, final ICommand predecessor, final Unit<?> indexUnits) {
		this(name, predecessor, indexUnits, SampleData.DEGREE_ANGLE);
	}

	public LocationDocumentBuilder(final String name, final ICommand predecessor, final Unit<?> indexUnits,
			final Unit<?> distanceUnits) {
	...
	@Override
	protected LocationDocument getDocument(final IDataset dataset, final ICommand predecessor) {
		return new LocationDocument((ObjectDataset) dataset, predecessor, _distanceUnits);
	}
