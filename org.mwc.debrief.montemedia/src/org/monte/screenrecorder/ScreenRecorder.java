 ...
 * <p>
 * This class records mouse clicks occurring on other Java Windows running in the same JVM. Mouse
 * clicks occurring in other JVM's and other processes are not recorded. This ability is useful for
 * performing in-JVM recordings of an application that is being tested.
 * <p>
 * This recorder uses four threads. Three capture threads for screen, mouse cursor and audio, and
 * one output thread for the movie writer.
 * <p>
...
public class ScreenRecorder extends AbstractStateModel
{
  ...
  /**
   * outputTime and ffrDuration are needed for conversion of the video stream from variable frame
   * rate to fixed frame rate. FIXME - Do this with a CodecChain.
   */
   ...
   *
   * @param cfg
   *          Graphics configuration of the capture screen.
   */
  public ScreenRecorder(GraphicsConfiguration cfg) throws IOException,
      AWTException
  {
    this(cfg, null,
        // the file format
        ...
        new Format(MediaTypeKey, MediaType.VIDEO, EncodingKey,
            ...
            COMPRESSOR_NAME_QUICKTIME_ANIMATION, DepthKey, 24, FrameRateKey,
            new Rational(15, 1)),
        //
        // the output format for mouse capture
        new Format(MediaTypeKey, MediaType.VIDEO, EncodingKey,
            ENCODING_BLACK_CURSOR, FrameRateKey, new Rational(30, 1)),
        //
        ...
        new Format(MediaTypeKey, MediaType.AUDIO, EncodingKey,
            ...
            SampleSizeInBitsKey, 16, ChannelsKey, 2, SampleRateKey,
            new Rational(48000, 1), SignedKey, true, ByteOrderKey,
   ...
   * @param mouseFormat
   *          The video format for mouse capture. The {@code EncodingKey} must be
   *          ENCODING_BLACK_CURSOR or ENCODING_WHITE_CURSOR. The {@code SampleRateKey} can be
   *          independent from the {@code screenFormat}. Specify null if you don't want to capture
   *          the mouse cursor.
   * @param audioFormat
   *          The audio format for audio capture. Specify null if you don't want audio capture.
   */
  public ScreenRecorder(GraphicsConfiguration cfg, Format fileFormat,
   ...
   *          The audio format for audio capture. Specify null if you don't want audio capture.
   */
  public ScreenRecorder(GraphicsConfiguration cfg, Rectangle captureArea,
      Format fileFormat, Format screenFormat, Format mouseFormat,
      Format audioFormat) throws IOException, AWTException
  {
    this(cfg, captureArea, fileFormat, screenFormat, mouseFormat, audioFormat,
        null);
  }
      ...
      Format audioFormat, File movieFolder) throws IOException, AWTException
  {
    ...
    {
      this.mouseFormat = new Format(FrameRateKey, new Rational(0, 0),
    ...
    {
      mouseCaptures = new ArrayBlockingQueue<Buffer>(mouseFormat.get(
          FrameRateKey).intValue() * 2);
      if (this.mouseFormat.get(EncodingKey).equals(ENCODING_BLACK_CURSOR))
      {
        cursorImg = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.black.png"));
        cursorImgPressed = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.black.pressed.png"));
      }
      else if (this.mouseFormat.get(EncodingKey).equals(ENCODING_YELLOW_CURSOR))
      {
        cursorImg = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.yellow.png"));
        cursorImgPressed = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.yellow.pressed.png"));
      }
      else
      {
        cursorImg = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.white.png"));
        cursorImgPressed = Images.toBufferedImage(Images.createImage(
            ScreenRecorder.class, "images/Cursor.white.pressed.png"));
      }
      cursorOffset = new Point(cursorImg.getWidth() / -2, cursorImg.getHeight()
          / -2);
    }
    ...
    if (this.movieFolder == null)
    {
      ...
      {
        this.movieFolder = new File(System.getProperty("user.home")
  ...
  /**
   * internal reference to the file created. We will use this to move the file to the path chosen by
   * the user.
   */
  ...
  public void moveFileTo(final String path)
  {
    try
    {
      Files.move(Paths.get(fileCreated.getAbsolutePath()), Paths.get(path),
  ...
  protected MovieWriter createMovieWriter() throws IOException
  {
    ...
    MovieWriter mw = w = Registry.getInstance().getWriter(fileFormat,
        fileCreated);

    // Create the video encoder
    Rational videoRate = Rational.max(screenFormat.get(FrameRateKey),
        mouseFormat.get(FrameRateKey));
    ffrDuration = videoRate.inverse();
    Format videoInputFormat = screenFormat.prepend(MediaTypeKey,
        MediaType.VIDEO, EncodingKey, ENCODING_BUFFERED_IMAGE, WidthKey,
        captureArea.width, HeightKey, captureArea.height, FrameRateKey,
        videoRate);
    Format videoOutputFormat = screenFormat.prepend(FrameRateKey, videoRate,
    ...
    if (frameEncoder.getOutputFormat() == null)
    {
      throw new IOException(
          "Unable to encode video frames in this output format:\n"
              + videoOutputFormat);
    }
        ...
        videoOutputFormat.intersectKeys(WidthKey, HeightKey)))
    {
      ...
      sic.setOutputFormat(videoOutputFormat.intersectKeys(WidthKey, HeightKey)
          .append(videoInputFormat));
   ...
   * <p>
   * This implementation creates a file in the users "Video" folder on Windows, or in the users
   * "Movies" folders on Mac OS X.
   * <p>
  ...
  protected File createMovieFile(Format fileFormat) throws IOException
  {
    ...
    File f = new File(movieFolder, //
        "ScreenRecording " + dateFormat.format(new Date()) + "." + Registry
  ...
  {
    return Files.createTempFile("", Registry.getInstance().getExtension(
        fileFormat)).toFile();
  }
  ...
  private void startScreenCapture() throws AWTException, IOException
  {
    ...
    int delay = max(1, (int) (1000 / screenFormat.get(FrameRateKey)
        .doubleValue()));
    screenGrabber = new ScreenGrabber(this, recordingStartTime);
    screenFuture = screenCaptureTimer.scheduleAtFixedRate(screenGrabber, delay,
  ...
  private static class ScreenGrabber implements Runnable
  {
    ...
    /**
     * Previously draw mouse location. This is used to have the last mouse location at hand, when a
     * new screen capture has been created, but the mouse has not been moved.
     */
    private Point prevDrawnMouseLocation = new Point(Integer.MAX_VALUE,
        Integer.MAX_VALUE);
    ...
    private Rectangle captureArea;
    /**
     * Holds the composed image (screen capture and super-imposed mouse cursor). This is the image
     * that is written into the video track of the file.
     */
    ...
    public ScreenGrabber(ScreenRecorder recorder, long startTime)
        throws AWTException, IOException
    {
      ...
      {
        videoImg = new BufferedImage(this.captureArea.width,
            this.captureArea.height, BufferedImage.TYPE_INT_RGB);
      }
      else if (screenFormat.get(DepthKey) == 16)
      {
        videoImg = new BufferedImage(this.captureArea.width,
            this.captureArea.height, BufferedImage.TYPE_USHORT_555_RGB);
      }
      else if (screenFormat.get(DepthKey) == 8)
      {
        videoImg = new BufferedImage(this.captureArea.width,
            this.captureArea.height, BufferedImage.TYPE_BYTE_INDEXED, Colors
                .createMacColors());
      }
      else
      {
    ...
    /**
     * Grabs a screen, generates video images with pending mouse captures and writes them into the
     * movie file.
     */
    private void grabScreen() throws IOException, InterruptedException
    {
      ...
      buf.format = new Format(MediaTypeKey, MediaType.VIDEO, EncodingKey,
          ENCODING_BUFFERED_IMAGE);
      // Generate video frames with mouse cursor painted on them
      boolean hasMouseCapture = false;
      if (mouseFormat != null && mouseFormat.get(FrameRateKey).intValue() > 0)
      {
        while (!mouseCaptures.isEmpty() && mouseCaptures.peek().timeStamp
            .compareTo(new Rational(timeAfterCapture, 1000)) < 0)
        {
          Buffer mouseCapture = mouseCaptures.poll();
          if (mouseCapture.timeStamp.compareTo(prevScreenCaptureTime) > 0)
          {
            if (mouseCapture.timeStamp.compareTo(new Rational(timeBeforeCapture,
                1000)) < 0)
            {
              previousScreenCapture = screenCapture;
              videoGraphics.drawImage(previousScreenCapture, 0, 0, null);
            ...
            prevDrawnMouseLocation.setLocation(mcp.x - captureArea.x, mcp.y
                - captureArea.y);
            Point p = prevDrawnMouseLocation;

            long localStopTime = getStopTime();
            if (mouseCapture.timeStamp.compareTo(new Rational(localStopTime,
                1000)) > 0)
            {
              break;
            }
            {
              ...
              if (prevMousePressed)
              {
                videoGraphics.drawImage(cursorImgPressed, p.x + cursorOffset.x,
                    p.y + cursorOffset.y, null);
              }
              else
              {
                videoGraphics.drawImage(cursorImg, p.x + cursorOffset.x, p.y
                    + cursorOffset.y, null);
              }
              buf.clearFlags();
              buf.data = videoImg;
              ...
              buf.sampleDuration = mouseCapture.timeStamp.subtract(
                  prevScreenCaptureTime);
              buf.timeStamp = prevScreenCaptureTime.subtract(new Rational(
                  startTime, 1000));
              ...
              videoGraphics.drawImage(previousScreenCapture, //
                  ...
                  p.x + cursorOffset.x + cursorImg.getWidth() - 1, p.y
                      + cursorOffset.y + cursorImg.getHeight() - 1, //
                  p.x + cursorOffset.x, p.y + cursorOffset.y, //
                  p.x + cursorOffset.x + cursorImg.getWidth() - 1, p.y
                      + cursorOffset.y + cursorImg.getHeight() - 1, //
                  null);
        ...
        if (!hasMouseCapture && prevScreenCaptureTime.compareTo(new Rational(
            getStopTime(), 1000)) < 0)
        {
          Point p = prevDrawnMouseLocation;
          if (p != null)
          ...
          buf.sampleDuration = new Rational(timeAfterCapture, 1000).subtract(
              prevScreenCaptureTime);
          buf.timeStamp = prevScreenCaptureTime.subtract(new Rational(startTime,
              1000));
          buf.track = videoTrack;
          buf.sequenceNumber = sequenceNumber++;
          buf.header = p.x == Integer.MAX_VALUE ? null : p;
      ...
      else if (prevScreenCaptureTime.compareTo(new Rational(getStopTime(),
          1000)) < 0)
      {
        buf.data = videoImg;
        buf.sampleDuration = new Rational(timeAfterCapture, 1000).subtract(
  ...
  protected void startMouseCapture() throws IOException
  {
    ...
    int delay = max(1, (int) (1000 / mouseFormat.get(FrameRateKey)
        .doubleValue()));
    mouseGrabber = new MouseGrabber(this, recordingStartTime,
        mouseCaptureTimer);
    mouseFuture = mouseCaptureTimer.scheduleAtFixedRate(mouseGrabber, delay,
        delay, TimeUnit.MILLISECONDS);
  ...
  /**
   * Stops mouse capturing. Use method {@link #waitUntilMouseCaptureStopped} to wait until the
   * capturing stopped.
   */
  ...
  /**
   * Waits until mouse capturing stopped. Invoke this method only after you invoked
   * {@link #stopMouseCapture}.
   */
  protected void waitUntilMouseCaptureStopped() throws InterruptedException
  {
    ...
    {
      /*
       * try { mouseFuture.get(); } catch (InterruptedException ex) { ex.printStackTrace(); } catch
       * (CancellationException ex) { ex.printStackTrace(); } catch (ExecutionException ex) {
       * ex.printStackTrace(); }
       */// Saul
  ...
  protected static class MouseGrabber implements Runnable
  {
    ...
    /**
     * Previously captured mouse location. This is used to coalesce mouse captures if the mouse has
     * not been moved.
     */
    private Point prevCapturedMouseLocation = new Point(Integer.MAX_VALUE,
        Integer.MAX_VALUE);
    ...
    public MouseGrabber(ScreenRecorder recorder, long startTime,
  ...
  private void startAudioCapture() throws LineUnavailableException
  {
    ...
    int delay = 500;
    // SAUL
    audioGrabber = new AudioGrabber(mixer, audioFormat, audioTrack,
        recordingStartTime, writerQueue);
    audioFuture = audioCaptureTimer.scheduleWithFixedDelay(audioGrabber, 0, 10,
  ...
  /**
   * This runnable grabs audio samples and enqueues them into the specified BlockingQueue. This
   * runnable must be called twice a second.
   */
  private static class AudioGrabber implements Runnable
  {
    ...
    public AudioGrabber(Mixer mixer, Format audioFormat, int audioTrack,
        long startTime, BlockingQueue<Buffer> queue)
        throws LineUnavailableException
    {
      ...
      try
      {
        BooleanControl ctrl = (BooleanControl) line.getControl(
      ...
      try
      {
        FloatControl ctrl = (FloatControl) line.getControl(
    ...
    public void run()
    {
      ...
      // one second of audio.
      int bufferSize = lineFormat.getFrameSize() * (int) lineFormat
          .getSampleRate();
      ...
      if (count > 0)
      {
        ...
        buf.sampleCount = count / (lineFormat.getSampleSizeInBits() / 8
        ...
        if (buf.timeStamp.add(buf.sampleDuration.multiply(buf.sampleCount))
            .compareTo(stopTS) > 0)
        {
          // we recorderd too much => truncate the buffer
          buf.sampleCount = Math.max(0, (int) Math.ceil(stopTS.subtract(
              buf.timeStamp).divide(buf.sampleDuration).floatValue()));
          buf.length = buf.sampleCount * (lineFormat.getSampleSizeInBits() / 8
              * lineFormat.getChannels());
    ...
    /**
     * Calculates the root-mean-square average of continuous samples. For four samples, the formula
     * looks like this:
     * 
     * <pre>
      ...
      if (format.getEncoding().equals(AudioFormat.Encoding.PCM_SIGNED))
      {
        switch (format.getSampleSizeInBits())
        {
          case 8:
            ...
            {
              case 1:
                audioLevelLeft = computeAudioLevelSigned8(data, 0, length,
                    format.getFrameSize());
                break;
              case 2:
                audioLevelLeft = computeAudioLevelSigned8(data, 0, length,
                    format.getFrameSize());
                audioLevelRight = computeAudioLevelSigned8(data, 1, length,
                    format.getFrameSize());
                break;
            }
            break;
          case 16:
            if (format.isBigEndian())
            {
              ...
              {
                case 1:
                  audioLevelLeft = computeAudioLevelSigned16BE(data, 0, length,
                      format.getFrameSize());
                  break;
                case 2:
                  audioLevelLeft = computeAudioLevelSigned16BE(data, 0, length,
                      format.getFrameSize());
                  audioLevelRight = computeAudioLevelSigned16BE(data, 2, length,
                      format.getFrameSize());
                  break;
              }
            }
    ...
    private float computeAudioLevelSigned16BE(byte[] data, int offset,
    ...
    private float computeAudioLevelSigned8(byte[] data, int offset, int length,
        int stride)
    {
      double sum = 0;
      for (int i = offset; i < length; i += stride)
  ...
  {
    writerQueue = new ArrayBlockingQueue<Buffer>(max(screenFormat.get(
        FrameRateKey).intValue(), mouseFormat.get(FrameRateKey).intValue())
        + 1);
    writerThread = new Thread()
    {
      ...
      public void run()
      {
        ...
        {
          e.printStackTrace();
          recordingFailed(e.getMessage() == null ? e.toString() : e
              .getMessage());
        }
      }
    };
   ...
   * <p>
   * Stopping the screen recorder may take several seconds, because audio capture uses a large
   * capture buffer. Also, the MovieWriter has to finish up a movie file which may take some time
  ...
  public void stop() throws IOException
  {
    if (state == State.RECORDING)
    {
      ...
      try
      {
        ...
        {
          /*
           * try { screenFuture.get(); } catch (InterruptedException ex) { } catch
           * (CancellationException ex) { } catch (ExecutionException ex) { }
           */ // Saul
      ...
      if (conversionEnabled())
      {
        ...
        File outputFile = Files.createTempFile("", "." + conversionExtension())
            .toFile();
        File inputFile = fileCreated;
        doConvert(inputFile.getAbsolutePath(), outputFile.getAbsolutePath(),
            fileFormat.get(ConversionKey));
  ...
  {
    return fileFormat != null && !ConvertFormat.NONE.equals(fileFormat.get(
        ConversionKey, ConvertFormat.NONE));
  }

  public boolean validateFfmpeg()
  {
    String commandExecute = fileFormat.get(FfmpegKey) + " -version";

    System.out.println("Validating FFMPEG: " + commandExecute);
  ...
  void doConvert(String inputFile, String outputFile, ConvertFormat config)
  {
    try
    {
      ...
      String commandExecute = fileFormat.get(FfmpegKey) + " -y" // Overwrite if file exist
          + " -i " // Declare input file
          + inputFile + " " + outputFile;

      System.out.println("Command to run: " + commandExecute);
  ...
  {
    BufferedReader stdError = new BufferedReader(new InputStreamReader(proc
        .getErrorStream()));
  ...
  /**
   * Writes a buffer into the movie. Since the file system may not be immediately available at all
   * times, we do this asynchronously.
   * <p>
   * The buffer is copied and passed to the writer queue, which is consumed by the writer thread.
   * See method startWriter().
   * <p>
   * AVI does not support a variable frame rate for the video track. Since we can not capture frames
   * at a fixed frame rate we have to resend the same captured screen multiple times to the writer.
   * <p>
   ...
   *
   * @param buf
   *          A buffer with un-encoded media data. If {@code buf.track==videoTrack}, then the buffer
   *          contains a {@code BufferedImage} in {@code buffer.data} and a {@code Point} in
   *          {@code buffer.header} with the recorded mouse location. The header is null if the
   *          mouse is outside the capture area, or mouse recording has not been enabled.
   *
   * @throws IOException
   */
  protected void write(Buffer buf) throws IOException, InterruptedException
  {
    ...
    if (buf.track == videoTrack)
    {
      ...
      {// variable frame rate not supported => convert to fixed frame rate
        ...
        while (outputTime.compareTo(inputTime) < 0)
        {
          ...
          {
            throw new IOException(
                "Codec failed or could not process frame in a single step.");
          }
          writerQueue.put(wbuf);
  ...
  private void doWrite(Buffer buf) throws IOException
  {
    ...
    // the same duration
    long now = System.currentTimeMillis();
    if (buf.track == videoTrack && buf.isFlag(BufferFlag.KEYFRAME) && (mw
        .isDataLimitReached() || now - fileStartTime > maxRecordingTime))
    {
  ...
  /**
   * Maximal recording time in milliseconds. If this time is exceeded, the recorder creates a new
   * file.
   */
