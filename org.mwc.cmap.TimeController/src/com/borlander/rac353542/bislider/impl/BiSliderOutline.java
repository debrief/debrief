...
class BiSliderOutline extends BiSliderComponentBase
{
	...
	void reloadConfig()
	{
		myForeground = updateColorDescriptor(myForeground, getUIModel()
				.getBiSliderForegroundRGB());
	}
	...
	public void paintOutline(GC gc)
	{
		...
		gc.drawRoundRectangle(drawArea.x, drawArea.y, drawArea.width,
				drawArea.height, arcRadius, arcRadius);
	...
	private void drawLabels(GC gc, boolean anchorAtMinEdge)
	{
		...
		Rectangle previousLabelBounds = drawLabelAndUpdateConstraints(gc, 0.0,
				totalMin, anchorAtMinEdge, null, null);
		Rectangle lastTextBounds = drawLabelAndUpdateConstraints(gc, 1.0, totalMax,
				anchorAtMinEdge,
		...
		for (double nextLabelValue = totalMin + segmentSize; nextLabelValue < totalMax; nextLabelValue += segmentSize)
		{
			...
			drawLabelAndUpdateConstraints(gc, nextLabelRate, nextLabelValue,
					anchorAtMinEdge, previousLabelBounds, lastTextBounds);
		}
		gc.setFont(oldFont);
	}
	 ...
	 */
	private Rectangle drawLabelAndUpdateConstraints(GC gc, double rate,
			double value, boolean anchorAtMinEdge, Rectangle leftConstraint,
			Rectangle rightConstraint)
	{
		...
		if (leftConstraint != null
				&& leftConstraint.intersects(adjustedX, adjustedY, textSize.x,
						textSize.y))
		{
			return null;
		}
		if (rightConstraint != null
				&& rightConstraint.intersects(adjustedX, adjustedY, textSize.x,
						textSize.y))
		{
			return null;
		}
	...
	private Point getLabelAdjustmentX(boolean atMinimumEdge, Point textSize,
			double valueRate)
	{
		...
		if (getUIModel().isVertical())
		{
			...
			if (verticalLabels)
			{
				myTempPoint.y = atMinimumEdge ? (int) (valueRate * textSize.x)
						: -(int) ((1 - valueRate) * textSize.x);
			}
			else
			{
			...
			boolean anchorAtMinEdge)
	{
		if (getUIModel().isVerticalLabels())
		{
			...
			myCachedRotatedTransform = setT2equalsToT1(myCachedRotatedTransform,
					myCachedOriginalTransform);
	...
	private Transform setT2equalsToT1(Transform t1, Transform t2)
	{
		...
		t1.setElements(matrix[0], matrix[1], matrix[2], matrix[3], matrix[4],
				matrix[5]);
		return t1;
	}
}
