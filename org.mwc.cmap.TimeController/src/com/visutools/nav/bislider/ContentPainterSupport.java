...
public class ContentPainterSupport implements java.io.Serializable {
	...
	/**
	 * Adds a feature to the ContentPainterListener attribute of the
	 * ContentPainterSupport object
	 *
	 * @param l The feature to be added to the ContentPainterListener attribute
	 */
	public synchronized void addContentPainterListener(final ContentPainterListener l) {
	 ...
	 */
	public void fireAsyncPaint(final Object Source_Arg, final Graphics Graphics_Arg, final double Min_Arg,
		...
		final Thread Thread1 = new Thread() {
			@Override
			public void run() {
				internFireAsyncPaint(Source_Arg, Graphics_Arg, Min_Arg, Max_Arg, Index_Arg, Color_Arg, Rectangle_Arg,
						BoundingRectangle_Arg);
			}
		};
		Thread1.start();
	} // fireAsyncPaint()
	...
	public void firePaint(final Object Source_Arg, final Graphics Graphics_Arg, final double Min_Arg,
			final double Max_Arg, final int Index_Arg, final Color Color_Arg, final Rectangle Rectangle_Arg,
			final Rectangle BoundingRectangle_Arg) {
		...
		final int count = Vector1.size();
		for (int i = 0; i < count; i++) {
			final ContentPainterListener listener = Vector1.elementAt(i);
			listener.paint(new ContentPainterEvent(Source_Arg, Graphics_Arg, Min_Arg, Max_Arg, Index_Arg, Color_Arg,
					Rectangle_Arg, BoundingRectangle_Arg));
		}
	} // firePaint()
	 ...
	 */
	@SuppressWarnings("unchecked")
	void internFireAsyncPaint(final Object Source_Arg, final Graphics Graphics_Arg, final double Min_Arg,
			final double Max_Arg, final int Index_Arg, final Color Color_Arg, final Rectangle Rectangle_Arg,
			final Rectangle BoundingRectangle_Arg) {
	 ...
	 */
	public synchronized void removeContentPainterListener(final ContentPainterListener l) {
