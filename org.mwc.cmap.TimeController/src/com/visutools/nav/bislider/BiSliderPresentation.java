...
public class BiSliderPresentation implements Serializable, MouseListener, MouseMotionListener, ComponentListener {
	//////////////////////////////////////////////////////
	// utility class that returns a string from a long value
	//////////////////////////////////////////////////////
	public static class FormatLong {
		public String format(final long val) {
			return "" + val;
		}
	}
	 ...
	 */
	public BiSliderPresentation(final SwingBiSlider Ctrl_Arg, final ContentPainterSupport ContentPainterSupport_Arg,
			final FormatLong formatter) {
		this.Ctrl = Ctrl_Arg;
		...
		JSlider1.addChangeListener(new ChangeListener() {
			@Override
			public void stateChanged(final ChangeEvent ChangeEvent_Arg) {

				double MiddleVal = PreciseOpenedValue;

				final double Amplitude = Ctrl.getSegmentSize() / Ctrl.getSegmentCount();
				...
				final double Val = MiddleVal + (JSlider1.getValue()) / 100d * Amplitude;
		...
		if (UIManager.getLookAndFeel().getName().equals("Metal")) {
			final int[] xp1 = { LeftValue, LeftValue - 7, LeftValue - 7, LeftValue - 6, LeftValue + 6, LeftValue + 7,
					LeftValue + 7 };
			final int[] yp1 = { Margin_Ruler_Top + RulerHeight + 1 - 14, Margin_Ruler_Top + RulerHeight + 1 - 7,
					Margin_Ruler_Top + RulerHeight + 1, Margin_Ruler_Top + RulerHeight + 1 + 1,
			...
			final int[] xp2 = { RightValue - 7, RightValue - 7, RightValue - 6, RightValue + 6, RightValue + 7,
					RightValue + 7, RightValue };
			final int[] yp2 = { Margin_Ruler_Top - 1 + 7, Margin_Ruler_Top - 1, Margin_Ruler_Top - 1 - 1,
		...
		// PreferredSize =
		// MinimumSize = );
		Ctrl.setPreferredSize(new Dimension(MARGIN_RULER_LEFT + MARGIN_RULER_RIGHT + 16 * Ctrl.getSegmentCount(),
				Margin_Ruler_Top + MARGIN_RULER_BOTTOM + MINIMUM_RULER_HEIGHT));
		Ctrl.setMinimumSize(new Dimension(MARGIN_RULER_LEFT + MARGIN_RULER_RIGHT + 8 * Ctrl.getSegmentCount(),
				Margin_Ruler_Top + MARGIN_RULER_BOTTOM + PREFERRED_RULER_HEIGHT));
		...
		JTextFieldMin.getDocument().addDocumentListener(new DocumentListener() {
			@Override
			public void changedUpdate(final DocumentEvent DocumentEvent_Arg) {
			}

			@Override
			public void insertUpdate(final DocumentEvent DocumentEvent_Arg) {
				try {
					JTextFieldMin.setBackground(Color.WHITE);
				} catch (final NumberFormatException NumberFormatException_Arg) {
					JTextFieldMin.setBackground(new Color(255, 128, 128));
				}
			}

			@Override
			public void removeUpdate(final DocumentEvent DocumentEvent_Arg) {
				try {
					JTextFieldMin.setBackground(Color.WHITE);
				} catch (final NumberFormatException NumberFormatException_Arg) {
					JTextFieldMin.setBackground(new Color(255, 128, 128));
		...
		JTextFieldMin.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(final KeyEvent e) {
				...
				JTextFieldMin.setSize(JTextFieldMin.getPreferredSize().width + 10,
						JTextFieldMin.getPreferredSize().height);
			}
		});
		...
		JTextFieldMax.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(final KeyEvent e) {
				...
				final int OldX = JTextFieldMax.getLocation().x;
				final int OldWidth = JTextFieldMax.getSize().width;
				JTextFieldMax.setSize(JTextFieldMax.getPreferredSize().width + 10,
						JTextFieldMax.getPreferredSize().height);
	 ...
	 */
	void changeMaxValue(final String NewValue_Arg) {
		try {
			final double NV = MWCXMLReader.readThisDouble(NewValue_Arg);
	 ...
	 * @param NewValue_Arg Description of the Parameter
	 */
	void changeMinValue(final String NewValue_Arg) {
		try {
			final double NV = MWCXMLReader.readThisDouble(NewValue_Arg);
	 ...
	 */
	@Override
	public void componentResized(final ComponentEvent ComponentEvent_Arg) {
	...
	private boolean isLastFiveEventsUpDown() {
		if (LastFiveEvents.size() > 4) {
			final Point PointM0 = LastFiveEvents.elementAt(0);
			final Point PointM1 = LastFiveEvents.elementAt(1);
			final Point PointM2 = LastFiveEvents.elementAt(2);
			final Point PointM3 = LastFiveEvents.elementAt(3);
			final Point PointM4 = LastFiveEvents.elementAt(4);
			final int D1x = PointM0.x - PointM1.x;
			final int D1y = PointM0.y - PointM1.y;
			final int D2x = PointM0.x - PointM2.x;
			final int D2y = PointM0.y - PointM2.y;
			final int D3x = PointM0.x - PointM3.x;
			final int D3y = PointM0.y - PointM3.y;
			final int D4x = PointM0.x - PointM4.x;
			final int D4y = PointM0.y - PointM4.y;

			return ((Math.abs(D1x) < 2 && Math.abs(D1y) > 5) || (Math.abs(D2x) < 2 && Math.abs(D2y) > 5)
					|| (Math.abs(D3x) < 2 && Math.abs(D3y) > 5) || (Math.abs(D4x) < 2 && Math.abs(D4y) > 5));
		} else {
			return false;
		}
	 ...
	 */
	@Override
	public void mouseClicked(final MouseEvent MouseEvent_Arg) {
		...
		if (MouseEvent_Arg.getClickCount() > 1
			...
			JTextFieldMax.setLocation(
					JComponent1.getWidth() - JTextFieldMax.getPreferredSize().width - MARGIN_RULER_RIGHT - 10, 0);
	 ...
	 */
	@Override
	public void mouseDragged(final MouseEvent MouseEvent_Arg) {

		// System.out.println("last="+LastFiveEvents.elementAt(LastFiveEvents.size()-1));
		// System.out.println("new="+MouseEvent_Arg.getPoint());
		if (LastFiveEvents.elementAt(0).x != MouseEvent_Arg.getX()
				|| LastFiveEvents.elementAt(0).y != MouseEvent_Arg.getY()) {
		...
		// "+MouseEvent_Arg.getX());
		final int MouseX = MouseEvent_Arg.getX();
		...
		if (Dragging == SEGMENT_SIZE) {
			double SegSize = (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()) * (MouseX - MARGIN_RULER_LEFT)
					/ RulerWidth;
		...
		if (Dragging == SEGMENT_SIZE_INT) {
			final double SegSize = Math.round(
					(Ctrl.getMaximumValue() - Ctrl.getMinimumValue()) * (MouseX - MARGIN_RULER_LEFT) / RulerWidth);
		...
		if (Dragging == ALT_LEFT_RULER) {
			double NewValue = Ctrl.getMinimumValue()
					+ ((MouseX - DeplBef - MARGIN_RULER_LEFT) * (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()))
							/ RulerWidth;
			...
			double NewValue = Ctrl.getMinimumValue()
					+ ((MouseX + DeplAft - MARGIN_RULER_LEFT) * (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()))
							/ RulerWidth;
			...
			final double DMin = ((MouseX - DeplBef - MARGIN_RULER_LEFT)
			...
			final double DMax = ((MouseX + DeplAft + MARGIN_RULER_LEFT)
					* (Ctrl.getMaximumValue() - Ctrl.getMinimumValue())) / RulerWidth;
			...
			final double Size = (float) (Ctrl.getMaximumColoredValue() - Ctrl.getMinimumColoredValue());
			...
			double DMin = ((MouseX - DeplBef - MARGIN_RULER_LEFT) * (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()))
					/ RulerWidth;
			final int NewMin = (int) Math.round(DMin / Ctrl.getSegmentSize());
			final double MagnetEffect = (Ctrl.getSegmentSize() * NewMin) - DMin;
	 ...
	 */
	@Override
	public void mouseEntered(final MouseEvent MouseEvent_Arg) {
	}
	 ...
	 */
	@Override
	public void mouseExited(final MouseEvent MouseEvent_Arg) {
		MouseUnder = NOTHING;
	 ...
	 */
	@Override
	public void mouseMoved(final MouseEvent MouseEvent_Arg) {
		final int OldMouseUnder = MouseUnder;
		final Rectangle2D OldRectangleSegment = RectangleSegment;

		// for mouseOver between the triangles
		final Rectangle Rect1 = TheRight_Polygon.getBounds();
		final Rectangle Rect2 = TheLeft_Polygon.getBounds();
		final Rectangle2D Rectangle2D1 = Rect1.createUnion(Rect2);

		// for segment mouse over
		final int d1 = MouseEvent_Arg.getX() - MARGIN_RULER_LEFT;
		final int GraduationCount = (int) Math.floor(d1 / GraduationWidth);

		final int LeftSegment = MARGIN_RULER_LEFT + (int) ((GraduationCount * GraduationWidth));
		...
		/*
		 * double ValuesWidth = Ctrl.getMaximumValue() - Ctrl.getMinimumValue(); int d1
		 * = MouseEvent_Arg.getX() - MARGIN_RULER_LEFT; double d2 = ((double)d1 *
		 ...
		 * (int)(MARGIN_RULER_LEFT + (RulerWidth * (d1 + d4 - Ctrl.getMinimumValue())) /
		 * ValuesWidth);
		 */
		if (MouseEvent_Arg.getX() >= MARGIN_RULER_LEFT && MouseEvent_Arg.getX() <= MARGIN_RULER_LEFT + RulerWidth) {
			RectangleSegment = new Rectangle(LeftSegment + 1, Margin_Ruler_Top + 1, RightSegment - LeftSegment - 2,
					RulerHeight - 2);
		} else {
	...
	/**
	 * Method called by the awt-swing mechanism when the user press the mouse button
	 * over this component
	 *
	 * @param MouseEvent_Arg the mouse event generatedby the awt-swing mechanism
	 */
	...
	public void mousePressed(final MouseEvent MouseEvent_Arg) {
		...
		if (Ctrl.isPrecise() && MouseEvent_Arg.getButton() == MouseEvent.BUTTON1 && MouseEvent_Arg.getClickCount() > 1
		...
		else if (MouseEvent_Arg.getX() > LeftValue && MouseEvent_Arg.getButton() == MouseEvent.BUTTON1
				&& MouseEvent_Arg.getX() < RightValue && MouseEvent_Arg.getY() > Margin_Ruler_Top
				&& MouseEvent_Arg.getY() < Margin_Ruler_Top + RulerHeight && !MouseEvent_Arg.isAltDown()
				&& !MouseEvent_Arg.isShiftDown()) {
			Dragging = SEGMENT;
			...
			// the segment
		else if (MouseEvent_Arg.isShiftDown() && MouseEvent_Arg.getButton() == MouseEvent.BUTTON1
				&& MouseEvent_Arg.getClickCount() > 1 && MouseEvent_Arg.getX() > MARGIN_RULER_LEFT
				&& MouseEvent_Arg.getX() < MARGIN_RULER_LEFT + RulerWidth && MouseEvent_Arg.getY() > Margin_Ruler_Top
				&& MouseEvent_Arg.getY() < Margin_Ruler_Top + RulerHeight) {

			final int d1 = MouseEvent_Arg.getX() - MARGIN_RULER_LEFT;
			final int GraduationCount = (int) Math.floor(d1 / GraduationWidth);

			final double Min = Ctrl.getMinimumValue() + (float) ((GraduationCount * GraduationWidth)
					* (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()) / RulerWidth);
			double Max = Ctrl.getMinimumValue() + (float) (((GraduationCount + 1) * GraduationWidth)
					* (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()) / RulerWidth);
		...
		else if (MouseEvent_Arg.getClickCount() > 1 && MouseEvent_Arg.getButton() == MouseEvent.BUTTON1
				&& MouseEvent_Arg.getX() > MARGIN_RULER_LEFT && MouseEvent_Arg.getX() < MARGIN_RULER_LEFT + RulerWidth
				&& MouseEvent_Arg.getY() > Margin_Ruler_Top && MouseEvent_Arg.getY() < Margin_Ruler_Top + RulerHeight) {
			 ...
			 * (3*GWd))); System.err.println(" 135.6D == 135.6f : "+(135.6D == 135.6f));
	 ...
	 */
	@Override
	public void mouseReleased(final MouseEvent MouseEvent_Arg) {
	 ...
	 */
	private void openPrecisionPopup(final int Thumb_Arg, final Point Point_Arg) {
		...
		final Hashtable<Integer, JLabel> Hashtable1 = new Hashtable<Integer, JLabel>();
		...
		for (int i = -100; i <= 100; i += 20) {
			double Val = 0d;
			Val = MiddleVal + (i) / 100d * Amplitude;
		...
		final PopupFactory PopupFactory1 = PopupFactory.getSharedInstance();
		final Point Point1 = (Point) Point_Arg.clone();
		...
		} catch (final AWTException AWTException_Arg) {
			AWTException_Arg.printStackTrace();
		} catch (final java.security.AccessControlException AccessControlException_Arg) {
	...
	public void paint(final Graphics Graphics_Arg) {
		...
		final Graphics2D Graphics2 = (Graphics2D) Graphics_Arg;
		...
		final Font Font1 = Ctrl.getFont();
		final Font Font2 = new Font(Font1.getName(), Font.BOLD, Font1.getSize());
		final Shape OldClip = Graphics2.getClip();
		// Shape NewClip = new Rectangle2D.Float(MARGIN_RULER_LEFT,
		// Margin_Ruler_Top,RulerWidth, RulerHeight);
		final Shape NewClip = new RoundRectangle2D.Float(MARGIN_RULER_LEFT + 2, Margin_Ruler_Top, RulerWidth - 3,
				RulerHeight, Ctrl.getArcSize(), Ctrl.getArcSize());

		final int SegmentCount = Ctrl.getSegmentCount();
		final FontMetrics TheFontMetrics = Graphics2.getFontMetrics();
		RulerWidth = JComponent1.getSize().width - MARGIN_RULER_LEFT - MARGIN_RULER_RIGHT;
		final double ValuesWidth = Ctrl.getMaximumValue() - Ctrl.getMinimumValue();

		if (NeverDrawn) {
			LeftValue = (int) (MARGIN_RULER_LEFT
					+ ((Ctrl.getMinimumColoredValue() - Ctrl.getMinimumValue()) * RulerWidth)
							/ (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()));
			RightValue = (int) (MARGIN_RULER_LEFT
					+ ((Ctrl.getMaximumColoredValue() - Ctrl.getMinimumValue()) * RulerWidth)
							/ (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()));
		...
		if (RulerWidth != JComponent1.getSize().width - MARGIN_RULER_LEFT - MARGIN_RULER_RIGHT) {
			...
			final int NewLeftValue = MARGIN_RULER_LEFT
					+ ((LeftValue - MARGIN_RULER_LEFT) * (NewRulerWidth)) / RulerWidth;
			final int NewRightValue = MARGIN_RULER_LEFT
			...
			GraduationWidth = (float) ((RulerWidth * Ctrl.getSegmentSize())
					/ (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()));
		}

		Color BackColor = null;
		final Object SliderBackColor = UIManager.getLookAndFeel().getDefaults().get("Slider.background");
		...
		Graphics2.fillRoundRect(MARGIN_RULER_LEFT, Margin_Ruler_Top, RulerWidth, RulerHeight, Ctrl.getArcSize(),
				Ctrl.getArcSize());

		Graphics2.setColor(Ctrl.getSliderBackground().darker());
		Graphics2.drawArc(MARGIN_RULER_LEFT + 1, Margin_Ruler_Top, Ctrl.getArcSize(), Ctrl.getArcSize(), 90, 90);
		Graphics2.drawArc(MARGIN_RULER_LEFT + RulerWidth - Ctrl.getArcSize() - 1, Margin_Ruler_Top, Ctrl.getArcSize(),
				Ctrl.getArcSize(), 0, 90);

		Graphics2.setColor(Ctrl.getSliderBackground().brighter());
		Graphics2.drawArc(MARGIN_RULER_LEFT + RulerWidth - Ctrl.getArcSize() - 1,
				Margin_Ruler_Top + RulerHeight - Ctrl.getArcSize(), Ctrl.getArcSize(), Ctrl.getArcSize(), 270, 90);
		Graphics2.drawArc(MARGIN_RULER_LEFT + 1, Margin_Ruler_Top + RulerHeight - Ctrl.getArcSize(), Ctrl.getArcSize(),
				Ctrl.getArcSize(), 180, 90);
		...
		Graphics2.drawLine(MARGIN_RULER_LEFT + 1, Margin_Ruler_Top + Ctrl.getArcSize() / 2, MARGIN_RULER_LEFT + 1,
				Margin_Ruler_Top + RulerHeight - Ctrl.getArcSize() / 2);
		Graphics2.setColor(Ctrl.getSliderBackground().brighter());
		...
		/*
		 * NewClip = new RoundRectangle2D.Float( MARGIN_RULER_LEFT+2, Margin_Ruler_Top,
		...
		final double[][] ColTable = Ctrl.getColorTable();

		// Graphics2.setClip(NewClip);
		for (int i = 0; i <= SegmentCount; i++) {
			double Val = Ctrl.getMinimumValue() + i * Ctrl.getSegmentSize();
			final double Val2 = Ctrl.getMinimumValue() + (i + 1) * Ctrl.getSegmentSize();
			...
			if (Val == Ctrl.getMaximumValue() || Val == Ctrl.getMinimumValue()) {
				...
				LastMax = x + NumberWidth;
					...
					- TheFontMetrics.stringWidth(MaxNumberString) && x >= LastMax + 8) {
				Graphics2.drawString(NumberString, x, Margin_Ruler_Top - 1);
				LastMax = x + NumberWidth;
			}
			...
			if (ContentPainterSupport1.getPainterListenerNumber() > 0 && Val < Ctrl.getMaximumValue()) {
				final int x0 = MARGIN_RULER_LEFT + (int) (GraduationWidth * i);
				int x3 = MARGIN_RULER_LEFT + (int) (GraduationWidth * (i + 1));
				final Rectangle Rect1bis = new Rectangle(x0, Margin_Ruler_Top + 2, x3 - x0, RulerHeight - 3);

				x3 = Math.min(x3, MARGIN_RULER_LEFT + RulerWidth);
				final Rectangle RectClip = new Rectangle(x0, Margin_Ruler_Top + 2, x3 - x0 + 2, RulerHeight - 3);

				final Shape ShapeClip = Graphics2.getClip();
				final Color Color0 = Graphics2.getColor();
			...
			if (i == 0) {
				RectFirstLabel = new Rectangle(x - 1, Margin_Ruler_Top - TheFontMetrics.getAscent(), NumberWidth + 1,
						TheFontMetrics.getAscent());
			}

			if (i == SegmentCount) {
				RectLastLabel = new Rectangle(x - 1, Margin_Ruler_Top - TheFontMetrics.getAscent(), NumberWidth + 1,
						TheFontMetrics.getAscent());
			}
		...
		for (int i = 0; i < ColTable.length; i++) {
			...
			if (ColTable[i][0] < ColTable[i][1]) {
				// the selection only
				final int x1 = (int) (MARGIN_RULER_LEFT
						+ (RulerWidth * (ColTable[i][0] - Ctrl.getMinimumValue())) / ValuesWidth) + 1;
				final int x2 = (int) (MARGIN_RULER_LEFT
						+ (RulerWidth * ((ColTable[i][1]) - Ctrl.getMinimumValue())) / ValuesWidth);

				// the full segment
				final int x0 = MARGIN_RULER_LEFT + (int) (GraduationWidth * (int) ColTable[i][3]);
				int x3 = MARGIN_RULER_LEFT + (int) (GraduationWidth * (int) (ColTable[i][3] + 1));
				final Rectangle Rect1bis = new Rectangle(x1, Margin_Ruler_Top + 2, x2 - x1, RulerHeight - 3);

				x3 = Math.min(x3, MARGIN_RULER_LEFT + RulerWidth);
				final Rectangle Rect1 = new Rectangle(x1, Margin_Ruler_Top + 2, x2 - x1, RulerHeight - 3);
				final Rectangle RectClip = new Rectangle(x1 - 1, Margin_Ruler_Top + 2, x2 - x1 + 2, RulerHeight - 3);
				final Rectangle Rect2 = new Rectangle(x0, Margin_Ruler_Top + 2, x3 - x0, RulerHeight - 3);

				if (ContentPainterSupport1.getPainterListenerNumber() == 0) {
					Graphics2.fill(Rect1);
					if (i != 0) {
						Graphics2.setColor(new Color(Graphics2.getColor().getRed(), Graphics2.getColor().getGreen(),
								Graphics2.getColor().getBlue(), 192));
					...
					ContentPainterSupport1.firePaint(Ctrl, Graphics2, ColTable[i][0], ColTable[i][1],
							i + SegmentCountBefore, Graphics2.getColor(), Rect1bis, Rect2);
					Graphics2.setClip(OldClip);
					Graphics2.setColor(Color0);
				}
			}
		...
		if (Dragging == ALT_LEFT_RULER || Dragging == ALT_RIGHT_RULER) {
			...
			Graphics2.fillRect((int) (Rectangle2D1.getX() + Rectangle2D1.getWidth() / 2), Margin_Ruler_Top, 2,
					RulerHeight);
		}

		// Graphics2.setColor(getOppositeColor(Ctrl.getBackground()));
		Graphics2.setColor(Color.BLACK);
		Graphics2.drawRoundRect(MARGIN_RULER_LEFT, Margin_Ruler_Top, RulerWidth, RulerHeight, Ctrl.getArcSize(),
				Ctrl.getArcSize());

		paintThumbs(Graphics2);

		if (Dragging == ALT_RIGHT_RULER || Dragging == ALT_LEFT_RULER || Dragging == RIGHT_RULER || Dragging == SEGMENT
				|| Dragging == SHIFT_SEGMENT || Dragging == SHIFT_RIGHT_RULER) {
			String ValString = "";
		...
		if (Dragging == ALT_RIGHT_RULER || Dragging == ALT_LEFT_RULER || Dragging == LEFT_RULER || Dragging == SEGMENT
				|| Dragging == SHIFT_SEGMENT || Dragging == SHIFT_LEFT_RULER) {
			String ValString = "";
			...
			Graphics2.setColor(getOppositeColor(Ctrl.getForeground()));
			Graphics2.drawString(ValString, LeftValue - TheFontMetrics.stringWidth(ValString) - 1,
					Margin_Ruler_Top + RulerHeight - 1);
			Graphics2.setColor(Ctrl.getForeground());
			Graphics2.drawString(ValString, LeftValue - TheFontMetrics.stringWidth(ValString) - 2,
	...
	/**
	 * Paints shadowed dots on the given <tt>Graphics</tt> objects. The coordinates
	 * of the points must be given in arrays.
	 *
	 * @param g       the <tt>Graphics</tt> object on which we wand to paint the
	 *                dots
	 ...
	 */
	private void paintShadowedDots(final Graphics g, final int[] xCoords, final int[] yCoords, final int num,
			final Color DarkColor_Arg, final Color BrightColor_Arg) {
		for (int s = 0; s < 2; s++) {
	 ...
	 *         painted downward
	 */
	private void paintThumb(final Graphics Graphics_Arg, final boolean up, final Polygon Thumb_Arg,
			final Color MainColor_Arg, final Color DarkColor_Arg, final Color BrightColor_Arg) {

		final Graphics2D G2 = (Graphics2D) Graphics_Arg;

		// HACK: commented out the next block of code. Because it's being compared with
		// 'false', it can never run anyway. I suspect it's part of Borlander's
		// workaround
...
//		{
		...
		final int yy = up ? Thumb_Arg.ypoints[0] + 8 : Thumb_Arg.ypoints[6] - 13;
		final int xx = up ? Thumb_Arg.xpoints[0] : Thumb_Arg.xpoints[6];
		final int[] xp = { xx - 5, xx - 1, xx + 3, xx - 3, xx + 1, xx - 5, xx - 1, xx + 3 };
		final int[] yp = { yy, yy, yy, yy + 2, yy + 2, yy + 4, yy + 4, yy + 4 };
	 ...
	 * paint a thumb
	 */
	public void paintThumbs(final Graphics Graphics_Arg) {
		final Graphics2D Graphics2 = (Graphics2D) Graphics_Arg;
		...
		if (UIManager.getLookAndFeel().getName().equals("Metal")) {
			if (Dragging == LEFT_RULER || Dragging == SHIFT_LEFT_RULER || Dragging == ALT_LEFT_RULER)
				paintThumb(Graphics_Arg, true, TheLeft_Polygon, MetalLookAndFeel.getPrimaryControlShadow(),
						MetalLookAndFeel.getPrimaryControlDarkShadow(), MetalLookAndFeel.getPrimaryControl());
			else if (MouseUnder == LEFT_POLYGON)
				paintThumb(Graphics_Arg, true, TheLeft_Polygon, MetalLookAndFeel.getControlShadow(),
			...
			if (Dragging == RIGHT_RULER || Dragging == SHIFT_RIGHT_RULER || Dragging == ALT_RIGHT_RULER)
				paintThumb(Graphics_Arg, false, TheRight_Polygon, MetalLookAndFeel.getPrimaryControlShadow(),
						MetalLookAndFeel.getPrimaryControlDarkShadow(), MetalLookAndFeel.getPrimaryControl());
			else if ((MouseUnder == RIGHT_POLYGON))
				paintThumb(Graphics_Arg, false, TheRight_Polygon, MetalLookAndFeel.getControlShadow(),
						MetalLookAndFeel.getControlDarkShadow(), MetalLookAndFeel.getControl());
			else
	 ...
	 */
	public void setRulerValues(final double Min_Arg, final double Max_Arg) {
		...
		final int NewLeftValue = (int) (MARGIN_RULER_LEFT + ((Min_Arg - Ctrl.getMinimumValue()) * RulerWidth)
				/ (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()));
		final int NewRightValue = (int) (MARGIN_RULER_LEFT + ((Max_Arg - Ctrl.getMinimumValue()) * RulerWidth)
				/ (Ctrl.getMaximumValue() - Ctrl.getMinimumValue()));
		...
		if (NewLeftValue <= NewRightValue && NewLeftValue >= MARGIN_RULER_LEFT
			...
			if (UIManager.getLookAndFeel().getName().equals("Metal")) {
				final int[] xp1 = { LeftValue, LeftValue - 7, LeftValue - 7, LeftValue - 6, LeftValue + 6,
						LeftValue + 7, LeftValue + 7 };
				final int[] yp1 = { Margin_Ruler_Top + RulerHeight + 1 - 14, Margin_Ruler_Top + RulerHeight + 1 - 7,
						Margin_Ruler_Top + RulerHeight + 1, Margin_Ruler_Top + RulerHeight + 1 + 1,
						Margin_Ruler_Top + RulerHeight + 1 + 1, Margin_Ruler_Top + RulerHeight + 1,
						Margin_Ruler_Top + RulerHeight + 1 - 7 };
