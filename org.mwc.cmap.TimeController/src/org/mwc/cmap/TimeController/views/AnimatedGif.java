...
public class AnimatedGif {
	...
	public AnimatedGif(final Label lbl, final String filename) {
		this.lbl = lbl;
		try {
			imageDataArray = loader.load(
					FileLocator.openStream(TimeControllerPlugin.getDefault().getBundle(), new Path(filename), false));
					...
					if (!cancel) {
						...
						try {
							...
							while (loader.repeatCount == 0 || repeatCount > 0) {
								...
								shellGC.drawImage(offScreenImage, bounds.width - imageData.width - 5,
										bounds.height / 2 - imageData.height / 2);
	...
	public void resume(final Rectangle newBounds) {
		this.cancel = false;
		bounds = newBounds;
	}
