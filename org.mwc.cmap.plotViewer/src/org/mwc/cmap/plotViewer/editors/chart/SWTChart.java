...
public abstract class SWTChart extends PlainChart implements ISelectionProvider {
	...
	public abstract class CustomisedSWTCanvas extends SWTCanvas {
		...
		public CustomisedSWTCanvas(final Composite parent, final PlainProjection projection) {
		 ...
		 */
		abstract public void doSupplementalRightClickProcessing(MenuManager menuManager, Plottable selected,
				Layer theParentLayer);
		...
		protected void fillContextMenu(final MenuManager mmgr, final Point scrPoint, final WorldLocation loc) {
			...
			if (HitTester.doesHit(new java.awt.Point(scrPoint.x, scrPoint.y), loc, dist, getProjection())) {
			...
			if (res != null) {
				...
				if (e != null) {
					RightClickSupport.getDropdownListFor(mmgr, new Editable[] { res }, new Layer[] { theParent },
				...
				RightClickSupport.getDropdownListFor(mmgr, new Editable[] { getProjection() }, null, null, getLayers(),
						true);
				...
				if (noPoints != null) {
					...
					for (final Iterator<Plottable> iter = noPoints.iterator(); iter.hasNext();) {
						...
						RightClickSupport.getDropdownListFor(mmgr, new Editable[] { pl }, null, null, getLayers(),
								true);
			...
			final Action editProjection = new Action("Edit Projection") {
				@Override
				public void run() {
					final EditableWrapper wrapped = new EditableWrapper(getProjection(), getLayers());
					final ISelection selected = new StructuredSelection(wrapped);
					parentFireSelectionChanged(selected);
				}
	...
	abstract public static class PlotMouseDragger {
		 ...
		 * @param theCanvas
		 */
		public void doMouseMove(final org.eclipse.swt.graphics.Point pt, final int JITTER, final Layers theLayers,
	...
	/**
	 * create the transparent image we need to for collating multiple layers into an
	 * image
	 *
	...
	protected static Image createSWTImage(final ImageData myImageTemplate) {
		...
		final int transPx = myImageTemplate.palette
	 ...
	 * @param _myProjection
	 */
	public SWTChart(final Layers theLayers, final Composite parent, final PlainProjection myProjection) {
		super(theLayers);
		...
		CorePlugin.getDefault().getPreferenceStore().addPropertyChangeListener(_propertyChangeListener);
	...
	private void clearImages() {
		...
		while (iter.hasNext()) {
			...
			if (nextI instanceof Image) {
				...
				CorePlugin.logError(IStatus.ERROR, "unexpected type of image found in buffer:" + nextI, null);
			}
		}
	...
	public void close() {
		...
		CorePlugin.getDefault().getPreferenceStore().removePropertyChangeListener(_propertyChangeListener);
	 ...
	 */
	public SWTCanvas createCanvas(final Composite parent, final PlainProjection projection) {
		return new CustomisedSWTCanvas(parent, projection) {
			...
			@Override
			public void doSupplementalRightClickProcessing(final MenuManager menuManager, final Plottable selected,
										...
										if (bl.isBuffered()) {
											...
											if (image == null) {
												...
												if (_myImageTemplate == null) {
													...
													final Image template = new Image(Display.getCurrent(), canvasWidth,
															canvasHeight);
													// and remember it.
												...
												final SWTCanvasAdapter ca = new SWTCanvasAdapter(dest.getProjection());
	...
	//
	// if
	// (this.getClass().getName().startsWith("org.mwc.cmap.overview.views.ChartOverview"))
	// {
