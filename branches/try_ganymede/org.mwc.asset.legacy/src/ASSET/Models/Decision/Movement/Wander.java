...
/**
 * Title: Wander
 * Description:  Wander around an origin, passing up to an indicated range from it
  ...
  java.io.Serializable
{
  ...
  /**
   * an offset to randomly apply to the course to the centre
   * of the area - to give the wandering effect
   */
                                     ...
                                     final long time)
  {
    ...
    if (demStat != null)
    {
      // if we have an existing demanded status, we will continue with it - there's no need to change it if
      // we don't need to
   ...
   */
  protected String setNewCourse(final Status status, Double oldDemCourse, SimpleDemandedStatus res)
  {
    StringBuffer activity = new StringBuffer();

    // what's the range to the origin?
    final MWC.GenericData.WorldVector offset = _origin.subtract(status.getLocation());
  ...
  public final ASSET.Participants.DemandedStatus decide(final ASSET.Participants.Status status,
                                                        ASSET.Models.Movement.MovementCharacteristics chars, final DemandedStatus demStatus, final ASSET.Models.Detection.DetectionList detections,
  ...
  public static class Wander2Test extends SupportTesting.EditableTesting
  {
    ...
    {
      Wander newWander = new Wander(createLocation(0, 0), new WorldDistance(21, WorldDistance.KM));
