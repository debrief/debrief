...
public class SWTCanvasAdapter implements CanvasType, Serializable, Editable
{
     ...
     * keep on failing if it's not. We should remember when its not avaialble,
     * and not bother calling from there on.
     */
    ...
    protected void switchAntiAliasOn(final boolean val)
    {
        ...
        if (_gdiAvailable)
        {
            ...
            {
                CorePlugin.logError(Status.INFO, "Graphics library not found",
                        e);
        ...
        if (!_theDest.isDisposed())
...

            {
                ...
                for (int thisC = 0; thisC < theString.length(); thisC++)
                {
                    ...
                    if (thisChar == ' ')
                    {
                        thisWid = _theDest.getFontMetrics()
                                .getAverageCharWidth();
                    }
                    else
                    {
            ...
            final ImageObserver observer)
    {
        ...
        for (int x = 0; x < 48; x++)
        {
            for (int y = 0; y < 48; y++)
            {
                ...
                {
                    imageData.setPixel(x, y, SWTRasterPainter.toSWTColor(255,
                            0, 0)); // Set
                    // the
                    // center
                    // to
                ...
                {
                    imageData.setPixel(x, y, SWTRasterPainter.toSWTColor(0,
                            255, 0)); // Set
                    // the
                    // outside
                    // to
    ...
    private static int[] getPolygonArray(int[] xPoints, int[] yPoints,
            int nPoints)
    {
     ...
     *            list of x coordinates
     * @param yPoints
     *            list of y coordinates (or null if xPoints contains both lists -
