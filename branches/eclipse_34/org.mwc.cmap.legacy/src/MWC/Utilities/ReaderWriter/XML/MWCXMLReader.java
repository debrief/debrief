...
public class MWCXMLReader extends DefaultHandler
{
	...
	protected void handleOurselves(final String name, final Attributes attributes)
	{
		...
		while (enumer.hasMoreElements())
		{
			...
			if (val != null)
			{
				...
				{
					MWC.Utilities.Errors.Trace.trace(e, "Trouble handling attribute: "
							+ ha.myName + " for:" + _myType);
				}
			}
			else
			{
				// let's not bother about parameters not being found, they're mostly
				// optional anyway
				// NO, KEEP IT IN! to give us some hints if somebody's application isn't
				// working too well.
	...
	abstract static public class HandleDoubleAttribute extends HandleAttribute
	{
		...
		public final void setValue(final String name, final String value)
		{
			...
			{
				MWC.Utilities.Errors.Trace.trace(pe, "Reader: Whilst reading in "
						+ name + " value of :" + value);
			}
		}
	...
	abstract static public class HandleDateTimeAttribute extends HandleAttribute
	{
		...
		public final void setValue(final String name, final String value)
		{
			...
			{
				MWC.Utilities.Errors.Trace.trace(e, "Failed to parse Date value of:"
						+ value + " for:" + name);
			}
		}
	...
	{
		// parse the text
		final double res = shortFormat.parse(val).doubleValue();
