...
public class StepperXYPlot extends XYPlot implements StepperListener
{
   ...
   */
  public StepperXYPlot(final XYDataset data,
   ...
   * a printer), together with a current time marker
   * <P>
   ...
   */
  public final void draw(final Graphics2D g2, final Rectangle2D plotArea, final ChartRenderingInfo info)
  {
    ...
    if (_currentTime != null)
    {
      ...
      CanBeRelativeToTimeStepper axis = (CanBeRelativeToTimeStepper) this.getHorizontalAxis();
      ...
      if (axis instanceof HorizontalDateAxis)
      {
        ...
        linePosition = dateAxis.translateDateToJava2D(new Date(theTime / 1000), dataArea);
      }
      else
      {
   ...
   * @param dataArea
   */
  protected void plotStepperLine(final Graphics2D g2, double linePosition, Rectangle2D dataArea)
  {
    // prepare to draw
    ...
    g2.drawLine((int) linePosition - 1, (int) dataArea.getY() + 1,
                (int) linePosition - 1, (int) dataArea.getY() + (int) dataArea.getHeight() - 1);
