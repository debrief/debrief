...
public class VPlot extends Composite implements IVPartMovement, IVTime, ParticipantDetectedListener {
	...
	public VPlot(final Composite parent, final int style) {
		...
		_myChart.getCanvas().getProjection().setRelativeProjectionParent(new RelativeProjectionParent() {
		...
		followTrack.addSelectionListener(new SelectionAdapter() {
			...
			@Override
			public void widgetSelected(final SelectionEvent e) {
				_myChart.getCanvas().getProjection().setRelativeMode(followTrack.getSelection(), false);
			}
	...
	public void moved(final Status status) {
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				theFix.setCourse(MWC.Algorithms.Conversions.Degs2Rads(status.getCourse()));
