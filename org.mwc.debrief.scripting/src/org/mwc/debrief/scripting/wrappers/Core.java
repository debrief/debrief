...
public class Core {
	...
	public static class TestCore extends TestCase {
		...
		public void testCreateDateCalendarFormat() {
			...
			final HiResDate created = createDateCalendarFormat(now.get(Calendar.YEAR), now.get(Calendar.MONTH),
	 ...
	 * @param date      Field number to indicate the day of the month. This is a
	 *                  synonym for DATE. The first day of the month has value 1.
	 * @param hourOfDay Field number to indicate the hour of the morning or
	 *                  afternoon. HOUR is used for the 12-hour clock (0 - 11). Noon
	 *                  and midnight are represented by 0, not by 12. E.g., at
	 *                  10:04:15.250 PM the HOUR is 10.
	 * @param minute    Field number for get and set indicating the minute within
	 *                  the hour. E.g., at 10:04:15.250 PM the MINUTE is 4
	 * @param second    Field number for get and set indicating the second within
	 *                  the minute. E.g., at 10:04:15.250 PM the SECOND is 15.
	 * @return // @type MWC.GenericData.HiResDate
	 */
	@WrapToScript
	public static HiResDate createDateCalendarFormat(final int year, final int month, final int date,
	 ...
	 */
	@WrapToScript
	public static HiResDate createDateFromString(final String date, final SimpleDateFormat formatter)
	 ...
	 * @param units Unit of the duration as an integer. Options available:
	...
	/**
	 * Function that creates a font object given a font name as string, an style and
	 * size.
	 * 
	 * @see java.awt.Font#Font(String, int, int)
	 * @param fontName Font name as String. For example: "Serif.plain"
	 * @param style    Style of the font. For example: java.awt.Font.PLAIN
	 * @param size     Size of the font created.
	 * @return // @type java.awt.Font
	 */
	@WrapToScript
	public static Font createFont(final String fontName, final int style, final int size) {
	 ...
	 * 
	 * @see org.mwc.debrief.scripting.wrappers.Plot
	 * @param filename Name of the plot editor.
	 * @return // @type org.mwc.debrief.scripting.wrappers.Plot
	 */
	@WrapToScript
	public static Plot getPlot(@ScriptParameter(defaultValue = "unset") final String filename) {
	...
	private static boolean validDescriptor(final String descriptor) {
		return "org.mwc.debrief.PlotEditor".equals(descriptor) || "org.mwc.debrief.TrackEditor".equals(descriptor);
	}
	...
	private void listenToMyParts() {
		...
		final IWorkbenchWindow window = editor.getSite().getPage().getWorkbenchWindow();
		...
		_partMonitor.addPartListener(TimeProvider.class, PartMonitor.ACTIVATED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				if (!provider.equals(_timeProvider)) {
					...
					if (_timeProvider != null) {
						_timeProvider.removeListener(listener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
					}

					_timeProvider = provider;
					_timeProvider.addListener(listener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
				}
			}
		});

		// Listen for anyone that can provide time
		_partMonitor.addPartListener(TimeProvider.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				final TimeProvider provider = (TimeProvider) part;
