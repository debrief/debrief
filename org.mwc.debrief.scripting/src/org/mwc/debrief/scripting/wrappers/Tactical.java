...
public class Tactical {
	public static class TaticalTest extends TestCase {
		...
		static final double speedYps = new WorldSpeed(speedMs, WorldSpeed.M_sec).getValueIn(WorldSpeed.ft_sec) / 3;
		...
		public void testCreateFix() {
			...
			assertEquals("Same course for Fix", courseDegs, fix.getCourseDegs(), 1e-5);
			assertEquals("Same speedYps for Fix", speedYps, fix.getFix().getSpeed(), 1e-5);
		}

		public void testCreateNarrative() {
			final NarrativeEntry entry = createNarrativeEntry(trackName, type, dtg, entryString);
			assertEquals("Same value for track name", trackName, entry.getTrackName());
			...
			assertEquals("Same name for Narrative Wrapper", narrativeTitle, narrativeWrapper.getName());
			assertEquals("Entry is contained in narrative wrapper", entry, narrativeWrapper.elements().nextElement());
		}
		...
		public void testCreateTrack() {
			...
			assertEquals("Same name for Track Wrapper", trackName, trackWrapper.getName());
		}
	}

	/**
	 * Create a fix given the time, location, course in radians and speed in yards
	 * per seconds.
	 * 
	 ...
	 */
	@WrapToScript
	public static FixWrapper createFix(final HiResDate time, final WorldLocation location, final double courseDegs,
			final double speedMs) {
		final double courseRads = MWC.Algorithms.Conversions.Degs2Rads(courseDegs);
		final double speedYps = new WorldSpeed(speedMs, WorldSpeed.M_sec).getValueIn(WorldSpeed.ft_sec) / 3d;
	 ...
	 *
	 * @param title Title of the narrative.
	 ...
	 */
	@WrapToScript
	public static NarrativeEntry createNarrativeEntry(final String track, final String type, final HiResDate DTG,
			final String entry) {
	 ...
	 * @param name Name of the new track created.
