...
public class RendererWithDynamicFeedback extends XYLineAndShapeRenderer {
	...
	private final Stroke myFeedbackStroke = new BasicStroke(1.0f, BasicStroke.CAP_SQUARE, BasicStroke.JOIN_MITER, 10.0f,
			new float[] { 5.0f, 5.0f }, 0.0f);

	/**
	 * All parameters are domain coordinates that have to be translated to Java2D
	 * points.
	 */
	...
	private void drawFeedbackItemLabel(final Graphics2D g2, final PlotOrientation orientation, final XYDataset dataset,
			final int series, final int item, final double xx, final double yy, final boolean b) {
		// no labels for now
	}
	...
	public void drawItem(final Graphics2D g2, //
		...
		if (hasFeedbackFor(series, item)) {
			drawItemFeedback(g2, state, dataArea, info, plot, domainAxis, rangeAxis, dataset, series, item,
					crosshairState, pass);
		}
	}
	...
	protected void drawItemFeedback(final Graphics2D g2, //
		...
		if (isLinePass(pass)) {
			...
			drawFeedBackNode(g2, plot, dataset, pass, series, item, domainAxis, dataArea, rangeAxis, crosshairState,
					entities);
		}
	}
	...
	private boolean hasFeedbackFor(final int series, final int item) {
		return myFeedBackRowAndColumn != null && myFeedBackValue != null && myFeedBackRowAndColumn.x == series
				&& myFeedBackRowAndColumn.y == item;
	}
