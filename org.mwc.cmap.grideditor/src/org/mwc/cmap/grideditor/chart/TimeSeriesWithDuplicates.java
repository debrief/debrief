...
class TimeSeriesWithDuplicates {
	...
	/**
	 * For every {@link TimeStampedDataItem} which is the <strong>duplicate</strong>
	 * maps it to the its counterpart {@link BackedTimeSeriesDataItem} which is
	...
	/**
	 * For every {@link TimeStampedDataItem} which is the <strong>duplicate</strong>
	 * maps it to the pre-created {@link BackedTimeSeriesDataItem} which is NOT in
	 * the series but stores the original time for the case when item's time will be
	 * changed.
	 */
	private final IdentityHashMap<TimeStampedDataItem, BackedTimeSeriesDataItem> myDomainDuplicate2Deferred;

	/**
	 * For every {@link TimeStampedDataItem} which actually presents in the series,
	 * maps it to its {@link BackedTimeSeriesDataItem}.
	 */
	...
	private List<TimeStampedDataItem> findRegisteredDuplicatesFor(final BackedTimeSeriesDataItem original) {
		...
		for (final Map.Entry<TimeStampedDataItem, BackedTimeSeriesDataItem> next : myDomainDuplicate2Existing
				.entrySet()) {
		...
		if (isDuplicate(removedItem)) {
			...
			if (!duplicates.isEmpty()) {
				...
				for (final TimeStampedDataItem next : duplicates) {
					final BackedTimeSeriesDataItem precreatedChartItem = myDomainDuplicate2Deferred
							.get(luckyDomainItem);
					removeDuplicate(next);
					if (next == luckyDomainItem) {
						addToSeries(precreatedChartItem);
					} else {
						// now it becomes the duplicate for the "lucky" one which is about to be added
						// to series
	...
	private BackedTimeSeriesDataItem searchForExistingDuplicate(final BackedTimeSeriesDataItem chartItem) {
		// we should consider the possibility that chartItem is duplicate for some
		// existing period
		// (because duplicates are not supported by TimeSeries)
		// but we don't want to call binarySearch for EVERY domain item added (or being
		// constructed on setInput)
		// so, we will check for most frequent case when we can state for sure that
		...
		// ok, bad luck, hopefully rare case, but we have to check anyway by
		// binarySearch
