...
public class ScatteredXYSeries extends XYSeries {
	...
	/**
	 * Unfortunately the updateBoundsForAddedItem method is private in super class
	 * as of 1.0.13 release.
	 *
	 * In order to allow insertion into the arbitrary index (for
	 * {@link ScatteredXYSeries}) we must somehow call this method after insertion
	 * in order to update bounds.
	 *
	...
	protected final void callUpdateBoundsForAddedItem(final XYDataItem item) {
		try {
			if (myUpdateBoundsForAddedItemMethod == null) {
				myUpdateBoundsForAddedItemMethod = XYSeries.class.getDeclaredMethod("updateBoundsForAddedItem",
						XYDataItem.class);
			}
			if (myUpdateBoundsForAddedItemMethod != null) {
