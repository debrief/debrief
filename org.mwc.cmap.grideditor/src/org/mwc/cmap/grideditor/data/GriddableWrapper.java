...
public class GriddableWrapper implements GriddableSeries {
	...
	public void addPropertyChangeListener(final PropertyChangeListener listener) {
		if (_item.getEditable() instanceof SupportsPropertyListeners) {
			final SupportsPropertyListeners pw = (SupportsPropertyListeners) _item.getEditable();
	...
	public void fireModified(final TimeStampedDataItem subject) {
		if (_item.getEditable() instanceof SupportsPropertyListeners) {
			...
			CorePlugin.logError(IStatus.ERROR, "Item in grid doesn't let us watch it's properties", null);
	...
	public GriddableItemDescriptor[] getAttributes() {
		if (_myAttributes == null) {
			...
			final GriddableSeriesMarker series = (GriddableSeriesMarker) _item.getEditable();
			...
			if (items.size() > 0) {
				_myAttributes = items.toArray(new GriddableItemDescriptor[] { null });
			}
		}
		return _myAttributes;
	}
