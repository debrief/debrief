...
public class GridEditorView extends ViewPart {
	...
	private static void activateOutlineView(final CorePlotEditor editor) {
		final IContentOutlinePage outline = (IContentOutlinePage) editor.getAdapter(IContentOutlinePage.class);
		if (outline != null) {
			outline.setFocus();
		}
	}
	...
	private static GriddableWrapper extractGriddableSeries(final ISelection selection) {
		...
		if (firstElement instanceof EditableWrapper) {
			...
			if (wrapped != null) {
				if ((value instanceof GriddableSeriesMarker) && !(value instanceof Editable.DoNoInspectChildren)) {
			...
			if (firstElement instanceof IAdaptable) {
				...
				final EditableWrapper wrapped = is.getAdapter(EditableWrapper.class);
				if (wrapped != null) {
					final Object value = wrapped.getEditableValue();
	...
	public void createPartControl(final Composite parent) {
		final GridEditorActionContext actionContext = new GridEditorActionContext(myUndoSupport);
	...
	private void handleWorkspaceSelectionChanged(final ISelection actualSelection) {
		...
		if (input == null) {
			...
			final GriddableWrapper existingInput = (GriddableWrapper) myUI.getTable().getTableViewer().getInput();

			// see if we're currently looking at something
			final EditableWrapper editable = existingInput != null ? existingInput.getWrapper() : null;
	...
	public void init(final IViewSite site) throws PartInitException {
		super.init(site);
		final ISelectionService selectionService = site.getWorkbenchWindow().getSelectionService();
	...
	private void initUndoSupport() {
		myUndoSupport = new GridEditorUndoSupport(
				PlatformUI.getWorkbench().getOperationSupport().getOperationHistory());
		// set up action handlers that operate on the current context
		myUndoAction = new UndoActionHandler(this.getSite(), myUndoSupport.getUndoContext());
		myRedoAction = new RedoActionHandler(this.getSite(), myUndoSupport.getUndoContext());
	}
