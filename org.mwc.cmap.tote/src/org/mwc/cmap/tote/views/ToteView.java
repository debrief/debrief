...
/**
 * View which provides a track tote. The track tote is a table of values who are calculated using
 * the current status of one or more vessel tracks
 * <p>
 */
...
public class ToteView extends ViewPart
{
      ...
      ITableFontProvider
  {
    ...
    /**
     * remember the fonts we're going to use Start off with the font for secondary tracks
     */
      ...
      if ((res == null) && (_theDTG != null))
      {
        if (_trackData != null)
        {
          ...
          if (_thePrimary != null)
          {
            ...
            final Watchable[] nearestPrimaries = _thePrimary.getNearestTo(
                _theDTG);
            ...
            if (secLists != null)
            {
              ...
              else
              {
                ...
                if (columnIndex < 2)
                {
                  CorePlugin.logError(IStatus.ERROR, "Wrong column index:"
                      + columnIndex, null);
                }
                else
                {
        ...
        if (_trackData != null)
        {
          ...
          else if (columnIndex == 1)
          {
            ...
            if (_thePrimary != null)
            {
              ...
              {
                final FixWrapper fix = (FixWrapper) pw;
  ...
  /**
   * the table content provider (containing both the calculations and the tracks)
   */
  ...
  /**
   * automatically pass through the data, and automatically assign the relevant watchable items to
   * primary, secondary, etc.
   * 
   * @param onlyAssignTracks
   *          - as we scan through the layers, only put TrackWrappers onto the tote
   */
  ...
  /**
   * This is a callback that will allow us to create the _tableViewer and initialize it.
   */
  @Override
  ...
  private void createViewActions()
  {
    ...
    _removeTrackAction = new Action("Remove this track", IAction.AS_PUSH_BUTTON)
    {
      /*
       * (non-Javadoc)
       * 
    ...
    // -------------------------------------------------------
    _autoGenerate = new Action("Auto-populate tote", IAction.AS_PUSH_BUTTON)
    {
      @Override
      public void run()
      {
    ...
    _autoGenerate.setImageDescriptor(CorePlugin.getImageDescriptor(
        "icons/16/gears_run.png"));
    // -------------------------------------------------------
    // put watchables on the tote
    // -------------------------------------------------------
    ...
    _autoGenerateJustTracks.setImageDescriptor(CorePlugin.getImageDescriptor(
        "icons/16/gears_run.png"));
    ...
    _removeTrackAction = new Action("Remove this track", IAction.AS_PUSH_BUTTON)
    {
      ...
      public void runWithEvent(final Event event)
      {
        ...
        else
        {
          ...
          final WatchableList thisSec = _trackData.getSecondaryTracks()[index
              - 2];
  ...
  protected int findSelectedColumn(final Event e, final Table theTable)
  {
    if (theTable.getColumnCount() < 2)
    {
      return -1;
    }
    final Point pt = Display.getCurrent().map(null, theTable, new Point(e.x,
        e.y));
    final Rectangle clientArea = theTable.getClientArea();
    final boolean header = clientArea.y <= pt.y && pt.y < (clientArea.y
        + theTable.getHeaderHeight());
    if (!header)
    {
      return -1;
    }
    if (theTable.getColumnCount() == 2)
    {
      final int w = theTable.getColumn(0).getWidth();
      if (pt.x > w)
      {
        return 1;
      }
    ...
    {
      final int w1 = theTable.getColumn(0).getWidth();
      final int w2 = w1 + theTable.getColumn(1).getWidth();
      final int w3 = w2 + theTable.getColumn(2).getWidth();
      if (pt.x > w3)
      {
        return 3;
      }
  ...
  private void hookContextMenu()
  {
    ...
    theTable.addListener(SWT.MenuDetect, new Listener()
    {
      ...
      {
        final int index = findSelectedColumn(e, theTable);
        if (index > 0 && !N_A.equals(theTable.getColumn(index).getText())
    ...
    theTable.addMouseListener(new MouseAdapter()
    {
      ...
      public void mouseUp(final MouseEvent e)
      {
        ...
        {
          // cool. sorted.
          final int index = findSelectedColumn(e.x, e.y, _tableViewer
              .getTable());
          if (index != -1)
          {
            final MenuManager mmgr = new MenuManager();
      ...
      throws PartInitException
  {
    ...
    _showUnits = new Action("Show calc units column", IAction.AS_CHECK_BOX)
    {
      @Override
      public void run()
      {
    ...
    _showUnits.setImageDescriptor(TotePlugin.getImageDescriptor(
        TotePlugin.IMG_SHOW_UNIT_COLUMN));
    ...
    if (memento != null)
    {
      ...
      if (unitsVal != null)
      {
        _showUnits.setChecked(Boolean.getBoolean(memento.getString(
            SHOW_UNITS)));
      }
    }
  ...
  void redoTableAfterTrackChanges()
  {
      ...
      for (int i = 0; i < secs.length; i++)
      {
        ...
        if (thisSec != null)
        {
          ...
          for (int j = 0; j < items.length; j++)
          {
            ...
            final Color whiteCol = ColorHelper.getColor(new java.awt.Color(255,
                255, 255));
    ...
    if (pri != null)
    {
      final Color blackShade = ColorHelper.getColor(new java.awt.Color(0, 0,
          0));
      final Color lightCol = ColorHelper.getColor(new java.awt.Color(240, 240,
          245));
