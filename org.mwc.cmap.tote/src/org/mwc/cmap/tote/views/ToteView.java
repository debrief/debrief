...
public class ToteView extends ViewPart {
	...
	public class ToteLabelProvider implements ITableLabelProvider, ITableFontProvider {
		...
		/**
		 * remember the fonts we're going to use Start off with the font for secondary
		 * tracks
		 */
		...
		public ToteLabelProvider(final Font coreFont) {
			...
			_primaryFont = new Font(Display.getCurrent(), theOnly.getName(), theOnly.getHeight(),
					theOnly.getStyle() | SWT.BOLD);
			_interpolatedSecondaryFont = new Font(Display.getCurrent(), theOnly.getName(), theOnly.getHeight(),
					theOnly.getStyle() | SWT.ITALIC);
			_interpolatedPrimaryFont = new Font(Display.getCurrent(), theOnly.getName(), theOnly.getHeight(),
					theOnly.getStyle() | SWT.ITALIC | SWT.BOLD);
					...
					if (_thePrimary != null) {
						...
						if (secLists != null) {
							...
							if (columnIndex == 1) {
								...
								if (columnIndex < 2) {
									CorePlugin.logError(IStatus.ERROR, "Wrong column index:" + columnIndex, null);
								} else {
	...
	private void createViewActions() {
		...
		_removeTrackAction = new Action("Remove this track", IAction.AS_PUSH_BUTTON) {
			...
			public void runWithEvent(final Event event) {
				// cool. sorted.
				final int index = findSelectedColumn(event.x, event.y, _tableViewer.getTable());
				if (index != -1) {
		...
		_removeTrackAction.setImageDescriptor(
		...
		// put watchables on the tote
		// -------------------------------------------------------
		_autoGenerateJustTracks = new Action("Auto-populate tote (tracks only)", IAction.AS_PUSH_BUTTON) {
			@Override
			public void run() {
				autoGenerate(true);
			}
	...
	protected int findSelectedColumn(final Event e, final Table theTable) {
		...
		final Point pt = Display.getCurrent().map(null, theTable, new Point(e.x, e.y));
		final Rectangle clientArea = theTable.getClientArea();
		final boolean header = clientArea.y <= pt.y && pt.y < (clientArea.y + theTable.getHeaderHeight());
		if (!header) {
			return -1;
		}
		if (theTable.getColumnCount() == 2) {
	...
	private void initialiseCalcLoaders() {
		...
		_loader = new CalculationLoaderManager(EXTENSION_POINT_ID, EXTENSION_TAG, PLUGIN_ID) {
			...
			@Override
			public toteCalculation createInstance(final IConfigurationElement configElement, final String label) {
				// get the attributes
				...
				// create the instance
				res = new CalculationLoaderManager.DeferredCalculation(configElement, theLabel, icon);
	...
	void redoTableAfterTrackChanges() {
		...
		final Color greyCol = Display.getCurrent().getSystemColor(SWT.COLOR_WIDGET_BACKGROUND);
			...
			for (int i = 0; i < secs.length; i++) {
				...
				if (thisSec != null) {
					...
					for (int j = 0; j < items.length; j++) {
						...
						final Color whiteCol = ColorHelper.getColor(new java.awt.Color(255, 255, 255));
		...
		if (pri != null) {
			final Color blackShade = ColorHelper.getColor(new java.awt.Color(0, 0, 0));
			final Color lightCol = ColorHelper.getColor(new java.awt.Color(240, 240, 245));
