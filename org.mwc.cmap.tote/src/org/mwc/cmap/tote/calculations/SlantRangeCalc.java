...
public class SlantRangeCalc extends rangeCalc {
	...
	/**
	 * override our paren't calculate method so we can choose whether to show slant
	 * range
	 *
	 * @param primary
	 * @param secondary
	...
	public double calculate(final Watchable primary, final Watchable secondary, final HiResDate thisTime) {
		...
		if ((primary != null) && (secondary != null) && (primary != secondary)) {
			...
			if (doSlant()) {
				...
				final double depthSep = Math.abs(primary.getLocation().getDepth() - secondary.getLocation().getDepth());
	...
	private boolean doSlant() {
		final String calcSlantStr = CorePlugin.getToolParent()
				.getProperty(PrefsPage.PreferenceConstants.CALC_SLANT_RANGE);
	...
	/**
	 * override our parent's update method so we can annotate the range if we're
	 * actually calculating the slant range.
	 *
	 * @param primary
	 * @param secondary
	 * @param time
	...
	public String update(final Watchable primary, final Watchable secondary, final HiResDate time) {
		...
		if (doSlant()) {
			...
			if (!res.equals(plainCalc.NOT_APPLICABLE)) {
