...
public class tesselate {
	...
	public static ArrayList<Geometry> ST_Tile(Geometry p_geom, double p_Tile_X,
		...
		// throw new Exception
		// (c_s_option_value.replace("*VALUE*",v_option_value));
		// }
		...
		if (p_geom.getGeometryType().equalsIgnoreCase("Point")
			...
			for (int i = 0; i < v_vertices.length; i++) {
				...
				grid.add(gf.createPoint(new Coordinate((v_loCol * p_Tile_X)
						+ v_half_x, (v_loRow * p_Tile_Y) + v_half_y)));
		...
		for (int v_col = v_loCol; v_col <= v_hiCol; v_col++) {
			for (int v_row = v_loRow; v_row <= v_hiRow; v_row++) {
				...
				if (!im.isDisjoint()) {
					// if ( !
					// ST_Relate(v_clip_geom,"DETERMINE",p_geom,p_precision).equalsIgnoreCase("DISJOINT")
					// ) {
	...
	public static double getPrecisionScale(int _numDecPlaces) {
		return _numDecPlaces < 0 ? (double) (1.0 / Math.pow(10,
				Math.abs(_numDecPlaces))) : (double) Math
				.pow(10, _numDecPlaces);
	}
