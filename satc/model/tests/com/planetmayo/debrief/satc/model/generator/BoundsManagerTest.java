...
public class BoundsManagerTest extends ModelTestBase
{
	...
	public void prepareBoundsManager() 
	{
		...
		boundsManager.addSteppingListener(new ISteppingListener()
		{
			
			@Override
			public void stepped(IBoundsManager eventManager, int thisStep, int totalSteps)
			{
			...
			@Override
			public void error(IBoundsManager eventManager, IncompatibleStateException ex)
			{
				assertSame(boundsManager, eventManager);
				error = ex;
			}
			...
			@Override
			public void complete(IBoundsManager eventManager)
			{
				assertSame(boundsManager, eventManager);
				complete = true;
			}
	...
	{
		VehicleType vehicleType = new MockVehicleTypesManager().getAllTypes().get(0);
	...
	public void testStep() 
	{
		...
		boundsManager.addSteppingListener(new SteppingAdapter() {
		...
		boundsManager.step();
		assertEquals(bearingMeasurementContribution.getNumObservations() + 2, boundsManager.getSpace().size());
	...
	public void testGetCurrentContributionAndStep() 
	{
		...
		assertSame(bearingMeasurementContribution, boundsManager.getCurrentContribution());
		boundsManager.step();
		assertEquals(2, boundsManager.getCurrentStep());
		assertSame(courseForecastContribution, boundsManager.getCurrentContribution());
		boundsManager.restart();
		assertEquals(0, boundsManager.getCurrentStep());
		assertNull(boundsManager.getCurrentContribution());		
	}
	...
	{
		assertFalse(0 == bearingMeasurementContribution.getPropertyChangeListenersCount());
		...
		assertEquals(0, bearingMeasurementContribution.getPropertyChangeListenersCount());
		assertFalse(0 == courseForecastContribution.getPropertyChangeListenersCount());
		boundsManager.clear();
