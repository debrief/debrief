		...
		public void testCustomQuery() {
			...
			try {
				ConfigurationReader.loadDatabaseConfiguration(_config, new LoaderOption[] {
						new LoaderOption(LoaderType.DEFAULT_FILE, DatabaseConnection.DEFAULT_POSTGRES_DATABASE_FILE) });
				...
				final Scanner scanner = new Scanner(
						OSUtils.getInputStreamResource(Measurement.class, MEASUREMENTS_FILE, Activator.PLUGIN_ID));
				final StringBuilder builder = new StringBuilder();
				while (scanner.hasNextLine()) {
					builder.append(scanner.nextLine());
					builder.append("\n");
				}
				final List<Measurement> list = postgresDatabaseConnection.listAll(Measurement.class, builder.toString(),
						null);
