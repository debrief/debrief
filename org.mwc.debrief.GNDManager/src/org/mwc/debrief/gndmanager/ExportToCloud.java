		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			if (docs.size() > 0) {
				...
				try {
					...
					CorePlugin.logError(IStatus.INFO, "CouchDb POST (upload) to: " + newURL, null);
					...
					if (result == 201) {
...

							{
								...
								for (int i = 0; i < len; i++) {
									...
									if (item.has("error")) {
										...
										Activator.logError(IStatus.ERROR, "Import to Cloud failed:" + reason.asText(),
												null);
									} else {
						...
						Activator.logError(IStatus.ERROR, "Post failed: " + result, null);
					}

				} catch (final IOException e) {
					Activator.logError(IStatus.ERROR, "Failed writing to String", e);
				}
			}
