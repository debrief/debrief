...
public class TrackStoreWrapper extends BaseLayer implements WatchableList, Serializable {
	...
	public static class CouchTrack implements Serializable, Plottable {
		...
		public CouchTrack(final TrackWrapper tw) {
			...
			while (posits.hasMoreElements()) {
				final Editable editable = posits.nextElement();
				...
				speeds.add(new WorldSpeed(fw.getSpeed() / 3d, WorldSpeed.ft_sec).getValueIn(WorldSpeed.M_sec));
		...
		public WorldLocation getLocationAt(final int i) {
			...
			if ((_latArray != null) && (_longArray != null)) {
				res = new WorldLocation(_latArray.get(i).asDouble(), _longArray.get(i).asDouble(), 0);
			}
			return res;
		}
			...
			if (_myName == null) {
				...
				if (metadata != null) {
					...
					if (_myName == null) {
						...
						_myName = _myName + "-" + theID.substring(theID.length() - 5, theID.length());
				}
			}
		...
		public boolean overlaps(final BaseTimePeriod targetPeriod) {
			...
			if (_myCoverage == null) {
				...
				if (times != null && times.size() > 0) {
					...
					_myCoverage = new TimePeriod.BaseTimePeriod(new HiResDate(fDate), new HiResDate(lDate));
				}
			}
		...
		public void paint(final CanvasType dest) {
			...
			final long startTime = _parent._currentFilterPeriod.getStartDTG().getDate().getTime();
			final long endTime = _parent._currentFilterPeriod.getEndDTG().getDate().getTime();
		...
		public PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final PropertyDescriptor[] res = { prop("Visible", "the Layer visibility", VISIBILITY),
						...
						displayProp("IncludeInTimeStep", "Include in time stepping",
								"include these tracks in time stepping", TEMPORAL), };
	...
	public void add(final Editable thePlottable) {
		if (thePlottable instanceof CouchTrack) {
			...
			CorePlugin.logError(IStatus.ERROR, "Track store cannot add this element:" + thePlottable, null);
	}
	...
	private synchronized String collateDateFilter(final HiResDate start, final HiResDate end) {
		...
		final String res = SearchRequestConverter.toRequestString(request, new ElasticFacet[] {},
				new ElasticFacet[] {});
	...
	private void doTheDownload(final ArrayList<String> docsToDownload) {
		...
		try {
			...
			CorePlugin.logError(IStatus.INFO, "CouchDb get: " + uri, null);
			...
			if (result == 200) {
				...
				CorePlugin.logError(IStatus.ERROR, "Get from CouchFailed. response is:" + result, null);
			}
		...
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				if (docsToDownload.size() > 0) {
					...
					if (_dataLoaded == null)
						_dataLoaded = new TimePeriod.BaseTimePeriod(_currentFilterPeriod.getStartDTG(),
								_currentFilterPeriod.getEndDTG());
					else {
	...
	public WorldArea getBounds() {
		...
		if (_includeInBounds) {
			...
			while (iter.hasNext()) {
				final TrackStoreWrapper.CouchTrack thisT = iter.next();
	...
	private ArrayList<String> getDownloadList(final String myFilter) {
		...
		try {
			CorePlugin.logError(IStatus.INFO, "ElasticSearch get: " + uri, null);
			...
			if (result == 200) {
				...
				if (rawRows.isArray()) {
					...
					Activator.logError(IStatus.INFO, "ES found " + rowCount + " rows", null);
					if (rowCount < MAX_ROWS_TO_IMPORT) {
						...
						Activator.logError(IStatus.WARNING, "More than " + MAX_ROWS_TO_IMPORT + " rows returned!",
								null);
				}

			}
		} catch (final MalformedURLException e) {
			CorePlugin.logError(IStatus.ERROR, "failed to create URI for ElasticSearch query", e);
		} catch (final JsonParseException e) {
			CorePlugin.logError(IStatus.ERROR, "failed to parse returned ES rows", e);
		} catch (final JsonMappingException e) {
			CorePlugin.logError(IStatus.ERROR, "failed to parse returned ES rows", e);
		} catch (final IOException e) {
			CorePlugin.logError(IStatus.ERROR, "failed to parse returned ES rows", e);
	...
	/**
	 * whether to return bounds objects for the tracks when requested (both temporal
	 * & spatial)
	 *
	 */
							...
							if (times != null) {
								...
								if (ctr < times.size() - 1) {
									...
									if (_interpolatePoints && (ctr > 0)) {
										...
										final FixWrapper before = new FixWrapper(new Fix(
												new HiResDate(times.get(ctr - 1)), thisT.getLocationAt(ctr - 1), 0, 0));
										final FixWrapper next = new FixWrapper(
												new Fix(new HiResDate(times.get(ctr)), thisT.getLocationAt(ctr), 0, 0));
	...
	public void paint(final CanvasType dest) {
		...
		if (_currentFilterPeriod == null) {
			CorePlugin.logError(IStatus.WARNING, "Track Store hasn't had filter applied yet", null);
	...
	public void removeElement(final Editable p) {
		if (p instanceof CouchTrack) {
			...
			CorePlugin.logError(IStatus.ERROR, "Track store cannot remove this element:" + p, null);
	}
