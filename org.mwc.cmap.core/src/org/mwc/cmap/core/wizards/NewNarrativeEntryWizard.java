...
public class NewNarrativeEntryWizard extends Wizard {
	...
	public void addPages() {
		...
		namePage = new EnterStringPage(null, "TrackNameHere", PAGE_TITLE,
				...
				"a one-word title for the track the entry relates to (e.g. NELSON)", imagePath, helpContext, true,
				null);
		addPage(namePage);

		final HiResDate date = defaultDate != null ? defaultDate : new HiResDate(new Date());

		dtgPage = new EnterDTGPage(null, date, PAGE_TITLE, "Please specify the date-time for the entry",
				"Date-time the observation was recorded", imagePath, helpContext);
		addPage(dtgPage);

		typePage = new EnterStringPage(null, "EntryType", PAGE_TITLE, "Now specify what type of entry this is.",
				"a one-word title for the type of observation (e.g. Command_Comment)", imagePath, helpContext, true,
				null);
		addPage(typePage);

		entryPage = new EnterStringPage(null, "Content of the entry", PAGE_TITLE, "Now specify the observation itself",
				"the observation (e.g. Start of trial Optimara)", imagePath, helpContext, true, null);
		addPage(entryPage);
	...
	public NarrativeEntry getEntry() {
		return new NarrativeEntry(namePage.getString(), typePage.getString(), dtgPage.getDate(), entryPage.getString());
	}
