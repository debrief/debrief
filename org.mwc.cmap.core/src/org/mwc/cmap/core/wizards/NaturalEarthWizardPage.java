...
public class NaturalEarthWizardPage extends CoreEditableWizardPage {
	...
	public NaturalEarthWizardPage(final ISelection selection) {
		super(selection, "nePage", "Add Vectored background data",
				"Use this page to add the Natural Earth dataset to your plot.", "images/ne_wizard.png", null);
	}

	@Override
	protected void addComponents(final Composite container) {
		final Link link1 = new Link(container, SWT.NONE);
		...
		final String path = getDataFolder();
		if (path == null || path.isEmpty() || !(new File(path).isDirectory())) {
			...
			link.addSelectionListener(new SelectionAdapter() {
				@Override
				public void widgetSelected(final SelectionEvent event) {
					final PreferenceDialog dialog = PreferencesUtil.createPreferenceDialogOn(link.getShell(),
	...
	protected Editable createMe() {
		if (_editable == null) {
			...
			try {
				_editable = (Editable) handlerService.executeCommand("org.mwc.cmap.naturalearth.ui.CreateNELayer",
						null);
	...
	private String getDataFolder() {
		return Platform.getPreferencesService().getString("org.mwc.cmap.NaturalEarth", "dataFolder", null, null);
	}
