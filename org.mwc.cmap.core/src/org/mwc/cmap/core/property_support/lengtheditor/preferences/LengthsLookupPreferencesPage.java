...
public class LengthsLookupPreferencesPage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	private void addFileEditor() {
		...
		myFileEditor = new FileFieldEditor(LengthsRegistry.FILE_NAME, Messages.LengthsLookupPreferencesPage_FileLabel,
	...
	private void addOpenHelpHyperlink() {
		...
		GridDataFactory.fillDefaults().span(numColumns, 1).align(SWT.BEGINNING, SWT.CENTER).applyTo(helpBtn);
	}
	...
	private void openSystemEditor() {
		...
		if ((fileName == null) || (fileName.length() == 0)) {
			...
			try {
				...
				CorePlugin.logError(IStatus.ERROR, Messages.LengthsLookupPreferencesPage_ErrorOnOpenFileEditor, e);
			}
		}
	}
