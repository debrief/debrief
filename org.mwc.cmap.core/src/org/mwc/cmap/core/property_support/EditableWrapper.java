...
public class EditableWrapper implements IPropertySource {
	...
	public static class OrderedEditableWrapper extends EditableWrapper implements Comparable<OrderedEditableWrapper> {
		...
		public OrderedEditableWrapper(final Editable plottable, final EditableWrapper parent, final Layers theLayers,
				final int myIndex) {
	...
	final protected static class PropertyChangeAction extends AbstractOperation {
		 ...
		 * @param parentLayer the parent layer - the one to be updated following a
		 *                    change
		 * @param wholeLayers the layers object we inform about the change
		 */
		public PropertyChangeAction(final Object oldValue, final Object newValue, final DebriefProperty subject,
				final String name, final Layers wholeLayers, final Layer topLevelLayer) {
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			if ((newValue == null && oldValue != null) || !newValue.equals(oldValue)) {
				...
				_property._subject.getInfo().fireChanged(_property.getEditable(), _property.getDisplayName(), _oldValue,
						_newValue);
		...
		public IStatus undo(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			// ok, and tell any listeners that want to know...
			_property._subject.getInfo().fireChanged(_property.getValue(), _property.getDisplayName(), _newValue,
					_oldValue);
					...
					if (pds != null) {
						for (int j = 0; j < pds.length; j++) {
							...
							if (pd.isExpert()) {
								...
								// ok, add this editor
								final IPropertyDescriptor newProp = new DebriefProperty(pd, obj, null);
	...
	private static void addPropertyEditors(final Vector<IPropertyDescriptor> list, final EditorType editor) {
		...
		if (properties != null) {
			for (int i = 0; i < properties.length; i++) {
				...
				if (thisProp instanceof DeprecatedPropertyDescriptor) {
					...
					final IPropertyDescriptor newProp = new DebriefProperty(thisProp, (Editable) editor.getData(),
							null);
					list.add(newProp);
				}
			}
	...
	final protected static Class getPropertyClass(final PropertyDescriptor thisProp) {
	 ...
	 * @param theLayers
	 */
	public EditableWrapper(final Editable plottable, final EditableWrapper parent, final Layers theLayers) {
		_editable = plottable;
	...
	public void addEditors(final Vector<IPropertyDescriptor> list, final Editable sample, final Griddable grid,
		...
		if (properties != null) {
			for (int i = 0; i < properties.length; i++) {
				...
				if (thisProp instanceof DeprecatedPropertyDescriptor) {
					...
					// ok, wrap it, and add it to our list.
					final IPropertyDescriptor newProp = new DebriefProperty(thisProp, sample, null);
					list.add(newProp);
				}
			}
		} else {
			final NonBeanPropertyDescriptor[] nonBean = grid.getNonBeanGriddableDescriptors();
			if (nonBean != null) {
	...
	public void addSuplementalEditors(final Vector<IPropertyDescriptor> list, final Editable.EditorType editor) {
		if (editor != null) {
			final BeanInfo[] others = editor.getAdditionalBeanInfo();
			if (others != null) {
				// adding more editors
			...
			if (opposite != null) {
				...
				for (int i = 0; i < _myDescriptors.length; i++) {
					...
					if (descriptor instanceof DebriefProperty) {
						...
						final DebriefProperty descriptorProperty = (DebriefProperty) descriptor;
						if (descriptorProperty.getName().contains(opposite)
								&& descriptorProperty.getName().contains(innerName)) {
							final DTGPropertySource oppositeValueObject = (DTGPropertySource) descriptorProperty
									.getValue();

							final HiResDate oppositeValue = oppositeValueObject.getValue();
							if (HiResDate.isNotInitialized(myValueHiResDate)
									|| HiResDate.isNotInitialized(oppositeValue)) {
								return true;
							}
							if (_END.equals(opposite)) {
								return myValueHiResDate.getMicros() <= oppositeValue.getMicros();
							} else if (_START.equals(opposite)) {
								return oppositeValue.getMicros() <= myValueHiResDate.getMicros();
							}
						}
					}
				}
	 ...
	 *
	 * @param id the string to look for
	...
	final public IPropertyDescriptor[] getGriddablePropertyDescriptors() {
		...
		if (_myGridDescriptors == null) {
			final Vector<IPropertyDescriptor> list = new Vector<IPropertyDescriptor>(0, 1);
			final IPropertyDescriptor[] res = new IPropertyDescriptor[] { null };
			if (_editable != null) {
				// is it griddable
				if (_editable instanceof GriddableSeriesMarker) {
					...
					final PropertyDescriptor[] properties = grid.getGriddablePropertyDescriptors();

					addEditors(list, sample, grid, properties);

					// hmm, are there any "supplemental" editors?
					addSuplementalEditors(list, editor);
				}
			}
	...
	final public void setPropertyValue(final Object id, final Object value) {
		...
		if (valueChanged) {
			...
			final PropertyChangeAction pca = new PropertyChangeAction(oldVal, value, thisProp, getEditable().getName(),
					getLayers(), getTopLevelLayer());
