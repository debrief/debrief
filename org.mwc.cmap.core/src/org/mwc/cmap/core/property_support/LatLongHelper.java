...
public class LatLongHelper extends EditorHelper {
	...
	public static class LatLongPropertySource implements IPropertySource2 {
		...
		static {
			...
			descriptors = new IPropertyDescriptor[] { new CategorisedDescriptor(ID_LAT_DEG, "1. Lat Degrees", "Lat"),
		...
		public boolean equals(final Object obj) {
			...
			if (obj instanceof LatLongPropertySource) {
				final LatLongPropertySource o = (LatLongPropertySource) obj;
		...
		private void firePropertyChanged(final String type) {
			final PropertyChangeEvent event = new PropertyChangeEvent(this, type, _originalLocation, getLocation());
		...
		private WorldLocation getLocation() {
			...
			try {
				...
				res = new WorldLocation(dLatDeg, dLatMin, dLatSec, _latHem.charAt(0), dLongDeg, dLongMin, dLongSec,
						_longHem.charAt(0), depth);

			} catch (final ParseException ee) {
				CorePlugin.logError(IStatus.ERROR, "Failed to correctly parse double in lat helper", ee);
		...
		public WorldLocation getValue() {
			...
			try {
				...
				CorePlugin.logError(IStatus.ERROR, "Failed to produce location", e);
				res = _originalLocation;
			}
			return res;
		}
		...
		public void refresh() {
			...
			_depth = _origDepth = new WorldDistance(_originalLocation.getDepth(), WorldDistance.METRES);
		}
	...
	private static class PasteLocationDialogCellEditor extends DialogCellEditor {
		...
		protected Object openDialogBox(final Control cellEditorWindow) {
			...
			if (val != null) {
				...
				CorePlugin.showMessage("Paste location", "Sorry, there is no suitable text on the clipboard");
			}
