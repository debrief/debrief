...
public class RightClickSupport {
	...
	private static class ListPropertyAction extends AbstractOperation {
		...
		public ListPropertyAction(final String propertyName, final Editable[] editable, final Method getter,
				final Method setter, final Object newValue, final Layers layers, final Layer parentLayer) {
			...
			try {
				...
				CorePlugin.logError(IStatus.ERROR,
						"Failed to retrieve old value for:" + "Multiple items starting with:" + _subjects[0].getName(),
						e);
			}
		...
		private IStatus doIt(final Object theValue) {
			...
			for (int cnt = 0; cnt < _subjects.length; cnt++) {
				...
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "Setter call failed:" + thisSubject.getName() + " Error was:"
							+ e.getTargetException().getMessage(), e.getTargetException());
					res = Status.CANCEL_STATUS;
				} catch (final IllegalArgumentException e) {
					CorePlugin.logError(IStatus.ERROR, "Wrong parameters pass to:" + thisSubject.getName(), e);
					res = Status.CANCEL_STATUS;
				} catch (final IllegalAccessException e) {
					CorePlugin.logError(IStatus.ERROR, "Illegal access problem for:" + thisSubject.getName(), e);
					res = Status.CANCEL_STATUS;
				}
			}
	...
	public static interface RightClickContextItemGenerator {
		public void generate(IMenuManager parent, Layers theLayers, Layer[] parentLayers, Editable[] subjects);
	}
	...
	private static class SubjectMethod extends Action {
		 ...
		 */
		public SubjectMethod(final String title, final Editable[] subject, final Method method, final Layer topLayer,
		...
		public void run() {
			...
			if ("exportThis".equals(_method.getName()) || "exportShape".equals(_method.getName())) {
			...
			for (int cnt = 0; cnt < len; cnt++) {
				...
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "whilst firing method from right-click", e);
				} catch (final IllegalAccessException e) {
					CorePlugin.logError(IStatus.ERROR, "whilst firing method from right-click", e);
				} catch (final InvocationTargetException e) {
					CorePlugin.logError(IStatus.ERROR, "whilst firing method from right-click", e);
	...
	static public final class testMe extends junit.framework.TestCase {
		...
		public final void testAdditionalNonePresent() {
			final ShapeWrapper sw = new ShapeWrapper("rect",
					new RectangleShape(new WorldLocation(12.1, 12.3, 12), new WorldLocation(1.1, 1.1, 12)), Color.red,
					new HiResDate(2222));
			final Editable[] editables = new Editable[] { sw };
		...
		public final void testAdditionalSomePresent() {
			final LabelWrapper lw = new LabelWrapper("Some label", new WorldLocation(1.1, 1.1, 12), Color.red);
			final Editable[] editables = new Editable[] { lw };
			final MenuManager menu = new MenuManager("Holder");
		...
		public final void testPropMgt() {
			final Editable itemOne = new FixWrapper(new Fix(new HiResDate(122333), new WorldLocation(1, 2, 3), 12, 14));
			final Editable itemTwo = new FixWrapper(new Fix(new HiResDate(122334), new WorldLocation(1, 2, 5), 13, 12));
	...
	public static class UndoableAction extends AbstractOperation {
		...
		public UndoableAction(final String propertyName, final Editable[] editable, final SubjectAction action,
		 ...
		 *
		 * @param fireExtended whether to fire extended (or just reformatted)
		 */
		...
		public IStatus redo(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			for (int cnt = 0; cnt < _subjects.length; cnt++) {
				...
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "Failed to set new value for:" + thisSubject.getName(), e);
					res = Status.CANCEL_STATUS;
				}
			}
	 ...
	 */
	public static void addRightClickGenerator(final RightClickContextItemGenerator generator) {
		if (_additionalRightClickItems == null) {
			_additionalRightClickItems = new Vector<RightClickContextItemGenerator>(1, 1);
		}
	...
	public static MenuManager createMenuFor(final IMenuManager manager, final Editable[] editables,
			final Layers theLayers, final Layer theTopLayer, MenuManager subMenu,
		...
		for (final String thisCat : map.keySet()) {
			...
			for (final PropertyDescriptor thisP : list) {
				...
				if (supportsBooleanEditor(thisP)) {
					...
					subMenu = generateBooleanEditorFor(manager, subMenu, thisP, editables, theLayers, theTopLayer);
				} else {
					// now the drop-down lists
					if (supportsListEditor(thisP)) {
						// generate boolean editors in the sub-menu
						subMenu = generateListEditorFor(manager, subMenu, thisP, editables, theLayers, theTopLayer);
					}
				}
			}
	...
	static private MenuManager generateBooleanEditorFor(final IMenuManager manager, final MenuManager subMenu,
			final PropertyDescriptor thisP, final Editable[] editables, final Layers theLayers,
		...
		try {
			...
			CorePlugin.logError(IStatus.ERROR, "Failed to retrieve old value for:" + editables[0].getName(), e);
		}

		final IAction changeThis = new Action(thisP.getDisplayName(), IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
				try {
					final ListPropertyAction la = new ListPropertyAction(thisP.getDisplayName(), editables, getter,
							setter, Boolean.valueOf(isChecked()), theLayers, topLevelLayer);
	...
	static private MenuManager generateListEditorFor(final IMenuManager manager, final MenuManager subMenu,
			final PropertyDescriptor thisP, final Editable[] editables, final Layers theLayers,
			final Layer topLevelLayer) {
					...
					public void run() {
						try {
							...
							if (isChecked()) {
								...
								final ListPropertyAction la = new ListPropertyAction(thisP.getDisplayName(), editables,
							...
							CorePlugin.logError(IStatus.INFO, "While executing select editor for:" + thisP, e);
						}
					}
	...
	static private IAction generateUndoableActionFor(final MWC.GUI.Tools.SubjectAction theAction,
			final Editable[] editables, final Layers theLayers, final Layer topLevelLayer) {

		final IAction changeThis = new Action(theAction.toString(), IAction.AS_PUSH_BUTTON) {
			@Override
			public void run() {
				try {
					final AbstractOperation la = new UndoableAction(theAction.toString(), editables, theAction,
	...
	/** have a look at the supplied editors, find which properties are common */
	protected static PropertyDescriptor[] getCommonPropertiesFor(final Editable[] editables) {
				...
				if (res != null) {
					...
					if (len > 1) {
						...
						for (int cnt = 1; cnt < len; cnt++) {
							...
							// do we have an editor?
							if (thisEditor != null) {
								final PropertyDescriptor[] newSet = thisEditor.getPropertyDescriptors();
	 ...
	 * @param topLevelLayers          the top-level layers that contain our elements
	 *                                (it's these that get updated)
	...
	static public void getDropdownListFor(final IMenuManager manager, final Editable[] editables,
			final Layer[] topLevelLayers, final HasEditables[] parentLayers, final Layers theLayers,
		...
		if (editables.length > 0) {
			...
			if (commonProps != null) {
				...
				final Map<String, SortedSet<PropertyDescriptor>> map = mapThese(commonProps);

				subMenu = createMenuFor(manager, editables, theLayers, theTopLayer, subMenu, map);
			}
					...
					if (additional != null) {
						...
						for (int i = 0; i < additional.length; i++) {
							...
							if (thisB instanceof EditorType) {
								...
								// and the properties
								final PropertyDescriptor[] theseProps = thisB.getPropertyDescriptors();

								// hey, can we group these descriptors?
								final Map<String, SortedSet<PropertyDescriptor>> map = mapThese(theseProps);

								// and wrap the object
								final Editable[] editables2 = new Editable[] { subject };

								subMenu = createMenuFor(manager, editables2, theLayers, theTopLayer, subMenu, map);
							}
						}
					}
				}
			...
			if (meths != null) {
				for (int i = 0; i < meths.length; i++) {
					...
					if (thisMethD == null) {
						CorePlugin.logError(IStatus.ERROR, "Failed to create method, props may be wrongly named", null);
					} else {
						// create button for this method
						final Action doThisAction = new SubjectMethod(thisMethD.getDisplayName(), editables,
			...
			if (actions != null) {
				for (int i = 0; i < actions.length; i++) {
					...
					// create button for this method
					final IAction doThisAction = generateUndoableActionFor(thisMethD, editables, theLayers,
							theTopLayer);
		...
		if (!hideClipboardOperations) {
			...
			RightClickCutCopyAdaptor.getDropdownListFor(manager, editables, topLevelLayers, parentLayers, theLayers,
					theClipboard);
			...
			RightClickPasteAdaptor.getDropdownListFor(manager, selectedItem, topLevelLayers, parentLayers, theLayers,
					theClipboard);
		...
		if (_additionalRightClickItems != null) {
			for (final Iterator<RightClickContextItemGenerator> thisItem = _additionalRightClickItems
				...
				try {
					...
					// and log the error
					CorePlugin.logError(IStatus.ERROR, "failed whilst creating context menu", e);
				}
			}
		}
	}
	 ...
	 */
	protected static MethodDescriptor[] getIntersectionFor(final MethodDescriptor[] a, final MethodDescriptor[] b,
	 ...
	 * @return the common elements
	 */
	protected static PropertyDescriptor[] getIntersectionFor(final PropertyDescriptor[] a, final PropertyDescriptor[] b,
	...
	private static SubjectAction[] getIntersectionFor(final SubjectAction[] a, final SubjectAction[] b,
	...
	/** have a look at the supplied editors, find which properties are common */
	protected static SubjectAction[] getUndoableActionsFor(final Editable[] editables) {
			...
			for (int i = 0; i < extensions.length; i++) {
				...
				for (int j = 0; j < confE.length; j++) {
					...
					try {
						...
						CorePlugin.logError(IStatus.ERROR, "Trouble whilst loading right-click handler extensions", e);
					}
				}
			}
		}
	...
	private static Map<String, SortedSet<PropertyDescriptor>> mapThese(final PropertyDescriptor[] theseProps) {
		final Map<String, SortedSet<PropertyDescriptor>> res = new TreeMap<String, SortedSet<PropertyDescriptor>>();
		if (theseProps != null) {
			for (final PropertyDescriptor thisP : theseProps) {
				final String cat;
				if (thisP instanceof CategorisedPropertyDescriptor) {
					final CategorisedPropertyDescriptor catProp = (CategorisedPropertyDescriptor) thisP;
				...
				if (thisL == null) {
					final Comparator<PropertyDescriptor> comparator = new PropertyComparator();
