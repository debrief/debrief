...
public class RightClickSupport
{
   ...
   * list of actions to be added to context-menu on right-click
   */
  private static Vector<AlternateRightClickContextItemGenerator> _additionalAlternateRightClickItems = null;
  ...
  /**
   * whether we've checked for any one that extends teh alternate right click
   * support via plugin xml
   * 
   */
  private static boolean _altRightClickExtensionsChecked = false;
   ...
   *          the generator to add...
   */
  public static void addAlternateRightClickGenerator(
      final AlternateRightClickContextItemGenerator generator)
  {
    if (_additionalAlternateRightClickItems == null)
      _additionalAlternateRightClickItems = new Vector<AlternateRightClickContextItemGenerator>(
          1, 1);

    _additionalAlternateRightClickItems.add(generator);
  }
  ...
  public static interface AlternateRightClickContextItemGenerator
  {
    public void generate(IMenuManager parent, Layers theLayers,
  ...
  public static void getAlt2DropdownListFor(IMenuManager manager,
      ArrayList<Editable> editables, ArrayList<Editable> parents,
      Layers theLayers)
  {
    // ok, see if we have any special helpers to help out with this

    if (!_altRightClickExtensionsChecked)
    {
      // TODO: once we've defined the plugin to support this
    //  loadAltLoaderExtensions();

      // ok, done
      _altRightClickExtensionsChecked = true;
    }

    // hmm, do we have any right-click generators?
    if (_additionalAlternateRightClickItems != null)
    {
      for (final Iterator<AlternateRightClickContextItemGenerator> thisItem = _additionalAlternateRightClickItems
          .iterator(); thisItem.hasNext();)
      {
        final AlternateRightClickContextItemGenerator thisGen = (AlternateRightClickContextItemGenerator) thisItem
            .next();

        try
        {
          thisGen.generate(manager, theLayers, parents, editables);
        }
        catch (final Exception e)
        {
          // and log the error
