...
public class DebriefProperty implements IPropertyDescriptor, IDebriefProperty {
	...
	public DebriefProperty(final PropertyDescriptor prop, final Editable subject, final Control theControl) {
		_thisProp = prop;
		_subject = subject;
	...
	private EditorHelper findHelperFor(final PropertyDescriptor prop, final Editable subject) {
		...
		if (res == null) {
			...
			if (res == null) {
				// ok, log the error
				final String msg = "editor not found for:" + EditableWrapper.getPropertyClass(prop) + "("
						+ prop.getDisplayName() + ")";
	...
	public String[] getFilterFlags() {
		...
		if (_thisProp.isExpert()) {
			res = new String[] { IPropertySheetEntry.FILTER_ID_EXPERT };
		} else {
			res = null;
		}
	...
	public void setValue(final Object value) {
		...
		try {
			if (write != null) {
				write.invoke(_subject, new Object[] { theValue });
			}
		} catch (final IllegalArgumentException e) {
			CorePlugin.logError(IStatus.ERROR, "Whilst setting property value for:" + theValue, e);
