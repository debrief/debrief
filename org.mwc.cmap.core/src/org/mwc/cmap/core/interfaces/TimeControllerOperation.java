...
public abstract class TimeControllerOperation {
	...
	/**
	 * whether there must be visible sensor data visible in the specified time
	 * period
	 */
	private final boolean _requiresSensorData;

	/**
	 * main constructor. we always want an operation name, so let the child classes
	 * call this with a name
	 *
	 * @param operationName what we put on the menu and the dialog
	 */
	protected TimeControllerOperation(final String operationName, final boolean requireSensor,
			final boolean singleSensor, final boolean doubleSensor, final boolean requiresSensorData) {
	 ...
	 * @param period
	 */
	abstract protected void executeExport(WatchableList primaryTrack, WatchableList[] secondaryTracks,
			TimePeriod period);
	...
	public void run(final WatchableList primary, final WatchableList[] secondaries, final TimePeriod period) {
		...
		if (_singleSensor) {
			...
			final SensorWrapper theSensor = FlatFileExporter.getSubjectSensor(primaryTrack);
		...
		if (_requiresSensorData) {
			...
			while (sensors.hasMoreElements()) {
				...
				if (thisS.getVisible()) {
					final Collection<Editable> theseCuts = thisS.getItemsBetween(period.getStartDTG(),
							period.getEndDTG());
					if (theseCuts != null)
