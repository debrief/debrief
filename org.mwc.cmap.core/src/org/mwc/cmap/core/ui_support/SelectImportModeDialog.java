...
public class SelectImportModeDialog extends ThreadSafeDialog<ImportSettings> implements SelectionListener {
	...
	public SelectImportModeDialog(final Shell parent, final String trackName, final int style) {
		...
		final String freq = CorePlugin.getToolParent().getProperty(RESAMPLE_FREQ);
	...
	public Composite createContents(final Composite shell) {
		...
		comboViewer.setInput(new Long[] { 0l, 5000l, 15000l, 60000l, 300000l, 600000l, 3600000l, Long.MAX_VALUE });

		comboViewer.getCombo().setLayoutData(new GridData(GridData.FILL_HORIZONTAL | GridData.GRAB_HORIZONTAL));
		comboViewer.setSelection(new StructuredSelection(Long.valueOf(_resampleFrequency)));
		...
		ok.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(final SelectionEvent event) {
				...
				if (_rememberIt) {
					...
					CorePlugin.getToolParent().setProperty(ImportReplay.RESAMPLE_FREQUENCY,
	...
	public void widgetSelected(final SelectionEvent e) {
		...
		final boolean isOTG = (_mode == ImportReplay.IMPORT_AS_OTG);
