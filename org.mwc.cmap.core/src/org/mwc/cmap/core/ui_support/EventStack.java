...
/**
 * queue of events, where the event only gets run if another doesn't get placed
 * on top of it before a specified time period passes
 *
 * @author ianmayo
 *
 */
public class EventStack {
	...
	/**
	 * a queue of events, where only the last one gets run - ideally placed for
	 * screen updates when changing lots of properties. Normally changing the
	 * property of hundreds of items will trigger hundreds of screen updates, this
	 * allows us to just fire the last one
	 *
	 * @param delay how (millis) long we should wait for another before running the
	 *              current operation
	 */
