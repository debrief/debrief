...
public class SWTClipper {
	 ...
	 */
	public static void drawLine(final GC theDest, final int x1, final int y1, final int x2, final int y2,
		...
		drawLine(theDest, x1, y1, mask1, x2, y2, mask2, xmin, xmax, ymin, ymax);
	}
	 ...
	 * @param ymax    &quot;upper&quot; side of rectangle
	 */
	protected static void drawLine(final GC theDest, final int x1, final int y1, final int mask1, final int x2,
			final int y2, final int mask2, final int xmin, final int xmax, final int ymin, final int ymax) {
		...
		if ((mask & OUTSIDE) == 0) {
			...
			// need clipping
			final Point[] p = getClipped(x1, y1, mask1, x2, y2, mask2, xmin, xmax, ymin, ymax);
			if (p != null) {
	 ...
	 */
	public static void drawLine(final GC g, final int x1, final int y1, final int x2, final int y2,
			final Rectangle rect) {
		drawLine(g, x1, y1, x2, y2, rect.x, rect.x + rect.width, rect.y, rect.y + rect.height);
	}
	 ...
	 * @param ymax &quot;upper&quot; side of rectangle
	 */
	public static void drawLine(final GC g, final Point p1, final Point p2, final int xmin, final int xmax,
			final int ymin, final int ymax) {
		drawLine(g, p1.x, p1.y, p2.x, p2.y, xmin, xmax, ymin, ymax);
	}
	 ...
	 * @param ymax    &quot;upper&quot; side of rectangle
	 */
	public static void drawPolyline(final GC g, final int[] x, final int[] y, final int nPoints, final int xmin,
		...
		if (nPoints == 1 && mask == INSIDE) {
			...
			for (int p = 1; p < nPoints; ++p) {
				drawLine(g, x[p - 1], y[p - 1], masks[p - 1], x[p], y[p], masks[p], xmin, xmax, ymin, ymax);
			}
		}
	}
	 ...
	 * @param rect    rectangle to clip to (assuming width and height are positive)
	 */
	public static void drawPolyline(final GC g, final int[] x, final int[] y, final int nPoints, final Rectangle rect) {
		drawPolyline(g, x, y, nPoints, rect.x, rect.x + rect.width, rect.y, rect.y + rect.height);
	}
	 ...
	 */
	protected static Point[] getClipped(final double x1, final double y1, final int mask1, final double x2,
		...
		/*
		 * System.out.println("mask1 = "+mask1); System.out.println("mask2 = "+mask2);
		...
		if (mask1 == INSIDE) {
			...
			// shortcut: no point is inside, but both are in the same sector, so no
	 ...
	 * @return <code>null</code> (does not clip) or array of two points
	 */
	public static Point[] getClipped(final int x1, final int y1, final int x2, final int y2, final int xmin,
			final int xmax, final int ymin, final int ymax) {
		int mask1 = 0; // position mask for first point
		int mask2 = 0; // position mask for second point
	 ...
	 */
	private static Point intersect(final double x11, final double y11, final double x12, final double y12,
		...
		if (det != 0.0) {
			...
			if (mu >= 0.0 && mu <= 1.0) {
				final Point p = new Point((int) (x21 + mu * dx2 + 0.5), (int) (y21 + mu * dy2 + 0.5));
