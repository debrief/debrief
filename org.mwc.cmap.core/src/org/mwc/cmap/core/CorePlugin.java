 ...
 */
public class CorePlugin extends AbstractUIPlugin implements ClipboardOwner, PreferenceProvider {
	...
	static public final class ClipboardTest extends junit.framework.TestCase {
		public void testNearGreenwich() {
			...
			assertEquals("Expected 26 degrees, 08 minutes, 0.455 seconds north", 26.134597222222222,
					fromClipboard.getLat());
			assertEquals("Expected 21 degrees, 56 minutes, 56.52 seconds north", 21.949033333333333,
					fromClipboard.getLong());
	...
	public static final String BULK_NARRATIVE_VIEWER = "org.mwc.cmap.narrative.views.BulkNarrativeViewer";
	...
	public static final String UNIT_CENTRIC = "org.mwc.debrief.core.view.unit_centric";
	...
	public static final String VIDEO_PLAYER_VIEW = "org.mwc.cmap.media.views.VideoPlayerView";
	public static final String IMAGE_VIEWER_VIEW = "org.mwc.cmap.media.views.ImagesView";

	public static final String[] SUPPORTED_MEDIA_FORMATS = { "*.avi", "*.vob", "*.mp4", "*.mov", "*.mpeg", "*.flv",
			"*.mp3", "*.wma", "*.wmv", "*.mp3", "*.*" };
	 ...
	 *
	 */
	static final public String PREF_BASE60_FORMAT_NO_SECONDS = PLUGIN_ID + ".base60-no-seconds";
	...
	public static Action createOpenHelpAction(final String target, final String description, final IViewPart host) {
		return createOpenHelpAction(target, description,
	 ...
	 * @return
	 */
	public static Action createOpenHelpAction(final String target, final String description,
			final IWorkbenchHelpSystem helpS) {
	 ...
	 * @param parentPart
	 */
	public static void editThisInProperties(final Vector<ISelectionChangedListener> selectionListeners,
			final StructuredSelection asSelection, final ISelectionProvider selectionProvider,
			...
			public void run() {
				try {
					...
					if (selectionListeners != null) {
						final SelectionChangedEvent sEvent = new SelectionChangedEvent(selectionProvider, asSelection);
						for (final Iterator<ISelectionChangedListener> stepper = selectionListeners.iterator(); stepper
								.hasNext();) {
					...
					logError(IStatus.ERROR, "Failed to open properties view when showing timer properties", e);
				}
			}
		});
	}
	...
	public static Image extendedGetImageFromRegistry(final String name) {
		...
		if (desc == null) {
			final Status status = new Status(IStatus.ERROR, "org.mwc.cmap.core", "Missing image " + name);
	...
	public static WorldLocation fromClipboard(final String txt) {
		...
		if (txt.startsWith(LOCATION_STRING_IDENTIFIER)) {
			...
			final String dataPart = txt.substring(LOCATION_STRING_IDENTIFIER.length(), txt.length());
			...
			res = new WorldLocation(_lat.doubleValue(), _long.doubleValue(), _depth.doubleValue());
		} else {
			...
			if (numTokens == 2) {
				try {
					...
					CorePlugin.logError(IStatus.ERROR, "whilst trying to get (lat,long) location off clipboard", e);
				}
				...
				if (latLong.countTokens() == 2) {
					...
					try {
						...
						final Sexagesimal latVal = SexagesimalSupport._DD_MM_SS_SSS
								.parse(latString.replaceAll("\"", "\u2033").replaceAll("'", "\u2032"), false);
						final Sexagesimal longVal = SexagesimalSupport._DD_MM_SS_SSS
								.parse(longString.replaceAll("\"", "\u2033").replaceAll("'", "\u2032"), true);
	...
	public static IUndoContext getUndoContext() {
		...
		if (Display.getCurrent() != null) {
			...
			try {
				...
				System.err.println("Class not found:" + dd.getMessage() + " (ok if in JUnit testing)");
			}
		}
		return contexts[0];
	}
	 ...
	 *                  applicable
	 */
	public static void logError(final int severity, final String message, Throwable exception,
	...
	public static IViewPart openSecondaryView(final String viewName, final String secondaryId, final int state) {
		IViewPart res = null;
		try {
			final IWorkbench wb = PlatformUI.getWorkbench();
			final IWorkbenchWindow win = wb.getActiveWorkbenchWindow();
			...
			logError(IStatus.ERROR, "Failed to open secondary " + viewName + "view", e);
		}
		return res;
	}
	...
	public static void run(final IUndoableOperation theAction) {
		...
		if (theAction != null) {
			...
			try {
				...
				logError(IStatus.ERROR, "Whilst adding new action to history buffer", e);
			}
		}
	}
	...
	public static String toClipboard(final WorldLocation loc) {
		final String res = LOCATION_STRING_IDENTIFIER + loc.getLat() + "," + loc.getLong() + "," + loc.getDepth();
		return res;
	}
	...
	public static void writeToClipboard(final String txt) {
		...
		final java.awt.datatransfer.Clipboard clip = java.awt.Toolkit.getDefaultToolkit().getSystemClipboard();
	...
	/**
	 * special-case handler that ensures that XY Plot views are closed when the app
	 * closes. XY-Plot views are "special" views because we don't wish them to
	...
	public CorePlugin() {
		...
		java.beans.PropertyEditorManager.registerEditor(Color.class, MWC.GUI.Properties.ColorPropertyEditor.class);
	}
	...
	/**
	 * Searches in the Eclipse Extension registry for EARTH_MODEL_PROVIDER
	 * extensions and creates one. Throws an error if there is more than one
	...
	private void evaluateEarthModelProviderExtension(final IExtensionRegistry registry) throws CoreException {
		final IConfigurationElement[] config = registry.getConfigurationElementsFor(EARTH_MODEL_PROVIDER);
	...
	private void executeExtension(final Object o) {
		final ISafeRunnable runnable = new ISafeRunnable() {
			...
			public void handleException(final Throwable e) {
				CorePlugin.logError(IStatus.ERROR, "Exception in providing Earth model", e);
			}
		...
		if (_defaultFont == null) {
			...
			if (pref != null && pref.length() > 0) {
				try {
					...
					_defaultFont = new Font(font.getName(), font.getStyle(), font.getHeight());
				} catch (final Exception e) {
	...
	public void start(final BundleContext context) throws Exception {
		...
		if (!iter2.hasNext()) {
			logError(IStatus.ERROR, "Failed to initialise TIFF reader for Java ImageIO", null);
			System.err.println("TIFF READER NOT READY");
		} else {
			logError(IStatus.INFO, "Successfully loaded TIFF reader for Java ImageIO", null);
		}
