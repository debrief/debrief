...
public class OTH_G_ContentDescriber extends TextContentDescriber {
	...
	public int describe(final InputStream contents, final IContentDescription description) throws IOException {
		...
		try {
			...
			final boolean canRead = OTH_Importer.canLoad(logger, r);
			res.set(canRead);
		} catch (final Exception e) {
			CorePlugin.logError(IStatus.ERROR, "OTH-G content type error", e);
		} finally {
			try {
				if (r != null)
					r.close();
