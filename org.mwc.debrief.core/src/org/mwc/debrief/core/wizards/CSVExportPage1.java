...
public class CSVExportPage1 extends CustomWizardPage {
	...
	public CSVExportPage1(final DropdownProvider provider, final String unit, final String provenance) {
		super(PAGE_ID);
	...
	protected Composite createDataSection(final Composite parent) {
		...
		sensorCmb = addCmbField(contents, "SENSOR", "Sensor:", "Source sensor", true, sensor, this, provider);

		unitNameTxt = addTxtField(contents, "Unit Name:", "Subject platform", unitName);

		flagCmb = addCmbField(contents, "FLAG", "Flag:", "Subject nationality", true, flag, this, provider);
		typeTxt = addTxtField(contents, "Type:", "Subject platform type", type);

		semiMajorAxisTxt = addTxtField(contents, "Semi-Major Axis (Nm):", "½ ellipse length", semiMajorAxis);

		semiMinorAxisTxt = addTxtField(contents, "Semi-Minor Axis (Nm):", "½ ellipse length", semiMinorAxis);
