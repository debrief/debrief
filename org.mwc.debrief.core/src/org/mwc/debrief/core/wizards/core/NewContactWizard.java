...
public class NewContactWizard extends Wizard {
	...
	public NewContactWizard(final HiResDate tNow, final TrackWrapper track, final SensorWrapper sensor) {
		_tNow = tNow;
		_track = track;
		_sensor = sensor;
	}
	...
	public void addPages() {
		...
		final MessageWizardPage introPage = new MessageWizardPage("introMessage", PAGE_TITLE, "Introduction",
				introMessage, imagePath);
		addPage(introPage);
		...
		// ok, we need to let the user enter the solution wrapper name
		rngBearingPage = new RangeBearingPage(null, PAGE_TITLE, "Specify the range/bearing to the contact",
				"range to contact (0.0 if not known)", "bearing to contact (degs)", imagePath, helpContext,
				new WorldDistance(5, WorldDistance.KYDS), 270d);
		addPage(rngBearingPage);

		// ok, we need to let the user enter the solution wrapper name
		colorPage = new SelectColorPage(null, Color.RED, PAGE_TITLE, "Now format the new sensor cut",
				"The color for this new sensor cut", imagePath, helpContext, null, false);
		addPage(colorPage);

		final String message = "The sensor cut will now be added to the specified sensor, \n" + " you "
				+ "can customise the cut further in the Properties window";
	...
	public SensorContactWrapper getContact() {
		...
		final String label = MWC.Utilities.TextFormatting.FormatRNDateTime.toString(_tNow.getDate().getTime());
		tw.setLabel(label);
		return tw;
	}
