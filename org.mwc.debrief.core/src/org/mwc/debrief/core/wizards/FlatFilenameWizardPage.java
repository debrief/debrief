...
public class FlatFilenameWizardPage extends WizardPage {
	...
	private static final String DOUBLE_SENSOR = "This wizard allows you to indicate the type of sensors used, "
			+ "and the " + "directory in which to place the output file. "
			+ "The output file will take the name of the primary file with a " + FILE_SUFFIX
			+ " suffix added. See online help for more details on the export format.";
	...
	// and the sensor type
	private final String[][] sensor2Types = new String[][] { { "Towed-LF", "TL" }, { "Towed-HF", "TH" },
			{ "HM-Bow", "HB" }, { "HM-Flank", "HF" }, { "HM-Intercept", "HI" } };

	// and the sensor type
	private final String[][] sensor1Types = new String[][] { { "Towed Array", "T" }, { "Hull mounted array", "H" } };
	...
	public FlatFilenameWizardPage(final String fileVersion, final int numSensors) {
		...
		super.setImageDescriptor(
				AbstractUIPlugin.imageDescriptorFromPlugin("org.mwc.debrief.core", "images/newplot_wizard.gif"));
	}
	...
	public void createControl(final Composite parent) {
		...
		if (_fileVersion.equals(FlatFileExporter.UPDATED_VERSION)) {
			...
			// ok, get the sensor1 depth
			final StringFieldEditor sensor1FwdEditor = new StringFieldEditor(sensor1fwdKey, "Sensor 1 fwd depth (m):",
					container) {
				@Override
				protected boolean doCheckState() {
		...
		if (_numSensors > 1) {
			_sensor2TypeEditor = new RadioGroupFieldEditor(sensor2Key, "Sensor 2 type:", 2, sensorTypes, container) {
				@Override
				protected void fireValueChanged(final String property, final Object oldValue, final Object newValue) {
					super.fireValueChanged(property, oldValue, newValue);
	...
	public boolean isPageComplete() {
		...
		if ((_filePath == null) || (_filePath.length() == 0) || (_serialName == null) || (_serialName.length() == 0))
			done = false;
