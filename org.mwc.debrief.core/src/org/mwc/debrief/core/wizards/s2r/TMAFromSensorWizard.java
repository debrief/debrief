...
public class TMAFromSensorWizard extends Wizard {
	...
	public TMAFromSensorWizard(final double brgDegs, final WorldDistance range, final double initialCourse,
			final WorldSpeed initialSpeed, final boolean showOffset, final Color color) {
		_brgDegs = brgDegs;
		_range = range;
	...
	public TMAFromSensorWizard(final double brgDegs, final WorldDistance range, final double initialCourse,
			final WorldSpeed initialSpeed, final Color color) {
	...
	public void addPages() {
		...
		selectOffsetPage = new RangeBearingPage(null, PAGE_TITLE, "Now specify the offset to the track start",
				"range from ownship to start of track", "bearing from ownship to start of track", imagePath,
				helpContext, _range, _brgDegs);

		if (_showOffset) {
			addPage(selectOffsetPage);
		}

		enterSolutionPage = new EnterSolutionPage(null, PAGE_TITLE, "Specify the initial Course and Speed values",
				imagePath, helpContext, _initialSpeed, _initialCourse);
		...
		if (_color != null) {
			// ok, we need to let the user enter the solution wrapper name
			colorPage = new SelectColorPage(null, _color, PAGE_TITLE, "Format the sensor cuts used for this leg",
					"The colour to shade the selected sensor cuts (or leave unchanged)", imagePath, helpContext, null,
					true);
			addPage(colorPage);
		}
	}
