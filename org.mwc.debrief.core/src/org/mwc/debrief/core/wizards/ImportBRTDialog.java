...
public class ImportBRTDialog extends Wizard implements BRTHelper {
	...
	public ImportBRTDialog(final TrackWrapper autoSelectedTrack, final TrackWrapper[] allTracks,
			final String defaultSensorName) {
		...
		this.isTowedPage = new EnterBooleanPage(null, true, PAGE_TITLE,
				"Please indicate if this data is for a Towed Array", "Towed Array Sensor? (yes/no)", imagePath,
				helpContext, "(You'll next be invited to enter the array length if it is)");
		this.towedOffsetPage = new EnterRangePage(null, PAGE_TITLE,
				...
				"Array Offset", defaultWidth, imagePath, helpContext, null, TOWED_OFFSET_PREF);
		this.colorPage = new SelectColorPage(null, DebriefColors.BLUE, PAGE_TITLE, "Please now format the new sensor",
		...
		this.cutLengthPage = new EnterRangePage(null, PAGE_TITLE,
				...
				"Default range", defRange, imagePath, helpContext, null, CUTLENGTH_PREF);

		if (defaultTrack == null && allTracks.length > 0) {
			this.trackPage = new SelectTrackPage(null, PAGE_TITLE, "Select a track",
					"Please, select the track to add the sensor data", imagePath, helpContext, false, null, allTracks,
					allTracks[0]);
		} else {
			this.trackPage = null;
		}
		...
		this.showSensorOnTrackPage = new EnterBooleanPage(null, true, PAGE_TITLE,
				"Please, indicate if want the sensor visible once loaded", "Sensor visibility (yes/no)", imagePath,
				helpContext, null);
	}
