...
class UnitCentricChart extends SWTChart {
	...
	abstract private class CorePaintOperation implements IOperateOnMatch {
		...
		@Override
		public void handlePrimary(final WatchableList primary, final WorldLocation origin, final long timeNow) {
		...
		private void plotSensorArcs(final WatchableList parent, final WorldLocation nearestOffset, final long time,
			...
			if (parent instanceof TrackWrapper) {
				...
				if (arcs.getVisible()) {
					...
					while (ele.hasMoreElements()) {
						final DynamicTrackShapeSetWrapper shapeSet = (DynamicTrackShapeSetWrapper) ele.nextElement();
						if (shapeSet.getVisible()) {
							shapeSet.paintOverride(dest, time, nearestOffset, primaryHeadingDegs);
						}
					}
				}
			}
		...
		@Override
		public void processNearest(final Watchable nearest, final WorldLocation nearestOffset,
				final double primaryHeadingDegs) {
			if (nearest instanceof FixWrapper) {
				final FixWrapper nearestInTime = (FixWrapper) nearest;
				...
				final LabelWrapper nearestInTime = (LabelWrapper) nearest;

				// draw the snail marker
				final PlainSymbol sym = nearestInTime.getSnailShape();
				sym.setColor(nearestInTime.getColor());
				sym.paint(dest, nearestOffset, MWC.Algorithms.Conversions.Degs2Rads(0d));
			}
	...
	private class NormalPaintOperation extends CorePaintOperation {
		...
		@Override
		public void doItTo(final Watchable rawSec, final WorldLocation offsetLocation, final double proportion) {
			dest.setLineWidth(2f);
			dest.setColor(rawSec.getColor());

			if (rawSec instanceof CanPlotFaded) {
				final CanPlotFaded faded = (CanPlotFaded) rawSec;
				faded.paintMe(dest, offsetLocation, rawSec.getColor());
			} else {
				CorePlugin.logError(IStatus.WARNING, "Expect all items here to implement CanPlotFaded interface", null);
			}
	...
	private class SnailPaintOperation extends CorePaintOperation {
		...
		@Override
		public void doItTo(final Watchable secondary, final WorldLocation offsetLocation, final double proportion) {
			if (secondary instanceof FixWrapper) {
				final FixWrapper fix = (FixWrapper) secondary;
				// try to use the same line width
				final LightweightTrackWrapper track = (LightweightTrackWrapper) fix.getTrackWrapper();
				if (track != null) {
			...
			// sort out the color
			final Color newCol = colorFor(secondary.getColor(), (float) proportion, getCanvas().getBackgroundColor());

			dest.setColor(newCol);

			if (secondary instanceof CanPlotFaded) {
				final CanPlotFaded faded = (CanPlotFaded) secondary;
	...
	protected static Color colorFor(final Color color, final float proportion, final Color backgroundColor) {
	...
	public UnitCentricChart(final Composite parent, final UnitDataProvider provider) {
		...
		_rangeRings = new RangeRingShape(new WorldLocation(0d, 0d, 0d), 5, new WorldDistance(5, WorldDistance.KM));
	}
	...
	private void checkDataCoverage(final Layers theLayers) {
		...
		if ((getCanvas().getProjection().getDataArea() == null) && (_provider.getTrackDataProvider() != null)) {
			final WatchableList primary = _provider.getTrackDataProvider().getPrimaryTrack();
			if (primary != null && primary instanceof WatchableList) {
				...
				final IOperateOnMatch getBounds = new IOperateOnMatch() {
					...
					@Override
					public void doItTo(final Watchable rawSec, final WorldLocation offsetLocation,
							final double proportion) {
						area.extend(offsetLocation);
					}

					@Override
					public void handlePrimary(final WatchableList primary, final WorldLocation origin,
							final long timeNow) {
						// ok, ignore
					}

					@Override
					public void processNearest(final Watchable nearestInTime, final WorldLocation nearestOffset,
							final double primaryHeadingDegs) {
						// ok, ignore
					}
				};
				UnitCentricView.walkTree(theLayers, primary, _provider.getTimeProvider().getTime(), getBounds,
						_provider.getSnailLength(), false);
	...
	private void doPaint(final CanvasType dest) {
		...
		// is it a track?
		final TrackWrapper priTrack = primary instanceof TrackWrapper ? (TrackWrapper) primary : null;
		...
		if (primary.getStartDTG().lessThan(subjectTime) && primary.getEndDTG().greaterThanOrEqualTo(subjectTime)) {
			...
			UnitCentricView.walkTree(_theLayers, primary, subjectTime, paintIt, _provider.getSnailLength(), _snailMode);
		}
	...
	public void paintMe(final CanvasType dest) {
		if (_theLayers == null) {
			CorePlugin.logError(IStatus.WARNING, "Unit centric view is missing layers", null);
		...
		if (_provider.getTrackDataProvider() == null) {
			CorePlugin.logError(IStatus.WARNING, "Unit centric view is missing track data provider", null);
			dest.setColor(new Color(200, 0, 0));
		...
		if (_provider.getTimeProvider() == null) {
			CorePlugin.logError(IStatus.WARNING, "Unit centric view is missing time provider", null);
			return;
		}
