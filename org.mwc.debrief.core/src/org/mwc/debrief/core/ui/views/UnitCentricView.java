...
public class UnitCentricView extends ViewPart implements PropertyChangeListener, UnitDataProvider {
	/**
	...
	private static class DistanceAction extends Action {
		...
		public DistanceAction(final String title, final WorldDistance distance, final DistanceOperation operation,
	...
	private static class DistanceActionBuilder {
		...
		private DistanceActionBuilder(final WorldDistance existingDistance, final DistanceOperation operation,
		...
		private DistanceAction createAction(final WorldDistance distance) {
			final DistanceAction distanceAction = new DistanceAction(distance.toString(), distance, _operation, _chart);
		...
		private ActionContributionItem createContribution(final WorldDistance distance) {
			...
			final ActionContributionItem action = new ActionContributionItem(distanceAction);
	...
	private class GridMenuCreator implements IMenuCreator {
		...
		public Menu getMenu(final Control parent) {
			...
			final DistanceActionBuilder builder = new DistanceActionBuilder(currentLen, setGridOperation,
					_myOverviewChart, gridMenu);
			...
			final ActionContributionItem pa7 = new ActionContributionItem(new Action("Format grid") {
				@Override
				public void run() {
	...
	public static interface IOperateOnMatch {
		 ...
		 */
		void doItTo(final Watchable secondary, final WorldLocation offsetLocation, final double proportion);
		 ...
		 */
		void processNearest(final Watchable nearestInTime, final WorldLocation nearestOffset,
				double primaryHeadingDegs);
	}
	...
	private static class NormalWalker implements TrackWalker {
		...
		private NormalWalker(final LightweightTrackWrapper secondary) {
		...
		@Override
		public HiResDate timeFor(final FixWrapper thisF) {
	...
	private static class PeriodAction extends Action {
		...
		public PeriodAction(final String title, final long period, final PeriodOperation operation,
				final UnitCentricChart chart) {
			super(title);
			_period = period;
	...
	private class ShowRingsMenuCreator implements IMenuCreator {
		...
		public Menu getMenu(final Control parent) {
			...
			final WorldDistance currentLen = _myOverviewChart.getRings().getRingWidth();

			final DistanceActionBuilder builder = new DistanceActionBuilder(currentLen, setRingsOperation,
					_myOverviewChart, ringsMenu);
			...
			final ActionContributionItem pa7 = new ActionContributionItem(new Action("Format rings") {
				@Override
				public void run() {
	...
	/**
	 * we need to make a single-point secondary track look like a track that's as
	 * long as the primary track needs it to be. Mock this behaviour.
	 *
	 * @author ian
	 *
	 */
	private static class SingleSecondaryWalker implements TrackWalker {
		...
		private SingleSecondaryWalker(final LightweightTrackWrapper primary, final LightweightTrackWrapper secondary,
				final HiResDate subjectTime, final boolean snailMode) {
			_secondaryFix = (FixWrapper) secondary.getPositionIterator().nextElement();
		...
		public HiResDate timeFor(final FixWrapper thisF) {
			...
			final HiResDate res = _cached.getDateTimeGroup();
	...
	private class SnailDropDownMenuCreator implements IMenuCreator {
		private ActionContributionItem createAction(final String name, final long period, final long existingPeriod,
			...
			final ActionContributionItem action = new ActionContributionItem(periodAction);
		...
		public Menu getMenu(final Control parent) {
			...
			int ctr = 0;
			final ActionContributionItem pa1 = createAction("5 Mins", 1000 * 60 * 5, _snailLength, setSnail,
					_myOverviewChart);
			pa1.fill(snailMenu, ctr++);
			final ActionContributionItem pa2 = createAction("15 Mins", 1000 * 60 * 15, _snailLength, setSnail,
					_myOverviewChart);
			pa2.fill(snailMenu, ctr++);
			final ActionContributionItem pa3 = createAction("30 Mins", 1000 * 60 * 30, _snailLength, setSnail,
					_myOverviewChart);
			pa3.fill(snailMenu, ctr++);
			final ActionContributionItem pa4 = createAction("1 Hour", 1000 * 60 * 60, _snailLength, setSnail,
					_myOverviewChart);
			pa4.fill(snailMenu, ctr++);
			final ActionContributionItem pa5 = createAction("2 Hours", 1000 * 60 * 60 * 2, _snailLength, setSnail,
					_myOverviewChart);
	...
	private static abstract class ToggleAction extends Action {
		...
		public ToggleAction(final String title, final int style, final String defaultImage,
	...
	/**
	 * we need to allow a custom way of walking through the data if we have a single
	 * point track.
	 *
	 */
	 ...
	 * @return
	 */
	private static WorldLocation processOffset(final FixWrapper primary, final WorldLocation other,
		...
		// update the bearing
		final WorldVector newDelta = new WorldVector(newBearing, delta.getRange(), 0d);
	...
	public static void walkTree(final Layers theLayers, final WatchableList primary, final HiResDate subjectTime,
		...
		final OperateFunction handleTracks = new OperateFunction() {
			...
			public void operateOn(final Editable item) {
				...
				if (other.equals(primary)) {
					...
					if (isSingle) {
						walker = new SingleSecondaryWalker((LightweightTrackWrapper) primary, other, subjectTime,
								snailMode);
					} else {
					...
					while (walker.hasMoreSecondaries()) {
						...
						if (subjectTime == null) {
							...
							if (snailLength == Long.MAX_VALUE) {
								...
								final long offset = subjectTime.getDate().getTime() - hisD.getDate().getTime();
							...
							if (nearest != null && nearest.length > 0) {
								...
								if (nItem instanceof FixWrapper) {
									...
									final long diff = Math
											.abs(hisD.getDate().getTime() - subjectTime.getDate().getTime());

									if (nearestInTime == null || diff < nearestDelta) {
										nearestInTime = thisF;
										nearestDelta = diff;
										relativeLocation = processOffset(priFix, thisF.getLocation(), origin);
										primaryHeading = priFix.getCourseDegs();
									}

									final WorldLocation pos = processOffset(priFix, thisF.getLocation(), origin);

									// work out how far back down the leg we are
									final long age = subjectTime.getDate().getTime() - hisD.getDate().getTime();
		...
		final OperateFunction handleLabels = new OperateFunction() {
			...
			public void operateOn(final Editable item) {
				...
				if (subjectTime == null) {
					...
					final Watchable[] matches = label.getNearestTo(subjectTime);
						...
						if (useIt) {
							...
							if (nearest != null && nearest.length > 0) {
								...
								if (nItem instanceof FixWrapper) {
									final FixWrapper priFix = (FixWrapper) nItem;
									final long diff = Math
											.abs(priFix.getDTG().getDate().getTime() - subjectTime.getDate().getTime());

									if (nearestInTime == null || diff < nearestDelta) {
										nearestInTime = label;
										nearestDelta = diff;
										relativeLocation = processOffset(priFix, label.getLocation(), origin);
										primaryHeading = priFix.getCourseDegs();
									}

									final WorldLocation pos = processOffset(priFix, label.getLocation(), origin);

									// work out how far back down the leg we are
									final long age = subjectTime.getDate().getTime()
											- nItem.getTime().getDate().getTime();
									final double proportion = age / (double) snailLength;
	...
	private void formatItem(final ClassWithProperty toFormat) {
		...
		// do we have any data?
		if (editable.hasEditor() && editable.getInfo().getPropertyDescriptors() != null) {
			...
			CorePlugin.logError(IStatus.WARNING, "No editable properties found for:" + editable, null);
		}
	}
	...
	private void makeActions() {
		...
		final SnailDropDownMenuCreator snailDropDownMenu = new SnailDropDownMenuCreator();
		...
		final ShowRingsMenuCreator ringRadiiMenuCreator = new ShowRingsMenuCreator();
			...
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				if (!provider.equals(_trackDataProvider)) {
					...
					if (_trackDataProvider.getPrimaryTrack() == null) {
						CorePlugin.logError(IStatus.WARNING, "Unit centric view is missing primary track", null);
						CorePlugin.showMessage("Unit Centric View", "Please assign a primary track");
					}
				}
			}
		});
			...
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				if (!provider.equals(_timeProvider)) {
					if (_timeProvider != null) {
						// ditch the old one
						_timeProvider.removeListener(_timeChangeListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
					}

					// ok, start listening to the new one
					_timeProvider = provider;
					_timeProvider.addListener(_timeChangeListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
				}
			}
		});
