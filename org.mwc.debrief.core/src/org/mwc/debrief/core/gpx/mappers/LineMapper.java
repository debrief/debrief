...
public class LineMapper implements DebriefJaxbContextAware {
	...
	public ShapeWrapper fromGpx(final GpxType gpx) {
		...
		if (extensions != null) {
			...
			try {
				...
				CorePlugin.logError(IStatus.ERROR, "Error while mapping Line from GPX", e);
			}
		}
		return null;
	}
	...
	private ShapeWrapper mapLine(final LayerType layerType) {
		...
		if (lineType != null) {
			...
			final WorldLocation btWl = new WorldLocation(bottomRight.getLat(), bottomRight.getLong(),
					bottomRight.getDepth());
