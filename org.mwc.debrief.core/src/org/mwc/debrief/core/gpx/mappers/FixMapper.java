...
public class FixMapper implements DebriefJaxbContextAware
{
  ...
  /**
   * Debrief and its datasets currently represent the 'z' dimension as Depth. GPX has elevation, so
   * clearly we'll need to invert this data - positive depths in Debrief will be stored as negative
   * elevations.
   */
   ...
   * @param previousFix
   *          the value of the previously read-in fix, used to calc course and speed, when necessary
   * 
   * @category gpx11
   */
  public FixWrapper fromGpx(final WptType trackType,
      final FixWrapper previousFix)
  {
    ...
    final WorldLocation val = new WorldLocation(trackType.getLat(), trackType
        .getLon(), convertElevationToDepth(ele));
    fix.setLocation(val);
    ...
    if (extensions != null)
    {
      ...
      try
      {
        ...
        final FixExtensionType fixExtension =
            (FixExtensionType) JAXBIntrospector.getValue(object);

        trackPoint.setCourse(MWCXMLReader.readThisDouble(fixExtension
            .getCourse()));
        trackPoint.setLabel(fixExtension.getLabel());
        trackPoint.setSpeed(MWCXMLReader.readThisDouble(fixExtension
            .getSpeed()));
        final LocationPropertyEditor locationConverter =
   ...
   * @category gpx10
   */
  public FixWrapper fromGpx10(final Trkpt pointType,
      final FixWrapper previousFix)
  {
    final Fix fix = new Fix();
    ...
    final WorldLocation val = new WorldLocation(pointType.getLat(), pointType
        .getLon(), convertElevationToDepth(ele));
    fix.setLocation(val);
  ...
  public Trkpt toGpx10(final FixWrapper fixWrapper)
  {
    ...
    gpxPoint.setLat(BigDecimal.valueOf(fixWrapper.getFix().getLocation()
        .getLat()));
    gpxPoint.setLon(BigDecimal.valueOf(fixWrapper.getFix().getLocation()
        .getLong()));
    gpxPoint.setEle(convertDepthToElevation(BigDecimal.valueOf(fixWrapper
        .getFix().getLocation().getDepth())));
    ...
    gpxPoint.setCourse(BigDecimal.valueOf(MWC.Algorithms.Conversions.Rads2Degs(
        fixWrapper.getFix().getCourse())).setScale(4,
            BigDecimal.ROUND_CEILING));
    gpxPoint.setSpeed(BigDecimal.valueOf(MWC.Algorithms.Conversions.Kts2Mps(
        fixWrapper.getSpeed())).setScale(4, BigDecimal.ROUND_CEILING));
    //
