...
public class ViewTest extends ViewPart {
	...
	public ViewTest() {
		...
		if (tiffReader != null) {
			...
			final GridReaderLayer res = new GridReaderLayer(tiffReader, defaultStyle, params);
			map.addLayer(res);
		}

		try {
			final URL url = DebriefPlugin.getDefault().getBundle().getEntry("data/50m_admin_0_countries.shp");
	...
	protected org.eclipse.swt.graphics.Image createSWTImage(final ImageData myImageTemplate) {
		_myImageTemplate.transparentPixel = _myImageTemplate.palette.getPixel(new RGB(255, 255, 255));
		final org.eclipse.swt.graphics.Image image = new org.eclipse.swt.graphics.Image(Display.getCurrent(),
				_myImageTemplate);
		return image;
	}
	...
	private void paintMe(final PaintEvent e) {
		...
		if (map != null) {
			...
			final org.eclipse.swt.graphics.Rectangle paintArea = new org.eclipse.swt.graphics.Rectangle(0, 0, width,
					height);
			...
			final Map rendererParams = new HashMap();
			rendererParams.put("optimizedDataLoadingEnabled", new Boolean(true));

			renderer.setRendererHints(rendererParams);

			final org.eclipse.swt.graphics.Rectangle curPaintArea = e.gc.getClipping();
			final BufferedImage baseImage = new BufferedImage(curPaintArea.width + 1, curPaintArea.height + 1,
			...
			swtImage = new Image(canvas.getDisplay(),
	 ...
	 * reference system.
	 * <p>
	 * Tne transform is calculated such that {@code envelope} will be centred in the
	 * display
	 *
