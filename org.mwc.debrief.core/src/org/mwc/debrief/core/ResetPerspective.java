...
public class ResetPerspective {
	...
	private static final long resetPerspective = 8;

	public void resetPerspective() {
		try {
			final long resetPerspectivePreference = DebriefPlugin.getDefault().getResetPerspectivePreference();
	...
	private void setResetPerspective() {
		final String value = new Long(resetPerspective).toString();
		DebriefPlugin.getDefault().getPreferenceStore().putValue(DebriefPlugin.RESET_PERSPECTIVE, value);
	}

	protected void showDialog(final Shell shell, final String info) {
		final PopupDialog dialog = new PopupDialog(shell, PopupDialog.HOVER_SHELLSTYLE, true, false, false, false,
			...
			protected Control createDialogArea(final Composite parent) {
				...
				final StyledText text = new StyledText(parent, SWT.MULTI | SWT.READ_ONLY | SWT.WRAP);
