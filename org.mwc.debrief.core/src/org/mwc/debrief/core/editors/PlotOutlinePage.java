...
public class PlotOutlinePage extends Page implements IContentOutlinePage
{
  ...
  public void init(IPageSite pageSite)
  {
    ...
    actionBars.setGlobalActionHandler(ActionFactory.UNDO.getId(),
        _plotEditor.getUndoAction());
    actionBars.setGlobalActionHandler(ActionFactory.REDO.getId(),
        _plotEditor.getRedoAction());
  ...
  void fillContextMenu(final IMenuManager manager)
  {
    ...
    final ArrayList<Editable> eList = new ArrayList<Editable>();
    final ArrayList<Layer> parentLayers = new ArrayList<Layer>();
    final ArrayList<Layer> updateLayers = new ArrayList<Layer>();

    final ArrayList<Editable> altList = new ArrayList<Editable>();
    final ArrayList<Editable> altParents = new ArrayList<Editable>();
    ...
    for (int i = 0; i < oList.length; i++)
    {
      ...
      {
        // ok, traditional Debrief fodder. get running
        eList.add(wrapper.getEditable());
        ...
        if (theParent != null)
          // yes, store it
          parentLayers.add((Layer) wrapper.getParent().getEditable());
        else
          // nope - store a null (to indicate it's a top-level layer)
          parentLayers.add(null);

        updateLayers.add(wrapper.getTopLevelLayer());
      }
    }
    ...
    if (eList.size() > 0)
    {
      Editable[] arrEditable = eList.toArray(new Editable[]
      { null });
      Layer[] arrUpdates = updateLayers.toArray(new Layer[]
      { null });
      Layer[] arrParents = parentLayers.toArray(new Layer[]
      { null });
      RightClickSupport.getDropdownListFor(manager, arrEditable, arrUpdates,
          arrParents, _myLayers, false);
    }

    // see if there's any other processing we can do
    RightClickSupport.getAlt2DropdownListFor(manager, altList, altParents,
        _myLayers);
