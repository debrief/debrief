...
public class PlotOutlinePage extends Page implements IContentOutlinePage
{
  ...
  public void createControl(Composite parent)
  {
    ...
    /**
     * Intentional not used TableEditor which would create lot of objects as this view usually has
     * more data.
     */
  ...
  public void connectActions()
  {
    ...
    actionBars.setGlobalActionHandler(ActionFactory.PASTE.getId(), new Action()
    {
      ...
      public void run()
      {
        ...
        {
          RightClickPasteAdaptor.createAction(selectedItem, _myLayers,
              _clipboard, tr).run();
        }
      ...
      public boolean isEnabled()
      {
        ...
        if ((selectedItem instanceof MWC.GUI.Layer) || (selectedItem == null))
        {
          return RightClickPasteAdaptor.createAction(selectedItem, _myLayers,
              _clipboard, tr) != null;
        }
  ...
  private void makeActions()
  {
    ...
    _followSelectionToggle =
        new Action("Jump to selection", Action.AS_CHECK_BOX)
        {
        };
   ...
   * @return
   */
  protected boolean isAtleastOnePlottableCheck(final StructuredSelection ss,
      boolean visible)
  {
  ...
  /**
   * class that embodies applying an operation to a series of selected points. the series of points
   * are remembered, so that they can be undone/redone
   * 
   * @author ian.mayo
   */
  ...
  /**
   * recursive class used to build up a list containing the item together with all child items
   * 
   * @param list
  ...
  /**
   * one or more layers have been changed. This method may get called lots of times. Stack up the
   * events - and just call our UI update method once at the end
   * 
   * @param changedLayer
