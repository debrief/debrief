...
public interface SWTPlotHighlighter extends Editable {
	...
	public final class RectangleHighlight implements SWTPlotHighlighter {
		...
		protected static void plotArrayCentre(final CanvasType dest, final MWC.GenericData.Watchable watch,
				final Rectangle areaCovered, final int mySize) {
			...
			if (watch instanceof FixWrapper) {
				...
				final WatchableList wList = fw.getTrackWrapper();
				if (wList instanceof TrackWrapper) {
					final TrackWrapper tw = (TrackWrapper) wList;
					...
					if (tw != null && tw.getPlotArrayCentre()) {
						...
						while (enumer.hasMoreElements()) {
							...
							if (sw.getVisible()) {
								...
								final WorldLocation centre = sw.getArrayCentre(fw.getTime(), watch.getLocation(), tw);

								// have we managed it?
								if (centre != null) {
									final Point pt = dest.toScreen(centre);
									dest.drawLine(pt.x - mySize, pt.y - mySize, pt.x + mySize, pt.y + mySize);
									dest.drawLine(pt.x + mySize, pt.y - mySize, pt.x - mySize, pt.y + mySize);

									// store the new screen update area
									thisR = new Rectangle(pt.x - mySize, pt.y - mySize, mySize, mySize);
									if (areaCovered != null) {
		...
		public final void highlightIt(final MWC.Algorithms.PlainProjection proj, final CanvasType dest,
				final MWC.GenericData.WatchableList list, MWC.GenericData.Watchable watch, final boolean isPrimary) {
	 ...
	 */
	void highlightIt(MWC.Algorithms.PlainProjection proj, CanvasType dest, WatchableList list, Watchable watch,
			boolean isPrimary);
