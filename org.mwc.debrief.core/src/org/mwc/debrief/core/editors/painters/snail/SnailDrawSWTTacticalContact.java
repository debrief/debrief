...
public abstract class SnailDrawSWTTacticalContact implements drawSWTHighLight, MWC.GUI.Editable {
	 ...
	 */
	@Override
	public final Rectangle drawMe(final MWC.Algorithms.PlainProjection proj, final CanvasType dest,
			final WatchableList list, final Watchable watch, final SnailHighlighter parent, final HiResDate dtg,
		...
		if (trail_len > 0) {
				...
				if (len > 0) {
					...
					while (cons.hasNext()) {
						...
						if (wa != null) {
							...
							// paint the object
							scw.paint(hostTrack, dest, keepItSimple, alpha);
			...
			contact.paint(null, dest, keepItSimple, alpha);
