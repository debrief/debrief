...
public class SnailHighlighter implements TemporalLayerPainter {
	...
	public static interface drawSWTHighLight {
		...
		public java.awt.Rectangle drawMe(final PlainProjection proj, final CanvasType dest, final WatchableList list,
				final Watchable watch, final SnailHighlighter parent, final HiResDate dtg, final Color backColor);
	}
	...
	private Watchable[] getNearestTo(final WatchableList list, final HiResDate dtg) {
		if (list instanceof TrackWrapper) {
			...
			if (track.getSegments().size() == 0) {
				...
				final Editable next = track.getPositionIterator().nextElement();
				final FixWrapper fix;
				if (next instanceof TrackSegment) {
					final TrackSegment seg = (TrackSegment) next;
					fix = (FixWrapper) seg.first();
				} else if (next instanceof FixWrapper) {
					fix = (FixWrapper) next;
				} else {
					CorePlugin.logError(IStatus.ERROR,
							"Item of type:" + next.getClass() + " should not be stored as a track position", null);
					fix = null;
				}
	...
	private void highlightIt(final PlainProjection projection, final CanvasType dest, final WatchableList list,
			final Watchable watch, final HiResDate newDTG, final Color backgroundColor) {
		...
		while (iter.hasMoreElements()) {
			final drawSWTHighLight plotter = iter.nextElement();
	...
	public void paintThisLayer(final Layer theLayer, final CanvasType dest, final HiResDate newDTG) {
		...
		final Vector<Plottable> watchables = TotePainter.getWatchables(theLayer);
