...
public final class SWTRangeHighlighter implements SWTPlotHighlighter {
	...
	public static final class RangeHighlightInfo extends Editable.EditorType {
		...
		public final java.beans.PropertyDescriptor[] getPropertyDescriptors() {
			try {
				final java.beans.PropertyDescriptor[] res = { prop("Color", "Color to paint highlight", FORMAT),
						...
						displayProp("ArcStart", "Arc start", "The rel angle where the arcs start (degs)", SPATIAL),
	...
	private void drawRangeRings(final WorldLocation worldCentre, final int rawAxisDegs, final CanvasType dest) {
		...
		final WorldLocation worldEdge = worldCentre.add(new WorldVector(
				MWC.Algorithms.Conversions.Degs2Rads(rawAxisDegs), MWC.Algorithms.Conversions.Yds2Degs(_radius), 0));
		...
		if ((_arcStart < 0) && (_arcEnd > 0)) {
			final double axisRads = MWC.Algorithms.Conversions.Degs2Rads(axis + _arcStart);
			edge.setLocation((int) (radius * Math.sin(axisRads)), -(int) (radius * Math.cos(axisRads)));
			edge.translate(centrex, centrey);
			dest.drawLine(centrex, centrey, edge.x, edge.y);
		}

		// now the start edge stalk
		final double startRads = MWC.Algorithms.Conversions.Degs2Rads(axis + _arcStart);
		 ...
		 * degrees either side (since in that case drawing it twice effectively hides it
		 */
	...
	private void drawRectangle(final MWC.GenericData.Watchable watch, final CanvasType dest,
	...
	public final void highlightIt(final MWC.Algorithms.PlainProjection proj, final CanvasType dest,
		...
		if (doPlot) {
			...
			if (_plainRectangle) {
				...
				final int headingDegs = (int) MWC.Algorithms.Conversions.Rads2Degs(watch.getCourse());
