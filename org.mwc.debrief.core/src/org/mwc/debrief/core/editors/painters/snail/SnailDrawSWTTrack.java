...
final class SnailDrawSWTTrack {
	...
	public final java.awt.Rectangle drawMe(final MWC.Algorithms.PlainProjection proj, final CanvasType dest,
			final Watchable watch, final SnailHighlighter parent, HiResDate dtg, final Color backColor) {
		...
		if (trk instanceof TrackWrapper) {
			final TrackWrapper track = (TrackWrapper) trk;
			final Duration customTrail = track.getCustomTrailLength();

			if (customTrail != null) {
				trail_len = (long) customTrail.getValueIn(Duration.MICROSECONDS);
			} else {
				trail_len = (long) parent.getSnailProperties().getTrailLength().getValueIn(Duration.MICROSECONDS);
			}
		} else {
			trail_len = (long) parent.getSnailProperties().getTrailLength().getValueIn(Duration.MICROSECONDS);
		}
		...
		if (trk instanceof LightweightTrackWrapper) {
			final LightweightTrackWrapper ft = (LightweightTrackWrapper) trk;
		...
		if (myList != null) {
			...
			dotPoints = getUnfilteredItems(trk, new HiResDate(0, dtg.getMicros() - trail_len),
			...
			if (!dotPoints.isEmpty()) {
				...
				while (iter.hasNext()) {
					...
					if (format.getNameVisible() && !titlePlotted) {
						final Enumeration<Editable> numer = format.getPositionIterator();
	 ...
	 * filtered, and which have valid depths. values
	 *
	 * @param start start DTG
	 * @param end   end DTG
	 * @return series of fixes
	 */
	public final Collection<Editable> getUnfilteredItems(final WatchableList list, final HiResDate start,
			final HiResDate end) {
