...
public class GenerateSensorRangePlot implements RightClickContextItemGenerator {
	...
	public static class TestCalcs extends TestCase {
		...
		private static FixWrapper getFix(final long time, final WorldLocation loc, final TrackWrapper track) {
		...
		public void NtestIt() {
			...
			GenerateSensorRangePlot.addDataSeries(target, sensors, start_time, end_time, data, true);
		...
		public void NtestMenu() {
			...
			final GenerateSensorRangePlot plot = new GenerateSensorRangePlot() {
				@Override
				protected Action getAction(final Vector<SensorWrapper> candidates, final Vector<TrackWrapper> primary) {
	 ...
	 * @param theSensors    the selected set of sensors
	 * @param start_time    the start time selected
	 * @param end_time      the end time selected
	 * @param useSensorName whether to label the lines with the sensor name or the
	 *                      track name
	...
	private static void addDataSeries(final TrackWrapper primaryTrack, final Vector<SensorWrapper> theSensors,
			final HiResDate start_time, final HiResDate end_time, final TimeSeriesCollection theSeriesCollection,
		...
		while (iter.hasMoreElements()) {
			final SensorWrapper thisSensor = iter.nextElement();
			...
			if (false) {
				...
				// step through the track
				//
				final Collection<Editable> primaryFixes = primaryTrack.getItemsBetween(start_time, end_time);
				...
				if (primaryFixes != null) {
					...
					while (theseFixes.hasNext()) {
						...
						if (thisPosition.getVisible()) {
							...
							final WorldLocation sensorLoc = thisSensor.getArrayCentre(currentTime, null,
									thisSensor.getHost());
							...
							if (sensorLoc == null) {
								...
								final rangeCalc rc = new rangeCalc();
								final String units = rc.getUnits();

								// check we were able to retrieve units
								if (units == null) {
									DebriefPlugin.logError(IStatus.ERROR,
											"Unable to show sensor plot, preferred range units not available", null);
									return;
								}

								final double thisVal = rangeCalc.convertRange(range.getValueIn(WorldDistance.DEGS),
										units);
								...
								final ColouredDataItem newItem = new ColouredDataItem(
										new FixedMillisecond(thisPosition.getDTG().getDate().getTime()), thisVal,
										thisColor, true, null, true, true);
			...
			if (thisSeries != null)
				if (thisSeries.getItemCount() > 0) {
					CorePlugin.logError(IStatus.OK, "created:" + thisSeries.getItemCount() + " pts", null);
	...
	public void generate(final IMenuManager parent, final Layers theLayers, final Layer[] parentLayers,
		...
		final Vector<SensorWrapper> sensorCandidates = new Vector<SensorWrapper>(0, 1);
		...
		if ((trackCandidates.size() >= 1) && (sensorCandidates.size() >= 1)) {
			...
			if ((trackCandidates.size() > 1) && (sensorCandidates.size() > 1)) {
				...
				viewPlot.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/document_chart.png"));
	 ...
	 * overring in tests)
	 *
	 * @param sensorCandidates the sensors to measure the range from
	 * @param trackCandidates  the tracks to measure to
	 * @return
	 */
		...
		return new Action(title) {
			...
			public void run() {
				...
				try {
					...
					final TimeProvider tp = editor.getAdapter(TimeProvider.class);
					...
					while (tIter.hasNext()) {
						...
						if (trackWrapper != null) {
							if (outerPeriod == null) {
								outerPeriod = new TimePeriod.BaseTimePeriod(trackWrapper.getStartDTG(),
										trackWrapper.getEndDTG());
							} else {
					...
					if (multiTrackRun) {
						...
						while (iter.hasNext()) {
							final TrackWrapper thisTrack = iter.next();

							// do run for this track
							addDataSeries(thisTrack, sensorCandidates, startTime, endTime, theSeriesCollection,
									!multiTrackRun);

						}
						theTitle = "(Sensor) Range to " + sensorCandidates.firstElement().getName() + " vs Time plot";

					} else {
						final TrackWrapper primaryTrack = trackCandidates.firstElement();

						// single track run, do as normal
						addDataSeries(primaryTrack, sensorCandidates, startTime, endTime, theSeriesCollection,
								!multiTrackRun);
						theTitle = "(Sensor) Range to " + primaryTrack.getName() + " vs Time plot";
					}
					...
					if (theSeriesCollection.getSeriesCount() >= 1) {
						...
						// create the plot
						plotter.showPlot(theTitle, theSeriesCollection, yLabel, null, thePlotId);
					} else {
