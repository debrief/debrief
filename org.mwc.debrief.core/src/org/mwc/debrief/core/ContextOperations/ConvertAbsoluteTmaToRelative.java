...
//      CMAPOperation op = new ConvertToRelative(items, theLayers, worldOffset, tgtCourse, tgtSpeed);
//      
//      // and run it
...
//			_layers.addThisLayerAllowDuplication(_newTrack);
			...
			final Layer[] parentLayers, final Editable[] subjects)
	{
		  ...
		  for (int i = 0; i < subjects.length; i++)
      {
        ...
        // ok, generate it
        final ConvertToRelative action = new ConvertToRelative(theLayers, suitableSegments);
		    
		    Action doIt = new Action("Convert " + phrase + " from absolute to relative")
		    {
