 ...
 *
 */
public class ConvertAbsoluteTmaToRelative implements RightClickContextItemGenerator {

	private static class ConvertToRelativeOperation extends CMAPOperation {

		protected static RelativeTMASegment createSegment(final Layers layers, final SuitableSegment seg,
				final TrackWrapper track, final AbsoluteTMASegment absSegment, final SensorWrapper sensor) {
			...
			final RelativeTMASegment rSeg = new RelativeTMASegment(sensor, offset, absSegment.getSpeed(),
		...
		private final TrackWrapper _commonParent;

		public ConvertToRelativeOperation(final Layers theLayers, final List<SuitableSegment> suitableSegments,
				final TrackWrapper commonParent) {
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			final Iterator<SuitableSegment> iter = _segments.iterator();
			while (iter.hasNext()) {
				final ConvertAbsoluteTmaToRelative.SuitableSegment seg = iter.next();
				...
				final RelativeTMASegment rSeg = createSegment(_layers, seg, track, absSegment, sensor);
		...
		public IStatus redo(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			final Iterator<RelativeTMASegment> rIter = _replacements.iterator();
			while (rIter.hasNext()) {
				final RelativeTMASegment relativeTMASegment = rIter.next();
	...
	private static class ShowMessage extends Action {
		...
		public ShowMessage(final String title, final String message) {
		...
		@Override
		public void run() {
			CorePlugin.errorDialog(_title, "Sorry can't convert segment: " + _message);
		}
	}

	private static class SuitableSegment {
		private final AbsoluteTMASegment _thisA;
		private final SensorWrapper _sensor;

		public SuitableSegment(final AbsoluteTMASegment thisA, final SensorWrapper sensor) {
			_thisA = thisA;
			_sensor = sensor;
		}
	}
	...
	static public final class testMe extends junit.framework.TestCase {
		private static class TestOperation extends AbstractOperation {
			...
			public TestOperation(final String label, final Layers theLayers,
					final List<SuitableSegment> suitableSegments, final TrackWrapper parent) {
				super(label);
		...
		private TrackWrapper getLongerTrack() {
			...
			final TrackSegment ts = new TrackSegment(false);
		...
		public void testApplicable() throws ExecutionException {
			...
			final TrackWrapper tw2 = getLongerTrack();
			tw2.setName("target track");

			final ConvertAbsoluteTmaToRelative op = new ConvertAbsoluteTmaToRelative() {
				@Override
			...
			Editable[] subjects = new Editable[] { tw, sw };
			op.generate(menu, theLayers, null, subjects);

			assertEquals("no items added", 0, actions.size());

			final WorldSpeed a1Speed = new WorldSpeed(12, WorldSpeed.Kts);

			final WorldLocation a1Origin1 = new WorldLocation(12, 12, 0);
			final HiResDate a1Start1 = new HiResDate(new Date(2016, 1, 14, 12, 30, 0).getTime());
			final HiResDate a1end1 = new HiResDate(new Date(2016, 1, 14, 12, 40, 0).getTime());
			final AbsoluteTMASegment a11 = new AbsoluteTMASegment(12, a1Speed, a1Origin1, a1Start1, a1end1);
			a11.setWrapper(tw);

			final WorldLocation a1Origin = new WorldLocation(12, 12, 0);
			final HiResDate a1Start = new HiResDate(new Date(2016, 1, 14, 12, 30, 0).getTime());
			final HiResDate a1end = new HiResDate(new Date(2016, 1, 14, 12, 40, 0).getTime());
			final AbsoluteTMASegment a1 = new AbsoluteTMASegment(12, a1Speed, a1Origin, a1Start, a1end);
			a1.setWrapper(tw2);

			subjects = new Editable[] { a1 };
			...
			final WorldVector theOffset = new WorldVector(12, 0.002, 0);
			// ok, add a relative segment before it
			final RelativeTMASegment r1 = new RelativeTMASegment(13, a1Speed, theOffset, theLayers, tw.getName(),
					"other sensor");

			long thisTime = new Date(2016, 1, 14, 12, 22, 0).getTime();
			FixWrapper fw = new FixWrapper(new Fix(new HiResDate(thisTime), a1Origin.add(getVector(25, 50)),
					MWC.Algorithms.Conversions.Degs2Rads(0), 110));
			fw.setLabel("fw1");
			r1.addFix(fw);

			thisTime = new Date(2016, 1, 14, 12, 24, 0).getTime();
			fw = new FixWrapper(new Fix(new HiResDate(thisTime), a1Origin.add(getVector(23, 200)),
					MWC.Algorithms.Conversions.Degs2Rads(0), 110));
			fw.setLabel("fw2");
			r1.addFix(fw);
			...
			final RelativeTMASegment r2 = new RelativeTMASegment(13, a1Speed, theOffset, theLayers, tw.getName(),
					sw.getName());
			final Enumeration<Editable> enumer = r1.elements();
			while (enumer.hasMoreElements()) {
				final Editable editable = enumer.nextElement();
				r2.add(editable);
			}
			...
			System.out.println("r1 ends at:" + r2.getDTG_End() + " a1 starts at:" + a1.getDTG_Start());
			...
			// - add later track
			// ok, add a relative segment before it
			final RelativeTMASegment r3 = new RelativeTMASegment(13, a1Speed, theOffset, theLayers, tw.getName(),
					sw2.getName());

			thisTime = new Date(2016, 1, 14, 12, 50, 0).getTime();
			fw = new FixWrapper(new Fix(new HiResDate(thisTime), a1Origin.add(getVector(25, 50)),
					MWC.Algorithms.Conversions.Degs2Rads(0), 110));
			fw.setLabel("fw3_1");
			r3.addFix(fw);
...
//      assertEquals("items added", 1, actions.size());
	...
	public void generate(final IMenuManager parent, final Layers theLayers, final Layer[] parentLayers,
		...
		if (subjects.length > 0) {
			...
			for (int i = 0; i < subjects.length; i++) {
				...
				if (editable instanceof CoreTMASegment) {
					if (editable instanceof AbsoluteTMASegment) {
						// cool, go for it.
						final AbsoluteTMASegment abs = (AbsoluteTMASegment) editable;

						// have a look at the parent
						final TrackWrapper thisParent = abs.getWrapper();
			...
			for (int i = 0; i < subjects.length; i++) {
				final AbsoluteTMASegment thisA = (AbsoluteTMASegment) subjects[i];

				// now check for peer relative segments
				RelativeTMASegment before = null;
				RelativeTMASegment after = null;
				final Enumeration<Editable> segs = commonParent.getSegments().elements();
				while (segs.hasMoreElements()) {
					final TrackSegment thisSeg = (TrackSegment) segs.nextElement();

					// is this one relative?
					if (thisSeg instanceof RelativeTMASegment) {
						final RelativeTMASegment thisRel = (RelativeTMASegment) thisSeg;
				...
				if (before == null && after == null) {
					...
					if (beforeS == null && afterS == null) {
						...
						parent.add(new ShowMessage("Can't convert track",
								"Adjacent TMA Segments must be relative to same sensor"));
						return;
					}
					...
					final SuitableSegment suitableSegment = new SuitableSegment(thisA, sensor);
			...
			if (suitableSegments != null) {
				...
				// ok, generate it
				final IUndoableOperation action = getOperation(theLayers, suitableSegments, commonParent);

				final Action doIt = new Action("Convert " + phrase + " from absolute to relative") {
					@Override
					public void run() {
						runIt(action);
					}
	 ...
	 * @return
	 */
	protected IUndoableOperation getOperation(final Layers theLayers, final List<SuitableSegment> suitableSegments,
			final TrackWrapper commonParent) {
