...
public class GenerateTMASegmentFromCuts implements RightClickContextItemGenerator {
	...
	static public final class testMe extends junit.framework.TestCase {
		private static class DummyMenuManager implements IMenuManager {
			...
			@Override
			public void appendToGroup(final String groupName, final IContributionItem item) {
				// TODO Auto-generated method stub
			...
			@Override
			public IMenuManager findMenuUsingPath(final String path) {
				// TODO Auto-generated method stub
				return null;
			}

			@Override
			public IContributionItem findUsingPath(final String path) {
				// TODO Auto-generated method stub
				return null;
			}
			...
			@Override
			public void prependToGroup(final String groupName, final IContributionItem item) {
				// TODO Auto-generated method stub
			...
			@Override
			public void setRemoveAllWhenShown(final boolean removeAll) {
				// TODO Auto-generated method stub
			...
			@Override
			public void updateAll(final boolean force) {
				// TODO Auto-generated method stub
		...
		public void testGenerateMultiSensor() {
			...
			final SensorWrapper sensor1 = new SensorWrapper("sensor1");
			host.add(sensor1);
			final SensorWrapper sensor2 = new SensorWrapper("sensor2");
			host.add(sensor2);
			...
			sensor1.add(new SensorContactWrapper("host", new HiResDate(800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(1800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(2800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(5800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(7800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(8800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor1.add(new SensorContactWrapper("host", new HiResDate(9800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor2.add(new SensorContactWrapper("host", new HiResDate(800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor2.add(new SensorContactWrapper("host", new HiResDate(1800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor2.add(new SensorContactWrapper("host", new HiResDate(2800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor2.add(new SensorContactWrapper("host", new HiResDate(5800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));

			final GenerateTMASegmentFromCuts genny = new GenerateTMASegmentFromCuts();
			final Editable[] sensors = new SensorWrapper[] { sensor1, sensor2 };
			final Layers theLayers = new Layers();

			final DummyMenuManager parent = new DummyMenuManager();
			genny.generate(parent, theLayers, null, sensors);
			assertEquals("has action", parent.actions.size(), 1);
			final TMAAction theAction = (TMAAction) parent.actions.get(0);
		...
		public void testSplitWithOffset() throws ExecutionException {
			...
			final CMAPOperation op = new TMAfromCuts(items, theLayers, worldOffset, tgtCourse, tgtSpeed, newColor);
			...
			final RelativeTMASegment seg = (RelativeTMASegment) sol.getSegments().elements().nextElement();
		...
		public void testTrimmingTrack() {
			...
			sensor.add(new SensorContactWrapper("host", new HiResDate(800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(1800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(2800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(5800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(7800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(8800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));
			sensor.add(new SensorContactWrapper("host", new HiResDate(9800), null, 100d, null, Color.RED, "label", 12,
					"sensor"));

			final Enumeration<Editable> cuts = sensor.elements();
			final SensorContactWrapper[] cutArr = new SensorContactWrapper[sensor.size()];
			int ctr = 0;
	...
	private static abstract class TMAAction extends Action {
		...
		public TMAAction(final String title, final SensorContactWrapper[] items) {
			super(title);
			_items = items;
		}
	...
	public static class TMAfromCuts extends CMAPOperation {
		...
		protected static SensorContactWrapper[] trimToHost(final SensorContactWrapper[] cuts) {
		...
		public TMAfromCuts(final SensorContactWrapper[] items, final Layers theLayers, final WorldVector offset,
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			// create it, then
			final TrackSegment seg = new RelativeTMASegment(_items, _offset, _speed, _courseDegs, _layers,
					colorOverride);
			...
			if (isRunning()) {
				...
				if (editor != null) {
					...
					final TrackManager mgr = editor.getAdapter(TrackManager.class);
					if (mgr != null) {
		...
		public String getTrackNameFor(final TrackWrapper newTrack) {
			return TrackSegment.TMA_LEADER + FormatRNDateTime.toString(newTrack.getStartDTG().getDate().getTime());
		}
	...
	private boolean allSensorCuts(final Editable[] subjects) {
	...
	private boolean allSensors(final Editable[] subjects) {
		for (final Editable e : subjects) {
	...
	private SensorContactWrapper[] cutsToArray(final Editable[] subjects) {
		final SensorContactWrapper[] res = new SensorContactWrapper[subjects.length];
	...
	public void generate(final IMenuManager parent, final Layers theLayers, final Layer[] parentLayers,
		...
		if (allSensors(subjects)) {
			...
			for (final Editable thisE : subjects) {
				final SensorWrapper sw = (SensorWrapper) thisE;
				// cool, go for it
				final Enumeration<Editable> numer = sw.elements();
				while (numer.hasMoreElements()) {
			...
			if (!wraps.isEmpty()) {
				...
				final Color firstColor = firstContact.getColor() != null ? firstContact.getColor()
						: firstContact.getSensor().getColor();

				// cool wrap it in an action.
				_myAction = new TMAAction("Generate TMA solution from all cuts", finalItems) {
					@Override
					public void run() {
						...
						final TMAFromSensorWizard wizard = new TMAFromSensorWizard(firstContact.getBearing(), theDist,
						...
						if (dialog.getReturnCode() == Window.OK) {
							...
							final IUndoableOperation convertToTrack1 = new TMAfromCuts(getItems(), theLayers,
									answers.res, answers.courseDegs, answers.speed, answers.newColor);
			...
			final SensorContactWrapper[] items = cutsToArray(subjects);
			if (items.length > 0) {
				final Editable editable = subjects[0];
				// cool, stick with it
				final SensorContactWrapper firstContact = (SensorContactWrapper) editable;

				final Color firstColor = firstContact.getColor() != null ? firstContact.getColor()
						: firstContact.getSensor().getColor();

				// cool wrap it in an action.
				_myAction = new TMAAction("Generate TMA solution from selected cuts", items) {
					@Override
					public void run() {
						// get the supporting data
						final TMAFromSensorWizard wizard = new TMAFromSensorWizard(firstContact.getBearing(),
								firstContact.getRange(), DEFAULT_TARGET_COURSE, DEFAULT_TARGET_SPEED, firstColor);
						final WizardDialog dialog = new WizardDialog(Display.getCurrent().getActiveShell(), wizard);
						dialog.create();
						dialog.open();
	...
	protected UserChoice getUserChoice(final TMAFromSensorWizard wizard, final Color firstColor) {
		final UserChoice choice = new UserChoice();
		choice.res = new WorldVector(0, new WorldDistance(5, WorldDistance.NM), null);
		choice.courseDegs = 0d;
		choice.speed = new WorldSpeed(5, WorldSpeed.Kts);

		final RangeBearingPage offsetPage = (RangeBearingPage) wizard.getPage(RangeBearingPage.NAME);
		if (offsetPage != null) {
			if (offsetPage.isPageComplete()) {
				choice.res = new WorldVector(MWC.Algorithms.Conversions.Degs2Rads(offsetPage.getBearingDegs()),
						offsetPage.getRange(), null);
			}
		}

		final EnterSolutionPage solutionPage = (EnterSolutionPage) wizard.getPage(EnterSolutionPage.NAME);
		if (solutionPage != null) {
			if (solutionPage.isPageComplete()) {
				final EnterSolutionPage.SolutionDataItem item = (SolutionDataItem) solutionPage.getEditable();
				choice.courseDegs = item.getCourse();
				choice.speed = item.getSpeed();
			}
		}

		final SelectColorPage colorPage = (SelectColorPage) wizard.getPage(SelectColorPage.NAME);
