...
public class GeneratePasteRepClipboard implements RightClickContextItemGenerator {
	...
	private static class PasteRepOperation extends CMAPOperation {
		...
		public PasteRepOperation(final String title, final Layers theLayers, final String contentToImport) {
			super(title);
	...
	public static Action createAction(final Layers theLayers, final String clipboardContent) {
		final Action doPasteAction = new Action("Paste REP from clipboard") {
			@Override
			public void run() {
				final PasteRepOperation operation = new PasteRepOperation("Paste from clipboard", theLayers,
						clipboardContent);
	 ...
	 * (non-Javadoc)
	 *
	 * @see org.mwc.cmap.core.property_support.RightClickSupport.
	 * RightClickContextItemGenerator#generate(
	 * org.eclipse.jface.action.IMenuManager, MWC.GUI.Layers, MWC.GUI.Layer[],
	 * MWC.GUI.Editable[])
	 */
	@Override
