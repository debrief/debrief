...
public class CopyBearingsToClipboard implements RightClickContextItemGenerator {
	...
	private static class CopyBearingData extends CMAPOperation {
		...
		private static BearingListList calculateOffsets(final List<TrackWrapper> tracks, final TrackWrapper refTrack) {

			final BearingListList res = new BearingListList();

			for (final TrackWrapper track : tracks) {
				final BearingList bearings = new BearingList(track.getName(), track.getColor());
				...
				while (posits.hasMoreElements()) {
					...
					if (nearest != null && nearest.length > 0) {
						...
						final WorldVector offset = nextF.getLocation().subtract(near.getLocation());
		...
		public CopyBearingData(final String title, final List<TrackWrapper> tracks, final TrackWrapper referenceTrack) {
			super(title);
			_tracks = tracks;
		...
		private void writeOffsets(final BearingListList offsets) {
			...
			// store our data
			final TransferableBearingList ourData = new TransferableBearingList(offsets);
	...
	private static class Doublet implements Serializable {
		...
		public Doublet(final WorldVector vector, final Color color) {
			_vector = vector;
			_color = color;
		}
	}
	...
	private static class PasteBearingData extends CMAPOperation {
		...
		private static void createPointsFor(final TrackWrapper newTrack, final BearingList offsets,
				final TrackWrapper refTrack, final boolean retainColor) {
			...
			for (final HiResDate dtg : offsets.keySet()) {
				final Doublet doublet = offsets.get(dtg);
		...
		public PasteBearingData(final String title, final Layers layers, final BearingListList bearingList,
				final TrackWrapper referenceTrack, final boolean retainColor) {
			super(title);
			_layers = layers;
			_referenceTrack = referenceTrack;
	...
	static public final class testMe extends junit.framework.TestCase {
		...
		private void doTestCopyBearings(final int startIndex, final int endIndex) throws ExecutionException {
			...
			for (int i = startIndex; i < endIndex; i++) {
				...
				if (i % 9 != 0) {
					final SensorContactWrapper contact = new SensorContactWrapper(host.getName(),
							new HiResDate(newDate2.getTime()), null, 12d, null, Color.red, "Some label", 1, "Sensor");
					sensor.add(contact);
				}
			}
			...
			final SensorContactWrapper[] obs = new SensorContactWrapper[sensor.size()];
			Enumeration<Editable> ele = sensor.elements();
			int ctr = 0;
			while (ele.hasMoreElements()) {
				final SensorContactWrapper cut = (SensorContactWrapper) ele.nextElement();
				obs[ctr++] = cut;
			}

			final WorldVector offset = new WorldVector(12, 0.002, 0.0d);
			final RelativeTMASegment seg = new RelativeTMASegment(obs, offset, new WorldSpeed(12, WorldSpeed.Kts), 12d,
					layers, Color.green);
			subject.add(seg);
			...
			final GenerateInfillOperation operation = new GenerateInfillOperation("title", subjects, layers, subject,
					getMyLogger(), true);
		...
		@SuppressWarnings("deprecation")
		private void doTestPasteBearings(final int startIndex, final int endIndex, final int numPoints,
				final int menuItems) {
			final Layers layers = new Layers();
			final TrackWrapper host = new TrackWrapper();
			host.setName("Host");
			...
			final ActionContributionItem first = (ActionContributionItem) menu.getItems()[1];
	...
	private static class TransferableBearingList implements Transferable {
		public static DataFlavor FLAVOR = new DataFlavor(BearingList.class, "BearingListList");
	...
	public void generate(final IMenuManager parent, final Layers theLayers, final Layer[] parentLayers,
		...
		if (subjects.length >= 1) {
			...
			if (tracks.size() == 1) {
				pasteAdded = generatePasteAction(parent, theLayers, tracks.get(0), copyAdded);
	...
	private boolean generateCopyAction(final IMenuManager parent, final List<TrackWrapper> tracks) {
		final boolean added;
		boolean canDo = true;
		...
		for (final TrackWrapper track : tracks) {
			...
			if (seg instanceof RelativeTMASegment) {
				...
				if (refTrack == null) {
					// ok, show error
					CorePlugin.logError(IStatus.ERROR, "Host track for TMA leg can't be determined", null);
					canDo = false;
					break;
					...
					if (host == null) {
						...
						CorePlugin.logError(IStatus.ERROR, "Tracks don't have common Host Track", null);
						canDo = false;
						break;
					}
				} else {
					CorePlugin.logError(IStatus.ERROR, "Host track for TMA leg isn't a TrackWrapper", null);
					canDo = false;
					break;
				}
			}
		...
		if (host != null && !hostSelected && canDo) {
			...
			final Action convertToTrack = new Action(title) {
				...
				public void run() {
					...
					final IUndoableOperation copyBearings = new CopyBearingData(title, tracks, theHost);
			...
			convertToTrack.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/copy.png"));
	...
	private boolean generatePasteAction(final IMenuManager parent, final Layers theLayers, final TrackWrapper track,
			final boolean copyAdded) {
		final boolean added;
		...
		if (clip.isDataFlavorAvailable(TransferableBearingList.FLAVOR)) {
			...
			try {
				...
				if (contents != null) {
					...
					if (bearingList != null) {
						for (final BearingList bearings : bearingList) {
							...
							final TimePeriod listP = new TimePeriod.BaseTimePeriod((HiResDate) dates[0],
									(HiResDate) dates[dates.length - 1]);
							final TimePeriod trackP = new TimePeriod.BaseTimePeriod(track.getStartDTG(),
									track.getEndDTG());
						...
						if (doIt) {
							final String subject = bearingList.size() > 1 ? "tracks" : "track";

							// ok, create the action
							final String title = "Create new " + subject + " by adding clipboard bearings to "
									+ track.getName();
							...
							final Action createNewTrack = new Action(title) {
								...
								public void run() {
									// ok, go for it.
									// sort it out as an operation
									final IUndoableOperation copyBearings = new PasteBearingData(title, theLayers,
											bearingList, track, false);
							...
							createNewTrack.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/paste.png"));
							createNewTrackWithColor
									.setImageDescriptor(CorePlugin.getImageDescriptor("icons/16/paste.png"));
				...
				CorePlugin.logError(IStatus.ERROR, "Problem creating new track from clipboard", e);
			}
		} else {
			added = false;
		}
