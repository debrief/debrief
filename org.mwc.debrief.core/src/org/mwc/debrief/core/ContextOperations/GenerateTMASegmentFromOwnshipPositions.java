 ...
 *
 */
public class GenerateTMASegmentFromOwnshipPositions implements RightClickContextItemGenerator {
	...
	private static class TMAfromPositions extends CMAPOperation {
		...
		public TMAfromPositions(final FixWrapper[] items, final WorldVector offset, final Layers theLayers,
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			final HiResDate startTime = _items[0].getDTG();
			final HiResDate endTime = _items[_items.length - 1].getDTG();
			final WorldLocation startPoint = _items[0].getLocation().add(_offset);
			final TrackSegment seg = new AbsoluteTMASegment(_courseDegs, _speed, startPoint, startTime, endTime);
		...
		if (subjects.length == 1 || subjects.length > 1000) {
			...
			for (int i = 0; i < subjects.length; i++) {
				...
				if (editable instanceof FixWrapper) {
					...
					final WatchableList wList = fix.getTrackWrapper();
					...
					_myAction = new Action("Generate TMA solution from selected positions") {
						...
						public void run() {
							...
							if (dialog.getReturnCode() == Window.OK) {
								WorldVector res = new WorldVector(0, new WorldDistance(5, WorldDistance.NM), null);
								double courseDegs = 0;
								...
								if (offsetPage != null) {
									if (offsetPage.isPageComplete()) {
										res = new WorldVector(
												MWC.Algorithms.Conversions.Degs2Rads(offsetPage.getBearingDegs()),
								...
								final IUndoableOperation convertToTrack1 = new TMAfromPositions(items, res, theLayers,
										courseDegs, speed);
