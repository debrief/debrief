 ...
 *
 */
public class ExportCSVPreferencesPage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	private static final String CONTEXT_ID = "org.mwc.debrief.help.ExportCSVsPrefs";
	...
	private void addFileEditor() {
		...
		myFileEditor = new FileFieldEditor(PreferenceConstants.PATH_TO_CSV,
				Messages.ExportCSVLookupPreferencesPage_FileLabel, true, StringFieldEditor.VALIDATE_ON_KEY_STROKE,
				parent) {
		...
		myFileEditor.setErrorMessage(Messages.ExportCSVLookupPreferencesPage_InvalidFileName);
	...
	private boolean isFileNameProvided(final String fileName) {
		...
		if ((fileName == null) || (fileName.length() == 0)) {
			MessageDialog.openInformation(Display.getCurrent().getActiveShell(), "Edit CSV Dropdowns file",
	...
	private void openSystemEditor() {
		...
		if (!isFileNameProvided(fileName)) {
			...
			try {
				...
				CorePlugin.logError(IStatus.ERROR, Messages.ExportCSVLookupPreferencesPage_ErrorOnOpenFileEditor, e);
			}
		}
	}
	...
	public boolean performOk() {
		return validateFields() && super.performOk();
	}

	private void reloadDataFromFile() {
		final CSVExportDropdownRegistry registry = CSVExportDropdownRegistry.getRegistry();
		registry.setFileName(getFileName());
		try {
			registry.reload();
		} catch (final FileNotFoundException e) {
			CorePlugin.logError(IStatus.ERROR, "Can't find registry file:" + registry.getFileName(), e);
		}
	}
