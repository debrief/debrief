...
public class GenerateTrackFromActiveCuts implements RightClickContextItemGenerator {
	...
	private class BulkTrackGenerate extends CMAPOperation {
		private final List<TrackfromSensorWrappers> list = new ArrayList<TrackfromSensorWrappers>();

		BulkTrackGenerate(final List<SensorWrapper> sensors, final Layers layers, final ErrorLogger logger) {
			super("Generate multiple tracks from active sensors");

			for (final SensorWrapper sensor : sensors) {
				final TrackfromSensorWrappers genny = new TrackfromSensorWrappers(sensor, layers, logger);
				list.add(genny);
			}
		}
	...
	static public final class testMe extends junit.framework.TestCase {
		...
		public final void testIWork() {
			...
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.0, 2.0, 0.0), 12, 12)));

			cal.set(2001, 10, 4, 4, 4, 01);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.0, 2.25, 0.0), 12, 12)));

			cal.set(2001, 10, 4, 4, 4, 02);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.0, 2.5, 0.0), 12, 12)));
			cal.set(2001, 10, 4, 4, 4, 05);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.0, 2.75, 0.0), 12, 12)));
			cal.set(2001, 10, 4, 4, 4, 23);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.25, 2.0, 0.0), 12, 12)));
			cal.set(2001, 10, 4, 4, 4, 25);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.5, 2.0, 0.0), 12, 12)));
			cal.set(2001, 10, 4, 4, 4, 28);
			final WorldLocation theLoc = new WorldLocation(2.75d, 2.0, 0.0);
			track.addFix(new FixWrapper(new Fix(new HiResDate(cal.getTime().getTime(), 0), theLoc, 12, 12)));
			cal.set(2001, 10, 4, 4, 4, 55);
			track.addFix(new FixWrapper(
					new Fix(new HiResDate(cal.getTime().getTime(), 0), new WorldLocation(2.25, 2.25, 0.0), 12, 12)));
			...
			sensor.add(new SensorContactWrapper("track", new HiResDate(cal.getTime().getTime()),
					new WorldDistance(1000, WorldDistance.YARDS), 23.3d, null, Color.RED, "Some lable", 1,
					sensor.getName()));
			...
			final GenerateTrackFromActiveCuts genny = new GenerateTrackFromActiveCuts(logger);
			final Layer[] parentLayers = new Layer[] { track };
			final Editable[] subjects = new Editable[] { sensor };
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			for (int cnt = 0; cnt < cuts.length; cnt++) {
				...
				if (cut.getHasBearing() && cut.getRange() != null && !cut.getHasAmbiguousBearing()) {
					...
					final WorldVector vec = new WorldVector(Math.toRadians(cut.getBearing()),
					...
					if (origin != null) {
						...
						logger.logError(IStatus.WARNING,
								"Ownship location not found for sensor at:" + cut.getDTG().getDate(), null);
					}
				}
			}
	...
	private static class TrackfromSensorWrappers extends TrackfromSensorData {
		...
		public TrackfromSensorWrappers(final SensorWrapper wrapper, final Layers theLayers, final ErrorLogger logger) {
			super("Create Track from Active cuts", theLayers, logger);
			_wrapper = wrapper;
		}
		...
		protected SensorContactWrapper[] getCuts() {
			...
			if (wraps.size() > 0) {
				final SensorContactWrapper[] sample = new SensorContactWrapper[wraps.size()];
			...
			if (!allGood) {
				...
				for (int i = 0; i < subjects.length; i++) {
					...
					if (editable instanceof SensorWrapper) {
						...
						if (sensor.size() > 0) {
							final SensorContactWrapper scw = (SensorContactWrapper) sensor.elements().nextElement();
				...
				if (!sensors.isEmpty()) {
					...
					_myAction = new Action("Generate Tracks from multiple Active Sensors") {
						...
						public void run() {
							// ok, go for it.
							// sort it out as an operation
							final IUndoableOperation convertToTrack1 = new BulkTrackGenerate(sensors, theLayers,
									_logger);
