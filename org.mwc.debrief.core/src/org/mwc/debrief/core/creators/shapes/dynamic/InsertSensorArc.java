...
public class InsertSensorArc extends CoreInsertSensorArc {
	...
	public void execute() {
		...
		if (!proceed) {
			...
			MessageDialog.openWarning(getShell(), "Warning!",
					"Sorry, one or more tracks must be present in order to add sensor arcs");
		} else {
			super.execute();
		}
	}
	...
	@Override
	protected DynamicTrackShapeWrapper getPlottable(final PlainChart thePlainChart) {
		final Layers layers = getChart().getLayers();
		Date startDate = null;
		Date endDate = null;

		final Map<String, Editable> tracks = new HashMap<>();
		final Enumeration<Editable> elements = layers.elements();
		while (elements.hasMoreElements()) {
		...
		final NewSensorArcWizard wizard = new NewSensorArcWizard(_selectedLayer, startDate, endDate);
