...
public class JaxbGpxHelper implements GpxHelper {
	...
	static {
		try {
			GPX_1_1_JAXB_CTX = JAXBContext.newInstance("com.topografix.gpx.v11",
					com.topografix.gpx.v11.ObjectFactory.class.getClassLoader());
			GPX_1_0_JAXB_CTX = JAXBContext.newInstance("com.topografix.gpx.v10",
					com.topografix.gpx.v10.ObjectFactory.class.getClassLoader());
			DEBRIEF_EXTENSIONS_JAXB_CTX = JAXBContext.newInstance("org.mwc.debrief.core.gpx",
					org.mwc.debrief.core.gpx.ObjectFactory.class.getClassLoader());
	...
	public void marshall(final List<TrackWrapper> tracks, final File fileName) {
		...
		if (direcotryPath == null) {
			CorePlugin.errorDialog("Export to GPS", "You have to selected the directory to save the GPS file.");
			return;
		}
		...
		try {
			...
			if (tracks.size() > 0) {
				CorePlugin.logError(IStatus.INFO,
						"Exporting " + tracks.size() + " tracks to gpx file " + saveTo.getAbsolutePath(), null);
				...
				CorePlugin.logError(IStatus.INFO, "No tracks vailable to export", null);
				CorePlugin.infoDialog("Export to GPS", "No tracks vailable to export");
			}
		} catch (final Exception e) {
			CorePlugin.logError(IStatus.ERROR,
					"Error while marshalling to file GPX format: " + fileName.getAbsolutePath(), e);
			String dialogMsg = "";
	...
	public Layers unmarshall(final InputStream gpxStream, final Layers theLayers) {
		...
		try {
			...
			CorePlugin.logError(IStatus.ERROR, "Problem reading GPX", e);
