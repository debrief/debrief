	...
	protected IRunnableWithProgress getImporter(final IAdaptable target, final Layers theLayers,
		...
		return new IRunnableWithProgress() {
			...
			public void run(final IProgressMonitor pm) {
				final IControllableViewport view = target.getAdapter(IControllableViewport.class);
				final PlotEditor plot = target.getAdapter(PlotEditor.class);
				try {
					_myReader.importThis(fileName, inputStream, theLayers, view, plot);
				} catch (final PlainImporter.ImportException ie) {
					Application.logError2(ToolParent.ERROR, ie.getMessage(), ie);
