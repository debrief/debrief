...
public final class StepperHandler implements SWTGUIHandler.ComponentCreator {
	...
	private void configureThisHighlighter(final SWTPlotHighlighter thisHighlighter, final ComponentDetails details) {
		if (thisHighlighter.getName().equals(SWTRangeHighlighter.RANGE_RING_HIGHLIGHT)) {
			...
			final String radius = details.properties.get(RADIUS);
			if (radius != null)
				try {
					rr.setRadius(MWCXMLReader.readThisDouble(radius));
				} catch (final ParseException pe) {
					MWC.Utilities.Errors.Trace.trace(pe,
							"Reader: Whilst reading in " + RADIUS + " value of :" + radius);
				}

			final String arcs = details.properties.get(ARCS);
			if (arcs != null)
				rr.setArcStart(Integer.parseInt(arcs));

			final String arcs_end = details.properties.get(ARCS_FINISH);
			if (arcs_end != null)
				rr.setArcEnd(Integer.parseInt(arcs_end));

			final String spokeSep = details.properties.get(SPOKE_SEPARATION);
			if (spokeSep != null)
				rr.setSpokeSeparation(Integer.parseInt(spokeSep));

			final String numRings = details.properties.get(NUM_RINGS);
			if (numRings != null)
				rr.setNumRings(Integer.parseInt(numRings));

			final String fillRings = details.properties.get(SHADE_ARCS);
			if (fillRings != null)
				rr.setFillArcs(Boolean.valueOf(fillRings));

			final String justPrimary = details.properties.get(JUST_PRIMARY);
			if (justPrimary != null)
				rr.setJustPlotPrimary(Boolean.valueOf(justPrimary));

			final String useTrackColor = details.properties.get(USE_TRACK_COLOR);
			if (useTrackColor != null)
				rr.setUseCurrentTrackColor(Boolean.valueOf(useTrackColor));

			final String colRed = details.properties.get(COL_RED);
			final String colGreen = details.properties.get(COL_GREEN);
			final String colBlue = details.properties.get(COL_BLUE);
			if ((colRed != null) && (colGreen != null) && (colBlue != null)) {
				final Color newCol = new Color(Integer.valueOf(colRed), Integer.valueOf(colGreen),
						Integer.valueOf(colBlue));
				rr.setColor(newCol);
			}

		} else if (thisHighlighter.getName().equals(SWTPlotHighlighter.RectangleHighlight.DEFAULT_HIGHLIGHT)) {
			final SWTPlotHighlighter.RectangleHighlight rect = (RectangleHighlight) thisHighlighter;
			final String theCol = details.properties.get(RECT_HIGHLIGHT_COLOR);
			if (theCol != null)
				rect.setColor(ColourHandler.fromString(theCol));
			final String theSize = details.properties.get(RECT_HIGHLIGHT_SIZE);
			if (theSize != null)
	...
	private void configureThisPainter(final TemporalLayerPainter thisPainter,
		...
		if (thisPainter.getName().equals(SnailPainter.SNAIL_NAME)) {
			...
			final String vector_stretch = details.properties.get(VECTOR_STRETCH);
			if (vector_stretch != null) {
				try {
					...
					MWC.Utilities.Errors.Trace.trace(pe,
							"Reader: Whilst reading in " + VECTOR_STRETCH + " value of :" + vector_stretch);
				}
			}

			final String linkPos = details.properties.get(LINK_POSITIONS);
			if (linkPos != null)
				sp.getSnailProperties().setLinkPositions(Boolean.parseBoolean(linkPos));

			final String plotTrkName = details.properties.get(PLOT_TRACK_NAME);
			if (plotTrkName != null)
				sp.getSnailProperties().setPlotTrackName(Boolean.parseBoolean(plotTrkName));

			final String fadePoints = details.properties.get(FADE_POINTS);
			if (fadePoints != null)
				sp.getSnailProperties().setFadePoints(Boolean.parseBoolean(fadePoints));

			final String pointSize = details.properties.get(POINT_SIZE);
			if (pointSize != null)
				sp.getSnailProperties().setPointSize(Integer.parseInt(pointSize));

			final String trailLength = details.properties.get(TRAIL_LENGTH);
			if (trailLength != null) {
				try {
					final Duration theLen = Duration.fromString(trailLength);
					sp.getSnailProperties().setTrailLength(theLen);
				} catch (final java.text.ParseException pe) {
					MWC.Utilities.Errors.Trace.trace(pe, "Failed reading duration value is:" + trailLength);
				}
			}
		}
	}

	public final GUIHandler.ComponentDetails exportThis(final TimeControlPreferences controller,
			final LayerPainterManager painterMgr, final TimeProvider timeProvider, final Document doc) {
	...
	public final void makeThis(final Debrief.ReaderWriter.XML.GUIHandler.ComponentDetails details,
		...
		if (tZero != null) {
			...
			// set the cursor
			// set the cursor
			CorePlugin.logError(IStatus.WARNING, "T-Zero not yet implemented", null);
		}
		// ////////////////////////////////////////////////////////////
		...
		// /////////////////////////////////////////////////////////////
		val = details.properties.get("SmallStep");
		if (val != null) {
			try {
				final Duration dur = Duration.fromString(val);
				timePrefs.setSmallStep(dur);
			} catch (final java.text.ParseException pe) {
				MWC.Utilities.Errors.Trace.trace(pe, "Failed reading duration value is:" + val);
			}
		}

		// /////////////////////////////////////////////////////////////
		val = details.properties.get("LargeStep");
		if (val != null) {
			try {
				final Duration dur = Duration.fromString(val);
		...
		// /////////////////////////////////////////////////////////////
		val = details.properties.get("AutoStepInterval");
		if (val != null) {
			try {
				final Duration dur = Duration.fromString(val);
	...
	private void storeThisHighlighter(final SWTPlotHighlighter thisHighlighter, final String name,
			final ComponentDetails details, final Document doc) {
		if (name.equals(SWTRangeHighlighter.RANGE_RING_HIGHLIGHT)) {
			...
			details.addProperty(ARCS, MWCXMLReader.writeThis(hi.getArcStart()));
			details.addProperty(ARCS_FINISH, MWCXMLReader.writeThis(hi.getArcEnd()));
		...
		if (name.equals(SWTPlotHighlighter.RectangleHighlight.DEFAULT_HIGHLIGHT)) {
			...
			details.addProperty(RECT_HIGHLIGHT_COLOR, ColourHandler.toString(rec.getColor()));
	...
	private void storeThisPainter(final TemporalLayerPainter thisPainter, final String name,
		...
		if (name.equals(SnailPainter.SNAIL_NAME)) {
			final SnailHighlighter sp = (SnailHighlighter) thisPainter;
			details.addProperty(LINK_POSITIONS, MWCXMLReader.writeThis(sp.getSnailProperties().getLinkPositions()));
			details.addProperty(PLOT_TRACK_NAME, MWCXMLReader.writeThis(sp.getSnailProperties().getPlotTrackName()));
			details.addProperty(FADE_POINTS, MWCXMLReader.writeThis(sp.getSnailProperties().getFadePoints()));
			details.addProperty(POINT_SIZE,
					MWCXMLReader.writeThis(sp.getSnailProperties().getPointSize().getCurrent()));
			details.addProperty(TRAIL_LENGTH, MWCXMLReader.writeThis(sp.getSnailProperties().getTrailLength()));
			details.addProperty(VECTOR_STRETCH, MWCXMLReader.writeThis(sp.getSnailProperties().getVectorStretch()));
		}
	}
