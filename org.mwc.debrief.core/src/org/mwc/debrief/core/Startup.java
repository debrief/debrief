...
public class Startup implements IStartup {
	...
	protected void changeIcon(IWorkbenchPart part) {
		try {
			...
			Method method = clazz.getDeclaredMethod("setTitleImage",
			...
			if (part instanceof ContentOutline) {
				if (outlineImage == null) {
					...
					outlineImage = JFaceResources.getResources()
			...
			if (part instanceof PropertySheet) {
				if (propertiesImage == null) {
					...
					propertiesImage = JFaceResources.getResources()
							.createImageWithDefault(descriptor);
				}
			...
			if (part instanceof ResourceNavigator) {
				if (navigatorImage == null) {
					ImageDescriptor descriptor = DebriefPlugin
							.getImageDescriptor("icons/16/navigator.png");
					navigatorImage = JFaceResources.getResources()
							.createImageWithDefault(descriptor);
				}
	...
	private void removePreferencePages() {
		...
		preferenceManager
				.remove("org.eclipse.debug.ui.LaunchingPreferencePage");
		preferenceManager
		...
		preferenceManager
				.remove("org.eclipse.debug.ui.PerspectivePreferencePage");
	...
	private void removePerspective() {
		...
		if (registry instanceof IExtensionChangeHandler
				&& !descriptors.isEmpty()) {
	...
	private static class ActionAccessSupport {
		...
		public ActionAccessSupport() {
			...
			Method method;
			try {
				method = clazz.getDeclaredMethod("getActionBarAdvisor",
						new Class[] {});
				method.setAccessible(true);
				actionBarAdvisor = (ActionBarAdvisor) method
						.invoke(activeWorkbenchWindow);
				assert actionBarAdvisor != null : "API CHANGE!";
				actionAccess = ActionBarAdvisor.class.getDeclaredMethod(
						"getAction", new Class[] { String.class });
