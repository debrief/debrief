...
public class ASSETPlotEditor extends CorePlotEditor {
	...
	public void createPartControl(final Composite parent) {
		...
		final IAction undoAction = new UndoActionHandler(getEditorSite(), ASSETPlugin.ASSET_CONTEXT);
		final IAction redoAction = new RedoActionHandler(getEditorSite(), ASSETPlugin.ASSET_CONTEXT);
	...
	private void lookForData() {
		...
		final IViewPart[] views = getSite().getWorkbenchWindow().getActivePage().getViews();
	...
	protected void updateScenario(final ScenarioWrapper scenarioLayers, final ScenarioType scenario) {
		// are we already listening to it?
		if (_myScenario != null) {
			stopListeningToThis(_myLayers);
			stopListeningToThis(_myScenario);
