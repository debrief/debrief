...
public class DetectionListenerHandler extends ASSETResource implements ListenerResource {
	...
	public int accept(final String listenerTxt) {
		...
		try {
			...
			res = host.getHost().newParticipantDetectionListener(getScenarioId(), getParticipantId(), listener);
	...
	public void remove() {
		...
		final ASSETHost.HostProvider host = (HostProvider) getApplication();
		host.getHost().deleteParticipantDetectionListener(getScenarioId(), getParticipantId(), theId);
	}
