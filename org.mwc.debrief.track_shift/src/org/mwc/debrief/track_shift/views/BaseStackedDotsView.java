...
abstract public class BaseStackedDotsView extends ViewPart implements ErrorLogger {
	...
	private class DragTrackSegment extends ControlContribution {
		...
		public DragTrackSegment(final Vector<Action> actions) {
		...
		protected Control createControl(final Composite parent) {
			...
			item.setImage(CorePlugin.getImageFromRegistry(CorePlugin.getImageDescriptor("icons/24/SelectSegment.png")));
			item.addListener(SWT.Selection, new Listener() {
				...
				public void handleEvent(final Event event) {
					...
					// menu location
					final org.eclipse.swt.graphics.Rectangle rect = item.getBounds();
					org.eclipse.swt.graphics.Point pt = new org.eclipse.swt.graphics.Point(rect.x,
							rect.y + rect.height);
	...
	private static class LegStorer implements ILegStorer {
		...
		private LegStorer(final ColorProvider randomProv, final List<Zone> legs) {
	...
	private static class MidnightDateFormat extends GMTDateFormat {
		...
		private static boolean isMidnight(final Date date) {
			...
			return myCal.get(Calendar.HOUR_OF_DAY) == 0 && myCal.get(Calendar.MINUTE) == 0
					&& myCal.get(Calendar.SECOND) == 0 && myCal.get(Calendar.MILLISECOND) == 0;
		}
		 ...
		 */
		public MidnightDateFormat(final String format, final DateFormat midnightFormat) {
			super(format);
	...
	protected class TargetZoneSlicer implements ZoneSlicer {
		...
		public boolean ambigDataPresent() {
			...
			System.err.println("Should not be trying to check ambig cuts on target track");
		...
		private List<Zone> performSlicingCombined() {
			...
			if (secondary != null) {
				...
				final TimePeriod period = new TimePeriod.BaseTimePeriod(secondary.getStartDTG(), secondary.getEndDTG());
				final List<SensorContactWrapper> bearings = StackedDotHelper.getBearings(_myHelper.getPrimaryTrack(),
						_onlyVisible.isChecked(), period);
		...
		private List<Zone> performSlicingSeparate() {
			...
			if (secondary != null) {
				...
				// note: the slicer depends upon bearing. check we have bearing
				if (bearings.size() > 0 && !Double.isNaN(bearings.get(0).getBearing())) {
					res = sliceTarget(ownshipZoneChart.getZones(), bearings, _randomProv, secondary, _slicePrecision);
				} else {
					res = null;
				}
			} else {
	...
	private static TickUnitSource createMyStandardDateTickUnits() {
		...
		units.add(new DateTickUnit(DateTickUnitType.HOUR, 2, DateTickUnitType.MINUTE, 10, f4));
		units.add(new DateTickUnit(DateTickUnitType.HOUR, 4, DateTickUnitType.MINUTE, 30, f4));
		units.add(new DateTickUnit(DateTickUnitType.HOUR, 6, DateTickUnitType.HOUR, 1, f4));
		units.add(new DateTickUnit(DateTickUnitType.HOUR, 12, DateTickUnitType.HOUR, 1, f4));

		// days
		units.add(new DateTickUnit(DateTickUnitType.DAY, 1, DateTickUnitType.HOUR, 1, f4));
		units.add(new DateTickUnit(DateTickUnitType.DAY, 2, DateTickUnitType.HOUR, 1, f4));
		units.add(new DateTickUnit(DateTickUnitType.DAY, 7, DateTickUnitType.DAY, 1, f4));
		units.add(new DateTickUnit(DateTickUnitType.DAY, 15, DateTickUnitType.DAY, 1, f4));
	...
	private static void getCutsForThisLeg(final List<SensorContactWrapper> cuts, final long wholeStart,
			final long wholeEnd, final List<Long> thisLegTimes, final List<Double> thisLegBearings) {
	...
	protected static List<Zone> legsFromZigs(final long startTime, final long endTime, final List<Zone> zigs,
			final ColorProvider randomProv) {
	...
	private static void showThisSelectionInOutline(final List<EditableWrapper> subjects, final IEditorPart editor) {
	...
	private static void sliceThisLeg(final List<SensorContactWrapper> cuts, final ZigDetector slicer,
			final IZigStorer zigStorer, final ILegStorer legStorer, final double optimiseTolerance,
		...
		// slice the leg
		slicer.sliceThis(log, TrackShiftActivator.PLUGIN_ID, "Some scenario", wholeStart, wholeEnd, legStorer,
				zigStorer, RMS_ZIG_RATIO, optimiseTolerance, thisLegTimes, thisLegBearings);
	}

	private static EditableWrapper wrapThisCut(final SensorContactWrapper cut, final Layers layers) {
		...
		final EditableWrapper sensors = new EditableWrapper(secTrack.getSensors(), parentP, layers);
		...
		if (seg != null) {
			...
			if (secTrack != null) {
				...
				if (secTrack.getSegments().size() > 1) {
					...
					final EditableWrapper segments = new EditableWrapper(secTrack.getSegments(), parentP, layers);
	...
	protected final ZoneUndoRedoProvider undoRedoProvider = new ZoneUndoRedoProvider() {
		@Override
	...
	protected BaseStackedDotsView(final boolean needBrg, final boolean needFreq) {
		...
		_myTrackDataListener = new TrackDataListener() {
			...
			public void tracksUpdated(final WatchableList primary, final WatchableList[] secondaries) {
				...
				final boolean secSame = ((_myHelper.getSecondaryTrack() != null) && secondaries != null
						&& secondaries.length == 1 && (_myHelper.getSecondaryTrack().equals(secondaries[0]))
				...
				if (_holder == null || _holder.isDisposed()) {
					...
					_myHelper.initialise(_switchableTrackDataProvider, false, _onlyVisible.isChecked(), logger,
							getType(), _needBrg, _needFreq);
				}
	...
	protected void addPullDownExtras(final IMenuManager manager) {
	...
	/**
	 * if we aren't showing any of the top 3 plots, hide the panel, to make the
	 * space available for zone charts
	 */
	...
	private void clearDateTickUnits() {
		...
		if (dAxis instanceof CachedTickDateAxis) {
			final CachedTickDateAxis axis = (CachedTickDateAxis) _combined.getDomainAxis();
			axis.clearTicks();
		}
	}
	...
	void clearPlots() {
		final Runnable runner = new Runnable() {
			@Override
			public void run() {
				final TimeSeriesCollection coll = (TimeSeriesCollection) _dotPlot.getDataset();
				coll.removeAllSeries();

				final TimeSeriesCollection line = (TimeSeriesCollection) _linePlot.getDataset();
	...
	private void clearZoneCharts(final boolean osChanged, final boolean tgtChanged, final boolean clearTgtData) {
		if (osChanged) {
	...
	private void createGlobalActionHandlers() {
		...
		undoAction = new HandlerAction() {
			...
			public void refreah() {
				...
				if (isEnabled()) {
					final String toolTipText = "Undo " + operationHistory.getUndoOperation(undoContext).getLabel();
					setText(toolTipText);
		...
		undoAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/24/undo.png"));
		undoAction.setDisabledImageDescriptor(CorePlugin.getImageDescriptor("icons/24/undo.png"));
		...
		redoAction = new HandlerAction() {
			...
			public void refreah() {
				...
				if (isEnabled()) {
					final String toolTipText = "Redo " + operationHistory.getRedoOperation(undoContext).getLabel();
					setText(toolTipText);
					setToolTipText(toolTipText);
				} else {
					setText("Redo");
		...
		redoAction.setImageDescriptor(CorePlugin.getImageDescriptor("icons/24/redo.png"));
		redoAction.setDisabledImageDescriptor(CorePlugin.getImageDescriptor("icons/24/redo.png"));
	...
	public void createPartControl(final Composite parent) {
		...
		_holder = new ChartComposite(sashForm, SWT.NONE, null, 400, 600, 300, 200, 1800, 1800, true, true, true, true,
				true, true) {
			@Override
		...
		final ZoneChartConfig oZoneConfig = new ZoneChart.ZoneChartConfig("Ownship Legs", "Course", DebriefColors.BLUE,
				goingHolistic);
		...
		final TimeSeriesCollection[] otherDatasets = new TimeSeriesCollection[] { measuredValuesColl, ambigValuesColl };
		...
		ownshipZoneChart = ZoneChart.create(oZoneConfig, undoRedoProvider, sashForm, osZones, ownshipCourseSeries,
				otherDatasets, scoreSeries, blueProv, ownshipLegSlicer, deleteCutsInTurn, resolveAmbiguity,
				deleteAmbiguousCuts);
		...
		final ZoneChartConfig tZoneConfig = new ZoneChart.ZoneChartConfig("Target Legs", "Bearing", DebriefColors.RED,
				false);
		final TimeSeriesCollection calcColl = new TimeSeriesCollection();
		calcColl.addSeries(targetCalculatedSeries);
		final TimeSeriesCollection[] otherSeries = new TimeSeriesCollection[] { calcColl };
		targetZoneChart = ZoneChart.create(tZoneConfig, undoRedoProvider, sashForm, tgtZones, targetBearingSeries,
				otherSeries, null, randomProv, targetLegSlicer, null, null, null);
	...
	protected void createStackedPlot() {
		...
		_overviewCourseRenderer = new WrappingResidualRenderer(null, null, _targetCourseSeries, 0, 360);
		...
		_overviewCourseRenderer.setSeriesShape(0, new Ellipse2D.Double(-4.0, -4.0, 8.0, 8.0));
		...
		final LegendItemSource[] sources = { _linePlot, _targetOverviewPlot };
		...
		_myChart.addProgressListener(new ChartProgressListener() {
			...
			public void chartProgress(final ChartProgressEvent cpe) {
				...
				final long crossDate = (long) (_linePlot.isDomainCrosshairVisible()
						? _linePlot.getDomainCrosshairValue()
						: _dotPlot.getDomainCrosshairValue());
				...
				if (_selectOnClick.isChecked() && _seriesToSearch != null) {
					...
					final TimeSeriesCollection tsc = (TimeSeriesCollection) _linePlot.getDataset();
					...
					if (tsc != null) {
						...
						if (t != null) {
							...
							CorePlugin.logError(IStatus.WARNING, "Failed to find series:" + targetSeries, null);
							nearest = null;
						}
						...
						if (nearest == null) {
							CorePlugin.logError(IStatus.WARNING,
									"Failed to find match in series:" + targetSeries + " at time:" + newDate, null);
						} else {
							// ok, get the editor
							...
							if (layers != null) {
								...
								if (payload != null) {
									...
									if (payload instanceof SensorContactWrapper) {
										final SensorContactWrapper theCut = (SensorContactWrapper) payload;
										subject = wrapThisCut(theCut, layers);
									} else if (payload instanceof FixWrapper) {
										final FixWrapper theFix = (FixWrapper) payload;
		...
		_holder.addChartMouseListener(new ChartMouseListener() {
			...
			public void chartMouseClicked(final ChartMouseEvent arg0) {
				...
				final String seriesName = getSeriesToSelect(arg0.getChart(), arg0.getTrigger(), arg0.getEntity());
				...
				if (seriesName != null) {
					...
					if (ent instanceof XYItemEntity) {
						...
						final long time = (long) xy.getDataset().getXValue(xy.getSeriesIndex(), xy.getItem());
						final double valueVal = xy.getDataset().getYValue(xy.getSeriesIndex(), xy.getItem());
						...
						if (findNearest) {
							...
							final CombinedDomainXYPlot dPlot = (CombinedDomainXYPlot) arg0.getChart().getPlot();
							final ValueAxis dateAxis = dPlot.getDomainAxis();

							// and the data value
							final XYPlot selectedPlot = findSelectedPlot(arg0.getChart(), arg0.getTrigger(),
									arg0.getEntity());
							final ValueAxis valueAxis = selectedPlot.getRangeAxis();

							final org.eclipse.swt.graphics.Rectangle area = _holder.getScreenDataArea(screenClick.x,
									screenClick.y);
							final Rectangle jRect = new Rectangle(area.width, area.height);
							jRect.setLocation(area.x, area.y);
							final double dateVal = dateAxis.java2DToValue(p.getY(), jRect, RectangleEdge.LEFT);
							final long dateMillis = (long) dateVal;

							final double valueVal = valueAxis.java2DToValue(p.getX(), jRect, RectangleEdge.TOP);

							// special case - if we are in the outer third of the plot, we don't get all
							// hits
							// don't find the nearest manually if we're within 60% of the origin
							final double delta = Math.abs(valueAxis.getRange().getCentralValue() - valueVal);
							final double range = valueAxis.getRange().getUpperBound()
							...
							if (delta > (range / 2d) * factor) {
								...
								CorePlugin.logError(IStatus.INFO,
										"Not selecting cut, not far out enough. delta:" + df.format(delta) + ", range:"
												+ df.format((range / 2d)) + ", centre:"
												+ df.format(valueAxis.getRange().getCentralValue()) + " proportion:"
												+ df.format((delta / (range / 2d))),
										null);
							}
						}
					}
			...
			private XYPlot findSelectedPlot(final JFreeChart chart, final java.awt.event.MouseEvent trigger,
				...
				if (entity instanceof PlotEntity) {
					...
					if (plot instanceof CombinedDomainXYPlot) {
						...
						final PlotRenderingInfo plotInfo = _holder.getChartRenderingInfo().getPlotInfo();
			...
			private String getSeriesToSelect(final JFreeChart chart, final java.awt.event.MouseEvent trigger,
					final ChartEntity entity) {
				...
				if (selectedPlot != null) {
					if (selectedPlot.equals(_linePlot)) {
						...
						if (entity instanceof XYItemEntity) {
							...
							// get the XY details
							final XYItemEntity xy = (XYItemEntity) entity;

							// retrieve the subject series
							final TimeSeries ts = coll.getSeries(xy.getSeriesIndex());
						...
						if (entity instanceof XYItemEntity) {
							...
							final XYDataset dataset = xyi.getDataset();
							final int seriesIndex = xyi.getSeriesIndex();
			...
			private void highlightDataItemNearest(final long dateMillis, final double valueVal, String seriesName) {
				...
				if (tsc == null) {
					CorePlugin.logError(IStatus.ERROR, "Trying to select item. Can't find line plot data", null);
				} else {
					...
					if (t == null) {
						...
						// a line plot name
						final String tmpSeriesName = seriesName.replace(ERROR_VALUES,
					...
					if (t == null) {
						CorePlugin.logError(IStatus.ERROR,
								"Trying to select item. Can't find series titled:" + seriesName, null);
					} else {
						final XYPlot plotToUse = seriesName.startsWith(MEASURED_VALUES) ? _linePlot : _dotPlot;
		...
		if (_myTrackDataProvider != null) {
			...
			if (secs != null && secs.length == 1) {
				...
				if (firstSec instanceof TrackWrapper) {
					...
					secT.removePropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _infillListener);
				}
			}
		}
	...
	protected void doSelectCore(final XYPlot subjectPlot, final String seriesName) {
		...
		final TimeSeriesCollection tsc = (TimeSeriesCollection) subjectPlot.getDataset();
		final TimeSeries measurements = tsc.getSeries(seriesName);
		final List<Editable> toSelect = new ArrayList<Editable>();
		if (measurements == null) {
			CorePlugin.logError(IStatus.ERROR, "Trying to select fixes/cuts, but can't find them:" + seriesName, null);
			...
			for (final Object item : list) {
				...
				if (valueRange.contains(value) && timeRange.contains(time) && thisI instanceof ColouredDataItem) {
					// add to list
		...
		if (!toSelect.isEmpty()) {
			...
			final List<EditableWrapper> wrappedItems = new ArrayList<EditableWrapper>();
	...
	protected void fillLocalToolBar(final IToolBarManager toolBarManager) {
		// Note: we have undo/redo buttons on the toolbar. Let's not bother with them
		// here, there are
		...
		final IContributionItem dropdown = new DragTrackSegment(dragModeActions);
		toolBarManager.add(dropdown);

	}

	/**
	 * format the value in a suitable way for marking the current value of the
	 * cursor
	 *
	 * @param value data value at cursor
	...
	private ZoneChart.ZoneListener getTargetListener() {
		return new ZoneChart.ZoneListener() {
			...
			public void deleted(final Zone zone) {
				...
				final TimePeriod zonePeriod = new TimePeriod.BaseTimePeriod(new HiResDate(zone.getStart()),
				...
				while (iter.hasMoreElements()) {
					...
					if (nextSeg instanceof SegmentList) {
						// oh, this track has already been split into legs. We need to iterate through
						// them
						// instead
					...
					final TimePeriod legPeriod = new TimePeriod.BaseTimePeriod(cSeg.startDTG(), cSeg.endDTG());
	...
	protected List<Zone> getTargetZones() {
		...
		if (_myTrackDataProvider != null) {
			...
			if (secTracks != null && secTracks.length == 1) {
				...
				final WatchableList nextT = secTracks[0];
				if (nextT instanceof ISecondaryTrack) {
					...
					while (iter.hasMoreElements()) {
						...
						if (thisSeg instanceof RelativeTMASegment) {
							...
							if (!thisSeg.isEmpty()) {
								final FixWrapper firstE = (FixWrapper) thisSeg.elements().nextElement();
	...
	public void logError(final int statusCode, final String string, final Exception object) {
		Display.getDefault().asyncExec(new Runnable() {
			...
			public void run() {
				...
				if (string != null && _myChart.getTitle() != null && !string.equals(_myChart.getTitle().getText())) {
	...
	protected void makeActions() {
		...
		_selectMeasurements = new Action("Select visible measurements", IAction.AS_PUSH_BUTTON) {
			@Override
			public void run() {
				super.run();
		...
		_selectMeasurements.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/16/sensor_contact.png"));

		_selectPositions = new Action("Select visible positions", IAction.AS_PUSH_BUTTON) {
			@Override
			public void run() {
				super.run();
				doSelectPositions();
		...
		_selectPositions.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/16/fix.png"));
		...
		_showZones.setImageDescriptor(CorePlugin.getImageDescriptor("icons/24/GanttBars.png"));
		...
		_showLinePlot.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/stacked_lines.png"));
		...
		_showDotPlot.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/stacked_dots.png"));
		...
		_showTargetOverview.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/tgt_overview.png"));

		// get an error logger
		final ErrorLogger logger = this;
		_onlyVisible = new Action("Only draw dots for visible data points", IAction.AS_CHECK_BOX) {
			...
			public void run() {
				...
				if (_holder == null || _holder.isDisposed()) {
					...
					_myHelper.initialise(_switchableTrackDataProvider, true, _onlyVisible.isChecked(), logger,
							getType(), _needBrg, _needFreq);
				}
		...
		_onlyVisible.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/reveal.png"));

		_selectOnClick = new Action("Select TMA position or Sensor Cut under crosshair", IAction.AS_CHECK_BOX) {
		};
		_selectOnClick.setChecked(true);
		_selectOnClick.setToolTipText("Reveal the respective TMA Fix when an error clicked on plot");
		_selectOnClick.setImageDescriptor(CorePlugin.getImageDescriptor("icons/24/outline.png"));

		_showCrossHairs = new Action("Show/hide crosshair marker", IAction.AS_CHECK_BOX) {
		...
		_showCrossHairs.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/crosshair.png"));
		...
		_switchPrimary.setImageDescriptor(DebriefPlugin.getImageDescriptor("icons/16/MultiPath.png"));
	 ...
	 * @param leg
	 */
	public void setLeg(final TrackWrapper primaryTrack, final ISecondaryTrack secTrack, final Zone leg) {
		final TimePeriod zonePeriod = new TimePeriod.BaseTimePeriod(new HiResDate(leg.getStart()),
		...
		while (iter.hasMoreElements()) {
			...
			if (zonePeriod.overlaps(legPeriod)) {
				// just check the periods don't match - if they match, we don't need to do
				// anything
				if (zonePeriod.equals(legPeriod)) {
					...
					if (legFound) {
						...
						CorePlugin.logError(IStatus.INFO,
								"Existing leg overlaps with auto-generated one. deleting:" + cSeg, null);
					} else {
						if (cSeg instanceof RelativeTMASegment) {
							final RelativeTMASegment seg = (RelativeTMASegment) cSeg;
							otherSegment = seg;
							...
							// share the good news
							if (_ourLayersSubject != null) {
								_ourLayersSubject.fireExtended(seg, _myHelper.getSecondaryTrack());
							}
							...
							CorePlugin.logError(IStatus.WARNING, "Ignoring this leg,  it's not relative TMA:" + cSeg,
									null);
							continue;
						}
		...
		if (!legFound) {
			...
			final TimePeriod period = new TimePeriod.BaseTimePeriod(new HiResDate(leg.getStart()),
					new HiResDate(leg.getEnd()));
			final List<SensorContactWrapper> cuts = StackedDotHelper.getBearings(primaryTrack, false, period);
			final SensorContactWrapper[] observations = cuts.toArray(new SensorContactWrapper[] {});
			...
			final WorldVector defaultOffset = new WorldVector(Math.toDegrees(135),
					new WorldDistance(2, WorldDistance.NM), new WorldDistance(0, WorldDistance.METRES));
			...
			if (otherSegment != null) {
				...
				if (lastFix != null) {
					...
					final long timePeriod = leg.getStart() - lastFix.getDTG().getDate().getTime();
					final double distTravelled = speed.getValueIn(WorldSpeed.M_sec) * timePeriod / 1000d;
					final WorldVector vector = new WorldVector(Math.toRadians(otherSegment.getCourse()),
					...
					final Watchable[] matches = primaryTrack.getNearestTo(new HiResDate(leg.getStart()), false);
					...
					CorePlugin.logError(IStatus.WARNING,
							"Couldn't create target leg properly,  couldn't last fix in existing leg", null);
					offset = defaultOffset;
				}
			} else {
				courseDegs = 0d;
			...
			if (observations == null || observations.length == 0) {
				...
				// ok, ready to go
				final RelativeTMASegment newLeg = new RelativeTMASegment(observations, offset, speed, courseDegs,
						theLayers, override);
	 ...
	 */
	protected List<Zone> sliceTarget(final List<Zone> ownshipZones, final List<SensorContactWrapper> cuts,
			final ColorProvider randomProv, final ISecondaryTrack tgtTrack, final Precision slicePrecision) {
		...
		if (ownshipZones == null || ownshipZones.isEmpty()) {
			Application.logError2(ToolParent.ERROR, "List of ownship legs is empty", null);
			return null;
		}
		...
		for (final Zone thisZ : ownshipZones) {
			sliceThisLeg(cuts, slicer, zigStorer, legStorer, optimiseTolerance, RMS_ZIG_RATIO, log, thisZ);
		}

		// special case: if we've manually deleted the target legs, and have to
		// re-create them
	 ...
	 * @return
	 */
	protected List<Zone> sliceTarget2(final List<SensorContactWrapper> cuts, final ColorProvider randomProv,
		...
		final List<Long> thisLegTimes = new ArrayList<Long>();
		final List<Double> thisLegBearings = new ArrayList<Double>();
		getCutsForThisLeg(cuts, Long.MIN_VALUE, Long.MAX_VALUE, thisLegTimes, thisLegBearings);

		// slice the leg
		slicer.sliceThis2(log, TrackShiftActivator.PLUGIN_ID, "Some scenario", legStorer, RMS_ZIG_RATIO,
	...
	/**
	 * the layesr manager has told use that the sec track has been extended. So,
	 * update the zones.
	 */
	...
	private final void watchMyParts() {
		...
		_myPartMonitor.addPartListener(ISelectionProvider.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				final ISelectionProvider prov = (ISelectionProvider) part;
			...
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				...
				if (_myTrackDataProvider != dataP) {
					...
					if (secs != null && secs.length == 1) {
						...
						if (firstSec instanceof TrackWrapper) {
							final TrackWrapper secT = (TrackWrapper) firstSec;
							secT.addPropertyChangeListener(PlainWrapper.LOCATION_CHANGED, _infillListener);
						}
					}
					...
					_layersListener = new Layers.DataListener2() {
						...
						public void dataReformatted(final Layers theData, final Layer changedLayer) {
							if (_holder == null || _holder.isDisposed()) {
								...
								_myHelper.initialise(_switchableTrackDataProvider, false, _onlyVisible.isChecked(),
										logger, getType(), _needBrg, _needFreq);
