    ...
    ErrorLogger
{
  ...
  protected class TargetZoneSlicer implements ZoneSlicer
  {
    ...
    {
      final boolean doCombined = TrackShiftActivator.getDefault()
          .getPreferenceStore().getBoolean(USE_HOLISTIC_SLICER);
      if (doCombined)
      {
    ...
    private List<Zone> performSlicingCombined()
    {
      ...
      if (secondary != null)
      {
        ...
        final List<SensorContactWrapper> bearings = StackedDotHelper
            .getBearings(_myHelper.getPrimaryTrack(), _onlyVisible.isChecked(),
                period);
  ...
  protected static List<Zone> legsFromZigs(final long startTime,
      final long endTime, final List<Zone> zigs, final ColorProvider randomProv)
  {
  ...
  private static void showThisSelectionInOutline(
      final List<EditableWrapper> subjects, final IEditorPart editor)
  {
  ...
  private static EditableWrapper wrapThisFix(final FixWrapper fix,
      final Layers layers)
  {
  ...
  void clearPlots()
  {
    final Runnable runner = new Runnable()
    {
      ...
      public void run()
      {
        final TimeSeriesCollection coll = (TimeSeriesCollection) _dotPlot
            .getDataset();
        coll.removeAllSeries();

        final TimeSeriesCollection line = (TimeSeriesCollection) _linePlot
            .getDataset();
  ...
  public void createPartControl(final Composite parent)
  {
    ...
    final boolean goingHolistic = TrackShiftActivator.getDefault()
        .getPreferenceStore().getBoolean(USE_HOLISTIC_SLICER);
  ...
  protected void createStackedPlot()
  {
    ...
    final DefaultXYItemRenderer lineRend = new ColourStandardXYItemRenderer(
        null, null, _linePlot);
    lineRend.setBasePaint(Color.DARK_GRAY);
    ...
    _overviewSpeedRenderer = new ResidualXYItemRenderer(null, null,
        _targetSpeedSeries);
    _overviewSpeedRenderer.setBasePaint(DebriefColors.RED.darker());
      ...
      public void chartProgress(final ChartProgressEvent cpe)
      {
        ...
        if (_selectOnClick.isChecked() && _seriesToSearch != null)
        {
          ...
          if (tsc != null)
          {
            ...
            {
              CorePlugin.logError(IStatus.WARNING, "Failed to find series:"
                  + targetSeries, null);
              nearest = null;
            }
            ...
            else
            {
              ...
              if (layers != null)
              {
                ...
                if (payload != null)
                {
                  ...
                  {
                    final SensorContactWrapper theCut =
    ...
    _holder.addChartMouseListener(new ChartMouseListener()
    {
      ...
      public void chartMouseClicked(final ChartMouseEvent arg0)
      {
        ...
        String seriesName = getSeriesToSelect(arg0.getChart(), arg0
            .getTrigger(), arg0.getEntity());

        if (ent instanceof XYItemEntity && seriesName != null)
        {
          // ok, we have our XY item, and we were able to find which dataset it is
        }
        else
        {
          // ok, it wasn't an XY item that was clicked. Shall we find the nearest item?
          ...
          if (findNearest)
          {
            ...
            Collection<?> entities = _holder.getChartRenderingInfo()
                .getEntityCollection().getEntities();
            ...
            for (Iterator<?> iter = entities.iterator(); iter.hasNext();)
            {
              ...
              if ((element instanceof XYItemEntity))
              {
                ...
                Point2D centerPoint = new Point2D.Double(rect.getCenterX(), rect
                    .getCenterY());

                double distance = point2d.distance(centerPoint);
      ...
      public XYPlot findTheSubplot(CombinedDomainXYPlot dPlot,
          PlotRenderingInfo info, Point2D source)
      {
        ...
        if (subplotIndex >= 0)
        {
          result = (XYPlot) dPlot.getSubplots().get(subplotIndex);
        }
        else
        {
          // special processing. find the plot with a centre nearest the supplied one
          final int index = getSubplotIndexNearestTo(info, source);
          result = (XYPlot) dPlot.getSubplots().get(index);
        }
        return result;
      }

      public int getSubplotIndexNearestTo(PlotRenderingInfo info,
          Point2D source)
      {
        ...
        else
        {
          ...
          {
            PlotRenderingInfo subInfo = info.getSubplotInfo(i);
            Rectangle2D area = subInfo.getDataArea();
            Point2D.Double centre = new Point2D.Double(area.getCenterX(), area
                .getCenterY());
            final int dist = (int) source.distance(centre);
            if (dist < minDistance)
            {
              minDistance = dist;
              res = i;
        ...
        else
        {
          ...
          if (plot instanceof CombinedDomainXYPlot)
          {
            ...
            final PlotRenderingInfo plotInfo = _holder.getChartRenderingInfo()
                .getPlotInfo();
            final XYPlot subPlot = findTheSubplot(dPlot, plotInfo, source);
        ...
        PartMonitor.ACTIVATED, new PartMonitor.ICallback()
        {
              ...
              final IWorkbenchPart parentPart)
          {
            ...
            if (_myTrackDataProvider != dataP)
            {
              ...
              final TimeSeriesCollection errorData =
