...
public class BearingResidualsView extends BaseStackedDotsView implements ITimeVariableProvider {
	...
	/**
	 * once an ambiguous cut has been resolved, we continue to remember the
	 * ambiguous bearing, to allow the analyst to change their mind. But, at some
	 * point, once the analyst is happy with the resolution, they may wish to drop
	 * the ambiguous bearing, and only deal with the resolved one.
	 *
	 * @author ian
	 *
	 */
	private static class DeleteAmbiguousBearingsOperation extends CMAPOperation {
		...
		public DeleteAmbiguousBearingsOperation(final List<SensorContactWrapper> cutsToDelete,
	...
	private static class DeleteCutsOperation extends CMAPOperation {
		...
		private static Map<SensorWrapper, LegOfCuts> deleteTheseCuts(final List<SensorContactWrapper> cutsToDelete) {
			final Map<SensorWrapper, LegOfCuts> deletedCuts = new HashMap<SensorWrapper, LegOfCuts>();
		...
		public DeleteCutsOperation(final List<SensorContactWrapper> cutsToDelete, final Runnable updateData,
		...
		private void fireModified() {
			...
			if (sensor != null) {
				...
				sensor.getHost().firePropertyChange(SupportsPropertyListeners.EXTENDED, null,
	...
	private class OwnshipZoneSlicer implements ZoneSlicer {
		...
		private TimePeriod getAnalysisPeriod(final boolean wholePeriod, final TimePeriod trackPeriod,
			...
			if (!wholePeriod && existingZones != null && !existingZones.isEmpty()) {
				...
				final TimePeriod sensorDataCoverage = timePeriodFor(_myHelper.getPrimaryTrack());
				...
				if (lastZoneEnd.equals(sensorDataCoverage.getEndDTG())) {
					...
					final TimePeriod visiblePeriod = new TimePeriod.BaseTimePeriod(lastZoneStart,
							sensorDataCoverage.getEndDTG());
		...
		private void handleThisContact(final Zone zone, final SensorContactWrapper contact, final ZoneModes zoneMode) {
				...
				while (sEnum.hasMoreElements() && !hasAmbiguous) {
					...
					if (sensor.size() > 0) {
						...
						while (elements.hasMoreElements() && !hasAmbiguous) {
							final SensorContactWrapper contact = (SensorContactWrapper) elements.nextElement();
		...
		public ArrayList<Zone> performSlicing(final boolean wholePeriod) {
			...
			if (hasAmbiguous) {
				...
				final double MIN_ZIG = TrackShiftActivator.getDefault().getPreferenceStore()
						.getDouble(PreferenceConstants.MIN_ZIG);
				final double MIN_BOTH = TrackShiftActivator.getDefault().getPreferenceStore()
						.getDouble(PreferenceConstants.MIN_TURN_RATE);
				final double MIN_LEG_LENGTH = TrackShiftActivator.getDefault().getPreferenceStore()
						.getDouble(PreferenceConstants.MIN_LEG_LENGTH);
				final double OS_TURN_MIN_COURSE_CHANGE = TrackShiftActivator.getDefault().getPreferenceStore()
						.getDouble(PreferenceConstants.OS_TURN_MIN_COURSE_CHANGE);
				final long OS_TURN_MIN_TIME_INTERVAL = TrackShiftActivator.getDefault().getPreferenceStore()
						.getLong(PreferenceConstants.OS_TURN_MIN_TIME_INTERVAL);
				...
				final TimePeriod analysisPeriod = getAnalysisPeriod(wholePeriod, trackPeriod,
						ownshipZoneChart.getZones());

				final LegsAndZigs legsAndCuts = AmbiguityResolver.sliceTrackIntoLegsUsingAmbiguity(
						_myHelper.getPrimaryTrack(), MIN_ZIG, MIN_BOTH, MIN_LEG_LENGTH, logger, ambigScores,
						OS_TURN_MIN_COURSE_CHANGE, OS_TURN_MIN_TIME_INTERVAL, analysisPeriod, MAX_LEGS);

				slicedZones = new ArrayList<Zone>();
				for (final LegOfCuts leg : legsAndCuts.getLegs()) {
					final Zone thisZone = new Zone(leg.get(0).getDTG().getDate().getTime(),
							leg.get(leg.size() - 1).getDTG().getDate().getTime(), _blueProv.getZoneColor());
				...
				if (!slicedZones.isEmpty()) {
					final TimePeriod period = new TimePeriod.BaseTimePeriod(
							new HiResDate(slicedZones.get(0).getStart()),
							new HiResDate(slicedZones.get(slicedZones.size() - 1).getEnd()));
			...
			while (sensors.hasMoreElements()) {
				...
				if (sensor.getVisible()) {
					...
					while (ele.hasMoreElements()) {
						final SensorContactWrapper scw = (SensorContactWrapper) ele.nextElement();
						if (scw.getVisible()) {
	...
	private class ResolveCutsOperationAmbig extends CMAPOperation {
		...
		public ResolveCutsOperationAmbig(final AmbiguityResolver resolver, final List<LegOfCuts> legs) {
		...
		public IStatus execute(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			if (_resolved != null) {
				...
				res = new Status(IStatus.OK, TrackShiftActivator.PLUGIN_ID, "Resolve legs successful", null);
			} else {
				res = new Status(IStatus.ERROR, TrackShiftActivator.PLUGIN_ID, "Tried to slice too many legs", null);
		...
		public IStatus undo(final IProgressMonitor monitor, final IAdaptable info) throws ExecutionException {
			...
			final IStatus res = new Status(IStatus.OK, TrackShiftActivator.PLUGIN_ID, "Un-resolve legs successful",
					null);
			return res;
		}
	...
	public static class TestResiduals extends TestCase {
		private static TrackWrapper getData(final String name) throws FileNotFoundException {
			...
			final String fName = "../org.mwc.cmap.combined.feature/root_installs/sample_data/S2R/" + name;
		...
		public void testDeleteAmbiguous() throws FileNotFoundException {
			...
			final SensorWrapper s1 = new SensorWrapper("s1");

			final SensorContactWrapper sc1 = new SensorContactWrapper(track.getName(), new HiResDate(100000), null,
					100d, 120d, null, null, Color.RED, "one", LineStylePropertyEditor.DOT_DASH, "sensor");
			final SensorContactWrapper sc2 = new SensorContactWrapper(track.getName(), new HiResDate(200000), null,
					101d, 121d, null, null, Color.RED, "one", LineStylePropertyEditor.DOT_DASH, "sensor");
			final SensorContactWrapper sc3 = new SensorContactWrapper(track.getName(), new HiResDate(300000), null,
					102d, 122d, null, null, Color.RED, "one", LineStylePropertyEditor.DOT_DASH, "sensor");
			final SensorContactWrapper sc4 = new SensorContactWrapper(track.getName(), new HiResDate(400000), null,
					103d, 123d, null, null, Color.RED, "one", LineStylePropertyEditor.DOT_DASH, "sensor");
			final SensorContactWrapper sc5 = new SensorContactWrapper(track.getName(), new HiResDate(500000), null,
					104d, 124d, null, null, Color.RED, "one", LineStylePropertyEditor.DOT_DASH, "sensor");
			...
			final TimePeriod period = new TimePeriod.BaseTimePeriod(new HiResDate(250000), new HiResDate(500000));
		...
		public void testDitchUsingAmbiguity() throws FileNotFoundException {
			...
			final SensorWrapper sensor = (SensorWrapper) track.getSensors().elements().nextElement();
			...
			final LegsAndZigs res = AmbiguityResolver.sliceTrackIntoLegsUsingAmbiguity(track, 0.2, 0.2, 240, null, null,
					null, null, timePeriod, null);
			...
			Map<SensorWrapper, LegOfCuts> deleted = DeleteCutsOperation.deleteTheseCuts(zigs);
			...
			assertEquals("correct leg", 251.33d, resolvedLegs.get(0).leg.get(0).getBearing(), 1d);
			assertEquals("correct leg", 253d, resolvedLegs.get(1).leg.get(0).getBearing(), 1d);
			assertEquals("correct leg", 251d, resolvedLegs.get(2).leg.get(0).getBearing(), 1d);
			assertEquals("correct leg", 254d, resolvedLegs.get(3).leg.get(0).getBearing(), 1d);
			assertEquals("correct leg", 258d, resolvedLegs.get(4).leg.get(0).getBearing(), 1d);
			assertEquals("correct leg", 269d, resolvedLegs.get(5).leg.get(0).getBearing(), 1d);
		...
		/**
		 * note: we're testing this zone chart functionality in here, so we can
		 * introduce it into the automated test suite.
		 */
	...
	protected static void deleteAmbiguousCuts(final TrackWrapper primaryTrack, final Runnable updateData,
			final ZoneChart ownshipZoneChart, final ZoneUndoRedoProvider undoRedoProvider) {
		...
		final LegOfCuts cutsToDelete = findResolvedAmbiguousCuts(outerPeriod, primaryTrack);

		// delete the ambiguous cuts
		final IUndoableOperation deleteOperation = new DeleteAmbiguousBearingsOperation(cutsToDelete, updateData,
				ownshipZoneChart);
	...
	protected static void deleteCutsInTurnB(final TrackWrapper primaryTrack, final Runnable updateData,
			final ZoneChart ownshipZoneChart, final ZoneUndoRedoProvider undoRedoProvider) {
		...
		if (ownshipZones != null && ownshipZones.size() > 0) {
			...
			// ok, produce the list of cuts to cull
			final LegOfCuts cutsToDelete = findCutsNotInZones(ownshipZones, outerPeriod, primaryTrack);

			// delete this list of cuts
			final IUndoableOperation deleteOperation = new DeleteCutsOperation(cutsToDelete, updateData,
					ownshipZoneChart);
	...
	public static LegOfCuts findCutsNotInZones(final List<Zone> ownshipZones, final TimePeriod outerPeriod,
	...
	public static LegOfCuts findResolvedAmbiguousCuts(final TimePeriod outerPeriod, final TrackWrapper primaryTrack) {
		final LegOfCuts res = new LegOfCuts();
		final Enumeration<Editable> sensors = primaryTrack.getSensors().elements();
		while (sensors.hasMoreElements()) {
			final SensorWrapper sensor = (SensorWrapper) sensors.nextElement();
	...
	private static Logger getLogger() {
		final Logger logger;
		final boolean doLogging = TrackShiftActivator.getDefault().getPreferenceStore()
				.getBoolean(PreferenceConstants.DIAGNOSTICS);
		if (doLogging) {
			...
			final Handler handler = new ConsoleHandler() {
				...
				public void publish(final LogRecord record) {
					...
					TrackShiftActivator.getDefault().getLog()
							.log(new Status(IStatus.INFO, TrackShiftActivator.PLUGIN_ID, record.getMessage()));
				}
			};
	...
	private static void restoreCuts(final Map<SensorWrapper, LegOfCuts> deletedCuts) {
	...
	protected String formatValue(final double value) {
		...
		if (_crossHairFormatter != null) {
			...
			res = MWC.Utilities.TextFormatting.GeneralFormat.formatOneDecimalPlace(value);
		}
			...
			public void run() {
				final Runnable wrappedDelete = new Runnable() {
					...
					public void run() {
						...
						deleteAmbiguousCuts(_myHelper.getPrimaryTrack(), updateData, ownshipZoneChart,
								undoRedoProvider);
					}
				};
			...
			public void run() {
				final Runnable wrappedDelete = new Runnable() {
					...
					public void run() {
						...
						deleteCutsInTurnB(_myHelper.getPrimaryTrack(), updateData, ownshipZoneChart, undoRedoProvider);
					}
				};
				BusyIndicator.showWhile(Display.getCurrent(), wrappedDelete);
			}
	...
	public long getValueAt(final HiResDate dtg) {
		// get the time
		final RegularTimePeriod myTime = new FixedMillisecond(dtg.getMicros() / 1000);

		// get the set of calculated error values that we have stored in the graph
		final TimeSeriesCollection dataset = (TimeSeriesCollection) _dotPlot.getDataset();
	...
	protected void makeActions() {
		...
		scaleError = new Action("Show the error when dragging", IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
				super.run();
			}
		...
		scaleError.setToolTipText("Show symbol scaled to per-cut error when dragging");
		scaleError.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/scale.png"));

		// see if there's an existing setting for this.

		// now the course action
		relativeAxes = new Action("Centre Bearing axis on North", IAction.AS_CHECK_BOX) {
			@Override
			public void run() {
				super.run();
		...
		relativeAxes.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/swap_axis.png"));
		...
		showCourse.setImageDescriptor(TrackShiftActivator.getImageDescriptor("icons/24/ShowCourse.png"));
	...
	protected void resolveAmbiguousCutsB() {
		...
		if (zones == null || zones.size() == 0) {
			...
			for (final Zone zone : zones) {
				final TimePeriod period = new TimePeriod.BaseTimePeriod(new HiResDate(zone.getStart()),
						new HiResDate(zone.getEnd()));
				final List<SensorContactWrapper> cuts = StackedDotHelper.getBearings(_myHelper.getPrimaryTrack(), true,
						period);
			...
			final IUndoableOperation resolveCuts = new ResolveCutsOperationAmbig(resolver, legs);
	...
	protected void updateData(final boolean updateDoublets) {
		...
		_myHelper.updateBearingData(errorData, lineData, _switchableTrackDataProvider, _onlyVisible.isChecked(),
				showCourse.isChecked(), relativeAxes.isChecked(), this, updateDoublets, _targetCourseSeries,
				_targetSpeedSeries, measuredValuesColl, ambigValuesColl, ownshipCourseSeries, targetBearingSeries,
				targetCalculatedSeries, _overviewSpeedRenderer, _overviewCourseRenderer, backShader);
