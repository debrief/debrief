 ...
 * @author Anuradha
 *
 *         Direct port from Jface FilteredTree make sure to override passed
 *         PatternFilter
 *
...
public abstract class FilteredGrid extends Composite {
	 ...
	 * @param gridStyle  the style bits for the <code>Grid</code>
	 * @param filter     the filter to be used
	 * @param useNewLook <code>true</code> if the new 3.5 look should be used
	 * @since 3.5
	 */
	public FilteredGrid(final Composite parent, final int gridStyle, final boolean useNewLook) {
	...
	protected void createFilterText(final Composite parent) {
		...
		filterText.addKeyListener(new KeyAdapter() {
			...
			public void keyPressed(final KeyEvent e) {
				...
				final boolean hasItems = getViewer().getGrid().getItemCount() > 0;
			...
			public void keyTraversed(final TraverseEvent e) {
				if (e.detail == SWT.TRAVERSE_RETURN) {
					...
					if (getViewer().getGrid().getItemCount() == 0) {
						...
						// to match
						final boolean hasFocus = getViewer().getGrid().setFocus();
						final boolean textChanged = !getInitialText().equals(filterText.getText().trim());
		...
		if ((filterText.getStyle() & SWT.ICON_CANCEL) != 0) {
			filterText.addSelectionListener(new SelectionAdapter() {
				 ...
				 *
				 * @see
				 * org.eclipse.swt.events.SelectionAdapter#widgetDefaultSelected(org.eclipse.swt
	 ...
	 * @param style  SWT style bits used to create the grid
	 * @return the grid
	 */
	 ...
	 *
	 * @param parent the parent composite
	 * @param style  SWT style bits used to create the grid viewer
	 * @return the grid viewer
	 *
	 * @since 3.3
	 */
	protected GridTableViewer doCreateGridViewer(final Composite parent, final int style) {
		return new GridTableViewer(parent, style) {

			/**
			 * override getSelection in GridTableViewer to avoid bug with
			 * ILazyContentProvider and GridTableViewer as it need to check
			 * grid.getFocusItem() before access getData()
			 */
			@Override
	...
	/**
	 * Creates a workbench job that will refresh the grid based on the current
			...
			public IStatus runInUIThread(final IProgressMonitor monitor) {
				...
				Display.getDefault().asyncExec(new Runnable() {
					...
					public void run() {
						...
						final Control redrawFalseControl = gridComposite != null ? gridComposite
	...
	public void setFilterMode(final boolean checked) {
		...
		if (checked) {
			...
			final GridData layoutData = new GridData(SWT.FILL, SWT.BEGINNING, false, false);
