...
public class NarrativeViewerStyleConfiguration extends AbstractRegistryConfiguration {
	private static final String[] PHRASES_COLORS = new String[] { " 178, 180, 255", "147, 232, 207", "232, 205, 167",
			" 255, 192, 215", };
	...
	private final RegexMarkupValue selectionSearchHighlighter = new RegexMarkupValue("",
	...
	private final Color selectionAnchorSelectionBgColor = GUIHelper.COLOR_LIST_SELECTION;
	...
	private final ICellPainter cellPainter = new BackgroundPainter(
			new PaddingDecorator(new RichTextCellPainter(true, true, false), 0, 5, 0, 5, false));

	// customized painter that also supports shrinking
	private final RichTextCellPainter wrappingAutomaticRowHeightPainter = new RichTextCellPainter(true, false, true) {
		@Override
		protected boolean performRowResize(final int contentHeight, final Rectangle rectangle) {
			return ((contentHeight != rectangle.height) && this.calculateByTextHeight);
		};
	};

	private final RichTextCellPainter automaticRowHeightPainter = new RichTextCellPainter(false, false, true) {
		@Override
		protected boolean performRowResize(final int contentHeight, final Rectangle rectangle) {
			return ((contentHeight != rectangle.height) && this.calculateByTextHeight);
		};
	};
	public final ICellPainter wrappingEntryLogPainter = new BackgroundPainter(
			new PaddingDecorator(wrappingAutomaticRowHeightPainter, 0, 5, 0, 5, false));
	public final ICellPainter automaticRowHeightLogPainter = new BackgroundPainter(
	...
	private final HorizontalAlignmentEnum headerHAlign = HorizontalAlignmentEnum.LEFT;
	private final VerticalAlignmentEnum headerVAlign = VerticalAlignmentEnum.MIDDLE;
	private final Boolean renderGridLines = Boolean.FALSE;

	private final ICellPainter columnHeaderCellPainter = new BeveledBorderDecorator(
	...
	private void configureColumnHeaderStyle(final IConfigRegistry configRegistry) {
		...
		columnHeaderCellStyle.setAttributeValue(CellStyleAttributes.BACKGROUND_COLOR, this.headerBgColor);
		columnHeaderCellStyle.setAttributeValue(CellStyleAttributes.FOREGROUND_COLOR, this.headerFgColor);
		columnHeaderCellStyle.setAttributeValue(CellStyleAttributes.HORIZONTAL_ALIGNMENT, this.headerHAlign);
		columnHeaderCellStyle.setAttributeValue(CellStyleAttributes.VERTICAL_ALIGNMENT, this.headerVAlign);
		columnHeaderCellStyle.setAttributeValue(CellStyleAttributes.FONT, this.headerFont);

		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_STYLE, columnHeaderCellStyle,
				DisplayMode.NORMAL, GridRegion.COLUMN_HEADER);

		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_PAINTER, this.columnHeaderCellPainter,
				DisplayMode.NORMAL, GridRegion.COLUMN_HEADER);
		...
		// should be disabled
		configRegistry.registerConfigAttribute(CellConfigAttributes.RENDER_GRID_LINES, this.renderGridLines,
				DisplayMode.NORMAL, GridRegion.COLUMN_HEADER);
	}
	...
	private void configureDefaultStyle(final IConfigRegistry configRegistry) {
		...
		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_PAINTER, this.wrappingEntryLogPainter,
	 ...
	 * @param configRegistry
	 */
	private void configureMarkupDisplayConverter(final IConfigRegistry configRegistry) {
		...
		configRegistry.registerConfigAttribute(CellConfigAttributes.DISPLAY_CONVERTER, selectionMarkupConverter,
	...
	private void configureSelectionStyle(final IConfigRegistry configRegistry) {
		...
		selectionCellStyle.setAttributeValue(CellStyleAttributes.BACKGROUND_COLOR, this.selectionBgColor);
		selectionCellStyle.setAttributeValue(CellStyleAttributes.FOREGROUND_COLOR, this.selectionFgColor);

		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_STYLE, selectionCellStyle, DisplayMode.SELECT);

		// selection anchor
		final IStyle selectionAnchorStyle = new Style();
		selectionAnchorStyle.setAttributeValue(CellStyleAttributes.BACKGROUND_COLOR,
				this.selectionAnchorSelectionBgColor);
		selectionAnchorStyle.setAttributeValue(CellStyleAttributes.FOREGROUND_COLOR,
				this.selectionAnchorSelectionFgColor);
		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_STYLE, selectionAnchorStyle,
		...
		columnHeaderSelectionCellStyle.setAttributeValue(CellStyleAttributes.FONT, this.headerFont);
		columnHeaderSelectionCellStyle.setAttributeValue(CellStyleAttributes.BACKGROUND_COLOR,
				this.headerSelectionBgColor);
		columnHeaderSelectionCellStyle.setAttributeValue(CellStyleAttributes.FOREGROUND_COLOR,
				this.headerSelectionFgColor);

		configRegistry.registerConfigAttribute(CellConfigAttributes.CELL_STYLE, columnHeaderSelectionCellStyle,
				DisplayMode.SELECT, GridRegion.COLUMN_HEADER);
	}
	...
	public void updatePhrasesStyle() {
		...
		for (int i = 0; i < phrases.length; i++) {
			...
			final RegexMarkupValue phraseHighlighter = new RegexMarkupValue("",
					"<span style=\"background-color:rgb(" + bg + ")\">", "</span>");

			final RegexMarkupValue phraseSearchHighlighter = new RegexMarkupValue("",
					"<span style=\"color:rgb(0, 0, 0)\"><span style=\"background-color:rgb(" + bg + ")\">",
					"</span></span>");

			markupConverter.registerMarkup(phrase, phraseHighlighter);
			selectionMarkupConverter.registerMarkup(phrases[i], phraseSearchHighlighter);
