...
public abstract class FilteredNatTable extends Composite {
	...
	static {
		...
		descriptor = AbstractUIPlugin.imageDescriptorFromPlugin(Activator.PLUGIN_ID, "icons/16/disabled_clear.gif"); //$NON-NLS-1$
		if (descriptor != null) {
	...
	private static Image getImageFromJFaceResources(final String imageDes) {
		final ImageDescriptor id = JFaceResources.getImageRegistry().getDescriptor(imageDes);
		if (id != null) {
	 ...
	 * @since 3.5
	 */
	public FilteredNatTable(final Composite parent, final int gridStyle, final boolean useNewLook) {
