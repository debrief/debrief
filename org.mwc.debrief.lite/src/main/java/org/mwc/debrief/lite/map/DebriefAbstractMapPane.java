...
implements MapPane, RenderingExecutorListener, MapLayerListListener, MapBoundsListener {
    ...
    protected ReferencedEnvelope mapBounds = null;
    ...
    boolean resizeCompleted = false;
    public void setPendingDisplayArea() {
    	//pendingDisplayArea = new ReferencedEnvelope();
    	if(resizeCompleted) {
    ...
    protected void setForNewSize() {
        ...
        try {
            if (mapContent != null) {
                ...
                }else if(resizeCompleted) {
                	
                	 // the following is the fix for 'stretch' issue in the map
                	 MapViewport mvp = new MapViewport();
                     mvp.setScreenArea(getBounds());
                     mvp.setMatchingAspectRatio(true);
