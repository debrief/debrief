...
public class GeoToolMapProjection extends PlainProjection {
	...
	public GeoToolMapProjection(final MapContent map) {
		super("GeoTools Map");
		_map = map;
	...
	public Point toScreen(WorldLocation val) {
		Point res = null;
		AffineTransform transform = _map.getViewport().getWorldToScreen();
	...
	public WorldLocation toWorld(Point val) {
		...
		try {
			...
			Application.logError2(Application.ERROR, "Whilst trying to set convert to world coords", e);
			...
			Application.logError2(Application.ERROR, "Unexpected transform problem whilst performing screen to world",
					e);
		}
		return res;
	}
