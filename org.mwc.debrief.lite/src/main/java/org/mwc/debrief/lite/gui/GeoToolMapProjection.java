...
public class GeoToolMapProjection extends PlainProjection implements GeoToolsHandler {
		...
		if (_view.getCoordinateReferenceSystem() != dataCRS) {
			try {
				...
				if (_view.getWorldToScreen() != null) {
					...
					// output the results
					res = new Point((int) workScreen.getCoordinate()[0], (int) workScreen.getCoordinate()[1]);
				}
			} catch (MismatchedDimensionException | TransformException e) {
				Application.logError2(ToolParent.ERROR, "Failure in projection transform in toScreen operation:"
						+ e.getMessage() + " from location:" + val, null);
			}
		}
		return res;
	}
	...
	public WorldLocation toWorld(final Point val) {
		...
		try {
			...
			if (dArea != null && (dArea.getWidth() > 0 || dArea.getHeight() > 0)) {
				...
				res = new WorldLocation(workDegs.getCoordinate()[1], workDegs.getCoordinate()[0], 0);
			}
		} catch (final MismatchedDimensionException e) {
			Application.logError2(ToolParent.ERROR, "Whilst trying to set convert to world coords", e);
		}
		return res;
	}
