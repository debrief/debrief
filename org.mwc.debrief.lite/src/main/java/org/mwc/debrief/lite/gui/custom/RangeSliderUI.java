...
class RangeSliderUI extends BasicSliderUI
{
    ...
    private void moveLowerThumb()
    {
      ...
      switch (slider.getOrientation())
      {
        case SwingConstants.VERTICAL:
          ...
          final int vMax = yPositionForValue(slider.getValue() + slider
              .getExtent());
        ...
        case SwingConstants.HORIZONTAL:
          ...
          final int hMax = xPositionForValue(slider.getValue() + slider
              .getExtent());
  ...
  private final Color rangeColor = Color.BLACK;
  ...
  protected void calculateThumbLocation()
  {
    ...
    if (slider.getSnapToTicks())
    {
      ...
      if (tickSpacing != 0)
      {
        ...
        {
          final float temp = (float) (upperValue - slider.getMinimum())
              / (float) tickSpacing;
    ...
    {
      final int upperPosition = xPositionForValue(slider.getValue() + slider
          .getExtent());
    ...
    {
      final int upperPosition = yPositionForValue(slider.getValue() + slider
          .getExtent());
  ...
  /**
   * Overrides superclass method to do nothing. Thumb painting is handled within the
  ...
  public void scrollByBlock(final int direction)
  {
    synchronized (slider)
    {
      ...
      final int delta = blockIncrement * ((direction > 0) ? POSITIVE_SCROLL
          : NEGATIVE_SCROLL);
  ...
  public void scrollByUnit(final int direction)
  {
    synchronized (slider)
    {
      final int delta = 1 * ((direction > 0) ? POSITIVE_SCROLL
          : NEGATIVE_SCROLL);
