...
public class NarrativeConfigurationModel implements AbstractNarrativeConfiguration {
	...
	private final List<AbstractSelection<NarrativeEntry>> _narrativeSelection = new ArrayList<>();

	private final HashMap<NarrativeWrapper, Set<NarrativeEntry>> _narrativeWrappers = new HashMap<>();

	public NarrativeConfigurationModel(final TimeManager timeManager) {
		this._timeManager = timeManager;

		final PropertyChangeListener timeChangedListener = new PropertyChangeListener() {
		...
		_timeManager.addListener(timeChangedListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
	}
	...
	public void highlightNarrative(final HiResDate object) {
		...
		for (final Set<NarrativeEntry> narrativeEntries : _narrativeWrappers.values()) {
			for (final NarrativeEntry narrativeEntry : narrativeEntries) {
				if (narrative == null
						|| closestDistance > Math.abs(narrativeEntry.getDTG().getMicros() - object.getMicros())) {
					closestDistance = Math.abs(narrativeEntry.getDTG().getMicros() - object.getMicros());
		...
		if (narrative != null) {
			final boolean update = _currentHighLight == null || !_currentHighLight.equals(narrative);

			if (update) {
				final NarrativeEntry oldValue = _currentHighLight;
				final NarrativeEntry newValue = narrative;
				_currentHighLight = narrative;
				notifyListenersStateChanged(narrative, NARRATIVE_HIGHLIGHT, oldValue, newValue);
			}
		}
	}
	...
	@Override
	public void registerNewNarrativeEntry(final NarrativeWrapper wrapper, final NarrativeEntry entry) {
	...
	@Override
	public void setActiveNarrative(final NarrativeEntry narrative, final boolean check) {
		...
		if (newValue != null && !oldValue.equals(newValue)) {
			...
			notifyListenersStateChanged(narrative, NARRATIVE_SELECTION, oldValue, check);
		}
	}
	...
	public void setNarrativeWrapper(final NarrativeWrapper narrativeWrapper) {
		...
		while (iter.hasMoreElements()) {
			...
			if (nextItem instanceof NarrativeEntry) {
				_narrativeSelection.add(new AbstractSelection<NarrativeEntry>((NarrativeEntry) nextItem, true));
			}
		}
	}
	...
	@Override
	public void unregisterNarrativeEntry(final NarrativeWrapper wrapper, final NarrativeEntry entry) {
		if (_narrativeWrappers.containsKey(wrapper)) {
