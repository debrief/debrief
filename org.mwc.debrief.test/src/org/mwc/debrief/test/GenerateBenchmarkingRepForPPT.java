...
public class GenerateBenchmarkingRepForPPT {
	public static void main(final String[] args) throws IOException {
		final DateFormat dateFormat = new SimpleDateFormat("yyMMdd HHmmss.SSS");
		...
		final File tempFile = File.createTempFile("benchmarking-", ".rep");
		...
		for (int i = 0; i < amountOfSteps; i++) {
			...
			final String worldLocationString = worldLocation.toString().replaceAll("[^a-zA-Z0-9-_\\\\. ]", " ");
			builder.append(worldLocationString);

			builder.append("0 5 0");

			final WorldVector movementDelta = new WorldVector(Math.random() * 100, Math.random() * 100,
					Math.random() * 100);
