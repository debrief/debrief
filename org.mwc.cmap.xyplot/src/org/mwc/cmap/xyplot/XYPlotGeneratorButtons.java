...
public class XYPlotGeneratorButtons implements RightClickContextItemGenerator {
	...
	public static class GetSelection extends MessageDialog {
		 ...
		 * @param _choices
		 */
		public GetSelection(final String dialogTitle, final String dialogMessage, final String[] titles,
				final Object[] choices) {
			super(Display.getCurrent().getActiveShell(), dialogTitle, null, null, MessageDialog.QUESTION,
	...
	public XYPlotGeneratorButtons() {
		...
		if (_theOperations == null) {
			...
			_theOperations
					.addElement(new CalculationHolder(new bearingRateCalc(), new BearingRateFormatter(), true, 180));
			_theOperations.addElement(new CalculationHolder(new dopplerCalc(), null, true, 0, new DopplerPlotWizard()));
			...
			_theOperations.addElement(
					new ShowTimeVariablePlot3.CalculationHolder(new relBearingCalc(), theFormatter, true, 180));
			_theOperations
					.addElement(new ShowTimeVariablePlot3.CalculationHolder(new atbCalc(), theFormatter, true, 180));
		}
	...
	private TimePeriod calculateOuterPeriod(final Vector<WatchableList> theTracks) {
		TimePeriod outerPeriod = null;

		for (final WatchableList track : theTracks) {
				...
				final Action viewPlot = new Action("View XY plot") {
					...
					public void run() {
						...
						try {
							...
							final CalculationWizard wizard = theHolder.getWizard();
							if (wizard != null) {
								final int res = wizard.open(theHolder._theCalc, thePrimary, subjects);
							...
							final IViewPart newPart = page.showView(plotId, theTitle, IWorkbenchPage.VIEW_ACTIVATE);
							...
							final IAdaptable timeC = timeRef.getView(true);

							// that's it, now get the data
							final TimePeriod period = timeC.getAdapter(TimePeriod.class);
							if (period == null) {
								CorePlugin.logError(IStatus.ERROR,
										"TimeController view no longer provides TimePeriod adapter", null);
								return;
							}
							...
							if (thePrimary != null) {
								...
								if (thePrimary.getStartDTG() == null || thePrimary.getEndDTG() == null
										|| thePrimary.getStartDTG().equals(thePrimary.getEndDTG())) {
									// ok, it's a singleton. Find the outer bounds of the other data
									final TimePeriod outerPeriod = calculateOuterPeriod(theTracks);
									...
									if (thePrimary.getStartDTG() != null) {
										startTime = (startTime.greaterThan(thePrimary.getStartDTG())) ? startTime
												: thePrimary.getStartDTG();
									}

									if (thePrimary.getEndDTG() != null) {
										endTime = (endTime.lessThan(thePrimary.getEndDTG())) ? endTime
							...
							if (plotRef == null) {
								CorePlugin.logError(IStatus.ERROR, "Faled to find existing plot", null);
							} else {

								final XYPlotView plotter = (XYPlotView) plotRef.getView(true);

								try {
									plotter.showPlot(theTitle, prov,
											myOperation.toString() + " (" + myOperation.getUnits() + ")",
	...
	ShowTimeVariablePlot3.CalculationHolder getChoice() {
		...
		for (int i = 0; i < _theOperations.size(); i++) {
			final ShowTimeVariablePlot3.CalculationHolder thisC = _theOperations.elementAt(i);
			choices[i] = thisC.toString();
			results[i] = thisC;
		}

		final GetSelection dialog = new GetSelection("View time-variable plot", "Please select the attribute to view",
				choices, results);
	...
	private HiResDate getEarliestEndTime(final Vector<WatchableList> theTracks) {
		HiResDate thisEnd = null;
		for (final WatchableList thisTrack : theTracks) {
			// check it's not a singleton
			if (thisTrack instanceof LightweightTrackWrapper) {
				final LightweightTrackWrapper track = (LightweightTrackWrapper) thisTrack;
	...
	protected WatchableList getPrimary(final Editable[] subjects) {
		...
		if (subjects != null) {
			...
			final GetSelection sel = new GetSelection("Select primary", "Which is the primary track", labels, values);
