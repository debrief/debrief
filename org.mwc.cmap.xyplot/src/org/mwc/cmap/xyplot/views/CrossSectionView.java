...
public class CrossSectionView extends ViewPart implements ISnailPeriodChangedListener {
	...
	protected final class LineChangeListener implements PropertyChangeListener {
		...
		public void propertyChange(final PropertyChangeEvent event) {
			if (event.getPropertyName().equals(PlainWrapper.LOCATION_CHANGED)) {
	 ...
	 */
	final private PropertyChangeListener _timeListener;
	...
	private final ICrossSectionDatasetProvider _datasetProvider = new CrossSectionDatasetProvider();
	 ...
	 */
	private final SnailPeriodTracker _snailMode = new SnailPeriodTracker();
	...
	void clearTimeListener() {
		if (_timeProvider != null) {
			_timeProvider.removeListener(_timeListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
			_timeProvider = null;
		}
	}
	...
	public void createPartControl(final Composite parent) {
		...
		_partMonitor = new PartMonitor(getSite().getWorkbenchWindow().getPartService());
	...
	private void listenToMyParts() {
		// Listen to Layers
		_partMonitor.addPartListener(Layers.class, PartMonitor.ACTIVATED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
				processNewLayers(part);
			}
		});
		_partMonitor.addPartListener(Layers.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
		...
		_partMonitor.addPartListener(ISelectionProvider.class, PartMonitor.ACTIVATED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
		...
		_partMonitor.addPartListener(ISelectionProvider.class, PartMonitor.CLOSED, new PartMonitor.ICallback() {
			@Override
			public void eventTriggered(final String type, final Object part, final IWorkbenchPart parentPart) {
	...
	private void setupFiringTimeChangesToChart() {
		...
		final TimeProvider provider = editor.getAdapter(TimeProvider.class);
		if (provider != null) {
			setUpTimeProvider(provider);
		}
	}

	private void setUpTimeProvider(final TimeProvider prov) {
		_timeProvider = prov;

		// add our listener to the time object
		_timeProvider.addListener(_timeListener, TimeProvider.TIME_CHANGED_PROPERTY_NAME);
