...
public class XYPlotView extends ViewPart {
	...
	protected void bitmapToClipBoard(final JComponent component) {
		final Point size = _plotControl.getSize();
		...
		final java.awt.datatransfer.Clipboard cb = Toolkit.getDefaultToolkit().getSystemClipboard();
		cb.setContents(t, co);
	...
	public void createPartControl(final Composite parent) {
		_plotControl = new ChartComposite(parent, SWT.NONE, null, 400, 600, 300, 200, 1800, 1800, true, false, true,
				true, true, true) {
			@Override
		...
		_modifiedListener = new DataListener() {
			...
			public void dataReformatted(final Layers theData, final Layer changedLayer) {
				try {
					regenerateData();
				} catch (final org.eclipse.swt.SWTException se) {
					CorePlugin.logError(IStatus.WARNING, "Trouble redrawing XT Plot", se);
				}
			}
		};
	...
	private final void doWMF(final MetafileCanvasGraphics2d mf) {
		...
		final Point size = _plotControl.getSize();
		final Dimension dim = new Dimension(size.x, size.y);
		...
		try {
			...
			CorePlugin.logError(IStatus.ERROR, "Problem writing WMF", e);
		}
	...
	private void fillThePlot(final String title, final String units, final formattingOperation theFormatter,
		...
		final Font labelFont = Defaults.getFont();
		...
		final Runnable runnable = new Runnable() {
			...
			public void run() {
				if (_plotControl != null && !_plotControl.isDisposed()) {
					final JFreeChart c = _plotControl.getChart();
					if (c != null) {
						c.setNotify(true);
					}
				}
	...
	private void restorePreviousPlot() {
		try {
			...
			CorePlugin.logError(IStatus.ERROR, "Failed to read in saved XY Plot data", e);
		}
	}
	...
	private void rtfToClipboard(final String fName, final Dimension dim) {
		...
		try {
			...
			final RTFWriter writer = new RTFWriter(os);
			...
			final Object[] rtfData = new Object[] { os.toString() };
			clipboard.setContents(rtfData, new Transfer[] { rtfTransfer });
		} catch (final Exception e1) {
			final IStatus status = new Status(IStatus.ERROR, PlotViewerPlugin.PLUGIN_ID, e1.getLocalizedMessage(), e1);
	...
	private void setupFiringChangesToChart() {
		...
		if (editor != null) {
			...
			prov = editor.getAdapter(TimeProvider.class);
		} else
			CorePlugin.logError(IStatus.WARNING, "Failed to identify time provider", null);
	...
	final void wmfToClipboard() {
		...
		final MetafileCanvasGraphics2d mf = new MetafileCanvasGraphics2d(dir, (Graphics2D) _chartInPanel.getGraphics());
	...
	final void wmfToFile() {
		...
		final String tmpDir = CorePlugin.getDefault().getPreferenceStore().getString(PreferenceConstants.WMF_DIRECTORY);
		...
		// ok, setup the export
		final Point size = _plotControl.getSize();
		final BufferedImage image = new BufferedImage(size.x, size.x, BufferedImage.TYPE_INT_RGB);
		final Graphics2D g2d = image.createGraphics();
		_chartInPanel.setSize(size.x, size.y);
		_chartInPanel.paint(g2d);
		final MetafileCanvasGraphics2d mf = new MetafileCanvasGraphics2d(dir, g2d);
